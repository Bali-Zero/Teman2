...
This is the `main`-based replacement for the feature-base PR #5832 package.
The existing protected-base check required the packaging correction; no guard or
repository setting changes are part of it. Base commit:
`cb322979455aff66459c1bd3b6b17d1b20df9370`. The replacement PR number is not yet
assigned. PR #5825 remains the first-merge dependency for the two overlapping
admission module/test files. Native source and all eleven manifest-bound files
retain the reviewed bytes. The new-base targeted rerun passed **125 tests in
3.31s**; Ruff passed on all ten new Python files. This separate receipt is in the
[main-base pack](../2026-09/agent-air-m5-infra-consul-native-main-9d42c5a0/pack.yml);
the original native test and probe receipts retain their original scope.
...
The conductor ran these three native commands separately from the original
`infra-consul-app-server` worktree root;
