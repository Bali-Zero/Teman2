...
ORGAN_ID="mini.iqoo_radar_relay"
LOG_DIR="$HOME/logs/mini-iqoo_radar_relay"
LOG="$LOG_DIR/run.log"
mkdir -p "$LOG_DIR"
SIDECAR_DIR="$HOME/.organism/last_seen"
PIDFILE="/tmp/nuzantara-mini-iqoo_radar_relay.pid"
...
if [ "${MINI_IQOO_RADAR_RELAY_ENABLED:-true}" = "false" ]; then
    log "kill switch MINI_IQOO_RADAR_RELAY_ENABLED=false — exiting"
