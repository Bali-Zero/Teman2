#!/usr/bin/env bash
# wa-session-liveness.sh — wrapper cron per scripts/wa_session_liveness.py.
#
...
#
# Exit: 0 = ho guardato (anche se una linea e ferma: quel verdetto viaggia via
# Telegram) · 2 = non ho potuto guardare.

set -uo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
ORGAN="$REPO/scripts/wa_session_liveness.py"
...
echo "wa-session-liveness rc=$RC"
exit "$RC"
