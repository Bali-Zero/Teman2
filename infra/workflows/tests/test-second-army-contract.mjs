#!/usr/bin/env node
// test-second-army-contract.mjs — contract test for infra/workflows/second-army.js and
// run-second-army.mjs (docs/architecture/dual-consul/army-map.md §1ter). Follows the
...
const RUNNER_PATH = path.resolve(__dirname, "../run-second-army.mjs");
...
// ---- dynamic driver: compile second-army.js the way run-second-army.mjs does ----------------
...
// Test 12 — the runner enforces the model pin.
// CHOICE OF FORM: run-second-army.mjs is a CLI entrypoint — importing it executes main()
// immediately (argv, possible process.exit) and it exports nothing to import selectively. A
// static source assertion that the guard clause exists is the simpler, side-effect-free form —
// and it is the one the mission brief explicitly prefers.
function test_runner_enforces_model_pin() {
  const src = readFileSync(RUNNER_PATH, "utf8");
  assert.match(
    src,
    /if\s*\(\s*!opts\s*\|\|\s*!opts\.model\s*\)\s*\{\s*[\s\S]{0,200}throw new Error/,
    "run-second-army.mjs must throw when opts.model is missing",
  );
  console.log(
    "PASS: run-second-army.mjs statically guards a throw on missing opts.model",
  );
}
...
const tests = [
  ...
  test_runner_enforces_model_pin,
