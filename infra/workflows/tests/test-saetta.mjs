...
const run = new AsyncFunction('args', 'agent', 'parallel', 'phase', 'log',
...
const tasks = [
  ...
  { key: 'consumer', brief: '/repo/brief-b.md', scope: ['b/'], dependsOn: ['upstream'] },
  { key: 'independent', brief: '/repo/brief-c.md', scope: ['c/'], dependsOn: [] },
];
const config = { mission: 'SAETTA-TEST', colour: 'BLUE', repo: '/repo',
  outputDir: '/repo/output/test', maxParallel: 2, tasks };
const built = () => ({ status: 'result', targets: [target] });
const gated = () => ({ verdict: 'PASS', targets: [target], posted: true,
  merged: true, reasons: [], receipt: '/repo/gate.json' });

async function simulate({ patchGate = {}, patchBuild = {}, patchRelease = {}, patchClose = {},
  fail = '', skipped = '', args = config } = {}) {
  const calls = [];
  let active = 0, peak = 0;
  const result = await run(args, async (prompt, options) => {
    ...
    if (options.label === 'dux:upstream') return { ...built(), ...patchBuild };
    if (options.label.startsWith('dux:')) return built();
    if (options.label === 'gate:upstream') return { ...gated(), ...patchGate };
    if (options.label.startsWith('gate:')) return gated();
    if (options.label.startsWith('release:')) return { verdict: 'PASS', targets: [target],
      live_receipt: '/repo/live.json', ...(options.label === 'release:upstream' ? patchRelease : {}) };
    if (options.label.startsWith('close:')) return { recorded: true,
      verdict: prompt.includes('"verdict":"BLOCK"') ? 'BLOCK' : 'PASS',
      receipt: '/repo/close.json', ...patchClose };
...
for (const [name, patchGate] of Object.entries({
  block: { verdict: 'BLOCK' }, missingReceipt: { posted: false },
  staleHead: { targets: [{ pr: 101, head: 'b'.repeat(40) }] },
  otherPR: { targets: [{ pr: 102, head: sha }] },
  unmerged: { merged: false }, emptyTargets: { targets: [] },
  missingFile: { receipt: '' }, nonBinary: { verdict: 'PASS-WITH-CONDITIONS' },
...
for (const status of ['needs_input', 'failed']) test(`Dux ${status} cannot trigger a gate or consumer`, async () => {
...
test('close failure is BLOCK even if all slices passed', async () => {
  assert.equal((await simulate({ fail: 'close:SAETTA-TEST' })).result.verdict, 'BLOCK');
});
...
for (const [name, patchRelease] of Object.entries({
  block: { verdict: 'BLOCK' }, staleHead: { targets: [{ pr: 101, head: 'b'.repeat(40) }] },
  otherPR: { targets: [{ pr: 102, head: sha }] }, relativeReceipt: { live_receipt: 'live.json' },
...
test('close cannot upgrade a scheduler BLOCK by reporting PASS', async () => {
  const { result } = await simulate({ patchGate: { verdict: 'BLOCK' }, patchClose: { verdict: 'PASS' } });
  assert.equal(result.close.verdict, 'PASS');
  assert.equal(result.verdict, 'BLOCK');
});

for (const patchClose of [{ recorded: false }, { verdict: 'BLOCK' }, { receipt: 'close.json' }])
...
for (const stage of ['dux:upstream', 'gate:upstream', 'release:upstream', 'close:SAETTA-TEST'])
...
test('capacity two is enforced with more than two initially ready slices', async () => {
  const extra = { key: 'fourth', brief: '/repo/d.md', scope: ['d/'], dependsOn: [] };
  const { result, peak, calls } = await simulate({ args: { ...config, tasks: [...tasks, extra] } });
  assert.equal(result.verdict, 'PASS');
...
test('unordered overlapping directory scopes are rejected before any write', async () => {
  await assert.rejects(simulate({ args: { ...config, tasks: [tasks[0],
    { ...tasks[2], scope: ['a/nested/file.py'] }] } }), /overlapping/);
});

test('overlap is allowed only when dependencies serialize both scopes', async () => {
  const ordered = [tasks[0], { ...tasks[1], scope: ['a/file.py'] },
    { ...tasks[2], scope: ['a/'], dependsOn: ['consumer'] }];
  assert.equal((await simulate({ args: { ...config, tasks: ordered } })).result.verdict, 'PASS');
});
...
for (const scope of ['apps/*/page.tsx', 'apps/?/page.tsx', 'apps/../private',
  'apps/./page.tsx', 'apps//page.tsx', 'apps/\tpage.tsx', 'apps/\u007fpage.tsx'])
  test(`unsafe scope ${JSON.stringify(scope)} is rejected before dispatch`, async () => {
    await assert.rejects(simulate({ args: { ...config,
      tasks: [{ ...tasks[0], scope: [scope] }] } }), /invalid/);
  });

test('frozen Kita pilot manifest completes scheduler validation with mocked PASS seats',
  { skip: !process.env.SAETTA_PILOT_MANIFEST }, async () => {
    const manifestPath = resolve(process.env.SAETTA_PILOT_MANIFEST);
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
    assert.equal(manifest.mission, 'KITA-TAX-LEADS');
    const args = { ...config, ...manifest, tasks: manifest.tasks.map(task => ({
      ...task, brief: resolve(dirname(manifestPath), task.brief),
    })) };
    const { result, calls, peak } = await simulate({ args });
    assert.equal(result.verdict, 'PASS');
    assert.equal(result.results.length, 4);
    assert.ok(peak <= 2);
    for (const task of manifest.tasks) for (const dep of task.dependsOn)
      assert.ok(calls.indexOf(`dux:${task.key}`) > calls.indexOf(`release:${dep}`));
  });

test('valid task names never collide with object prototypes', async () => {
  assert.equal((await simulate({ args: { ...config, tasks: [
    { ...tasks[0], key: 'constructor' }] } })).result.verdict, 'PASS');
});
...
test('same runner accepts host-resolved paths and serialized native args', async () => {
  for (const home of ['/Users/balizero', '/Users/nuzantara']) {
    const args = { ...config, repo: `${home}/nuzantara`, outputDir: `${home}/out` };
    assert.equal((await simulate({ args: JSON.stringify(args) })).result.verdict, 'PASS');
  }
});
