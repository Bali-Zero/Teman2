...
//
// USAGE: node infra/workflows/run-second-army.mjs --args-file <path-to-json>
//          [--script <path-to-workflow-script>] [--dry-run]
// --script defaults to second-army.js beside this file. It is a real flag, not
// decoration: the behavioural tests point the runner at fixture scripts, which is the
// only way to measure what the runner DOES with a lane it should refuse.
...
const DEFAULT_SCRIPT_PATH = path.resolve(__dirname, "second-army.js");
...
async function main() {
  ...
  if (scriptIdx !== -1 && !argv[scriptIdx + 1]) {
    log("usage: --script needs a path");
    process.exitCode = 1;
    return;
  }
  const scriptPath =
    scriptIdx === -1 ? DEFAULT_SCRIPT_PATH : path.resolve(argv[scriptIdx + 1]);
  ...
  try {
    ...
    log(`failed to read ${scriptPath}: ${e.message}`);
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    log(`failed to compile ${scriptPath}: ${e.message}`);
    process.exitCode = 1;
    return;
  }
