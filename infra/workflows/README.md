...
- **Floor-3 promotion**: `floor >= 3` PROMOTES the mission to the champions' chain of §1 —
  no build, verify or refute lane runs; only a report lane records the promotion. `deadTiers`
  still carries the four declared-dead seats (see below), unprobed.
- **Who builds** (`CHAIN[colour].builders`, and roster order IS fallthrough order): BLUE, under a
  ...
  ORANGE, under a Terra Dux, tries `haiku` → `flash` → `deepseek-flash` → `qwen-plus`. A seat of
  the Dux's OWN family is skipped outright unless it is `HAIKU_GRUNT_SEAT` — the Anthropic-native
  grunt seat, which sits LAST so it is reached only once every cross-family door has probed dead,
  and its use is logged, never hidden. Every build lane is `model:"haiku"`: either Haiku is the
  chosen builder, or Haiku is only the grunt shell that reaches the external door.
...
executes `second-army.js` OUTSIDE the Workflow tool — from a shell, a cron, or CI — by
injecting the same `args`/`agent`/`log`/`phase`/`parallel`/`pipeline`/`budget` bindings the
harness would otherwise supply, using the same `AsyncFunction` compile trick as
`infra/workflows/tests/test-second-army-contract.mjs`.

```
node infra/workflows/run-second-army.mjs --args-file <path-to-json> [--dry-run]
```

- Prints ONLY the script's JSON return value to stdout; `log()`/`phase()` and every diagnostic
  go to stderr, prefixed `[second-army]`.
- `--dry-run` swaps in a canned `agent()` that spawns nothing (deterministic per-label-prefix
  responses) — used by CI (`.github/workflows/second-army-contract.yml`) and for a human
  sanity-check, with no `claude`/`codex`/`agy` binary required.
- **Model-pin enforcement**: `agent()` throws immediately if `opts.model` is missing — the
  runner is itself an enforcer of the Builder Contract's "every `agent()` call pins `model:`"
  rule, independent of the contract test's static/dynamic checks.
- **The fleet-mail hook cure**: measured on M5 2026-09-15, `claude -p` inherits a SessionStart
  hook that injects fleet mail, and a small model then answers the mail instead of the prompt.
  The runner always passes `--output-format json` and, when a lane requests one, `--json-schema
  '<schema>'` — this makes the prompt dominant. It never uses `--bare` (that skips hooks but
  also breaks OAuth).
