...
def main() -> None:
    ...
    log.write_text("")
    capture = Path(__file__).with_name("native_child_probe.py")
    ...
    prompt = "Bounded native child probe, explicitly authorized delegation. Use Agent exactly once, subagent_type Explore and model haiku explicitly. Give only this assignment: Read fixture.txt with one Read tool, report its number. Read-only, no edits, no network, no replacements, no memories, expected result42. Wait for its result then independently Read fixture.txt yourself, compare42 and return CLAUDE_CHILD_PROBE_OK. No other work."
    env = {k: v for k, v in os.environ.items() if k not in ("ANTHROPIC_API_KEY", "SUBAGENT_STOP_VERIFY_OFF", "STOP_VERIFY_ALLOW_DIRTY", "CONTEXT_GUARD_OFF", "MANDATE_BUDGET_OFF")}
