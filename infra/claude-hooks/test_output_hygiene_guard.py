...
CASES: list[tuple[str, str, str]] = [
    ...
    ("npx vitest run", "BLOCK", "npx-prefixed runner is still a runner (gate 6724 r2)"),
    ("npx jest", "BLOCK", "npx jest (gate 6724 r2)"),
    ("pnpm exec vitest run", "BLOCK", "pnpm exec vitest"),
    ("npx vitest run 2>&1 | tail -20", "ALLOW", "npx vitest bounded by tail"),
    ("npx jest -q", "ALLOW", "npx jest -q"),
    ...
    ("git show -s --format=%H HEAD", "ALLOW", "git show -s prints no patch (gate 6724 r2)"),
    ("git show --no-patch --format=%cd HEAD", "ALLOW", "git show --no-patch (gate 6724 r2)"),
    ("git diff --quiet && echo clean", "ALLOW", "git diff --quiet prints nothing (gate 6724 r2)"),
    ("git diff --exit-code", "ALLOW", "git diff --exit-code prints nothing (gate 6724 r2)"),
    ("git diff --check", "ALLOW", "git diff --check reports only whitespace errors (gate 6724 r2)"),
    ("git show -s", "ALLOW", "bare -s is still no-patch"),
