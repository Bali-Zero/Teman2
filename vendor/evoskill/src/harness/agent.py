...
# always False at runtime). See vendor/evoskill/UPSTREAM.md.
...
class Agent(Generic[T]):
    ...
    async def _execute_query(self, query: str) -> list[Any]:
        ...
        if sdk == "claude":
            # Bali Zero Nuzantara vendor strip (CLAUDE.md hard rule):
            # claude executor physically removed from this fork. Use
            # DeepSeek harness instead (sdk="deepseek"). The check
            # remains here so a misconfigured run fails LOUDLY rather
            # than silently routing to the wrong executor. See
            # vendor/evoskill/UPSTREAM.md for the full rationale.
            raise ImportError(
        ...
        if sdk == "codex":
            # Bali Zero Nuzantara vendor strip: codex executor removed.
            # v2 Codex panel finding #2 — autonomous loops trigger
            # Cloudflare protections + ChatGPT Pro daily quota risk.
            # DeepSeek V4 Pro API used as cheap insurance ($0.10-0.30/run).
            raise ImportError(
