...
4. Shared components require a Phase 4 waiver to be promoted here. Current count: 19 (`BZLogo`, `NavShell`, `ThemeProvider`, `ProgressRing`, `DeadlineBadge`, `TrustBand`, `CTAHandoff`, `FunnelFrame`, `MatterCard`, `CommandPalette`, `Money`, `ListPageHeader`, `SearchBox`, `FilterBar`, `StatChips`, `SubNav`, `ContextPanel`, `WhatsAppFAB`, `FactBadge`). FactBadge (WS1, 2026-07-21): promoted under the Phase 4 waiver — rationale: one verifiable-facts badge (KBLI codes, citations, regulation codes) shared by every surface; semantic tokens only, funnel-agnostic. Granted with the WS1 merge.
