...
const QUESTIONS = [
  ...
  },
  {
    category: "Identity",
    query: "Who made you?",
    expected: ["Bali Zero", "team", "developers"],
  },
  {
    category: "Identity",
  ...
  {
    category: "RAG",
    query: "What is KBLI 56101?",
    expected: ["Restaurant", "56101"],
  },
  {
    category: "RAG",
  ...
  },
  {
    category: "Complex",
    query:
      "I am a digital nomad earning $3000/month. Can I live in Bali legally tax-free?",
  ...
  },
  {
    category: "Complex",
    query:
      "Compare the cost of VOA runs vs getting an E33G Digital Nomad Visa for 6 months.",
    expected: ["cost", "visa"],
  },
  ...
  // Edge
  {
    ...
    expected: ["costa", "PT PMA"],
  },
  {
    category: "Edge",
    query: "Berapa biaya visa investor?",
    expected: ["biaya", "investor"],
  },
  {
    category: "Edge",
...
test.describe("Zantara Full Stress Test (30 Qs)", () => {
  ...
  test("should pass all 30 questions in one session", async ({ page }) => {
    for (const [index, q] of QUESTIONS.entries()) {
      ...
      const passed = q.expected.some((k) =>
        lastText.toLowerCase().includes(k.toLowerCase()),
      );
      if (!passed)
        console.warn(`[FAIL] Q${index + 1} Expected one of: ${q.expected}`);
