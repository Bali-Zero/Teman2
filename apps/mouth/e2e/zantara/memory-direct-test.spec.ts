...
async function saveConversation(
  ...
  const response = await fetch(
    `${TEST_CONFIG.apiUrl}/api/bali-zero/conversations/save`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to save conversation: ${response.status} - ${error}`,
    );
  }
...
async function getConversationHistory(
  ...
  if (!response.ok) {
    const error = await response.text();
    throw new Error(
      `Failed to get conversation history: ${response.status} - ${error}`,
    );
  }
...
test.describe("Direct Memory Test", () => {
  ...
  test("should save and retrieve conversation correctly", async ({ page }) => {
    ...
    console.log(
      `[TEST] Step 1: Saving conversation with ${messages1.length} messages`,
    );
    ...
    console.log(
      `[TEST] Step 3: Saving updated conversation with ${messages2.length} messages`,
    );
  ...
  test("should extract entities from conversation history", async ({
    page,
  }) => {
