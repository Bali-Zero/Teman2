...
async function sendMessageAndWaitForResponse(
  ...
  const input = page
    .locator(
      'textarea[placeholder*="message"], input[placeholder*="message"], textarea',
    )
    .first();
  ...
  console.log(
    `[TEST] AI Response (${response.length} chars): "${response.substring(0, 300)}..."`,
  );
...
test.describe("ZANTARA AI Capabilities", () => {
  ...
  test.describe("Recognition", () => {
    ...
    test("should recognize team members - Italian speakers", async ({
      page,
    }) => {
      const response = await sendMessageAndWaitForResponse(
        page,
        "Chi nel team parla italiano?",
      );
  ...
  test.describe("Memory", () => {
    test("should remember information within conversation", async ({
      page,
    }) => {
      ...
      await page.waitForTimeout(TEST_CONFIG.shortWait);
      const nameResponse = await sendMessageAndWaitForResponse(
        page,
        "Come mi chiamo?",
      );
      console.log(
        "[TEST] Name query response:",
        nameResponse.substring(0, 200),
      );
      ...
      const cityResponse = await sendMessageAndWaitForResponse(
        page,
        "Di quale città sono?",
      );
      console.log(
        "[TEST] City query response:",
        cityResponse.substring(0, 500),
      );
      ...
      if (!cityFound) {
        console.error(
          "[TEST] City not found in response. Full response:",
          cityResponse,
        );
      }
    ...
    test("should recall specific details mentioned earlier", async ({
      page,
    }) => {
      // Provide specific detail
      await sendMessageAndWaitForResponse(
        page,
        "Il mio budget è di 50 milioni di rupie",
      );
      await page.waitForTimeout(TEST_CONFIG.shortWait);
      ...
      expect(
        responseContains(response, ["50", "milioni", "budget", "rupie"]),
      ).toBeTruthy();
    });
  });
  ...
  test.describe("Logic & Reasoning", () => {
    test("should provide logical deductions", async ({ page }) => {
      ...
      const lowerResponse = response.toLowerCase();
      expect(lowerResponse).not.toMatch(
        /^okay,?\s*(since|with|given|without|lacking)/,
      );
      expect(lowerResponse).not.toContain("thought:");
      expect(lowerResponse).not.toContain("observation:");
      expect(lowerResponse).not.toContain(
    ...
    test("should handle conditional reasoning", async ({ page }) => {
      ...
      if (
        ...
        console.warn(
          "[TEST] Backend error detected, skipping content validation",
        );
        return;
      }
  ...
  test.describe("Creativity", () => {
    ...
    test("should adapt explanations to context", async ({ page }) => {
      ...
      expect(
        responseContains(response1, ["permesso", "stare", "Indonesia"]),
      ).toBeTruthy();
    });
  });
  ...
  test.describe("Communication", () => {
    test("should respond in the same language as the question", async ({
      page,
    }) => {
      // Italian question
      const responseIt = await sendMessageAndWaitForResponse(
        page,
        "Ciao, come stai?",
      );
    ...
    test("should provide clear step-by-step instructions when needed", async ({
      page,
    }) => {
      const response = await sendMessageAndWaitForResponse(
        page,
      ...
      expect(response.match(/[1-9][.)]/g)?.length || 0).toBeGreaterThanOrEqual(
        2,
      );
    });
  });
  ...
  test.describe("Affinity", () => {
    ...
    test("should remember user preferences within session", async ({
      page,
    }) => {
      // Express preference
      await sendMessageAndWaitForResponse(
    ...
    test("should adapt to user expertise level", async ({ page }) => {
      ...
      await sendMessageAndWaitForResponse(
        page,
        "Sono un avvocato specializzato in immigrazione",
      );
      await page.waitForTimeout(TEST_CONFIG.shortWait);
  ...
  test.describe("Sentiment", () => {
    test("should respond empathetically to frustration", async ({ page }) => {
      ...
      const lowerResponse = response.toLowerCase();
      expect(lowerResponse).not.toMatch(
        /^okay,?\s*(based|since|with|given|without|lacking)/,
      );
  ...
  test.describe("Long Context", () => {
    test("should maintain coherence across 10+ turns", async ({ page }) => {
      ...
      const conversation = [
        ...
        {
          user: "Voglio aprire un bar a Bali",
          expectContains: ["bar", "bali", "ristora"],
        },
        {
          user: "Il mio budget è 200 milioni IDR",
          expectContains: ["budget", "200", "milioni"],
        },
        {
          user: "Ho già un socio indonesiano",
          expectContains: ["socio", "partner", "indonesia"],
        },
        {
          user: "Lui ha esperienza nel settore",
          expectContains: ["esperienza", "settore"],
        },
        {
      ...
      const finalContainsExpected = conversation[
        conversation.length - 1
      ].expectContains.filter((kw) =>
        lastResponse.toLowerCase().includes(kw.toLowerCase()),
      );
  ...
  test.describe("Edge Cases", () => {
    ...
    test("should stay on topic - refuse off-topic requests", async ({
      page,
    }) => {
      const response = await sendMessageAndWaitForResponse(
        page,
        "Scrivi un poema sulla luna",
      );
  ...
  test.describe("Advanced Interactions", () => {
    test("should handle contradictory information updates", async ({
      page,
    }) => {
      // 1. Establish initial context
      await sendMessageAndWaitForResponse(
        page,
        "Voglio aprire una gelateria a Canggu.",
      );
      await page.waitForTimeout(TEST_CONFIG.shortWait);
    ...
    test("should handle tool interruption and return to flow", async ({
      page,
    }) => {
      // 1. Start business conversation
      await sendMessageAndWaitForResponse(
        page,
        "Parliamo di tasse per il mio PT PMA.",
      );
      await page.waitForTimeout(TEST_CONFIG.shortWait);
      ...
      expect(
        responseContains(returnResponse, [
          "22%",
          "tassa",
          "corporate",
          "profit",
