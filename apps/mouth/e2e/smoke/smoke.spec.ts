...
test.describe("Smoke Tests - Critical Path", () => {
  test("1. Login page loads", async ({ page }) => {
    ...
    await expect(
      page.locator('input[name="pin"], input[type="password"]').first(),
    ).toBeVisible();
  ...
  test("2. Login with valid credentials", async ({ page }) => {
    ...
    await page
      ...
      .fill(CONFIG.email);
    await page
      .locator('input[name="pin"], input[type="password"]')
      .first()
      .fill(CONFIG.pin);
...
test.describe("Smoke Tests - RAG & Memory", () => {
  test("RAG returns sources for legal question", async ({ page }) => {
    ...
    const input = page
      ...
      .first();
    await input.fill(
      "Quali documenti servono per aprire una PT PMA in Indonesia?",
    );
    ...
    await page.screenshot({
      path: "test-results/rag-response.png",
      fullPage: true,
    });
  });
  ...
  test("Memory persists user information", async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({
      path: "test-results/memory-response.png",
      fullPage: true,
    });
