...
test.describe("Company & Licenses Page", () => {
  ...
  test("should load company-licenses page correctly", async ({ page }) => {
    ...
    // Check description
    await expect(
      page.locator("text=Choose between Company setup guides"),
    ).toBeVisible();

    // Check back button
    const backButton = page.locator(
      'button:has-text("Back to Knowledge Base")',
    );
    ...
    const companyButton = page
      .locator('h2:has-text("Company")')
      .locator("..")
      .locator("..");
    await expect(
      companyButton.locator('p:has-text("KBLI Blueprints")'),
    ).toBeVisible();

    const licensesButton = page
      .locator('h2:has-text("Licenses")')
      .locator("..")
      .locator("..");
    await expect(
      licensesButton.locator('p:has-text("Business Licenses")'),
    ).toBeVisible();
  });
  ...
  test("should navigate to Company (KBLI Blueprints) page", async ({
    page,
  }) => {
    // Wait for page to be fully loaded
    await page
  ...
  test("should display correct Company button content", async ({ page }) => {
    ...
    await expect(
      companyButton.locator('span:has-text("View KBLI Blueprints")'),
    ).toBeVisible();
  });
  ...
  test("should display correct Licenses button content", async ({ page }) => {
    ...
    await expect(
      licensesButton.locator('span:has-text("View Licenses Guide")'),
    ).toBeVisible();
  });
