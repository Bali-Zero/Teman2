...
test.describe("Knowledge Base Main Page", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/search/", async (route) => {
      ...
      if (
        queryLower.includes("xyzabc123nonexistent") ||
        queryLower.includes("nonexistent")
      ) {
  ...
  test("should load knowledge page correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('main h1:has-text("Knowledge Base")'),
    ).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should display category cards with correct information", async ({
    ...
    const companyCard = page
      .locator('h3:has-text("Company & Licenses")')
      .locator("..");
    await expect(companyCard).toBeVisible();
    await expect(
      companyCard.locator('p:has-text("Company & Licenses")'),
    ).toBeVisible();
    ...
    const journeyCard = page
      .locator('h3:has-text("Our Journey")')
      .locator("..");
  ...
  test("should navigate to Company & Licenses page", async ({ page }) => {
    ...
    await page.waitForURL(/.*\/knowledge\/company-licenses/, {
      timeout: 10000,
    });
  ...
  test("should display info box with correct text", async ({ page }) => {
    const infoBox = page.locator(
      "text=The Knowledge Base contains all company documents",
    );
