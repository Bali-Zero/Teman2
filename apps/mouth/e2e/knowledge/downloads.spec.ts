...
test.describe("Knowledge Base Downloads", () => {
  ...
  test("should have download buttons on blueprints page", async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(buttonCount, 10); i++) {
      ...
      if (
        hasSvg &&
        (buttonText?.includes("EN") ||
          buttonText?.includes("ID") ||
          buttonText?.includes("Bisnis") ||
          buttonText?.includes("Teknis"))
      ) {
  ...
  test("should download PDF from blueprints without Google Drive redirect", async ({
    page,
    context,
  }) => {
  ...
  test("should have download buttons on KITAS detail pages", async ({
    page,
  }) => {
    ...
    if ((await firstVisaCard.count()) > 0) {
      ...
      const downloadButton = page
        .locator("button")
        .filter({ hasText: /Download.*PDF|PDF.*Download/i });
  ...
  test("should not have window.open calls to Google Drive", async ({
    page,
  }) => {
    // Navigate to blueprints page
    await page.goto("/knowledge/blueprints");
  ...
  test("should handle download when PDF URL is missing gracefully", async ({
    page,
  }) => {
    // Navigate to blueprints page
    await page.goto("/knowledge/blueprints");
