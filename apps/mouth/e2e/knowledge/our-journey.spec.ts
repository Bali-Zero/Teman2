...
test.describe("Our Journey Page", () => {
  ...
  test("should load our-journey page correctly", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=This section will showcase our journey and milestones",
      ),
    ).toBeVisible();
  ...
  test("should display empty state with coming soon message", async ({
    page,
  }) => {
    // Wait for page to be fully loaded
    await page
    ...
    await expect(page.locator('h3:has-text("Coming Soon")')).toBeVisible({
      timeout: 10000,
    });

    // Check for empty state message
    await expect(
      page.locator("text=This section is being prepared"),
    ).toBeVisible();
    await expect(
      page.locator("text=Check back soon for updates"),
    ).toBeVisible();
  });
