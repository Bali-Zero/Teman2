...
test.describe("Property Landing Page", () => {
  ...
  test("renders 3 service cards", async ({ page }) => {
    await expect(
      page.locator('h3:has-text("Leasehold Advisory")'),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Freehold Structuring")'),
    ).toBeVisible();
    await expect(
      page.locator('h3:has-text("Investment Analysis")'),
    ).toBeVisible();
  });

  test("has WhatsApp CTA link", async ({ page }) => {
    const whatsappLinks = page.locator('a[href*="wa.me/6282230102328"]');
  ...
  test("has Prime Intelligence section", async ({ page }) => {
    await expect(
      page.locator("text=Powered by Prime Intelligence"),
    ).toBeVisible();
