...
test.describe("Intelligence Center - Complete Workflow", () => {
  ...
  test("Visa Oracle: View Content button opens preview, Hide Preview closes it", async ({
    ...
    await expect(
      page.getByText("This is the full content preview for testing."),
    ).toBeVisible();
  ...
  test("Visa Oracle: Approve button shows confirmation, then removes item on confirm", async ({
    ...
    page.once("dialog", async (dialog) => {
      expect(dialog.message()).toContain(
        "This will ingest the content into the Knowledge Base",
      );
      await dialog.accept();
    });
    ...
    const approveButton = page
      .getByRole("button", { name: /Approve & Ingest/i })
      .first();
    await approveButton.click();

    // Verify item is removed after approval
    await expect(page.getByText("Test Visa Regulation")).not.toBeVisible({
      timeout: 5000,
    });
  ...
  test("Visa Oracle: Cancel on confirmation dialog keeps item", async ({
    page,
  }) => {
    await expect(page.getByText("Test Visa Regulation")).toBeVisible();
  ...
  test("News Room: Sync Sources button reloads news items", async ({
    page,
  }) => {
    ...
    // Verify new data loaded
    await expect(page.getByText(/News Article 2/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("System Pulse: Error state shows Retry button, retry reloads metrics", async ({
    page,
  }) => {
    // Navigate to System Pulse
    await page.getByRole("link", { name: /System Pulse/i }).click();
  ...
  test('All pages: Header always shows "Agent Active" indicator', async ({
    page,
  }) => {
