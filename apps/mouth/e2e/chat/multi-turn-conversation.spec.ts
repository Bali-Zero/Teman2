...
test.describe("Multi-Turn Conversation Handling", () => {
  ...
  test("should handle 10+ turn conversation without input getting disabled", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      await verifyInputEnabled(
        page,
        `Turn ${turnNumber}: Input should be enabled before sending`,
      );
      ...
      await verifyInputEnabled(
        page,
        `Turn ${turnNumber}: Input should be enabled after response`,
      );
    ...
    await verifyInputEnabled(
      page,
      "Final check: Input should be enabled after all turns",
    );
  ...
  test("should reset isLoading state even if streaming fails silently", async ({
    page,
  }) => {
    let requestCount = 0;
  ...
  test("should handle rate limiting gracefully without blocking conversation", async ({
    page,
  }) => {
    let requestCount = 0;
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      // Attendi che il messaggio appaia
      await expect(page.locator(`text="Messaggio ${i}"`)).toBeVisible({
        timeout: 5000,
      });
      ...
      if (i <= 5) {
        ...
        await verifyInputEnabled(
          page,
          `After rate limit error for message ${i}`,
        );
      }
  ...
  test("should maintain conversation context across multiple turns", async ({
    ...
    const lastResponse = await lastAssistantMessage
      .textContent()
      .catch(() => "");
  ...
  test("should handle rapid successive messages without blocking", async ({
    page,
  }) => {
    ...
    for (let i = 1; i <= 5; i++) {
      await expect(page.locator(`text="Messaggio rapido ${i}"`)).toBeVisible({
        timeout: 10000,
      });
    }
  });
  ...
  test("should handle timeout errors gracefully", async ({ page }) => {
    ...
    // Attendi che il messaggio appaia
    await expect(page.locator('text="Messaggio con timeout"')).toBeVisible({
      timeout: 5000,
    });
    ...
    await sendButton.click();
    await expect(page.locator('text="Messaggio dopo timeout"')).toBeVisible({
      timeout: 5000,
    });
