...
test.describe("Chat Flow", () => {
  ...
  test("should send text message and receive streaming response", async ({
    page,
  }) => {
    // Mock Stream
  ...
  test("should generate image", async ({ page }) => {
    ...
    await expect(
      page.locator('textarea[placeholder="Describe your image..."]'),
    ).toBeVisible();
    ...
    // Verify text first
    await expect(page.getByText("Here is your generated image:")).toBeVisible({
      timeout: 10000,
    });

    // Verify image
    // The alt text in MessageBubble is "Generated content"
    await expect(page.locator('img[alt="Generated content"]')).toBeVisible({
      timeout: 10000,
    });
  });
});
