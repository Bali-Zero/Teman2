...
test.describe("Chat Flow", () => {
  ...
  test("should display AI response", async ({ page }) => {
    ...
    await expect(page.locator("text=/PT PMA|passaggi|azienda/i")).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should show loading state while waiting for AI response", async ({
    page,
  }) => {
    ...
    const loadingIndicator = page
      .locator("text=/loading|sending|waiting/i")
      .first();
