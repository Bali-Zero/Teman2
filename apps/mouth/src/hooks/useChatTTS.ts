      ...
      try {
        ...
        audio.onended = () => {
          if (audioUrlRef.current === audioUrl) {
            ...
            trackEvent(
              'chat_tts_completed',
              { messageId, duration: ttsDuration },
              userProfile?.email
            );
        ...
        audio.onerror = (e) => {
          ...
          if (audioUrlRef.current === audioUrl) {
            ...
            trackEvent(
              'chat_tts_error',
              { messageId, errorType: 'playback', errorMessage },
              userProfile?.email
            );
        ...
        try {
          ...
          trackEvent(
            'chat_tts_started',
            { messageId, textLength: text.length },
            userProfile?.email
          );
        } catch (playError) {
          const errorMessage =
          ...
          logger.error(
            'TTS audio play error',
            {
              component: 'useChatTTS',
              action: 'handleTTS',
              metadata: {
                messageId,
                errorType: playError instanceof Error ? playError.name : 'Unknown',
              },
            },
          ...
          trackEvent(
            'chat_tts_error',
            { messageId, errorType: 'play', errorMessage },
            userProfile?.email
          );
