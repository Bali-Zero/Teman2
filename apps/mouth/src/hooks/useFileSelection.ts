...
interface UseFileSelectionReturn {
  ...
  handleSelect: (file: FileItem, index: number, event: React.MouseEvent | React.KeyboardEvent) => void;
...
export function useFileSelection({
  ...
  const notifyChange = useCallback((newIds: Set<string>) => {
  ...
  const selectSingle = useCallback((file: FileItem) => {
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      ...
      case ' ':
        event.preventDefault();
        if (files[currentIndex]) {
          onSelect(files[currentIndex], currentIndex, { ...event, ctrlKey: true } as React.KeyboardEvent);
        }
        return;
