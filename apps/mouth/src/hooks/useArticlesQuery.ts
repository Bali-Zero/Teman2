...
export function useArticlesQuery(filters: ArticleFilters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetchApi<PaginatedArticles>(`/api/articles?${queryString}`);
 ...
 */
export function useNewsFeedQuery(type: 'latest' | 'trending' | 'verified' = 'latest') {
  return useQuery({
    queryKey: articleKeys.feed(type),
    queryFn: async () => {
      const response = await fetchApi<Article[]>(`/api/intelligence/feed?type=${type}`);
...
export function useArticleMutation() {
  ...
  return useMutation({
    mutationFn: async (article: Partial<Article>) => {
      ...
      if (!response.data) throw new Error(response.error || 'Failed to save article');
      return response.data;
    },
    onSuccess: (data) => {
...
export function usePublishArticleMutation() {
  ...
  return useMutation({
    mutationFn: async ({ id, publish }: { id: number; publish: boolean }) => {
      ...
      if (!response.data) throw new Error(response.error || 'Failed to publish article');
      return response.data;
    },
    // Optimistic update
    onMutate: async ({ id, publish }) => {
      ...
      const previousArticle = queryClient.getQueryData<Article>(articleKeys.detail(id));
    ...
    onError: (err, variables, context) => {
      if (context?.previousArticle) {
        queryClient.setQueryData(articleKeys.detail(variables.id), context.previousArticle);
      }
    },
