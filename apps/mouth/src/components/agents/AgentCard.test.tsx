...
describe("AgentCard", () => {
  ...
  describe("Status Indicators", () => {
    it("displays green indicator for running status", () => {
      const { container } = render(
        <AgentCard {...defaultProps} status="running" />,
      );
    ...
    it("displays yellow indicator for idle status", () => {
      const { container } = render(
        <AgentCard {...defaultProps} status="idle" />,
      );
    ...
    it("displays red indicator for error status", () => {
      const { container } = render(
        <AgentCard {...defaultProps} status="error" />,
      );
  ...
  describe("User Interactions", () => {
    ...
    it("renders Details button when onDetails is provided", () => {
      const mockOnDetails = vi.fn();
      render(<AgentCard {...defaultProps} onDetails={mockOnDetails} />);

      const detailsButton = screen.getByLabelText("Details");
      expect(detailsButton).toBeInTheDocument();
    });

    it("does not render Details button when onDetails is not provided", () => {
      render(<AgentCard {...defaultProps} />);
  ...
  describe("Accessibility", () => {
    it("has proper aria-label for Details button when onDetails is provided", () => {
      const mockOnDetails = vi.fn();
      render(<AgentCard {...defaultProps} onDetails={mockOnDetails} />);
