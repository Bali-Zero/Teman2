...
function MessageBubbleComponent({
  ...
  const handleCopy = async () => {
    // Prefer the rendered text (markdown stripped) so users get clean output
    // instead of raw `**bold**` syntax. Falls back to the raw content when the
    // ref hasn't mounted yet (e.g. during the typewriter animation on the very
    // first paint) — copying source markdown is still better than nothing.
    const rendered = bubbleContentRef.current?.innerText?.trim();
    const text = rendered && rendered.length > 0 ? rendered : content;
 ...
 * streaming. Walks `steps` once, pairs each `tool_call`/`tool_start` with the
 * next `observation`/`tool_end`, and emits one chip per unique invocation.
 * Unknown tools are hidden — they remain available in the "Thinking Process"
