    ...
    async (enabled: boolean) => {
      ...
      try {
        ...
        for (const feature of features) {
          ...
          const color: string = feature.properties?.color ?? "#6B7280";
          if (!geom) continue;
        ...
        logger.info(
          `[Prime] Zone layer: ${zonePolygonsRef.current.length} polygons drawn`,
        );
      } catch (e) {
        logger.error(
        ...
        <div className="absolute bottom-6 right-6 z-20 flex flex-col items-end gap-2">
          ...
          {showFilters && (
            ...
            >
              ...
              <LayerToggle
                icon="🌾"
                label="Protected farmland"
                enabled={layers.lp2b}
                onChange={(v) => handleLayerChange("lp2b", v)}
                disabled
              />
              <LayerToggle
                icon="🌊"
                label="Tsunami risk"
                enabled={layers.tsunami}
                onChange={(v) => handleLayerChange("tsunami", v)}
                disabled
              />
            </div>
          )}
