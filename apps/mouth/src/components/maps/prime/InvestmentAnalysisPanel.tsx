...
function TrendBadge({ zoneCode }: { zoneCode: string }) {
  const { data } = useQuery<PredictData>({
    ...
    queryFn: () =>
      fetch(
        `/api/prime/v2/predict?zone_code=${encodeURIComponent(zoneCode)}`,
      ).then((r) => r.json()),
    enabled: !!zoneCode,
...
function DensitySection({ zoneCode }: { zoneCode: string }) {
  const { data, isLoading: loading } = useQuery<DensityData>({
    ...
    queryFn: () =>
      fetch(
        `/api/prime/v2/density?zone_code=${encodeURIComponent(zoneCode)}`,
      ).then((r) => r.json()),
    enabled: !!zoneCode,
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${satColor}`}
        >
      ...
      <div className="text-xs text-slate-400">
        ...
        {data.total_companies === 1 ? "company" : "companies"} in zone
      </div>
          ...
          return (
            <div key={sector} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full bg-accent-warm/70"
                  style={{ width: `${pct}%` }}
                />
              </div>
  ...
  return (
    <div className="px-4 py-3 space-y-3 border-b border-white/5">
      ...
      {analysis.zone?.zone_code ? (
        <TrendBadge zoneCode={String(analysis.zone.zone_code)} />
      ) : null}
      ...
      {showDealFlow && (
        <DealFlowWizard onClose={() => setShowDealFlow(false)} />
      )}
    </div>
  );
}
