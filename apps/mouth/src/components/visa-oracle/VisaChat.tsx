...
const FALLBACK_WA_URL = 'https://wa.me/6281338051876?text=Hi%2C%20I%20used%20Visa%20Oracle.';
...
export function VisaChat({ quizAnswers, initialSessionId, visas }: VisaChatProps) {
  ...
  const [remaining, setRemaining] = useState<number>(() => getRemainingQuestions());
  const [whatsappUrl, setWhatsappUrl] = useState<string | null>(null);
  const [showCTA, setShowCTA] = useState(false);

  const sessionIdRef = useRef<string>(initialSessionId ?? getSession().sessionId);
  ...
  async function handleSend() {
    ...
    try {
      const response = await sendChatMessage(sessionIdRef.current, trimmed, quizAnswers, messages);
    ...
    >
      ...
      >
        <input
          ...
          placeholder={remaining > 0 ? 'Ask about Indonesian visas...' : 'No questions remaining'}
