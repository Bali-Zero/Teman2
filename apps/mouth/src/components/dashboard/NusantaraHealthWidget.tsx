...
const ISLAND_PATHS: Record<
  string,
  { path: string; viewBox: string; scale: number }
> = {
  sumatra: {
...
export function NusantaraHealthWidget({
  className = "",
}: NusantaraHealthWidgetProps) {
  ...
  const fetchHealth = useCallback(async () => {
    try {
      ...
      logger.error("Failed to fetch Nusantara health", {}, err as Error);
  ...
  if (error && !health) {
    return (
      <div
        className={`rounded-lg border border-red-500/20 bg-red-500/10 p-6 ${className}`}
      >
        ...
        <div className="flex items-center justify-center gap-4 p-2 bg-black/30 border-t border-white/5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-white/60">
              {health.status_counts.healthy} healthy
            </span>
          </div>
          {health.status_counts.warning > 0 && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-white/60">
                {health.status_counts.warning} warning
              </span>
            </div>
          )}
