...
function parseRisk(category: string): { label: string; color: string } {
  ...
  if (lower === "tinggi" || (lower.includes("tinggi") && !lower.includes("rendah") && !lower.includes("menengah")))
    return { label: "High", color: "var(--kbli-risk-high, #ef4444)" };
  if (lower.includes("menengah") && lower.includes("tinggi"))
    return { label: "Medium-High", color: "var(--kbli-risk-medium-high, #f59e0b)" };
  if (lower.includes("menengah") && lower.includes("rendah"))
    return { label: "Medium-Low", color: "var(--kbli-risk-medium-low, #3b82f6)" };
