...
/**
 * WS2 kita slice 7 — admin explorers drain guard.
 *
 * Pins the token drain of the DbExplorer / VectorExplorer shared admin
 * components: no raw hex, no raw status-rgba tuples, no Tailwind palette
 * utilities (zinc surfaces + blue/purple accents -> --bz-bg-* /
 * --bz-chart-1 / --bz-neon-purple), panels read the dashboard recipe +
 * --bz-border, mirroring scripts/token_lint.py.
 */

const FILES = [
  join(__dirname, "DbExplorer.tsx"),
  join(__dirname, "VectorExplorer.tsx"),
];
...
describe("admin explorers drain guard (WS2 slice 7)", () => {
  for (const file of FILES) {
    ...
    it(`${name} carries no Tailwind palette utilities (status + neutral)`, () => {
      for (const line of codeLines(file)) {
  ...
  it("DbExplorer accents read --bz-chart-1", () => {
    expect(readFileSync(FILES[0], "utf8")).toContain("var(--bz-chart-1)");
  });

  it("VectorExplorer accents read --bz-neon-purple", () => {
    expect(readFileSync(FILES[1], "utf8")).toContain("var(--bz-neon-purple)");
  });
});
