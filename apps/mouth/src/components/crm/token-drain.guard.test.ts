import { describe, expect, it } from "vitest";
import { readFileSync } from "node:fs";
import { join } from "node:path";

/**
 * WS2 kita slice 7 — TaxCompanyPilotWorkspace drain guard.
 *
 * Pins the token drain of the shared CRM tax-pilot workspace (was a full
 * light surface — white cards, slate/emerald/red palette utilities,
 * bg-[#f4f6f1] — rendering inside the dark operative clients/tax-pilot
 * page): no raw hex colors, no raw status-rgba tuples, no Tailwind
 * palette utilities (status + neutral families). Severity reads
 * --state-* color-mix tints; panels read the dashboard recipe +
 * --bz-border, mirroring scripts/token_lint.py.
 */

const COMPONENT = join(__dirname, "TaxCompanyPilotWorkspace.tsx");

// token_lint.py HEX_RE — 3/4/6/8 digits, word-boundary aware.
const HEX_RE =
  /(?<![0-9A-Za-z#&])#(?:[0-9a-fA-F]{8}|[0-9a-fA-F]{6}|[0-9a-fA-F]{4}|[0-9a-fA-F]{3})(?![0-9A-Za-z])/;

// Raw status/accent rgba tuples the drain replaced with --state-* tints.
const STATUS_RGBA_TUPLES = [
  "rgba(239,68,68", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(244,63,94", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(16,185,129", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(5,150,105", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(52,211,153", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(34,197,94", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(251,191,36", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(245,158,11", // token-lint-ok: drain-guard assertion string, not a color use
  "rgba(14,165,233", // token-lint-ok: drain-guard assertion string, not a color use
];

// Status AND neutral families (slices 4-6 extended regex).
const PALETTE_UTIL_RE =
  /\b(?:bg|text|border|ring|divide|fill|stroke|from|to|via)-(?:red|green|emerald|amber|rose|blue|sky|cyan|purple|violet|indigo|yellow|orange|teal|fuchsia|pink|lime|zinc|gray|slate|neutral|stone)-\d/;

/** Code lines only: drops full-line comments and token-lint-ok one-offs. */
function codeLines(path: string): string[] {
  return readFileSync(path, "utf8")
    .split("\n")
    .filter((l) => {
      const t = l.trimStart();
      if (t.startsWith("//") || t.startsWith("*") || t.startsWith("/*")) {
        return false;
      }
      return !l.includes("token-lint-ok:");
    });
}

describe("TaxCompanyPilotWorkspace drain guard (WS2 slice 7)", () => {
  it("carries no unmarked raw hex colors", () => {
    for (const line of codeLines(COMPONENT)) {
      expect(HEX_RE.test(line), `hex in line: ${line.trim()}`).toBe(false);
    }
  });

  it("carries no raw status/accent rgba tuples", () => {
    const src = codeLines(COMPONENT).join("\n");
    for (const tuple of STATUS_RGBA_TUPLES) {
      expect(src.includes(tuple), `found ${tuple}`).toBe(false);
    }
  });

  it("carries no Tailwind palette utilities (status + neutral)", () => {
    for (const line of codeLines(COMPONENT)) {
      expect(PALETTE_UTIL_RE.test(line), `palette util: ${line.trim()}`).toBe(
        false,
      );
    }
  });

  it("carries no light-surface remnants (white/light panels)", () => {
    const src = codeLines(COMPONENT).join("\n");
    for (const remnant of ["bg-white", "bg-slate-50", "bg-emerald-50"]) {
      expect(src.includes(remnant), `light remnant ${remnant}`).toBe(false);
    }
  });

  it("severity map reads --state-* color-mix tints", () => {
    const src = readFileSync(COMPONENT, "utf8");
    expect(src).toContain("var(--state-danger)");
    expect(src).toContain("var(--state-warning)");
    expect(src).toContain("var(--state-info)");
    expect(src).toContain("color-mix(in_srgb,var(--state-danger)_10%");
  });

  it("panels read the dashboard recipe + --bz-border", () => {
    const src = readFileSync(COMPONENT, "utf8");
    expect(src).toContain("rgba(35,35,40,0.65)");
    expect(src).toContain("var(--bz-border)");
  });
});
