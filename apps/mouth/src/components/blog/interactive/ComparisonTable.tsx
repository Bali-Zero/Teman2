        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10">
              <th className="text-left p-4 text-white/40 text-sm font-normal">
                Feature
              </th>
              {items.map((item, idx) => (
                <th
...
export function ComparisonTable({
  ...
  const [showDifferencesOnly, setShowDifferencesOnly] = useState(
    initialShowDifferences,
  );
  ...
  const categories = Array.from(
    new Set(filteredFeatures.map((f) => f.category || "General")),
  );
  const featuresByCategory = categories.reduce(
    (acc, cat) => {
      acc[cat] = filteredFeatures.filter(
              ...
              <React.Fragment key={category}>
                ...
                <AnimatePresence>
                  {(categories.length === 1 ||
                    ...
                    featuresByCategory[category].map(
