---
title: "The Silent Earthquake: Indonesia's FDI Overhaul Under BKPM Regulation No. 5 of 2025"
slug: "bkpm-regulation-5-2025-fdi"
description: "BKPM Regulation 5/2025 slashes PT PMA paid-up capital from IDR 10B to IDR 2.5B. But the real revolution is how 'qualified expenditure' now includes your burn rate. Complete analysis inside."
excerpt: "PT PMA paid-up capital drops 75% to IDR 2.5 billion. But the KITAS visa still requires IDR 10B. Here's the full picture of Indonesia's FDI reform — traps included."
category: "business"
tags:
  - PT PMA
  - BKPM regulation
  - FDI Indonesia
  - paid-up capital
  - KITAS visa
  - capital reform 2025
  - qualified expenditure
  - LKPM reporting
  - investment requirements
  - foreign investment reform
publishedAt: "2026-02-12"
coverImage: "/static/insights/business/bkpm-regulation-5-2025-fdi.jpg"
updatedAt: "2026-02-12"
author:
  name: "Zantara AI"
  avatar: "/static/zantara-avatar.png"
  role: "AI Business Advisor"
featured: false
image:
  src: "/static/insights/business/bkpm-fdi-regulation.jpg"
  alt: "Indonesia FDI reform under BKPM Regulation 5/2025"
seo:
  title: "BKPM Regulation 5/2025 | PT PMA Capital Reform & FDI Guide"
  description: "Complete analysis of BKPM Regulation 5/2025: PT PMA capital reduced to IDR 2.5B, qualified expenditure redefined, KITAS visa disconnect, and LKPM reporting traps."
  keywords:
    - BKPM regulation 5 2025
    - PT PMA capital requirement
    - FDI Indonesia 2025
    - KITAS investor visa
    - LKPM reporting
readingTime: 17
difficulty: "advanced"
aiGenerated: true
kbSources:
  - collection: "legal_unified"
    documentsUsed: 14
lastKbSync: "2026-02-12T10:00:00Z"
relatedArticles:
  - "pt-pma-registration-guide"
  - "pt-pma-capital-requirements"
  - "kbli-2025-gold-rush-migration"
tableOfContents: true
seoTitle: "BKPM Regulation 5/2025 | PT PMA Capital Reform Guide"
seoDescription: "BKPM Regulation 5/2025 cuts PT PMA paid-up capital to IDR 2.5B. Qualified expenditure redefined. KITAS visa disconnect and LKPM traps explained."
aiOptimization:
  primaryQuestion: "How does BKPM Regulation 5/2025 change PT PMA capital requirements?"
  answerSnippet: "BKPM Regulation 5/2025 slashes PT PMA paid-up capital from IDR 10B to IDR 2.5B and redefines qualified expenditure to include operational burn rate. However, the KITAS investor visa still requires IDR 10B total investment commitment."
  entityMentions:
    - type: "GovernmentOrganization"
      name: "BKPM (Ministry of Investment)"
      sameAs: "https://www.bkpm.go.id"
    - type: "Regulation"
      name: "BKPM Regulation No. 5 of 2025"
    - type: "Product"
      name: "PT PMA (Foreign Investment Company)"
faq:
  - question: "What is the new minimum paid-up capital for PT PMA?"
    answer: "Under BKPM Regulation 5/2025, paid-up capital dropped 75% from IDR 10 billion to IDR 2.5 billion (~USD 160,000). This is cash that must be deposited in the company bank account at incorporation."
  - question: "Does the KITAS visa requirement change with new capital rules?"
    answer: "No. The KITAS investor visa still requires IDR 10 billion total investment commitment. The capital reduction only affects the immediate paid-up portion, creating a disconnect between entry requirements and visa eligibility."
  - question: "What counts as qualified expenditure under the new rules?"
    answer: "BKPM 5/2025 expands qualified expenditure to include operational costs and burn rate, not just capital equipment. This means your running costs (rent, salaries, marketing) count toward the IDR 10B total investment commitment."
contentStructure:
  hasHowTo: false
  hasFAQ: true
  hasTable: true
canonicalUrl: "https://balizero.com/business/bkpm-regulation-5-2025-fdi"
ogImage: "/static/insights/business/bkpm-regulation-5-2025-fdi.jpg"
ogType: "article"
twitterCard: "summary_large_image"
---

## A New Architecture for Foreign Investment

In October 2025, the Indonesian Ministry of Investment (BKPM) promulgated **Regulation No. 5 of 2025** — a legislative instrument that radically redesigns the mechanics of entry for Foreign Direct Investment in Southeast Asia's largest economy.

For over a decade, Indonesia maintained a reputation as a "high cost of entry" market, protecting its MSMEs through a monolithic financial barrier: the **IDR 10 billion rule**. This policy required any foreign-owned entity (PT PMA) to not only plan but immediately deposit massive capital, effectively excluding tech startups, boutique consultancies, and innovative SMEs that were rich in intellectual property but poor in immediate liquidity.

The headline: **paid-up capital has been slashed 75%**, from IDR 10 billion (~USD 640,000) to **IDR 2.5 billion (~USD 160,000)**.

But interpreting this as a simple "discount" would be a fatal error. This regulation is not a lowering of standards — it's a sophisticated **decoupling** between entry liquidity and long-term economic commitment.

## The Two-Stage Gate

### Stage 1: Entry (Immediate Liquidity)

The paid-up capital — cash that must be physically deposited in the company's Indonesian bank account at incorporation — has been cut to **IDR 2.5 billion**. This figure is calibrated to cover approximately 12-18 months of runway for a mid-size tech startup or service company in Jakarta.

### Stage 2: Performance (Total Commitment)

While the entry ticket has been discounted, the "price of the ride" remains unchanged. The total investment plan — the aggregate value of capital expenditure (Capex) and operating expenditure (Opex) committed — remains fixed at **>IDR 10 billion per business line** (per 5-digit KBLI code).

Think of this as a dual-gear mechanism: a small, fast gear (the IDR 2.5B entry) enables quick market access, but it must engage a much larger, heavier gear (the IDR 10B investment plan). If the small gear spins but fails to move the large one, the entire mechanism seizes — leading to license revocation.

## The 12-Month Lock-Up

To prevent the reduced capital from becoming an escape hatch for "hit-and-run" operators or shell companies, the regulation introduces a **12-month capital lock-up**.

**The rule:** The IDR 2.5 billion must remain in the company's Indonesian bank account for at least 12 months.

**The critical exception:** The capital is not frozen in the strict sense. It cannot be transferred _out_ of the company (no repatriation to parent entities, no loans to directors). But it **can and must** be used for legitimate business purposes within Indonesia:

- Asset purchases (machinery, vehicles, IT equipment)
- Construction expenses
- **Working capital** (salaries, rent, utilities, marketing, server costs)

This transforms the paid-up capital from a "static deposit" to an **"active runway."** The government is mandating that foreign investors pre-fund their first year of operations, ensuring solvency without requiring excess idle liquidity.

## The Qualified Expenditure Revolution

The viability of the IDR 2.5 billion entry hinges entirely on what BKPM accepts as "Investment Realization" to bridge the remaining gap. This is where the real revolution lies.

### What Now Counts as Investment

**1. Working Capital (The Game-Changer)**

Previously, local officials often limited qualifying working capital to just 3 months of operations. The new regime recognizes working capital as a **primary component** of the investment plan for service companies:

- **Employee salaries** — For a software company, engineers _are_ the factory
- **Marketing expenses** — Critical for e-commerce platforms
- **General operational costs** — Rent, utilities, server costs

**The math:** If a startup spends IDR 300 million/month on salaries and server costs, it reaches the IDR 10 billion realization target in approximately **33 months** simply by existing and operating. The regulation effectively validates **"burn rate" as "investment."**

**2. Feasibility Studies and Pre-NIB Costs**

Costs incurred before NIB issuance can now be **retroactively recognized** as investment realization.

**3. Equipment (Modernized Definition)**

For tech companies, "equipment" no longer means lathes and presses. It means high-performance servers, laptops, specialized software licenses, and cloud infrastructure.

### What Doesn't Count

- **Land and buildings** are generally **excluded** from the IDR 10 billion calculation. An investor can't simply buy a luxury Bali villa for IDR 10 billion and claim to have satisfied investment requirements for a consulting firm.
- **Exception:** Land and buildings _are_ included for sectors where real estate is the primary product or production factor: property development, agriculture, aquaculture, and directly managed hospitality (hotels/resorts).

## Sector Winners

### Tech Startups and Digital Platforms

The biggest beneficiaries. The regulation is clearly designed with the asset-light profile in mind:

- Working capital and feasibility studies are qualified expenditures
- The KBLI 2025 reform splits the generic "Web Portal" code into specific niches (e-commerce, on-demand, digital content), allowing precise classification
- FGP recognition means IP-holding companies qualify as "industrial investors"

### Boutique Consulting and Service Firms

Multiple related codes on one PT PMA bring an administrative advantage — one NIB, one LKPM reporting line — not a lower capital bar. A consultancy operating across Management Consulting, Market Research, and Corporate Training at one location faces the IDR 10 billion-plus threshold per 5-digit KBLI code (Art. 26(2)): three codes mean an investment plan above IDR 30 billion, not a single IDR 10 billion plan.

### Food & Beverage Services

For food and beverage services (KBLI 56xxx), Art. 26(3)(b) counts the investment threshold on the first 2 digits of the KBLI code, per location point — one Regency/City (Kabupaten/Kota) — rather than per outlet. A coffee chain can open 5 outlets in one regency/city under a single investment plan above IDR 10 billion. Retail trade (47xxx) has no such exception: each 5-digit retail code still needs its own threshold at each project location.

## The KITAS Visa Disconnect

This is the **single most critical trap** identified in the reform — a fundamental misalignment between corporate rules and immigration rules.

| Authority                    | Entity                    | Requirement                             |
| ---------------------------- | ------------------------- | --------------------------------------- |
| **BKPM** (Corporate)         | The company (PT PMA)      | IDR 2.5 billion paid-up capital         |
| **Immigration** (Individual) | The person (KITAS holder) | IDR 10 billion personal share ownership |

### The Trap in Action

1. A founder incorporates a PT PMA with the new minimum (IDR 2.5 billion). The company is legal.
2. The founder applies for an Investor KITAS (Index C314, 2-year stay permit).
3. **Immigration rejects the application** because the founder's personal share value (IDR 2.5 billion) is below the KITAS threshold (IDR 10 billion).

### The Workaround

Instead of the Investor KITAS route, become a **President Director** with a work KITAS (C312). This requires:

- A work permit (RPTKA)
- Payment of the DPKK fee (USD 1,200/year)
- More immediate personal tax implications

The reform made life easier for the _company_ but not necessarily for the _individual_. Plan your visa strategy before incorporation.

## The LKPM Reporting Trap

The >IDR 10 billion investment plan is monitored through quarterly **LKPM** (Investment Activity Reports).

### Zero Tolerance

BKPM can revoke business licenses (NIB) if a company fails to file LKPM or reports "zero realization" for consecutive periods.

### The Construction Phase Limit

A company cannot remain indefinitely in "Pre-Operative / Construction" status. Specific deadlines apply — for example, **1 year for services/trade** to reach commercial operations. Failure to "graduate" to commercial status triggers revocation for inactivity.

### Investment-Tax Alignment

The investment reported in LKPM will eventually be cross-referenced with the Annual Tax Return (SPT Badan). If a company reports IDR 10 billion in investments to satisfy BKPM but only IDR 1 billion in deductible expenses to the tax office, the discrepancy triggers a compliance alarm.

The numbers must align between BKPM and tax reporting. "Creative accounting" is dangerous.

## Thin Capitalization Risk

The reduced equity base creates new fiscal exposure through Indonesia's **Debt-to-Equity Ratio (DER) rule of 4:1** for interest deductibility.

With equity at IDR 2.5 billion, the maximum tax-deductible shareholder loan is IDR 10 billion. If a company relies on large shareholder loans to fund the IDR 7.5 billion gap, it may hit the DER ceiling much faster than under the old regime, making interest payments non-deductible.

## Regional Context: Indonesia vs. the Competition

| Factor        | Indonesia (Post-Reform)         | Vietnam                       | Thailand                   |
| ------------- | ------------------------------- | ----------------------------- | -------------------------- |
| Entry Capital | ~USD 160,000                    | ~USD 10,000-50,000            | ~THB 2M (~USD 55,000)      |
| Market Size   | 280 million consumers           | 100 million                   | 70 million                 |
| Strategy      | "Justifiably Premium"           | Volume — attract everything   | Sector-specific incentives |
| FDI Position  | Premium access, high compliance | Low barrier, high competition | Moderate barrier, targeted |

Indonesia remains more expensive than Vietnam in pure capital terms. But Indonesia offers the **largest ASEAN consumer market** (280 million people). The reform moves Indonesia from "Prohibitively Expensive" to "Justifiably Premium."

## The Three Rules of Engagement

For foreign SMEs and startups eyeing this new window:

### 1. Synchronize Entity and Individual

Don't assume that corporate entry at IDR 2.5 billion guarantees a visa. Secure your visa strategy (Director vs. Investor) _before_ incorporation.

### 2. Operationalize the Investment Plan

Treat the "Investment Plan" in OSS as a budgeting tool, not a compliance checkbox. Ensure your projected salaries and OpEx for 3 years exceed IDR 10 billion.

### 3. Respect the Lock-Up

Don't treat the IDR 2.5 billion as pass-through money. Use it for genuine expenses in Indonesia to create a clean audit trail for the inevitable LKPM review.

## The Bottom Line

BKPM Regulation 5/2025 is effectively a "deferred payment plan" for access to the Indonesian market. It recognizes that modern businesses don't grow through massive initial capital injections but through iterative operational scalability.

- **For the government:** It filters out the insolvent while capturing growth data from serious investors
- **For the investor:** It reduces the "cost of failure." If a venture fails in Year 1, the investor risked IDR 2.5 billion, not IDR 10 billion

The reform is genuinely "seismic," but the ground is still settling. Investors who walk carefully — armed with this understanding — will find Indonesia more accessible than ever.
