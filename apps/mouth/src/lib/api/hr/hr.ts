...
export const calculatePayroll = (month: number, year: number) =>
  fetchHR<any>('/payroll/calculate', { method: 'POST', body: JSON.stringify({ month, year }) });
...
export const requestLeave = (data: any) =>
  fetchHR<any>('/leave/request', { method: 'POST', body: JSON.stringify(data) });
...
export const rejectLeave = (id: number, reason?: string) =>
  fetchHR<any>(`/leave/${id}/reject`, { method: 'POST', body: JSON.stringify({ reason }) });
