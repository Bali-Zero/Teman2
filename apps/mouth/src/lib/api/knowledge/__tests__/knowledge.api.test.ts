...
describe('KnowledgeApi', () => {
  ...
  describe('searchDocs', () => {
    ...
    it('should handle empty results', async () => {
      const mockResponse = { results: [], total: 0, query: 'nonexistent' } as any;
    ...
    it('should handle API errors', async () => {
      ...
      await expect(knowledgeApi.searchDocs({ query: 'test' })).rejects.toThrow('API Error');
    });

    it('should handle network errors', async () => {
      vi.mocked(mockClient.request).mockRejectedValue(new Error('Network error'));

      await expect(knowledgeApi.searchDocs({ query: 'test' })).rejects.toThrow('Network error');
    });
  });
});
