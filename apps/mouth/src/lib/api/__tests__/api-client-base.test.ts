...
describe('ApiClientBase', () => {
  ...
  describe('Token Management', () => {
    it('should set token in memory and storage', () => {
      ...
      expect(safeStorage.setItem).toHaveBeenCalledWith('auth_token', 'new-token');
    });
  ...
  describe('User Profile Management', () => {
    it('should set user profile in memory and storage', () => {
      const profile = { id: '1', email: 'user@example.com', name: 'User', role: 'admin' };
      vi.mocked(safeStorage.setItem).mockReturnValue(true);

      client.setUserProfile(profile);

      expect(client.getUserProfile()).toEqual(profile);
      expect(safeStorage.setItem).toHaveBeenCalledWith('user_profile', JSON.stringify(profile));
    });

    it('should warn when localStorage is blocked for profile', () => {
      const profile = { id: '1', email: 'user@example.com', name: 'User', role: 'user' };
      vi.mocked(safeStorage.setItem).mockReturnValue(false);
    ...
    it('should sync profile from storage on getUserProfile', () => {
      const oldProfile = { id: '1', email: 'old@example.com', name: 'Old', role: 'user' };
      const newProfile = { id: '2', email: 'new@example.com', name: 'New', role: 'admin' };

      client.setUserProfile(oldProfile);
      vi.mocked(safeStorage.getItem).mockReturnValue(JSON.stringify(newProfile));
    ...
    it('should handle profile parse errors gracefully', () => {
      const validProfile = { id: '1', email: 'test@example.com', name: 'Test', role: 'user' };
  ...
  describe('Role-Based Access', () => {
    it('should identify admin role', () => {
      client.setUserProfile({ id: '1', email: 'admin@example.com', name: 'Admin', role: 'admin' });
    ...
    it('should identify owner role as admin', () => {
      client.setUserProfile({ id: '1', email: 'owner@example.com', name: 'Owner', role: 'owner' });

      expect(client.isAdmin()).toBe(true);
    });

    it('should identify board role as admin', () => {
      client.setUserProfile({ id: '1', email: 'board@example.com', name: 'Board', role: 'board' });

      expect(client.isAdmin()).toBe(true);
    });

    it('should not identify regular user as admin', () => {
      client.setUserProfile({ id: '1', email: 'user@example.com', name: 'User', role: 'user' });
  ...
  describe('HTTP Request - Error Handling', () => {
    ...
    it('should handle 405 Method Not Allowed', async () => {
      ...
      await expect(client.request('/test', { method: 'POST' })).rejects.toThrow('INVALID_METHOD');
    });
    ...
    it('should handle generic HTTP errors', async () => {
      ...
      await expect(client.request('/test')).rejects.toThrow('Internal server error');
    });
    ...
    it('should handle request timeout', async () => {
      ...
      await expect(client.request('/test', {}, 100)).rejects.toThrow('Request timeout');
    });
  ...
  describe('Convenience Methods', () => {
    ...
    it('should support custom timeout in post method', async () => {
      ...
      await expect(client.post('/test', {}, 100)).rejects.toThrow('Request timeout');
    });
  });
