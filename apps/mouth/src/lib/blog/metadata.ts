...
export function generateArticleMetadata(article: Article): Metadata {
  ...
  return {
    ...
    other: {
      // AI citation meta tags (used by Perplexity, ChatGPT browse, Google AI Overviews)
      ...(article.aiOptimization?.answerSnippet
        ? { "data-ai-summary": article.aiOptimization.answerSnippet }
        : {}),
      // Citation meta (academic/AI citation format - Perplexity prioritizes these)
      citation_title: metaTitle,
      citation_author: article.author.name,
      ...(article.publishedAt
        ? {
            citation_publication_date: new Date(article.publishedAt)
              .toISOString()
              .split("T")[0],
          }
        : {}),
...
export function generateInsightsMetadata(): Metadata {
  return {
    ...
    openGraph: {
      ...
      description:
        "Expert guides on Indonesia visas, business, tax, and life in Bali.",
      siteName: "Bali Zero",
      images: [
        {
          url: `${baseUrl}/static/og-image.jpg`,
