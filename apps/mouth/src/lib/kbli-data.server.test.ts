...
describe("kbli-data.server — section derivation (Mandate 12 fix)", () => {
  ...
  it("does not advertise generated gold content for a declared PMA gap", () => {
    ...
    // 15 -> 9 (against main's post-#6596-merge registry baseline): W-H
    // PR-3c v3 de-certified 41020/50133/65121/79122/96210/96220 from
    // mouthGold, whose prose still claimed an openness their own tuple
    // denies (withdrawn rather than hand-edited, no compiler exists for
    // non-whatYouNeed gold fields). 47111 lost ONLY canonicalIntel in the
    // same PR — its mouthGold entry carries different, untouched content
    // and stays certified.
    expect(getGoldCodes()).toContain("47111");
    ...
    expect(getGoldCodes()).toHaveLength(9);
