...
describe("real dataset: the gate binds, and v3 actually differentiates", () => {
  ...
  it("actually degrades on the real dataset (the gate is not a no-op here)", () => {
    ...
    // 1410 - 330 = 1080 on 2026-09-18 (naso PR-4, residual lot 1): the codes
    ...
    // plain neutral codes in this count. The rule reaches 507; 177 are
    // withheld — 61 under Pasal 11(2) (finance/banking), 68 in sectors whose
    // own statute this lane never read, 29 with an adjudicated 4-digit
    // sibling, 19 body-stated categories.
    expect(pmaGaps).toBe(1080);
