...
describe("MagicLinkRequestPage (WS3 day pass)", () => {
  ...
  it("drain guard: no forced-dark shell or gold hexes remain", () => {
    ...
    expect(html).not.toContain("#c9a96e"); // token-lint-ok: drain-guard assertion string, not a color use // token-lint-ok: drain-guard assertion string, not a color use
    expect(html).not.toContain("#d9bd7a"); // token-lint-ok: drain-guard assertion string, not a color use // token-lint-ok: drain-guard assertion string, not a color use
    expect(html).not.toContain("#a07838"); // token-lint-ok: drain-guard assertion string, not a color use // token-lint-ok: drain-guard assertion string, not a color use
    expect(html).not.toContain("#f0ece4"); // token-lint-ok: drain-guard assertion string, not a color use // token-lint-ok: drain-guard assertion string, not a color use
    expect(html).not.toContain("#c94a4a"); // token-lint-ok: drain-guard assertion string, not a color use
