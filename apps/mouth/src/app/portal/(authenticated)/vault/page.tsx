...
export default function VaultPage() {
  ...
  const uploadType = searchParams.get("upload") || "general";
  ...
  // Auto-open file picker when arriving with ?upload=passport
  useEffect(() => {
    if (uploadType !== "general" && fileInputRef.current && !isLoading) {
  ...
  useEffect(() => {
    loadDocuments();
    // Silent refresh every 2 minutes so new uploads from team appear
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setIsUploading(true);
      const uploadedDoc = await api.portal.uploadDocument(file, uploadType);
