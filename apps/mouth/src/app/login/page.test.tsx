 ...
 * characters `/`, TAB, `/`, `e`… , which begins with one slash, carries no
 * leading `//`, and passes every prefix test that can be written. The browser
 * then drops the TAB and resolves it to https://evil.test/.
 *
...
const ACCEPTED: ReadonlyArray<readonly [string, string]> = [
  ["/dashboard?tab=1#x", "/dashboard?tab=1#x"],
  [`${ORIGIN}/clients/42`, "/clients/42"],
  [`${ORIGIN}/clients?filter=mine#top`, "/clients?filter=mine#top"],
  ["/", "/"],
];
...
const REFUSED: ReadonlyArray<readonly [string, string]> = [
  ...
  ["javascript:alert(1)", "not http or https"],
  ["data:text/html,x", "not http or https"],
  ["https://evil.test/x", "another origin"],
  ["https://kita.balizero.com.evil.test/", "look-alike host"],
  ["https://evil.test\\@kita.balizero.com/", "userinfo trick"],
  ["/login?expired=true", "no loop back into the gate"],
  ["", "empty"],
];

describe("safeRedirect — judge the resolved URL, never the string", () => {
  for (const [written, expected] of ACCEPTED) {
    it(`accepts ${written} and returns ${expected} (innocence)`, () => {
      expect(safeRedirect(fromQuery(written), ORIGIN, FALLBACK)).toBe(expected);
    });
  }

  for (const [written, why] of REFUSED) {
    it(`refuses ${JSON.stringify(written)} — ${why} (guilt)`, () => {
      expect(safeRedirect(fromQuery(written), ORIGIN, FALLBACK)).toBe(FALLBACK);
    });
  }
  ...
  it("a prefix test would have passed the three whitespace vectors", () => {
    // The guard this replaces: starts with "/", not "//", not "/\".
    ...
    for (const v of ["/%09/evil.test", "/%0A/evil.test", "/%0D/evil.test"]) {
      ...
      expect(safeRedirect(fromQuery(v), ORIGIN, FALLBACK)).toBe(FALLBACK);
    }
  });

  it("refuses nothing at all", () => {
    expect(safeRedirect(null, ORIGIN, FALLBACK)).toBe(FALLBACK);
  });
  ...
  it("returns the RESOLVED path, never the caller's string", () => {
    ...
    expect(safeRedirect("/a/../clients", ORIGIN, FALLBACK)).toBe("/clients");
    expect(safeRedirect(`${ORIGIN}/a/../clients`, ORIGIN, FALLBACK)).toBe(
      "/clients",
    );
  });

  it("honours the caller's fallback, which is how a refusal is detected", () => {
    // page.tsx passes "" so it can log the refusal and then choose the role
    // destination itself.
    expect(safeRedirect("https://evil.test/x", ORIGIN, "")).toBe("");
    expect(safeRedirect("/clients", ORIGIN, "")).toBe("/clients");
  });

  it("defaults to /dashboard when no fallback is given", () => {
    expect(safeRedirect("https://evil.test/x", ORIGIN)).toBe("/dashboard");
  });
});
...
describe("LoginPage — the five plates", () => {
  ...
  it("refuses an off-origin ?redirect= and says so in the log (guilt)", async () => {
    // The same parameter, pointing off this origin: the role destination wins
