 ...
 * the TAB before it parses. `firstPartyRedirect` resolves every candidate
 * first and judges the parsed URL — keeping the SSO bounce back to a sibling
 * subdomain, which same-origin-only would have broken.
 *
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const allowed = firstPartyRedirect(requested, globalThis.location.origin);
      if (requested && !allowed) {
        logger.warn("Refused a redirect target that leaves Bali Zero", {
          component: "LoginPage",
          action: "handleLogin",
        });
      }
