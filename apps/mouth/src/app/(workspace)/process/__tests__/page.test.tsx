...
vi.mock("@/lib/analytics", () => ({
  ...
  trackFilterRemoved: vi.fn(),
  trackSortApplied: vi.fn(),
  trackSearch: vi.fn(),
  trackCaseStatusChanged: vi.fn(),
  trackPaginationChange: vi.fn(),
...
vi.mock("@/components/process/MonthPillTabs", () => ({
  MonthPillTabs: ({ onChange }: { onChange?: (m: string | null) => void }) => (
    <div data-testid="month-pill-tabs" onClick={() => onChange?.(null)} />
  ),
}));
vi.mock("@/components/process/GhostCard", () => ({
  GhostCard: () => <div data-testid="ghost-card" />,
}));
vi.mock("@/components/process/kanban-colors", () => {
  const cfg = {
    label: "Test",
    gradientStart: "#000",
    gradientEnd: "#000",
    tintBg: "rgba(0,0,0,0.05)",
    tintBorder: "rgba(0,0,0,0.08)",
...
describe("Cases Page", () => {
  ...
  describe("Component Rendering", () => {
    it("should render page title and description", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Manage KITAS, Visa, PT PMA/),
        ).toBeInTheDocument();
      });
    });
    ...
    it('should render "+ New Process" button', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /New Process/i }),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Analytics Tracking", () => {
    ...
    it("should track sort operations", async () => {
      ...
      expect(analytics.trackSortApplied).toHaveBeenCalledWith(
        "created_at",
        "desc",
      );
    });
  ...
  describe("Error Handling", () => {
    it("should handle API error gracefully", async () => {
      vi.mocked(api.crm.getPractices).mockRejectedValue(
    ...
    it("should handle status update failure gracefully", async () => {
      const user = userEvent.setup();
      (api.crm.updatePractice as any) = vi
        .fn()
