...
import type { TeamPhotoMap } from "@/lib/workspace/roster-directory";
...
// The photo table is NOT here any more: it is derived from the roster on the
// server in `@/lib/workspace/roster-directory` and arrives as a prop. This file is
// "use client", so a constant naming staff shipped to the browser and sat in this
// route's public chunk — which is fetchable without a session even though the PAGE
// is behind auth. See that module for the measurement.

// Get photo URL from email
const getTeamPhoto = (photos: TeamPhotoMap, email: string): string | null => {
  const prefix = email.split("@")[0].toLowerCase();
  return photos[prefix] || null;
};
...
export default function TeamManagementClient({
  ...
  /** Derived from the roster on the server; required so nobody renders a blank directory. */
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-[var(--bz-border)]">
            {teamMembers
              ...
              .map((member) => {
                // Resolved ONCE per member. It used to be called twice — in the
                // condition and again in `src`, the second with a non-null
                // assertion — which is a shape where one call gets updated and the
                // other is missed forever.
                const photo = getTeamPhoto(teamPhotos, member.email);
                return (
                  <div
                    key={member.user_id}
