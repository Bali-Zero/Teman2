      ...
      <div>
        ...
        {loading ? (
          ...
          <>
            ...
            {trend !== undefined && (
              <span className={`text-xs font-medium flex items-center gap-1 mt-1 ${trend >= 0 ? "text-green-500" : "text-red-500"}`}>
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const aging: OutstandingByAge[] = [
        { bucket: "0-30 days", amount: revenueRes.total_outstanding * 0.4, count: 12 },
        { bucket: "31-60 days", amount: revenueRes.total_outstanding * 0.3, count: 8 },
        { bucket: "61-90 days", amount: revenueRes.total_outstanding * 0.2, count: 5 },
        { bucket: "90+ days", amount: revenueRes.total_outstanding * 0.1, count: 3 },
      ];
      setOutstanding(aging);
      
      // Mock recent payments
      setRecentPayments([
        { id: 1, client_name: "PT Example Indonesia", amount: 15000000, status: "paid", due_date: "2026-02-15", paid_date: "2026-02-14" },
        { id: 2, client_name: "John Smith", amount: 8500000, status: "pending", due_date: "2026-02-20" },
        { id: 3, client_name: "PT Bali Business", amount: 22000000, status: "overdue", due_date: "2026-02-01" },
        { id: 4, client_name: "Sarah Johnson", amount: 12000000, status: "paid", due_date: "2026-02-10", paid_date: "2026-02-10" },
      ]);
    } catch (err) {
              ...
              <>
                ...
                <div className="mt-4 grid grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {formatCurrency(revenueTrend.length ? revenueTrend.reduce((a, b) => a + b, 0) / revenueTrend.length : 0)}
                    </p>
                  </div>
                  <div>
                    ...
                    {revenue?.by_month?.map((month, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-muted/50">
                          ...
                          <td className="p-3 text-right font-medium">{formatCurrencyFull(month.revenue)}</td>
                          <td className="p-3 text-right text-green-500">{formatCurrencyFull(month.paid)}</td>
                          <td className="p-3 text-right text-muted-foreground">{formatCurrencyFull(month.target)}</td>
                          <td className="p-3 text-right">
                            <span className={`font-medium ${percent >= 100 ? "text-green-500" : percent >= 80 ? "text-amber-500" : "text-red-500"}`}>
                              {percent.toFixed(0)}%
                            </span>
                          </td>
                        </tr>
                ...
                <tbody className="divide-y">
                  {recentPayments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-muted/50">
                      ...
                      <td className="p-3 text-right font-medium">{formatCurrencyFull(payment.amount)}</td>
                  ...
                  <div key={idx}>
                    ...
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${(item.amount / (outstanding[0]?.amount || 1)) * 100}%` }}
                      />
                    </div>
