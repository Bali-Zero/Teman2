...
export default function HRSettingsPage() {
  ...
  const [newRate, setNewRate] = useState({ practice_type_code: '', amount_idr: '' });
  const [showNew, setShowNew] = useState(false);

  useEffect(() => {
    hrApi.listBonusRates().then(data => {
      setRates(data.rates || []);
      setLoading(false);
  ...
  const handleSave = async (code: string, amount: number) => {
    try {
      await hrApi.upsertBonusRate({ practice_type_code: code, amount_idr: amount });
    ...
    <div className="space-y-6">
      ...
      {showNew && (
        <div className="bg-zinc-900 border border-[var(--bz-accent)]/30 rounded-lg p-4 flex items-end gap-3">
          <div className="flex-1">
            <label className="block text-xs text-zinc-400 mb-1">Practice Type Code</label>
            <input
              type="text"
              value={newRate.practice_type_code}
              onChange={e => setNewRate(prev => ({ ...prev, practice_type_code: e.target.value }))}
          ...
          <div className="w-40">
            <label className="block text-xs text-zinc-400 mb-1">Amount (IDR)</label>
            <input
              type="number"
              value={newRate.amount_idr}
              onChange={e => setNewRate(prev => ({ ...prev, amount_idr: e.target.value }))}
              placeholder="500000"
              className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-sm text-zinc-200"
            />
          </div>
      ...
      <div className="space-y-1">
        ...
        {rates.map((rate) => (
          ...
          >
            <div>
              <span className="text-zinc-200">{rate.practice_type_code?.replace(/_/g, ' ')}</span>
              {rate.practice_type_name && (
                <span className="text-xs text-zinc-500 ml-2">({rate.practice_type_name})</span>
              )}
            </div>
            <div>
              {editingId === rate.id ? (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button
                    onClick={() => handleSave(rate.practice_type_code, Number(editAmount))}
                ...
                <button
                  onClick={() => { setEditingId(rate.id); setEditAmount(String(rate.amount_idr)); }}
            ...
            <span className={`text-xs ${rate.is_active ? 'text-emerald-400' : 'text-zinc-500'}`}>
