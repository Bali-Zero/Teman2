...
export default function LeavePage() {
  ...
  const handleReject = async (id: number) => {
    ...
    if (!reason) return;
    try {
      await hrApi.rejectLeave(id, reason);
  ...
  return (
    <div className="space-y-6">
      ...
      {balances.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-zinc-300 mb-3">Leave Balances</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {balances.map((bal) => {
              const remaining = bal.allocated_days + (bal.carried_over || 0) - bal.used_days - bal.pending_days;
              return (
                <div key={bal.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
                  <div className="text-sm text-zinc-400">{bal.leave_type_name || bal.name}</div>
                  <div className="text-2xl font-bold text-zinc-100 mt-1">{remaining} days</div>
                  <div className="text-xs text-zinc-500 mt-1">
                    {bal.allocated_days} allocated · {bal.used_days} used · {bal.pending_days} pending
                  </div>
                </div>
              );
            })}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 flex-1">
                    <StatusIcon size={18} className={statusColors[req.status]} />
                    <div>
