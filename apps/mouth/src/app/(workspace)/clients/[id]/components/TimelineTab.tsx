...
import { Clock, MessageCircle, Mail, Phone, Send, Calendar, FileText } from 'lucide-react';
...
export function TimelineTab({
  ...
  const sentimentCount = useMemo(() => {
    ...
    interactions.forEach((i) => {
      if (i.sentiment && counts[i.sentiment] !== undefined) counts[i.sentiment]++;
    });
    return counts;
  }, [interactions]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        {filtered.map((interaction, idx) => {
          const style = CHANNEL_STYLES[interaction.interaction_type] || CHANNEL_STYLES.note;
