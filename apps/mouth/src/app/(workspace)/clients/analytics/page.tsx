...
const StatCard = ({ title, value, subtitle, trend, icon: Icon, color, loading, onClick }: StatCardProps) => {
  const colorClasses = {
        ...
        <div className="min-w-0 flex-1">
          ...
          {loading ? (
            ...
            <>
              ...
              {trend !== undefined && <div className="mt-1"><TrendIndicator value={trend} /></div>}
            </>
          )}
        </div>
...
export default function ClientAnalyticsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "team" | "revenue" | "processes">("overview");
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [
        ...
        api.get<TrendPoint[]>(`/api/crm/analytics/clients/trend?months=${dateRange === "6m" ? 6 : dateRange === "1y" ? 12 : 6}`),
      ]);
      ...
      if (overviewRes?.by_status) {
        ...
        const funnelData: FunnelStage[] = [
          { stage: "Leads", count: overviewRes.by_status.lead || 0, percentage: Math.round(((overviewRes.by_status.lead || 0) / total) * 100), drop_off: 0 },
          { stage: "Active", count: overviewRes.by_status.active || 0, percentage: Math.round(((overviewRes.by_status.active || 0) / total) * 100), drop_off: Math.round(((overviewRes.by_status.lead || 0) - (overviewRes.by_status.active || 0)) / (overviewRes.by_status.lead || 1) * 100) },
          { stage: "Completed", count: overviewRes.by_status.completed || 0, percentage: Math.round(((overviewRes.by_status.completed || 0) / total) * 100), drop_off: Math.round(((overviewRes.by_status.active || 0) - (overviewRes.by_status.completed || 0)) / (overviewRes.by_status.active || 1) * 100) },
        ];
        ...
        <section className="mb-6 sm:mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <StatCard
              ...
              value={isLoading ? "-" : `${overview?.by_status?.active && overview?.total_clients 
                ...
                <table className="w-full text-sm">
                  <thead className="bg-muted">
                    <tr>
                      ...
                      <th className="text-right p-3 font-medium hidden sm:table-cell">Collection %</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {revenue?.by_month?.map((month, idx) => (
                      <tr key={idx} className="hover:bg-muted/50">
                        ...
                        <td className="p-3 text-right font-medium">{formatCurrency(month.revenue)}</td>
                        <td className="p-3 text-right text-green-500">{formatCurrency(month.paid)}</td>
                ...
                <table className="w-full text-xs sm:text-sm">
                  <thead className="bg-muted">
                    <tr>
                      ...
                      <th className="text-center p-3 font-medium hidden md:table-cell">Completed</th>
                      ...
                      <tr key={idx} className="hover:bg-muted/50">
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="truncate max-w-[120px] sm:max-w-[200px]">{member.member_email}</span>
                          </div>
                        </td>
                        <td className="p-3 text-center">{member.total_clients}</td>
                        <td className="p-3 text-center text-green-500 hidden sm:table-cell">{member.active_clients}</td>
                        <td className="p-3 text-center hidden md:table-cell">{member.completed_cases}</td>
                        <td className="p-3 text-center">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            member.conversion_rate >= 50 ? "bg-green-500/20 text-green-500" :
                            member.conversion_rate >= 30 ? "bg-amber-500/20 text-amber-500" :
                            "bg-red-500/20 text-red-500"
                          }`}>
                            {member.conversion_rate}%
                          </span>
                        </td>
                        <td className="p-3 text-right font-medium">{formatCurrency(member.revenue_generated)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {processTypes.slice(0, viewMode === "mobile" ? 4 : 6).map((process, idx) => (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{formatCurrency(process.total_revenue)}</span>
                    </div>
                    <ProgressBar 
