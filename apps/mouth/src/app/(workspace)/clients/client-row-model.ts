 ...
 * record is still moving. A terminal record can never need the viewer
 * (fusion DISPOSITION F15), and a date can never make it copper (F3).
 */
export function viewerIsNext(
  ...
  return !isTerminalClientStatus(client.status);
}
...
/** Counts over the rows currently in view. `needsYou` uses viewerIsNext. */
export function deskCounts(clients: Client[], viewerEmail: string): DeskCounts {
  ...
  for (const client of clients) {
    ...
    if (client.status === "completed") completed += 1;
    if (viewerIsNext(client, viewerEmail)) needsYou += 1;
  }
  return { total, active, needsYou, completed };
}
