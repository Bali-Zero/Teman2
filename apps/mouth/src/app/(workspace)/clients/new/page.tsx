...
export default function NewClientPage() {
  ...
  const [entryMode, setEntryMode] = useState<"choosing" | "scan" | "form">(
    "choosing",
  );
  ...
  // upload happens right after it — same shape as the passport scan below.
  // `avatarPreview` is a local object URL for display only; it is never sent.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload the avatar now that the client has an id. A failure here must not
      // lose the client that was just created — warn and carry on, the photo can
      // be added later from the client page.
      if (avatarFile) {
        try {
          await api.crm.uploadClientAvatar(newClient.id, avatarFile);
        } catch (err) {
          logger.error(
            "Avatar upload failed after client creation",
            { component: "NewClientPage", action: "uploadClientAvatar" },
            err instanceof Error ? err : new Error(String(err)),
          );
          toastError(
            "Photo upload failed",
            "Client created, but the photo could not be uploaded. You can add it from the client page.",
          );
        }
      }
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Crop to square and resize to 400x400px, keeping BYTES (not a base64
      // data: URI): they go to storage via POST /clients/{id}/avatar once the
      // client exists. avatar_url must never be an inline blob — the backend
      // rejects it, and a persisted one blocks every later edit of the client.
  ...
  const uploadPassportWithRetry = async (
    clientId: number,
  ): Promise<boolean> => {
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        type="email"
                        className={
                          fieldErrors.email ? inputErrorClass : inputClass
                        }
                ...
                {formData.assigned_to && (
                  <span className="px-2 py-1 rounded bg-[var(--accent)]/20 text-[var(--accent)]">
                    ...
                    {teamMemberOptions.find(
                      (m) => m.value === formData.assigned_to,
                    )?.label ?? formData.assigned_to.split("@")[0]}
                  </span>
                )}
