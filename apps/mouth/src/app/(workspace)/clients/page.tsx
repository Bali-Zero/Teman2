...
function ClientsListContent() {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--bz-text-2)' }}>
            {isMounted ? filteredClients.length.toLocaleString('en-US') : filteredClients.length}{' '}
            client
      ...
      {isMounted &&
        stats &&
        (stats.passportExpired > 0 || stats.passportExpiringSoon > 0 || stats.silent30d > 0) && (
                ...
                >
                  ...
                  {currentUserEmail && !uniqueAssignees.includes(currentUserEmail) && (
                    <option value={currentUserEmail}>
                      {teamMemberOptions.find((m) => m.value === currentUserEmail)?.label ||
