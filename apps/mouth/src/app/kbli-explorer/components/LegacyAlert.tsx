...
export default function LegacyAlert({ code, onOpenBlackBook }: LegacyAlertProps) {
    ...
    >
      <div className="flex items-start gap-4">
        <div className={`p-2 rounded-lg ${isDanger ? "bg-red-500/20" : "bg-amber-500/20"}`}>
          {isDanger ? <ShieldAlert size={24} className="text-red-500" /> : <AlertTriangle size={24} className="text-amber-500" />}
        </div>
        <div className="flex-1">
          ...
          <p className="text-sm opacity-90 leading-relaxed mb-4">
            <span className="font-bold uppercase tracking-wider">{entry.status}:</span> {entry.reason} 
            <br />
