...
// Backup first
const backupFile = INDEX_FILE + '.backup_phase1_' + new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19);
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('["01111"') || lines[i].includes('[  "01111"')) {
    ...
    for (let j = i; j >= Math.max(0, i - 20); j--) {
      if (lines[j].includes('const K') || lines[j].includes('let K') || lines[j].includes('var K')) {
        startLine = j;
        break;
      }
    }
