      ...
      if (line.startsWith("data: ")) {
        ...
        try {
          ...
          if (data.type === "token") {
            ...
            console.log(
              `   ✅ TOOL END. Result length: ${data.data.result.length}`,
            );
...
(async () => {
  try {
    ...
    // TEST 1: CALCULATOR
    await testStreamQuery(
      token,
      "Calculate the PPN (11%) of 500,000,000 IDR.",
      "Tool: Calculator",
    );
    ...
    await testStreamQuery(
      token,
      "How much does the Investor KITAS (2 years) cost exactly in USD?",
      "Tool: Pricing",
    );
    ...
    await testStreamQuery(
      token,
      "What are the requirements for a PT PMA?",
      "Stream: RAG Flow",
    );
