...
test('SSR Smoke Test: Googlebot Verification', async ({ request }) => {
  ...
  const response = await request.get('http://localhost:3000/services/investor-kitas');
  ...
  if (
    !html.includes('<dl class="sr-only" itemscope itemtype="https://schema.org/GovernmentService">')
  ) {
