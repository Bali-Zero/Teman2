  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {message.has_attachments && (
                <Paperclip className="w-3.5 h-3.5 text-[var(--foreground-muted)]" aria-label="Ha allegati" />
              )}
        ...
        {!message.is_read && (
          <span className="shrink-0 mt-1.5 w-2 h-2 rounded-full bg-[var(--accent)]" aria-label="Non letto" />
        )}
      </button>
          ...
          {hasContent && fullContent.attachments && fullContent.attachments.length > 0 && (
            <div className="mt-4 pt-4 border-t border-[var(--border)]">
              <p className="text-xs font-medium text-[var(--foreground-muted)] mb-2 flex items-center gap-1.5">
                ...
                {fullContent.attachments.length} Allegat{fullContent.attachments.length === 1 ? 'o' : 'i'}
              </p>
...
export function ThreadView({
  ...
  const currentThread = useMemo<EmailSummary[]>(() => {
    ...
    return emails
      .filter((e) => {
        const n = e.subject.replace(/^(re|fwd|fw|sv|vs):\s*/gi, '').trim().toLowerCase();
        return n === normalizedSubject;
      })
      .sort((a, b) => {
        const aTime = Number(a.date) > 1_000_000_000_000 ? Number(a.date) : new Date(a.date).getTime();
        const bTime = Number(b.date) > 1_000_000_000_000 ? Number(b.date) : new Date(b.date).getTime();
