...
describe("FolderSidebar", () => {
  ...
  it("calls onSelectFolder when a folder is clicked", () => {
    ...
    render(
      <FolderSidebar {...defaultProps} onSelectFolder={onSelectFolder} />,
    );
  ...
  it("renders disconnect button", () => {
    render(<FolderSidebar {...defaultProps} />);
    expect(
      screen.getByLabelText("Disconnetti account Zoho Mail"),
    ).toBeInTheDocument();
  });
  ...
  it("calls onDisconnect when disconnect is clicked", () => {
    ...
    render(
      <FolderSidebar {...defaultProps} onDisconnect={onDisconnect} />,
    );
    fireEvent.click(
      screen.getByLabelText("Disconnetti account Zoho Mail"),
    );
  ...
  it("displays connected email when provided", () => {
    render(
      <FolderSidebar
        {...defaultProps}
        connectedEmail="user@balizero.com"
      />,
    );
  ...
  it("shows loading skeletons when isLoading is true", () => {
    const { container } = render(
      <FolderSidebar {...defaultProps} isLoading={true} />,
    );
  ...
  it("renders navigation landmark", () => {
    render(<FolderSidebar {...defaultProps} />);
    expect(
      screen.getByRole("navigation", { name: "Cartelle email" }),
    ).toBeInTheDocument();
  });
  ...
  it("shows 99+ for very high unread counts", () => {
    ...
    render(
      <FolderSidebar {...defaultProps} folders={foldersWithHighCount} />,
    );
