...
describe('EmailList', () => {
  ...
  it('renders email snippets', () => {
    render(<EmailList {...defaultProps} />);
    expect(screen.getByText('Here is an important update about...')).toBeInTheDocument();
  });
  ...
  it('calls onSelectEmail when email content is clicked', () => {
    ...
    render(<EmailList {...defaultProps} onSelectEmail={onSelectEmail} />);
  ...
  it('renders select all checkbox', () => {
    render(<EmailList {...defaultProps} />);
    const selectAllBtn = screen.getByLabelText('Seleziona tutte le email');
  ...
  it('calls onSelectAll when select-all is clicked', () => {
    ...
    fireEvent.click(screen.getByLabelText('Seleziona tutte le email'));
    expect(onSelectAll).toHaveBeenCalledWith(true);
  });

  it('shows batch action buttons when emails are selected', () => {
    render(<EmailList {...defaultProps} selectedIds={new Set(['msg-1'])} />);
    expect(screen.getByLabelText('Segna come lette')).toBeInTheDocument();
    expect(screen.getByLabelText('Segna come non lette')).toBeInTheDocument();
    expect(screen.getByLabelText('Elimina selezionate')).toBeInTheDocument();
  });

  it('does not show batch actions when nothing selected', () => {
    render(<EmailList {...defaultProps} />);
    expect(screen.queryByLabelText('Segna come lette')).not.toBeInTheDocument();
  });
  ...
  it('disables next page button when no more', () => {
    render(<EmailList {...defaultProps} totalEmails={50} currentPage={1} hasMore={false} />);
