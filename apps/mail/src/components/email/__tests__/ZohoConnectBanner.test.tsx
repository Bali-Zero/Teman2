...
describe('ZohoConnectBanner', () => {
  ...
  it('renders the connect description', () => {
    render(<ZohoConnectBanner onConnect={vi.fn()} isConnecting={false} />);
    expect(
      screen.getByText(/Connect your Zoho Mail account to read, send, and manage emails/)
    ).toBeInTheDocument();
  });
  ...
  it('renders email icon SVG', () => {
    const { container } = render(<ZohoConnectBanner onConnect={vi.fn()} isConnecting={false} />);
