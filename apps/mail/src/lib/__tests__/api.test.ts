...
describe('emailApi', () => {
  ...
  describe('error handling', () => {
    it('redirects on 401', async () => {
      ...
      await expect(emailApi.getConnectionStatus()).rejects.toThrow('Authentication required');
      expect(replaceSpy).toHaveBeenCalledWith(expect.stringContaining('kita.balizero.com/login'));
    });
  });
});
