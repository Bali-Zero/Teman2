#!/usr/bin/env bash
# NB-7 Editorial & Content Strategy — NLM pipeline cron wrapper
# Schedule: 40 2 * * 1-6   (02:40 WITA Mon-Sat)
# Machine:  Pro (nuzantara@Nuzantara)
# Log:      ~/.openclaw/logs/nlm_nb7_pipeline.log
# Brief:    ~/.agent/decisions/nlm_briefs/daily_intelligence_brief_nb7.json
set -euo pipefail
...
LOCK_FILE="$SCRIPT_DIR/../nb7.lock"
LOG_FILE="${HOME}/.openclaw/logs/nlm_nb7_pipeline.log"
...
if [ -f "$LOCK_FILE" ]; then
    ...
    if kill -0 "$PID" 2>/dev/null; then
        echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [SKIP] NB-7 pipeline already running (PID $PID)" >> "$LOG_FILE"
        exit 0
    fi
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [WARN] Stale NB-7 lock (PID $PID) — cleaning up" >> "$LOG_FILE"
    rm -f "$LOCK_FILE"
fi
echo $$ > "$LOCK_FILE"
...
cd "$PROJECT_ROOT"
echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [START] NB-7 Editorial pipeline (PID $$)" >> "$LOG_FILE"

# Run pipeline
PYTHONPATH=. "$PYTHON" -m apps.evaluator.nlm_deep_research.nb7_pipeline "$@" \
    2>&1 | tee -a "$LOG_FILE"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ -n "${TELEGRAM_OWNER_CHAT_ID:-}" ]; then
        MSG="⚠️ NB-7 Editorial pipeline FAILED (exit $EXIT_CODE) — check ~/.openclaw/logs/nlm_nb7_pipeline.log"
