#!/usr/bin/env bash
# NB-3 Company Setup Indonesia — NLM pipeline cron wrapper
# Schedule: 45 2 * * 1-6   (02:45 WITA Mon-Sat)
...
LOCK_FILE="$SCRIPT_DIR/../nb3.lock"
LOG_FILE="${HOME}/.openclaw/logs/nlm_nb3_pipeline.log"
...
if [ -f "$LOCK_FILE" ]; then
    ...
    if kill -0 "$PID" 2>/dev/null; then
        echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [SKIP] NB-3 pipeline already running (PID $PID)" >>"$LOG_FILE"
        exit 0
    fi
    echo "$(date -u '+%Y-%m-%dT%H:%M:%SZ') [WARN] Stale NB-3 lock (PID $PID) — cleaning up" >>"$LOG_FILE"
    rm -f "$LOCK_FILE"
fi
echo $$ >"$LOCK_FILE"
