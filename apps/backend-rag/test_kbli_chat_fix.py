...
async def test_llm_gateway():
    ...
    try:
        from backend.services.rag.agentic.llm_gateway import LLMGateway, TIER_FLASH
        
        gateway = LLMGateway()
        print(f"✅ LLMGateway initialized")
...
async def test_kbli_explanation():
    ...
    try:
        from backend.app.routers.kbli_notebook import _generate_kbli_explanation, KBLISearchResult
