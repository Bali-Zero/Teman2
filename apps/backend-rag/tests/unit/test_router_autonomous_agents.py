...
if "langgraph.graph" not in sys.modules:
    ...
    graph_stub = types.SimpleNamespace(
        END="END", StateGraph=_StateGraph, redis_url="redis://localhost:6379"
    )
    sys.modules["langgraph"] = types.SimpleNamespace(
        graph=graph_stub, redis_url="redis://localhost:6379"
    )
...
def _install_main_cloud_stub(monkeypatch, retriever=None, db_pool=None):
    app_stub = types.SimpleNamespace(
        state=types.SimpleNamespace(
            retriever=retriever, db_pool=db_pool, redis_url="redis://localhost:6379"
        )
    )
...
def test_persist_kg_sample_success(monkeypatch):
    ...
    kg_module = types.SimpleNamespace(
        Entity=_Entity,
        KnowledgeGraphBuilder=_KnowledgeGraphBuilder,
...
def _install_scheduler_stub(monkeypatch, scheduler):
    module_stub = types.SimpleNamespace(
        get_autonomous_scheduler=lambda: scheduler, redis_url="redis://localhost:6379"
    )
...
def test_get_scheduler_status_failure(monkeypatch):
    ...
    module_stub = types.SimpleNamespace(
        get_autonomous_scheduler=_raise, redis_url="redis://localhost:6379"
    )
    monkeypatch.setitem(sys.modules, "backend.services.misc.autonomous_scheduler", module_stub)
