...
class TestVerifyAdmin:
    ...
    def test_verify_admin_valid_zero_email(self):
        ...
        assert verify_admin({"email": "zero@balizero.com"}) is True
    ...
    def test_verify_admin_valid_balizero_admin(self):
        ...
        assert verify_admin({"email": "admin@balizero.com"}) is True
...
# ============================================================================
# Clock-in/out Identity Gate Tests (TOURNIQUET, 2026-07-21)
# ============================================================================


class TestClockIdentityGate:
    """Tests for the clock-in/out auth gate — closes the public unauth
    impersonation vector (memory
    `discovery_crm_pii_public_exposure_blog_ask_timesheet_2026_07_21`).
    """

    # ── GUILT: no session at all ───────────────────────────────────────

    def test_clock_in_no_auth_rejected(self, client, test_app):
        """No session → 401, never reaches the timesheet service."""
    ...
    # ── GUILT: authenticated non-admin, mismatched body identity ────────

    def test_clock_in_non_admin_impersonation_rejected(
        self, client, test_app, mock_timesheet_service
    ):
        """Authenticated as damar@, body claims to clock in someone else → 403.

        This is the exact impersonation vector: a valid non-admin session
        must not be able to clock ANY team member in/out via the body.
        """
        test_app.state.current_user = {"email": "damar@balizero.com", "role": "team"}
        ...
        with patch(
            ...
            response = client.post(
                "/api/team/clock-in",
                json={"user_id": "someone_else", "email": "victim@balizero.com"},
            )

        assert response.status_code == 403
        assert "yourself" in response.json()["detail"].lower()
        mock_timesheet_service.clock_in.assert_not_called()

    def test_clock_out_non_admin_impersonation_rejected(
        self, client, test_app, mock_timesheet_service
    ):
        """Same impersonation vector on clock-out."""
        test_app.state.current_user = {"email": "damar@balizero.com", "role": "team"}
        ...
        assert response.status_code == 403
        mock_timesheet_service.clock_out.assert_not_called()

    # ── INNOCENCE: authenticated non-admin clocking themselves ──────────

    def test_clock_in_non_admin_self_allowed(self, client, test_app, mock_timesheet_service):
        """A team member clocking THEMSELVES in must keep working."""
        test_app.state.current_user = {"email": "damar@balizero.com", "role": "team"}
        mock_timesheet_service.clock_in.return_value = {
            "success": True,
            "action": "clock_in",
        ...
        with patch(
            ...
            response = client.post(
                "/api/team/clock-in",
                json={"user_id": "damar_id", "email": "damar@balizero.com"},
            )
    ...
    def test_clock_in_non_admin_self_case_insensitive_email(
        self, client, test_app, mock_timesheet_service
    ):
        """Email match must not be defeated by case differences."""
        test_app.state.current_user = {"email": "Damar@Balizero.com", "role": "team"}
        mock_timesheet_service.clock_in.return_value = {
            "success": True,
            "action": "clock_in",
    ...
    # ── INNOCENCE: admin keeps the ability to act on behalf of others ───

    def test_clock_in_admin_on_behalf_of_other_still_allowed(
        self, client, test_app, mock_timesheet_service
    ):
        """Existing admin precedent (`is_crm_admin`) must not regress."""
        ...
        with patch(
            ...
            response = client.post(
                "/api/team/clock-in",
                json={"user_id": "someone_else_id", "email": "victim@balizero.com"},
            )

        assert response.status_code == 200
        mock_timesheet_service.clock_in.assert_called_once_with(
            user_id="someone_else_id",
            email="victim@balizero.com",
            metadata=None,
        )
...
class TestTeamStatusEndpoint:
    ...
    def test_get_team_status_no_auth(self, client, test_app, mock_timesheet_service):
        """Test team status without authentication"""
        test_app.state.admin_user = None
        test_app.state.current_user = None
        with patch(
            "backend.services.analytics.team_timesheet_service.get_timesheet_service",
            return_value=mock_timesheet_service,
...
class TestDailyHoursEndpoint:
    ...
    def test_get_daily_hours_forbidden_non_admin(self, client, test_app, mock_timesheet_service):
        ...
        with patch(
            ...
            response = client.get("/api/team/hours")
...
class TestExportTimesheetEndpoint:
    ...
    def test_export_timesheet_forbidden(self, client, test_app, mock_timesheet_service):
        ...
        with patch(
            ...
            response = client.get(
                "/api/team/export",
                params={"format": "csv", "start_date": "2025-12-01", "end_date": "2025-12-31"},
            )
