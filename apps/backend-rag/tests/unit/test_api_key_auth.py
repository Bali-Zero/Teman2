...
def api_key_auth():
    ...
    with patch("backend.app.services.api_key_auth.settings") as mock_settings:
        mock_settings.api_keys = "REDACTED-ROTATED-KEY,zantara-test-2024"
        return APIKeyAuth()
...
def admin_api_key_auth():
    ...
    `REDACTED-ROTATED-KEY` stopped being admin-by-legacy-allowlist once it was
    ...
    with patch("backend.app.services.api_key_auth.settings") as mock_settings:
        mock_settings.api_keys = "REDACTED-ROTATED-KEY,zantara-test-2024"
        mock_settings.api_key_roles = "REDACTED-ROTATED-KEY:admin"
        return APIKeyAuth()
...
def test_init_creates_valid_keys(api_key_auth):
    ...
    assert "REDACTED-ROTATED-KEY" in api_key_auth.valid_keys
...
def test_init_creates_admin_key(admin_api_key_auth):
    ...
    admin_key = admin_api_key_auth.valid_keys["REDACTED-ROTATED-KEY"]
...
def test_init_creates_key_stats(api_key_auth):
    ...
    assert "REDACTED-ROTATED-KEY" in api_key_auth.key_stats
...
def test_validate_api_key_success_admin(admin_api_key_auth):
    ...
    result = admin_api_key_auth.validate_api_key("REDACTED-ROTATED-KEY")
...
def test_validate_api_key_returns_metadata(api_key_auth):
    ...
    result = api_key_auth.validate_api_key("REDACTED-ROTATED-KEY")
...
def test_validate_api_key_metadata_values(api_key_auth):
    ...
    api_key_auth.validate_api_key("REDACTED-ROTATED-KEY")
    result = api_key_auth.validate_api_key("REDACTED-ROTATED-KEY")
...
def test_is_valid_key_admin_key(api_key_auth):
    ...
    assert api_key_auth.is_valid_key("REDACTED-ROTATED-KEY") is True
...
def test_get_key_info_admin_key(admin_api_key_auth):
    ...
    info = admin_api_key_auth.get_key_info("REDACTED-ROTATED-KEY")
...
def test_get_service_stats_key_usage_details(api_key_auth):
    ...
    assert "REDACTED-ROTATED-KEY" in key_usage
    assert "zantara-test-2024" in key_usage
    assert key_usage["REDACTED-ROTATED-KEY"]["usage_count"] == 1
...
def test_add_key_duplicate_returns_false(api_key_auth):
    ...
    result = api_key_auth.add_key("REDACTED-ROTATED-KEY", role="developer")
    assert result is False


def test_add_key_duplicate_does_not_modify(api_key_auth):
    """Test that adding duplicate key does not modify existing key"""
    original_role = api_key_auth.valid_keys["REDACTED-ROTATED-KEY"]["role"]
    api_key_auth.add_key("REDACTED-ROTATED-KEY", role="different")

    assert api_key_auth.valid_keys["REDACTED-ROTATED-KEY"]["role"] == original_role


def test_add_key_duplicate_logs_warning(api_key_auth):
    """Test that adding duplicate key logs warning"""
    with patch("backend.app.services.api_key_auth.logger") as mock_logger:
        api_key_auth.add_key("REDACTED-ROTATED-KEY", role="developer")
        mock_logger.warning.assert_called_once()
...
def test_remove_default_key(api_key_auth):
    ...
    result = api_key_auth.remove_key("REDACTED-ROTATED-KEY")
    assert result is True
    assert "REDACTED-ROTATED-KEY" not in api_key_auth.valid_keys
...
def test_multiple_key_validations_track_separately(api_key_auth):
    ...
    assert api_key_auth.key_stats["REDACTED-ROTATED-KEY"]["usage_count"] == 2
...
def test_different_roles_have_different_permissions(admin_api_key_auth):
    ...
    admin_result = admin_api_key_auth.validate_api_key("REDACTED-ROTATED-KEY")
...
def test_concurrent_usage_tracking(api_key_auth):
    ...
    assert stats["key_usage"]["REDACTED-ROTATED-KEY"]["usage_count"] == 2
