...
def _load_module(monkeypatch, aggregator_results=None):
    ...
    monkeypatch.setitem(
        sys.modules,
        "backend.services.analytics.analytics_aggregator",
    ...
    module_name = "backend.app.routers.analytics"
    if module_name in sys.modules:
        del sys.modules[module_name]

    spec = importlib.util.spec_from_file_location(
        module_name, backend_path / "backend" / "app" / "routers" / "analytics.py"
    )
    ...
    monkeypatch.setitem(
        ...
        types.SimpleNamespace(
            REGISTRY=types.SimpleNamespace(
                collect=lambda: metrics, redis_url="redis://localhost:6379"
            )
        ),
    )
...
def test_llm_usage_registry_error(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        "prometheus_client",
        types.SimpleNamespace(
            REGISTRY=types.SimpleNamespace(collect=_raise, redis_url="redis://localhost:6379")
        ),
    )
