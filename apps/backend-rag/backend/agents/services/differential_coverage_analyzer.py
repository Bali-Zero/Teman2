...
class DifferentialCoverageAnalyzer:
    ...
    def calculate_delta(
        self,
        current_report: UnifiedCoverageReport,
        ...
        for comp_name, current_comp in current_report.components.items():
            ...
            if baseline_comp:
                ...
                comp_delta = CoverageDelta(
                    ...
                    files_added=max(
                        0,
                        current_comp.files_analyzed - baseline_comp.get("files_analyzed", 0),
                    ),
                    files_removed=max(
                        0,
                        baseline_comp.get("files_analyzed", 0) - current_comp.files_analyzed,
                    ),
                )
