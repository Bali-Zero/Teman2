    ...
    async def run_full_scan(self, options: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if parallel_execution:
                logger.info(
                    "🚀 Running agents in PARALLEL mode (max %s concurrent)", max_concurrent
                )
...
async def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["scan", "coverage", "create", "maintain", "clean", "watch"],
        default="scan",
        help="Operation mode",
    )
    ...
    parser.add_argument(
        "--no-parallel",
        action="store_true",
        help="Disable parallel execution (use sequential)",
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
