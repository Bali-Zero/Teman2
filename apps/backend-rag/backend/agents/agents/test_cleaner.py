...
class TestAnalyzer:
    ...
    def _extract_test_classes(self, tree: ast.AST) -> list[dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef) and any(
                name.startswith("test") for name in node.name.lower()
            ):
                classes.append(
                    {
                        "name": node.name,
    ...
    def _detect_quality_issues(self, analysis: dict[str, Any]) -> list[str]:
        ...
        if analysis.get("mock_usage", {}).get("mock_calls", 0) == 0 and any(
            "import" in imp for imp in analysis.get("imports", [])
        ):
            issues.append("no_mocks_for_external_deps")

        # Too simple (always green)
        if (
            analysis.get("assertions", 0) <= 1
            and analysis.get("mock_usage", {}).get("mock_calls", 0) == 0
        ):
...
class OrphanDetector:
    ...
    def _check_if_orphaned(self, test_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"test_file": str(test_file), "is_orphaned": False, "error": str(e)}
    ...
    async def _cleanup_item(self, recommendation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            if not file_path.exists():
                return {"file": str(file_path), "success": False, "error": "File does not exist"}
