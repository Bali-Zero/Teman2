...
same driver-token custody as ``probe_evaluate.py``/``gold_replay_driver.py``
...
* The report is rewritten to disk after every walk, so a kill mid-run costs
  nothing: re-running with the same ``--report`` path resumes, skipping any
  walk already recorded with a terminal result (engine verdict or harness
  red). The report's header pins the manifest's own sha256 -- resuming
  against a DIFFERENT manifest (a different walk set) is refused, not
  silently merged.
...
def validate_budget(max_requests: int | None, rate_per_minute: float) -> None:
...
def _new_report(
    *, manifest_path: Path, digest: str, total_walks: int, url: str, max_requests: int, rate_per_minute: float
) -> dict[str, Any]:
    return {
...
def _dry_run_plan(
    *,
    ...
    pending = total_walks - already_recorded
...
async def run_live_enumeration(
    *,
    ...
    recorded_ids = {row["walk_id"] for row in (existing or {}).get("walks", [])}
    pending_walks = [w for w in walks if w["label"] not in recorded_ids]
    ...
    if dry_run:
        ...
        return _dry_run_plan(
            ...
            already_recorded=len(recorded_ids),
            max_requests=max_requests,
            rate_per_minute=rate_per_minute,
        )
    ...
    async with httpx.AsyncClient() as client:
        ...
        for walk in pending_walks:
            ...
            if result is None:
                ...
                break
            report["walks"].append(result.to_json())
            _write_report(report_path, report)
            if result.classification == "harness_red" and result.harness_detail == "http_429":
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "bounded and rate-limited, writing a resumable enumeration report."
        )
    )
...
def run(args: argparse.Namespace) -> int:
    try:
        validate_budget(args.max_requests, args.rate_per_minute)
    ...
    print(
        "wrote {recorded}/{total} walks to {path} (stopped_reason={reason})".format(
