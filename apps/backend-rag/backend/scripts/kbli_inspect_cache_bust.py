...
whole assembled `KBLIDetail` under `kbli_inspect_v8_{code}`, with a TTL from
...
WHAT IT DOES: for each `--only` code, reports whether `kbli_inspect_v8_{code}`
...
# missed. CI caught the pair coming apart, not a reader. It earned it a SECOND
# time on 2026-09-21 (v7 -> v8, the F9 demotion): the same constant was missed
# again, by a different author, for the same reason.
CACHE_KEY_TEMPLATE = "kbli_inspect_v8_{code}"
