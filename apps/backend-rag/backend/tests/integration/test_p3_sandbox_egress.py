...
docker itself). Run on Pro:
    ...
    docker compose \
      ...
      up -d --build
    cd apps/backend-rag && PYTHONPATH=. pytest backend/tests/integration/test_p3_sandbox_egress.py -v
...
_BACKEND = _HERE.parent.parent.parent.parent  # apps/backend-rag (integration -> tests -> backend -> backend-rag)
COMPOSE = [
    "docker", "compose",
