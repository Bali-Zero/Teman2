...
INTENTIONALLY_PUBLIC_MUTATIONS: tuple[IntentionalPublicMutation, ...] = (
    ...
    IntentionalPublicMutation(
        ...
        "GARUDA VOA magic-link preview (L4, this PR) — must be public for the "
        "same structural reason as the two rows above: the caller has not yet "
        "obtained a session (that is the whole point of previewing before "
        "exchanging), so there is no credential to require. Reviewed against "
        "the handler (preview_magic_link) and PostgresMagicLinkStore.peek() "
        "this PR: (1) NON-MUTATING — peek() issues a single `SELECT ... FROM "
        "garuda_magic_link_tokens`, never touches `used_at`, performs no "
        "INSERT/UPDATE/DELETE, takes no advisory lock, and increments no "
        "rate-limit counter of its own (see next point) — it is a read of "
        "the exact same row exchange() would read, just without the atomic "
        "UPDATE that spends it, so calling this route repeatedly changes "
        "nothing server-side. Classified MUTATING only because it is a POST, "
        "not because it mutates anything. (2) NON-ENUMERATING — a well-"
        "formed but unknown, expired, already-consumed, or foreign token "
        "all fall through the same `PeekOutcome(valid=False)` branch and "
        "answer the identical 401 MAGIC_LINK_INVALID as its exchange "
        "sibling; PeekOutcome carries no reason field a router could leak "
        "through. CORRECTED (Gear-3 council finding, codex-gpt-5.6-sol, "
        "2026-09-02): an earlier draft of this entry additionally claimed "
        "'absent, malformed' tokens land on the same 401 — false. Those "
        "fail MagicLinkExchange's `min_length=32` Pydantic constraint "
        "before store.peek ever runs and answer 422 INVALID_REQUEST "
        "instead; that is an orthogonal parse-error signal (the caller "
        "already knows its own request didn't parse), not a second "
        "distinguishable outcome about any real token's existence. "
        "(3) RATE LIMITING — no store-level throttle, same reasoning "
        "exchange() already documents for itself and this route's own "
        "docstring restates: the Protocol carries no client IP to key a "
        "per-identity limit on, so it answers under the generic per-IP "
        "`/api/` RateLimitMiddleware bucket (120 req/min) its two frozen "
        "siblings already share — not a new exemption, the same posture. "
        "(4) METHOD — deliberately kept POST, not narrowed to GET to dodge "
        "this gate: the actual caller (continue/page.tsx's server-side "
        "previewMagicLink()) sends the raw 32-byte bearer token in a JSON "
        "body, mirroring exchangeMagicLink's exact request shape; moving to "
        "GET would relocate that same secret into a URL query string, which "
        "access/CDN/error-tracking logs capture far more readily than a "
        "request body — a regression, not a hardening, for a route whose "
        "whole payload is a bearer credential. RESIDUAL, stated honestly "
        "rather than omitted (same Gear-3 council finding): masking.py "
        "reveals only the local part's first 2 characters, a fixed-width "
        "mask that a self-chosen attacker-controlled email can be crafted "
        "to collide with (e.g. 'jo-attacker@gmail.com' and 'john@gmail.com' "
        "both mask to 'jo***@gmail.com'). This does not affect THIS gate's "
        "mutating/non-enumerating verdict — it is a separate question about "
        "how effectively the masked preview defeats the login-CSRF spoof "
        "this endpoint exists to mitigate, tracked as a real, CONFIRMED, "
        "not-yet-fixed finding for the feature owner, not a hole in the "
        "authz posture this registry accounts for.",
    ),
