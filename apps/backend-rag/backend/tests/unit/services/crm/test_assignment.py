...
class TestNormalizePhone:
    ...
    def test_delegates_to_e164(self):
        ...
        with patch(
            "backend.services.crm.assignment.normalize_phone_e164", return_value="+6281234567890"
        ):
...
class TestClientIdentityResolver:
    ...
    async def test_resolve_creates_new(self):
        ...
        client_id, is_new = await resolver.resolve_or_create_client(
            "email", "new@x.com", client_data={"full_name": "New"}
        )
...
class TestCheckDuplicates:
    def _make_state(self, **kw):
        state = {
            "client_id": 1,
            "client_data": {"email": "j@x.com", "phone": "+62123"},
            "is_duplicate": False,
    ...
    async def test_no_duplicate(self):
        ...
        result = await check_duplicates(
            self._make_state(client_data={"email": None, "phone": None}), pool
        )
...
class TestAssignLead:
    ...
    async def test_duplicate_uses_existing(self):
        ...
        state = self._make_state(
            is_duplicate=True, matched_client_id=99, assigned_lead="existing@x.com"
        )
...
class TestSendTelegramNotification:
    def _make_state(self, **kw):
        state = {
            ...
            "is_duplicate": False,
            "matched_client_id": None,
            "assigned_lead": "lead@x.com",
            "assigned_lead_name": "Lead",
            "assignment_reason": "Dept match",
            "telegram_chat_id": None,
            "notification_sent": False,
            "success": True,
            "errors": [],
    ...
    async def test_no_lead_assigned(self):
        ...
        result = await send_telegram_notification(
            self._make_state(assigned_lead=None), AsyncMock(), AsyncMock()
        )
...
class TestTriggerLeadAssignment:
    async def test_trigger_success(self):
        ...
        with patch("backend.services.crm.assignment.create_lead_assignment_workflow") as m:
            mock_wf = AsyncMock()
            mock_wf.ainvoke.return_value = {
                "client_id": 1,
                "assigned_lead": "l@x.com",
                "notification_sent": True,
                "errors": [],
            }
            m.return_value = mock_wf
            result = await trigger_lead_assignment(
                1, {"full_name": "John"}, AsyncMock(), AsyncMock()
            )
