...
class TestAICRMExtractor:
    async def test_extract_from_conversation_success(self):
            ...
            "text": json.dumps(
                {
                    ...
                    "sentiment": "positive",
                    "urgency": "normal",
                    "summary": "KITAS inquiry",
                    "action_items": [],
                    "topics_discussed": ["visa"],
                    "extracted_entities": {
                        "dates": [],
                        "amounts": [],
                        "locations": [],
        ...
        result = await ext.extract_from_conversation(
            [{"role": "user", "content": "I need a KITAS visa"}]
        )
...
class TestShouldCreatePractice:
    async def test_should_create_high_confidence(self):
        ext, _ = _make_extractor()
        assert (
            await ext.should_create_practice(
                {
                    "practice_intent": {
                        "detected": True,
                        "confidence": 0.85,
...
class TestBirthplaceEnrichmentService:
    ...
    async def test_enrich_client_no_ollama_response(self):
        ...
        assert (
            await svc.enrich_client({"id": 1, "birthplace": "Rome", "nationality": "IT"}) is False
        )
...
class TestConversationTitleGenerator:
    ...
    async def test_generates_via_ollama(self):
        ...
        with patch(
            "backend.services.crm.enrichment._generate_title_via_ollama", new_callable=AsyncMock
        ) as m:
            m.return_value = "KITAS Visa Inquiry"
            result = await generate_conversation_title(
                "c1", "I need help with my KITAS visa application"
            )
    ...
    async def test_falls_back_to_gemini(self):
        ...
        with (
            ...
            patch(
                "backend.services.crm.enrichment._generate_title_via_gemini", new_callable=AsyncMock
            ) as m_g,
        ):
    ...
    async def test_all_methods_fail(self):
        ...
        with (
            ...
            assert (
                await generate_conversation_title("c1", "A long enough message for generation")
                is None
            )
