...
def test_every_unlocated_record_withholds_cap_and_condition(records: list[dict]) -> None:
    ...
    # 3(2), TERBUKA/100 unchanged. The rule reaches 507; 178 are withheld (61
    # finance/banking under Pasal 11(2), 67 in sectors whose own statute this
...
def test_umkm_reserved_is_tri_state_and_provenance_gated(records: list[dict]) -> None:
    ...
    # categories, 2 whose 2025 judul IS the activity a Lampiran III row caps
    # under a different number).
    # Each of the 329 is located + TERBUKA and named by no Lampiran II row, so
