...
def test_every_unlocated_record_withholds_cap_and_condition(records: list[dict]) -> None:
    """All 1,494 declared gaps fail closed, not just a hand-picked sample."""
    ...
    # 47722) moved declared_gap→located under Perpres 49/2021 Lampiran II
    # entry 46 (Koperasi/UMKM reservation), 1502→1494.
    assert len(unlocated) == 1494
...
def test_umkm_reserved_is_tri_state_and_provenance_gated(records: list[dict]) -> None:
    """Only the 65 located tuples may emit either a positive or negative claim."""
    # W-H PR-3b: 8 more codes (47241 47242 47244 47245 47246 47249 47712
    ...
    assert verdicts.count(True) == 26
    assert verdicts.count(False) == 3
    assert verdicts.count(None) == 1530
...
def test_only_located_zero_caps_enter_the_rejected_bucket(records: list[dict]) -> None:
    ...
    # 57 -> 65 located / 22 -> 30 rejected: W-H PR-3b's 8 codes (47241 47242
    # 47244 47245 47246 47249 47712 47722) are located with a cap of 0
    ...
    assert len(located) == 65
    assert len(new_rejected) == 30
