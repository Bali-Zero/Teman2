...
@pytest.mark.asyncio
async def test_submit_data_maps_locked_lifecycle_to_client_safe_conflict(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = SimpleNamespace(
        resolve_portal_client_id=AsyncMock(return_value=42),
        submit_form_data_for_client=AsyncMock(
            side_effect=LKPMDraftLockedError("private lifecycle state"),
        ),
    )
    monkeypatch.setattr(lkpm_router, "_get_service", lambda _pool: service)
    submission = LKPMClientSubmission(client_id=0, quarter="Q1", year=2026)
    ...
    assert (
        ...
        == "This LKPM report is locked after approval or OSS submission"
    )
    assert "private lifecycle state" not in str(exc_info.value.detail)
