...
from backend.services.compliance.lkpm_service import LKPMDraftLockedError, LKPMService
...
async def test_approve_service_atomically_rejects_other_tenant() -> None:
    ...
    assert "r.status IN ('validated', 'client_review', 'approved')" in sql
    assert "r.oss_submitted = FALSE" in sql
...
@pytest.mark.asyncio
async def test_save_draft_resets_stale_validation_only_for_mutable_report() -> None:
    pool, connection = _pool()
    connection.fetchrow.return_value = {"id": 301}
    service = _service_without_constructor(pool)

    result = await service._save_draft(
        LKPMDraft(
            client_id=CLIENT_A_ID,
            company_id=COMPANY_A_ID,
            quarter="Q3",
    ...
    assert "validation_alerts = '[]'::jsonb" in sql
    assert "validated_at = NULL" in sql
    assert "validated_by = NULL" in sql
    assert "lkpm_reports.status NOT IN ('approved', 'submitted', 'archived')" in sql
    assert "lkpm_reports.client_approved = FALSE" in sql
    assert "lkpm_reports.oss_submitted = FALSE" in sql


@pytest.mark.asyncio
async def test_save_draft_rejects_locked_report_atomically() -> None:
    pool, connection = _pool()
    connection.fetchrow.return_value = None
    service = _service_without_constructor(pool)

    with pytest.raises(
        LKPMDraftLockedError,
        match="Approved or submitted LKPM reports cannot be replaced",
    ):
        await service._save_draft(
            LKPMDraft(
                client_id=CLIENT_A_ID,
                company_id=COMPANY_A_ID,
                quarter="Q2",
