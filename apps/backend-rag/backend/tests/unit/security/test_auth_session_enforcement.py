...
async def test_revoked_session_is_rejected_for_header_and_cookie(
    ...
    async def exists(key: str) -> int:
        return int(revoked_scope == "token" and key == "revoked:synthetic-jti-1")

    async def get(key: str) -> str | None:
        if revoked_scope == "user" and key == "revoked_user:synthetic.user@example.test":
            return str(datetime.now(timezone.utc).timestamp() + 60)
        return None

    redis_client.exists = AsyncMock(side_effect=exists)
    redis_client.get = AsyncMock(side_effect=get)
...
@pytest.mark.asyncio
async def test_emergency_revocation_switch_skips_async_store_lookup() -> None:
    from backend.services.security.token_revocation import is_session_revoked

    with (
        patch("backend.app.core.config.settings.enable_token_revocation", False),
        patch("backend.core.redis_manager.RedisManager.get_instance") as get_manager,
    ):
        assert await is_session_revoked({"jti": "synthetic-jti-1"}) is False
