...
def test_valid_fixtures_round_trip(load_json: Any) -> None:
    ...
    assert len(fixture_paths) >= 3, "expected at least the 3 known valid conductor_handoff fixtures"
...
def test_invalid_fixtures_reject_with_exact_expected_reason(load_json: Any) -> None:
    ...
    assert len(fixture_paths) >= 4, "expected at least the 4 known invalid conductor_handoff fixtures"
    for fixture_path in fixture_paths:
