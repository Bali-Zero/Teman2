...
def monitor() -> MagicMock:
    fake = MagicMock()
    fake.get_dashboard_data = AsyncMock(
        return_value={
            "time_range": "24h",
            "total_queries": 10,
    ...
    fake.get_latency_percentiles = AsyncMock(
        return_value={
            "period_days": 7,
            "total_queries": 10,
...
def app(monitor: MagicMock) -> FastAPI:
    ...
    application.dependency_overrides[monitoring_module.verify_admin_access] = lambda: {
        "id": "1",
        "role": "admin",
    }
    application.dependency_overrides[monitoring_module.get_retrieval_quality_monitor] = lambda: (
        monitor
    )
    return application
