...
full Kleene tables for all/any/not (including mixed children) and the
known/unknown presence operators.

Spec: research/visa/2026-07-17-visa-oracle-v2-round2-codex-engine-concretization.md
§4.1 (lines 2341-2359).
"""
...
KNOWN_INT_SNAPSHOT = FactSnapshot(
    values={
        "intent.stay_days": KnownFact(value=30),
        "intent.purposes": KnownFact(value=frozenset({"TOURISM", "BUSINESS_MEETINGS"})),
    }
)
UNKNOWN_SNAPSHOT = FactSnapshot(
    values={
        "intent.stay_days": UnknownFact(reason=UnknownReason.NOT_ASKED),
        "intent.purposes": UnknownFact(reason=UnknownReason.NOT_PROVIDED),
    }
)
...
class TestPresenceOperators:
    ...
    def test_unknown_op_false_on_known_fact(self) -> None:
        cond = UnknownCondition(op="unknown", fact="intent.stay_days")
...
def test_comparison_operators_on_known_fact(condition, expected: TruthValue) -> None:
...
def test_comparison_operators_on_unknown_fact_always_unknown(condition) -> None:
    ...
    assert "intent.stay_days" in result.unknown_facts or "intent.purposes" in result.unknown_facts
...
def _leaf(truth: str) -> object:
    """Build a leaf condition against a snapshot where the fact's known
    value is engineered to make the leaf evaluate to TRUE/FALSE, or a leaf
    referencing an unknown fact for UNKNOWN."""
...
MIXED_SNAPSHOT = FactSnapshot(
    values={
        "intent.stay_days": KnownFact(value=30),
        "intent.desired_entry_date": UnknownFact(reason=UnknownReason.NOT_ASKED),
    }
)
