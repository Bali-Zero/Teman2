...
SNAPSHOT = FactSnapshot(
    values={
        "immigration.overstay_days": KnownFact(value=100),  # deciding fact, evaluated first
        "intent.stay_days": UnknownFact(reason=UnknownReason.NOT_ASKED),
        "intent.desired_entry_date": UnknownFact(reason=UnknownReason.NOT_PROVIDED),
        "work.employer_country_code": UnknownFact(reason=UnknownReason.UNVERIFIED),
    }
)
...
def test_all_first_child_false_still_evaluates_and_records_later_children() -> None:
    ...
    second_child = KnownCondition(op="known", fact="intent.stay_days")  # references unknown fact
    third_child = KnownCondition(op="known", fact="intent.desired_entry_date")  # also unknown
    ...
    assert result.referenced_facts == frozenset(
        {"immigration.overstay_days", "intent.stay_days", "intent.desired_entry_date"}
    )
    assert result.unknown_facts == frozenset({"intent.stay_days", "intent.desired_entry_date"})
...
def test_any_first_child_true_still_evaluates_and_records_later_children() -> None:
    ...
    second_child = KnownCondition(op="known", fact="work.employer_country_code")  # unknown
    ...
    assert result.referenced_facts == frozenset(
        {"immigration.overstay_days", "work.employer_country_code", "intent.stay_days"}
    )
    # third_child's "known" is FALSE for work.employer_country_code (it IS unknown -> known() is FALSE,
    # but the fact itself is unknown so it still belongs in unknown_facts).
    assert "work.employer_country_code" in result.unknown_facts
    assert "intent.stay_days" in result.unknown_facts
...
def test_nested_all_any_not_every_leaf_contributes_to_referenced_facts() -> None:
        ...
        args=[
            ...
            NotCondition(
                ...
                arg=AnyCondition(
                    op="any",
                    args=[
                        KnownCondition(op="known", fact="intent.stay_days"),
                        KnownCondition(op="known", fact="intent.desired_entry_date"),
                        KnownCondition(op="known", fact="work.employer_country_code"),
                    ],
                ),
            ),
        ],
    ...
    # collect_fact_paths (pure AST walk, no evaluation) must find the same set.
    assert collect_fact_paths(tree) == expected_paths
...
def test_collect_fact_paths_single_leaf() -> None:
    leaf = KnownCondition(op="known", fact="intent.stay_days")
