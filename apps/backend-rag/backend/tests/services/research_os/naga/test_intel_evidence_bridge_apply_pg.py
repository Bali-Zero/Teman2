...
async def test_a_claim_missing_a_required_field_is_refused_by_name_and_writes_nothing(
    ...
    def _claim_write_missing_confidence(mapped, *, item, evidence_write):
        write = real_build_claim_write(mapped, item=item, evidence_write=evidence_write)
...
# --------------------------------------------------------------------------------------------
# 7. Idempotency ACROSS A MOVED COHORT -- and its RED twin, which is the defect PR #7004 was
#    BLOCKED for (2026-09-21). Test 1 runs both applies on ONE manifest, the only condition in
#    which a manifest-derived value inside a per-object payload is invisible; `regulatory_watcher`
#    is a daily cron, so in production the cohort moves overnight, every night.
# --------------------------------------------------------------------------------------------


async def _seed_second_item(conn: asyncpg.Connection) -> None:
    await _seed_intel_item(
        conn,
        canonical_url="https://example.invalid/reg-cohort-moved-second",
        citation="PMK 3/2026",
        verbatim_excerpt="Sesuai dengan PMK 3/2026, ketentuan berlaku efektif mulai Maret.",
        published_at=datetime(2026, 3, 1, tzinfo=timezone.utc),
    )
...
async def _seed_first_item(conn: asyncpg.Connection) -> None:
    await _seed_intel_item(
        conn,
        canonical_url="https://example.invalid/reg-cohort-moved-first",
        citation="PMK 2/2026",
        verbatim_excerpt="Sesuai dengan PMK 2/2026, ketentuan berlaku efektif mulai Februari.",
        published_at=datetime(2026, 2, 1, tzinfo=timezone.utc),
    )
...
def _claim_write_with_manifest_bound_run_id(manifest: str):
    """PR #7004's exact derivation, reinstated: `Claim.lineage.run_id = uuid5(_RUN_NAMESPACE,
    manifest)`. Everything else in the payload is the real builder's own output."""

    real = ieb._build_claim_write

    def _build(mapped, *, item, evidence_write):
        write = real(mapped, item=item, evidence_write=evidence_write)
        payload = dict(write.payload)
        payload["lineage"] = dict(
            payload["lineage"], run_id=str(uuid.uuid5(ieb._RUN_NAMESPACE, manifest))
        )
        payload["object_hash"] = "0" * 64
        payload["object_hash"] = _real_object_hash(payload)
        return ObjectWrite(
            object_kind=write.object_kind, object_id=write.object_id, payload=payload
        )

    return _build


async def test_apply_after_the_cohort_moved_rewrites_nothing_for_the_unchanged_item(
    db: asyncpg.Connection,
) -> None:
    await _seed_first_item(db)
    first = await ieb.run_dry_run(db)
    assert first.admitted == 1, first.excluded_by_reason
    report1 = await ieb.run_apply(db, manifest=first.manifest_hash)
    assert report1.objects_inserted == 2
    hashes_before = {
        object_id: object_hash for _kind, object_id, object_hash in report1.written
    }

    await _seed_second_item(db)
    second = await ieb.run_dry_run(db)
    assert second.admitted == 2, second.excluded_by_reason
    # Without this the test would prove nothing: the cohort really did move.
    assert second.manifest_hash != first.manifest_hash

    report2 = await ieb.run_apply(db, manifest=second.manifest_hash)
    # Only the NEW item's pair is new. The first item's two objects come back
    # `already_present`, which `_insert_object` reports ONLY when the recomputed `object_hash`
    # equals the stored one -- a differing hash raises `object_id_hash_collision` instead.
    assert report2.objects_inserted == 2
    assert report2.objects_already_present == 2

    stored = {
        row["object_id"]: row["object_hash"]
        for row in await db.fetch(
            "SELECT object_id::text AS object_id, object_hash FROM research_os_objects "
            "WHERE object_id::text = ANY($1::text[])",
            list(hashes_before),
        )
    }
    assert stored == hashes_before


async def test_a_claim_run_id_bound_to_the_manifest_collides_once_the_cohort_moves(
    db: asyncpg.Connection, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The RED twin of the test above: reinstate PR #7004's derivation and the SAME sequence
    dies on `object_id_hash_collision`, rolling back the new item's pair with it -- the bridge
    would have been wedged from the first night onwards, not merely non-idempotent."""

    await _seed_first_item(db)
    first = await ieb.run_dry_run(db)
    monkeypatch.setattr(
        ieb, "_build_claim_write", _claim_write_with_manifest_bound_run_id(first.manifest_hash)
    )
    report1 = await ieb.run_apply(db, manifest=first.manifest_hash)
    assert report1.objects_inserted == 2
    ...
    assert second.manifest_hash != first.manifest_hash
    monkeypatch.setattr(
        ieb, "_build_claim_write", _claim_write_with_manifest_bound_run_id(second.manifest_hash)
    )

    before = await _counts(db)
    with pytest.raises(NagaWriteRejected) as excinfo:
        await ieb.run_apply(db, manifest=second.manifest_hash)
