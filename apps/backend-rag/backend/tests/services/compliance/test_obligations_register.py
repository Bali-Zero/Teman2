...
def test_a_roll_none_rule_ignores_the_holiday_block_after_it():
    # Sat 2026-08-15 is followed by Sun 16 and Independence Day on Mon 17, so the next business day
    # is Tue 18 Aug -- and none of that reaches a rule with roll: none. This was pinned on
    # bpjs_ketenagakerjaan_monthly until M4 (#6336) read PP 44/2015 art. 21(3) and PP 46/2015
    # art. 19(3) and flipped BOTH BPJS rules to next_business_day, which left the assertion
    # failing on main. The subject is the engine's roll semantics, not which rule the catalog
    # happens to exempt, so it is pinned on a synthetic rule the catalog cannot invalidate.
    rule = ObligationRule(
        "synthetic_15", "n", "a", "s", False, (), DueRule("monthly", 15, None, 1, "none", "fiscal")
    )
    rolling = replace(rule, due=replace(rule.due, roll="next_business_day"))
    assert due_dates(rule, PMA, date(2026, 8, 1), 30) == [("2026-07", date(2026, 8, 15))]
    assert due_dates(rolling, PMA, date(2026, 8, 1), 30) == [("2026-07", date(2026, 8, 18))]
...
def test_a_roll_none_rule_is_not_flagged_for_an_undecreed_year():
    # Nothing about an unknown holiday table can change a date that never moves: Sun 2027-01-10
    # stays, and the proposal carries the rule's own reason and nothing else. Also ex-catalog
    # since M4 flipped bpjs_kesehatan_monthly to next_business_day (Perpres 82/2018 art. 39(4)).
    rule = ObligationRule(
        "synthetic_10",
        "n", "a", "s", False, (),
        DueRule("monthly", 10, None, 1, "none", "fiscal"),
        needs_review_reason="the rule's own reason",
    )  # fmt: skip
    [proposed] = propose([rule], PMA, date(2027, 1, 1), 15)
    assert proposed.due_date == date(2027, 1, 10)
    assert proposed.needs_review_reason == "the rule's own reason"
...
def test_propose_is_sorted_and_carries_review_reasons(catalog):
    ...
    for p in proposed:
        if catalog[p.rule_id].due.frequency == "one_time":
            continue  # a "once" proposal carries the M2 note too; pinned by its own tests below
