"""
Localized server-owned stub messages shared across service boundaries.

This stdlib-only module is the single source of truth. It deliberately lives
outside ``backend.services.rag.agentic`` so API-light consumers such as the
WhatsApp outbox generator do not execute the eager agentic package imports.
...
_STUB_LANGUAGE_BY_CODE: dict[str, str] = {
    "it": "ITALIAN",
    "en": "ENGLISH",
    "id": "INDONESIAN",
    "ru": "RUSSIAN",
...
def get_localized_stub_for_text(
    key: str,
    text: str,
    history: Sequence[Mapping[str, str]] | None = None,
) -> str:
    """Resolve a stub from client text, using recent user history if needed."""
    language = _STUB_LANGUAGE_BY_CODE.get(detect_language(text, history), "ENGLISH")
