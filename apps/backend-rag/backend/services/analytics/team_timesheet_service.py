    ...
    async def get_my_status(self, user_id: str) -> dict[str, Any]:
        ...
        async with self.pool.acquire() as conn:
            ...
            return {
                ...
                "today_hours": float(today_hours["hours_worked"])
                if today_hours and today_hours["hours_worked"] is not None
                else 0.0,
                "week_hours": float(week_summary["total_hours"])
                if week_summary and week_summary["total_hours"] is not None
                else 0.0,
                "week_days": int(week_summary["days_worked"])
                if week_summary and week_summary["days_worked"] is not None
                else 0,
            }
