    ...
    async def get_experiment_results(self, experiment: str) -> dict[str, Any]:
        ...
        try:
            ...
            for metric_name in ["ctr", "satisfaction", "response_time", "evidence_score"]:
                control_data = variant_metrics.get(config.variants[0], {}).get("raw", {}).get(metric_name, [])
                treatment_data = variant_metrics.get(config.variants[1], {}).get("raw", {}).get(metric_name, [])
                
                if len(control_data) >= config.min_sample_size and len(treatment_data) >= config.min_sample_size:
            ...
            return {
                ...
                "total_queries": sum(
                    v.get("count", 0) for v in variant_metrics.values()
                ),
                "is_active": config.enabled and (
...
n        t-test for continuous metrics (response_time, evidence_score).
    ...
    def _calculate_significance(
        ...
        df = (
            (control_var / len(control_data) + treatment_var / len(treatment_data)) ** 2
        ) / (
    ...
    async def get_dashboard_data(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_experiments": sum(
                1 for e in self.experiments.values() if e.enabled
            ),
