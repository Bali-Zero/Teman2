...
REFERENCE_PATTERNS = [
    ...
    (r"sebagaimana\s+(?:telah\s+)?diatur\s+(?:dalam|oleh)\s+(UU|PP|Perpres|Permen)\s+(?:No\.?\s*)?(\d+)\s+Tahun\s+(\d{4})", "REFERENCES"),

    # "berdasarkan UU No. X/Y"
    (r"berdasarkan\s+(UU|PP|Perpres|Permen)\s+(?:No\.?\s*)?(\d+)[/\s](\d{4})", "REFERENCES"),

    # "mengacu pada PP No. X Tahun Y"
    (r"mengacu\s+(?:pada|ke)\s+(UU|PP|Perpres|Permen)\s+(?:No\.?\s*)?(\d+)\s+Tahun\s+(\d{4})", "REFERENCES"),

    # "diubah dengan UU No. X Tahun Y"
    (r"(?:telah\s+)?diubah\s+(?:dengan|oleh)\s+(UU|PP|Perpres|Permen)\s+(?:No\.?\s*)?(\d+)\s+Tahun\s+(\d{4})", "AMENDS"),

    # "mencabut UU No. X Tahun Y"
    (r"mencabut\s+(UU|PP|Perpres|Permen)\s+(?:No\.?\s*)?(\d+)\s+Tahun\s+(\d{4})", "REPLACES"),

    # "pelaksanaan UU No. X Tahun Y"
    (r"(?:untuk\s+)?pelaksanaan\s+(UU|PP|Perpres)\s+(?:No\.?\s*)?(\d+)\s+Tahun\s+(\d{4})", "IMPLEMENTS"),
]
...
async def find_semantic_links(
    ...
    try:
        ...
        connected_entities = [e for e in all_entities if e["id"] not in {o["id"] for o in orphan_entities}]
