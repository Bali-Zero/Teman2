            ...
            async with self.semaphore:
                try:
                    async with pool.acquire() as conn:
                        ...
                        if not row:
                            logger.warning(f"⚠️ [KBLI:{code}] Node not found in Postgres. Creating placeholder node...")
                            ...
                            await conn.execute(
                                ...
                                """,
                                entity_id, f"KBLI {code}", json.dumps({})
                            )
                            current_props = {}
                        else:
                    ...
                    attempt += 1
                    logger.error(f"❌ [KBLI:{code}] Postgres update failed (Attempt {attempt}): {e}")
