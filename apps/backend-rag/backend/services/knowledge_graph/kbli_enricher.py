...
class KBLIEnricher:
        ...
        search_filter = models.Filter(
            must=[
                models.FieldCondition(
                    key="kode_kbli",
                    match=models.MatchValue(value=section_prefix) # This is a simplified logic, we might need MatchText or Range
                )
            ],
            must_not=[
                models.HasIdCondition(has_id=[]) # Placeholder for logic to find "not yet processed"
            ]
        )
    ...
    async def enrich_single_code(self, kbli_payload: dict, external_research: Optional[str] = None):
        ...
        while attempt < self.retries:
            async with self.semaphore:
                try:
                    logger.info(f"[KBLI:{code}] - [STEP:ENRICH] - [ATTEMPT:{attempt+1}] - Starting...")
                    ...
                    market_insights = external_research if external_research else "Pending deep research..."
                    ...
                    logger.info(f"[KBLI:{code}] - [STEP:FUSION] - [STATUS:OK] - Successfully enriched")
                    return True
    ...
    async def run_batch(self, section_prefix: str, research_data: Optional[dict] = None):
