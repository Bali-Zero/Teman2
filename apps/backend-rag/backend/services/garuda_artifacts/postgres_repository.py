    ...
    async def insert(
        ...
        *,
        ...
        try:
            ...
            # Either `artifact_id`'s PK (astronomically unlikely -- 128
            # random bits from journal.new_opaque_id), `storage_key`'s
            # UNIQUE, or `ux_garuda_practice_artifacts_live`'s partial
            # unique index (a live artifact already exists for this
            # practice -- by far the reachable case: a second
            # putPracticeArtifact call with a fresh Idempotency-Key while
            # the first artifact is still live). All three collapse to the
            # same caller-facing fact: this insert did not happen.
    ...
    async def get_live_for_order(
        ...
        row = await conn.fetchrow(
            ...
            SELECT a.artifact_id, a.practice_id, a.storage_key, a.artifact_digest,
                   ...
                   a.created_at, a.retention_until, a.superseded_at
