...
async def main() -> int:
    ...
    try:
        ...
        # Self-commit cure (cure-A, PR #7 mandate): kpi_timeline.csv / the
        # weekly report / retrain_log.jsonl above land straight into the main
        # checkout's tracked tree and this job never committed them — same
        # disease PR #3756 promoted the one-shot backlog for. See
        # _promote.py for the pattern (translate-articles.py /
        # regulatory-watcher-run.sh).
        promote_paths = [
            "research/sota-social-2026-v1/kpi_timeline.csv",
            f"research/sota-social-2026-v1/weekly_report_{today}.md",
        ]
        if retrain_needed:
            promote_paths.append("research/sota-social-2026-v1/retrain_log.jsonl")
        promote_research_output(
            repo,
            promote_paths,
            commit_subject=f"chore(sota): promote weekly report {today}",
            commit_body=(
                "Auto-generated by backend.services.sota_loop.m13_weekly "
                "(com.balizero.sota.m13-weekly.plist). See "
                "apps/backend-rag/backend/services/sota_loop/_promote.py."
            ),
            task_prefix="sota-weekly",
        )
