...
def main() -> int:
    ...
    # Self-commit cure (cure-A, PR #7 mandate): the monthly report + archived
    # weights above land straight into the main checkout's tracked tree and
    # this job never committed them — same disease PR #3756 promoted the
    # one-shot backlog for. See _promote.py for the pattern
    # (translate-articles.py / regulatory-watcher-run.sh).
    promote_paths = [f"research/sota-social-2026-v1/monthly_report_{month}.md"]
    if weights_path.is_file():
        promote_paths.append(f"research/sota-social-2026-v1/09_wr2_weights_{month}.json")
    promote_research_output(
        repo,
        promote_paths,
        commit_subject=f"chore(sota): promote monthly report {month}",
        commit_body=(
            "Auto-generated by backend.services.sota_loop.m13_monthly "
            "(com.balizero.sota.m13-monthly.plist). See "
            "apps/backend-rag/backend/services/sota_loop/_promote.py."
        ),
        task_prefix="sota-monthly",
    )
    return 0
