...
def main() -> int:
    ...
    # Self-commit cure (cure-A, PR #7 mandate): the checkpoint report above
    # landed straight into the main checkout's tracked tree and this job
    # never committed it — same disease PR #3756 promoted the one-shot
    # backlog for. See _promote.py for the pattern (translate-articles.py /
    # regulatory-watcher-run.sh).
    promote_research_output(
        repo,
        [f"research/sota-social-2026-v1/checkpoint_day_{days}.md"],
        commit_subject=f"chore(sota): promote checkpoint day {days}",
        commit_body=(
            "Auto-generated by backend.services.sota_loop.m13_checkpoint "
            "(com.balizero.sota.m13-checkpoint.plist). See "
            "apps/backend-rag/backend/services/sota_loop/_promote.py."
        ),
        task_prefix="sota-checkpoint",
    )
    return 0
