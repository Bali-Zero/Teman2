...
class _HookContext:
    """What a commit hook is told: the scoped key digest and a monotonic call counter.
    Never a payload, an outcome or an actor id -- a hook is a scheduling seam, not a
    second reader of the document."""

    key_sha256: bytes
    call_index: int


_CommitHook = Callable[[_HookContext], Awaitable[None]]


@dataclass(frozen=True, slots=True)
class _CommitHooks:
    """TEST SEAM. Two optional awaitables `commit()` calls at the two points a race test
    needs to control (SPEC v2 §4.2):

    - `after_lookup`: inside the transaction, after the `SELECT ... FOR UPDATE` and before
      the policy check. Exactly once per `commit()`. Two racers that both block here have
      both observed the key ABSENT inside their own transaction, so neither can have
      committed: whichever inserts second MUST collide on the primary key. That is what
      makes the collision branch forced rather than scheduler-dependent.
    - `after_collision_before_reread`: after the INSERT lost on `garuda_documents_pkey` and
      the transaction unwound, before the fresh-connection re-read. Once per collision.
      Holding a loser here is how a test purges the winner (schedule C) or re-occupies the
      key (schedule D) between the collision and the re-read.

    Production never sets this: the keyword is underscore-named, defaults to `None`, and
    `test_the_default_constructor_has_no_hooks` pins that a store built the way a factory
    builds one carries none. PR3, which writes the factory, asserts it there as well.
    """

    after_lookup: _CommitHook | None = None
    after_collision_before_reread: _CommitHook | None = None
    ...
    async def commit(
        ...
        try:
            async with self._pool.acquire() as conn, conn.transaction():
                ...
                if self._hooks is not None and self._hooks.after_lookup is not None:
                    await self._hooks.after_lookup(_HookContext(key_hash, self._commit_calls))
            ...
            #   no row at all  -> the key is bound to NOTHING now: IdempotencyKeyVanished.
            #                     304's `guard_garuda_document_mutation` forbids DELETE only
            #                     while `clock_timestamp() < retention_until`, so the winner
            #                     can legitimately have been purged between the collision
            #                     and this re-read (SPEC v2 §2, outcome 3). Not corruption,
            #                     not impossible, not a lost race; typed so a caller can
            #                     catch it without importing this store. No retry.
            # What the loser CANNOT observe, and the port does not promise: whether the row
            # it reads belongs to the writer that beat it or to a third caller who took the
            # key after a purge. The collision was its first knowledge that any row existed,
            # so there is no earlier generation to compare against. Re-occupation is
            ...
            if winner is None:
                raise IdempotencyKeyVanished(idempotency_key)
