...
def verification_commands_for_paths(target_paths: list[str]) -> list[str]:
    ...
    if any(path.startswith("apps/admin-dashboard/") for path in target_paths):
        commands.append(ADMIN_DASHBOARD_LINT_COMMAND)
...
def plan_for_allowlisted_command(
    ...
    *,
    ...
    if normalized == PYTEST_AUTONOMOUS_LAB_COMMAND:
        pytest = _pytest_executable(backend_root)
        if pytest is None:
            return None
    ...
    if normalized == ADMIN_DASHBOARD_LINT_COMMAND:
        ...
        return CommandExecutionPlan(
            command=normalized,
            argv=[npm, "run", "lint"],
            cwd=repo_root / "apps" / "admin-dashboard",
...
def _pytest_executable(backend_root: Path) -> str | None:
...
def autonomous_lab_pytest_env() -> dict[str, str]:
...
def git_diff_check_env() -> dict[str, str]:
    """Return the minimal environment needed for shell-free git diff checks."""
    return {"CI": "true", "PATH": "/usr/bin:/bin"}


def _tool_path_with_optional_executable(executable: str | None) -> str:
...
def expected_env_for_allowlisted_command(command: str) -> dict[str, str] | None:
    """Return the exact environment allowed for a verification command."""
    normalized = command.strip()
    if normalized == PYTEST_AUTONOMOUS_LAB_COMMAND:
    ...
    if normalized in {GENERIC_GIT_DIFF_CHECK_COMMAND, GIT_DIFF_RESEARCH_COMMAND}:
        return git_diff_check_env()
    return None
