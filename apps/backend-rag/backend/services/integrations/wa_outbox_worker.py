...
async def _process_claimed_row(
    ...
    *,
    ...
    async with conn.transaction():
        ...
        # clock read. served_by (migration 322) rides the same statement
        # unconditionally, same as evidence_score: fly.toml's
        # `release_command` (`migrate apply-all && schema_audit`) runs on
        # the NEW image before Fly replaces any machine and fails the
        # release outright on a pending migration (schema_audit.py's
        # `_check_pending_migrations`), so this column is guaranteed
        # present the moment this worker code is ever live — no
        # deploy-window fallback needed here (same guarantee migration
        # 314's abstained_at/evidence_score already relies on).
