...
class DriveAuthManager:
    ...
    async def _get_or_refresh_oauth_token(self, user_id: str) -> Optional[str]:
        """
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if time_to_expiry < 300:
                    logger.info(f"Token OAuth per {user_id} in scadenza (TTL: {time_to_expiry}s). Inizio refresh.")
                    return await self._refresh_oauth_token(user_id, refresh_token)
                
                return access_token

        except Exception as e:
            logger.error(f"Errore critico nel recupero token OAuth per {user_id}: {e}", exc_info=True)
            return None

    async def _refresh_oauth_token(self, user_id: str, refresh_token: str) -> Optional[str]:
        """
        ...
        if not refresh_token:
            logger.error(f"Impossibile effettuare il refresh per {user_id}: refresh_token mancante.")
            if METRICS_ENABLED:
        ...
        if not client_id or not client_secret:
            logger.error("Credenziali OAuth (Client ID/Secret) non configurate nelle variabili d'ambiente.")
            return None
        ...
        try:
            ...
            logger.error(f"Fallimento HTTP durante il refresh del token Google OAuth per {user_id}: {e.response.text}", exc_info=True)
            if METRICS_ENABLED:
                drive_oauth_refresh_total.labels(status="failed").inc()
            return None
        except Exception as e:
            logger.error(f"Errore di sistema imprevisto durante il refresh token per {user_id}: {e}", exc_info=True)
            if METRICS_ENABLED:
                drive_oauth_refresh_total.labels(status="failed").inc()
            return None
    ...
    async def _get_service_account_token(self) -> Optional[str]:
        ...
        try:
            ...
            if not sa_json_str:
                logger.error("La variabile d'ambiente GOOGLE_SERVICE_ACCOUNT_JSON non è configurata.")
                return None
            ...
            logger.error(f"GOOGLE_SERVICE_ACCOUNT_JSON contiene JSON non valido: {e}", exc_info=True)
            return None
        except Exception as e:
    ...
    async def get_user_allowed_folders(self, user_email: str) -> Tuple[List[str], bool]:
        ...
        try:
            ...
            logger.error(f"Fallimento recupero permessi folder per l'utente {user_email}: {e}", exc_info=True)
            return ([], False)
