...
class DrivePermissionsManager:
    ...
    def __init__(self, auth_manager: Any, http_client: httpx.AsyncClient, audit: Optional[Any] = None):
    ...
    async def add_permission(self, user_email: str, file_id: str, request: Any) -> Dict[str, Any]:
        ...
        email = getattr(request, "email", None) or (request.get("email") if isinstance(request, dict) else None)
        role = getattr(request, "role", None) or (request.get("role") if isinstance(request, dict) else None)
    ...
    async def remove_permission(self, user_email: str, file_id: str, permission_id: str) -> Dict[str, bool]:
        """
