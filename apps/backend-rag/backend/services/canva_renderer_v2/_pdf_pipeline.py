...
)  # canva_renderer_v2/services/backend/backend-rag/apps/<repo> → repo root
...
def render_pdf(
    ...
    *,
    ...
    if result.returncode != 0:
        raise PdfRenderError(f"subprocess exit≠0 for draft {draft_id}: {result.stderr[:500]}")
    if not out_path.exists() or out_path.stat().st_size == 0:
        raise PdfRenderError(f"renderer produced zero-size output for draft {draft_id}")
    return out_path
