...
def _is_garuda_flow_enabled() -> bool:
    """Condition for the owner-only GARUDA archive support router."""
    from backend.app.core.config import settings
...
ROUTER_MANIFEST: tuple[RouterEntry, ...] = (
    ...
    RouterEntry(name="crm_guardian_drive", process_groups=_API, tags=("crm", "drive")),
    ...
    RouterEntry(
        name="kbli_notebook_chat", process_groups=_RAG, prefix="__API_V1__", tags=("kbli",)
    ),
    # ── KG ──
    ...
    RouterEntry(
        name="observed_shell", process_groups=_API, tags=("observability", "cell-core", "internal")
    ),
    # ── Omnichannel ──
    ...
    # ── Visa Oracle consultant-assignment event (C3 — public, exact path, rate-limited) ──
    RouterEntry(name="visa_oracle_consultant", process_groups=_API, tags=("visa",)),
    # ── Voice ──
...
def validate_manifest() -> list[str]:
    ...
    for entry in ROUTER_MANIFEST:
        ...
        if invalid:
            errors.append(f"Router '{entry.name}' has invalid process_groups: {invalid}")
