...
async def kg_query(
    ...
    with trace_span("kg_agentic_query", {"query_length": len(request.query)}):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"KG query processing failed: {e!s}",
            ) from e
...
async def list_golden_routes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to list golden routes: {e!s}",
        ) from e
...
async def kg_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get KG stats: {e!s}",
        ) from e
...
async def find_kg_path(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to find path: {e!s}",
        ) from e
...
async def kg_visualize(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to visualize KG: {e!s}",
        ) from e
