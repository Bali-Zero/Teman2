...
class MessageLog(SQLModel, table=True):
    ...
    message_id: str = Field(
        primary_key=True,
        max_length=255,
        description="Unique message ID from OpenClaw",
    )
    ...
    timestamp: datetime = Field(
        nullable=False,
        index=True,
        description="Message timestamp from OpenClaw",
    )
    ...
    channel: str = Field(
        nullable=False,
        max_length=50,
        description="Communication channel (e.g., whatsapp, sms)",
    )
    direction: str = Field(
        nullable=False,
        max_length=20,
    ...
    media_type: str | None = Field(
        default=None,
        max_length=50,
        description="Media type if present (image, video, etc.)",
    )
    ...
    session_id: str | None = Field(
        ...
        max_length=255,
        index=True,
        description="Session ID from metadata",
    )
    reply_to: str | None = Field(
        default=None,
        max_length=255,
        description="Reply-to message ID from metadata",
    )
