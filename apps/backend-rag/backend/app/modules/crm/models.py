...
class PracticeType(SQLModel, table=True):
    ...
    base_price: float | None = Field(
        default=None,
        sa_column=Column("base_price", type_=DECIMAL(12, 2)),
    )
...
class Practice(SQLModel, table=True):
    ...
    quoted_price: float | None = Field(
        default=None,
        sa_column=Column("quoted_price", type_=DECIMAL(12, 2)),
    )
    actual_price: float | None = Field(
        default=None,
        sa_column=Column("actual_price", type_=DECIMAL(12, 2)),
    )
    currency: str = Field(default="IDR", max_length=10, sa_column_kwargs={"server_default": "IDR"})
    payment_status: str = Field(
        default="unpaid", max_length=50, sa_column_kwargs={"server_default": "unpaid"}
    )
