-- ============================================================================
-- 315_visa_oracle_consultant_requests.sql
--
-- ----------------------------------------------------------------------------
-- RENUMBERED 2026-09-14 (split from PR #5037, per the gate's own recommended
-- shape): drafted as 293 on the original, now-suspended PR — gated for 4
-- failed rounds on this repo's tripwire test, unrelated to this table
-- (Builder Contract §1 depth-1 cap on a fix-of-a-fix). origin/main advanced
-- to 314 while that PR sat suspended. Measured fresh this turn:
--   ls apps/backend-rag/backend/db/migrations_v2 | sort -V | tail
--     -> highest present is 314 (314_wa_outbox_evidence.sql)
--   gh pr list --state open --json files | grep migrations_v2
--     -> no open PR claims 315/316/317
-- -> renumbered 293->315 here, siblings 294->316 and 295->317 alongside it.
-- Everything below, including this file's own now-stale bind-time provenance
-- comment naming 281, is otherwise UNCHANGED from the version the split-gate
-- already reviewed as clean.
-- ----------------------------------------------------------------------------
--
