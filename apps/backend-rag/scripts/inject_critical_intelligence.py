...
async def run():
    ...
    await update_group(red_zone, "Zona Rossa Bali", "⚠️ MORATORIA ATTIVA (INGUB 6/2025): Nuove licenze per retail/catene sospese a Bali.")
    await update_group(umkm_trap, "Trappola UMKM", "🚫 RISERVATO LOCALI: Questo codice è limitato alle Micro/Piccole imprese. Vietato alle PMA stranieri.", "RESERVAT_LOCALS")
    await update_group(high_stakes, "High Stakes PMA", "💰 INVESTIMENTO MASSICCIO: Richiesto capitale versato min. 10 Miliardi IDR per investitori stranieri.")
    await update_group(tech_economy, "Tech & Consulting", "🌐 VIRTUAL OFFICE ALLOWED: Ideale per startup digitali e consulenti. Possibilità di registrazione presso uffici virtuali accreditati.")
    await update_group(closed_sectors, "Settore Chiuso", "❌ BLACKLIST TOTALE: Attività proibita in Indonesia o completamente chiusa agli investimenti privati/stranieri.", "TERTUTUP")
