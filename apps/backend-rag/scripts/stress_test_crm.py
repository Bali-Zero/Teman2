...
class CRMStressTester:
    ...
    async def run(self, num_clients: int, cases_per_client: int):
        ...
        logger.info(f"🎯 Target: {num_clients} Clients, {num_clients * cases_per_client} Cases")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(f"✅ Created {len(created_client_ids)} clients. Starting cases...")
    ...
    def _print_report(self, total_time: float):
        ...
        print(
            f"👥 Clients Created: {self.stats['clients_created']} (Failed: {self.stats['clients_failed']})"
        )
        print(
            f"📁 Cases Created:   {self.stats['cases_created']} (Failed: {self.stats['cases_failed']})"
        )
