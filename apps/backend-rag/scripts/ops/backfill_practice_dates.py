...
async def backfill():
    ...
    # Get all practices
    practices = await conn.fetch("SELECT id, practice_type_code, status, created_at, completion_date FROM practices")
    ...
    for p in practices:
        ...
        await conn.execute("""
            ...
            WHERE id = $4
        """, expected_completion_date, expiry_date, next_renewal_date, p_id)
