...
- Plan for long-term property strategy""",
    ...
    try:
        ...
        assert "80 years" in result['facts'] or "80-year" in result['facts'], "Facts should contain key information"
        assert len(result['bali_zero_take']['hidden_insight']) > 50, "Hidden insight should be extracted"
        assert len(result['bali_zero_take']['our_analysis']) > 50, "Our analysis should be extracted"
...
def test_conversion_minimal_data():
    ...
    try:
        ...
        print(f"✅ Conversion successful with minimal data!")
        print(f"\n📊 Results:")
        print(f"   Title: {result['title']}")
        ...
        assert result['tldr']['should_worry'] in ["Yes", "No", "Depends"], "Should have valid worry level"
...
def test_enriched_article_structure():
    ...
    try:
        ...
        required_fields = [
            "title", "headline", "tldr", "facts", "bali_zero_take",
            "next_steps", "category", "priority", "relevance_score",
            "ai_summary", "ai_tags", "suggested_components",
            "source", "source_url", "enriched_at"
        ]
        ...
        assert isinstance(result['suggested_components'], list), "Suggested components should be list"
