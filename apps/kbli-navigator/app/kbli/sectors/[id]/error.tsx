...
export default function SectorDetailError({
  ...
  useEffect(() => {
    console.error('KBLI sector detail error:', error);
  }, [error]);

  return (
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
      <p className="text-4xl">⚠️</p>
      <h2 className="text-xl font-semibold text-white">Sector not available</h2>
      <p className="text-sm text-[var(--foreground-muted)] max-w-sm">
        Could not load this sector. It may not exist or there was a temporary error.
      </p>
