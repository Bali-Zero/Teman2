...
class DatabaseBackup:
    ...
    async def create_backup(self, backup_name: Optional[str] = None) -> Path:
        ...
        logger.info(
            f"Creating database backup: {backup_name}",
    ...
    async def restore_backup(self, backup_path: Path) -> None:
        ...
        logger.info(
            f"Restoring backup: {backup_path}",
            action=LogAction.START
        )
        ...
        try:
            ...
            if backup_path.suffix == '.sql' and backup_path.name.endswith('_decompressed.sql'):
                backup_path.unlink()
