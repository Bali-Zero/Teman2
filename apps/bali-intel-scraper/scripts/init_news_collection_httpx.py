...
def init_collection():
    ...
    with httpx.Client(timeout=30.0, headers=headers) as client:
        ...
        try:
            ...
            response = client.put(
                f"{QDRANT_URL}/collections/{COLLECTION_NAME}", json=collection_config
            )
            response.raise_for_status()
            ...
            for field_name, field_type in indices:
                try:
                    ...
                    if e.response.status_code == 400:
                        logger.warning(
                            f"   ⚠️ Indice '{field_name}' già esistente o errore: {e.response.text[:100]}"
                        )
                    else:
                        raise
            ...
            logger.error(
                f"❌ Errore HTTP: {e.response.status_code} - {e.response.text[:200]}"
            )
            raise
