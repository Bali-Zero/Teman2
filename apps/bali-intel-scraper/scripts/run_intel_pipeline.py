...
def build_staging_payload(art: dict) -> dict:
    ...
    if enr.get("live_news_score") is not None:
        ...
        payload["liveness_tier"] = (
            _tier if _tier else _derive_tier_from_score(enr["live_news_score"])
        )
...
class IntelPipeline:
    ...
    def run_step(self, step: str) -> bool:
        ...
        try:
            ...
            self.update_step_status(
                step,
                "failed",
                {"error": str(e), "fatal": True, "reason": "unhandled_exception"},
            )
            return False
    ...
    def step_scraping(self) -> bool:
        ...
        try:
            ...
            categories = self.config.get(
                "categories", ["immigration", "tax", "business"]
            )
            ...
            self.log(
                f"Scraped {len(articles)} articles from {scraper.stats.get('sources_processed', 0)} sources"
            )
            ...
            if os.environ.get("EXA_API_KEY"):
                try:
                    ...
                    _exa_thread = threading.Thread(
                        target=_exa_worker, daemon=True, name="exa-search"
                    )
                    _exa_thread.start()
                    if _exa_done.wait(timeout=600):  # 10min max
                        ...
                        self.log(
                            "Exa augmentation timed out after 10min — skipping", "WARN"
                        )
                        exa_articles = []
                    ...
                    self.log(
                        f"Exa added {len(new_exa)} unique articles (filtered {len(exa_articles) - len(new_exa)} dupes)"
                    )
                except Exception as e:
            ...
            if os.environ.get("INTEL_RADAR_PERSIST_DB", "false").lower() == "true":
                try:
                    ...
                    self.log(
                        f"intel-radar augmentation failed (non-fatal): {e}", "WARN"
                    )
    ...
    def step_validation(self) -> bool:
        ...
        if not articles:
            self.update_step_status(
                "2_validation", "skipped", {"reason": "no_articles"}
            )
            return True
        ...
        if published_file.exists():
            try:
                ...
                self.log(
                    f"  Loaded {len(published_urls)} published URLs for history check"
                )
            except Exception as e:
            ...
            if published:
                try:
                    ...
                    if (now - pub_date).days > 30:
                        # old article, skip
                        self.log(
                            f"    Skipping old article ({published}): {title[:50]}"
                        )
                        continue
                except Exception:
    ...
    def step_qwen_filter(self) -> bool:
        ...
        if not articles:
            self.update_step_status(
                "2.5_qwen_filter", "skipped", {"reason": "no_articles"}
            )
            return True
        ...
        try:
            ...
            if not ollama_model:
                self.log(
                    f"No suitable model in Ollama ({models}), falling back", "WARN"
                )
                return self._qwen_filter_fallback(articles)
        except Exception as e:
            self.log(
                f"Ollama not available ({e}), falling back to keyword filter", "WARN"
            )
            return self._qwen_filter_fallback(articles)
        ...
        for batch_start in range(0, len(articles), BATCH_SIZE):
            ...
            try:
                resp = _httpx.post(
                    ...
                    timeout=90
                    + 20 * len(batch),  # 90s base (cold start) + ~20s per article
                )
                ...
                self.log(
                    f"  Batch {batch_num}/{total_batches}: {len(results)}/{len(batch)} scored "
        ...
        if geo_demoted:
            self.log(
                f"  Geo-filter: {geo_demoted} non-Indonesia articles demoted to ≤10"
            )
        ...
        self.log(
            f"Qwen filter: {len(passed)} passed / {len(filtered)} filtered "
    ...
    def step_quality_gate(self) -> bool:
        ...
        if not articles:
            self.update_step_status(
                "2.6_quality_gate", "skipped", {"reason": "no_articles"}
            )
            return True
        ...
        try:
            ...
            self.log(
                "quality_gate module not found — passing all articles through", "WARN"
            )
            self.update_step_status(
                "2.6_quality_gate", "skipped", {"reason": "module_not_found"}
            )
            return True
        ...
        self.state["archived_articles"] = (
            self.state.get("archived_articles", []) + archived
        )
    ...
    def step_verification(self) -> bool:
        ...
        if not articles:
            self.update_step_status(
                "2.7_verification", "skipped", {"reason": "no_articles"}
            )
            return True
        ...
        regulatory = [
            a
            for a in articles
            if a.get("qwen_category", a.get("category", ""))
            in self.REGULATORY_CATEGORIES
        ]
        non_regulatory = [a for a in articles if a not in regulatory]

        self.log(
            f"  Regulatory: {len(regulatory)}, Non-regulatory: {len(non_regulatory)}"
        )
                ...
                for art in regulatory:
                    ...
                    try:
                        ...
                        if relevant:
                            ...
                            verified_count += 1
                            self.log(
                                f"    T1 verified: {title[:50]}... ({len(relevant)} relevant official sources)"
                            )
                        else:
                            art["verification"] = {
                                "status": "unverified",
                            ...
                            self.log(
                                f"    Unverified: {title[:50]}... (no relevant T1 source)"
                            )
                    except Exception as e:
                        art["verification"] = {
                            "status": "check_failed",
        ...
        if openai_key:
            try:
                ...
                self.log(
                    f"  KB cross-check: {kb_match_count}/{len(articles)} have existing KB context"
                )
    ...
    def step_clustering(self) -> bool:
        ...
        if not articles or len(articles) <= 1:
            self.update_step_status(
                "2.8_clustering", "skipped", {"reason": "too_few_articles"}
            )
            return True
        ...
        if not openai_key:
            ...
            self.update_step_status(
                "2.8_clustering", "skipped", {"reason": "no_api_key"}
            )
            return True
        ...
        try:
            ...
            embeddings = [
                d["embedding"]
                for d in sorted(emb_resp.json()["data"], key=lambda x: x["index"])
            ]
        except Exception as e:
        ...
        for i in range(n):
            ...
            while changed:
                ...
                for j in range(n):
                    ...
                    max_sim = max(
                        cosine_sim(embeddings[k], embeddings[j]) for k in cluster
                    )
        ...
        for cluster_idx, cluster in enumerate(clusters):
            ...
            for sup in supporting:
                ...
                if sup.get("verification"):
                    source_info["verification"] = sup["verification"].get(
                        "status", "unknown"
                    )
        ...
        self.log(
            f"  Single-article: {singles}, Multi-source: {merged} (absorbed {total_merged_in} articles)"
        )
        for d in dossiers:
            size = d["dossier"]["cluster_size"]
            marker = f"[{size} sources]" if size > 1 else ""
            self.log(
                f"    {d['quality_score']:3d}/100 [{d['tier']}] {d['title'][:55]} {marker}"
            )
    ...
    def step_nlm_context(self) -> bool:
        ...
        if not articles:
            self.update_step_status(
                "2.9_nlm_context", "skipped", {"reason": "no_articles"}
            )
            return True
    ...
    def _select_top_articles(self, articles: list[dict], max_count: int) -> list[dict]:
        ...
        ranked = sorted(
            articles,
            key=lambda a: (
                tier_order.get(a.get("tier", "T3"), 2),
                -a.get("quality_score", 50),
            ),
        )
    ...
    def step_enrichment(self) -> bool:
        ...
        if not articles:
            ...
            self.update_step_status(
                "3_enrichment", "skipped", {"reason": "no_articles"}
            )
            return True
        ...
        self.log(
            f"Selected {len(top_articles)}/{len(articles)} articles for enrichment "
        ...
        try:
            ...
            self.log(
                f"Enriched {ok}/{len(enriched)} articles ({len(remaining)} unenriched kept)"
            )

            if top_articles and ok == 0:
                self.update_step_status(
                    "3_enrichment",
                    "failed",
                    {
                        ...
                        "enriched": 0,
                        "reason": "all_enrichment_providers_failed",
                    },
                )
                return False
            ...
            if digest:
                ...
                self.log(
                    f"Intel digest generated ({len(digest.get('topics', []))} topics)"
                )
            ...
            self.update_step_status(
                "3_enrichment",
                "failed",
                {"fatal": True, "error": str(e)},
            )
            return False

    def _generate_intel_digest(
        self, enriched: list[dict], unenriched: list[dict]
    ) -> dict | None:
        ...
        articles_text = "\n".join(
            article_lines[:80]
        )  # Cap at 80 to stay within context
...
"""
    ...
    def step_images(self) -> bool:
        ...
        all_enriched = [
            a for a in self.state.get("articles", []) if a.get("enrichment")
        ]
        published = [
            a for a in all_enriched if a.get("tier") == "T1" or a.get("featured")
        ]
        if not published:
            # Fallback: if no T1/featured, take top-scored articles (up to 5)
            published = sorted(
                all_enriched, key=lambda a: a.get("quality_score", 0), reverse=True
            )[:5]
        if not published:
        ...
        self.log(
            f"Generating images for {len(published)} T1/featured articles (of {len(all_enriched)} enriched)"
        )
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=1800,  # 30min max (~2min/image × 15)
        ...
        if result.returncode != 0:
            ...
            self.log(
                f"Image generation failed (exit {result.returncode}): {stderr_full[-500:]}",
                "ERROR",
            )
            if stdout_full:
        ...
        try:
            ...
            self.state["articles"] = updated_state.get(
                ...
                for art in self.state.get("articles", []):
                    ...
                    try:
                        ...
                        r = await c.post(
                            ...
                            json={
                                "cover_image_base64": img_b64,
                                "cover_image_filename": p.name,
                            },
                        ...
                        if r.status_code in (200, 201):
                            ...
                            self.log(
                                f"Cover upload failed for {item_id}: {r.text}", "WARN"
                            )
                    except Exception as e:
        ...
        self.update_step_status(
            "8_images",
            "completed",
            {"images_generated": images_count, "covers_uploaded": covers_uploaded},
        )
        return True
    ...
    def step_seo(self) -> bool:
        ...
        if not enriched:
            ...
            self.update_step_status(
                "5_seo", "skipped", {"reason": "no_enriched_articles"}
            )
            return True
        ...
        try:
            ...
            seo_by_url = {
                a.get("url", ""): a.get("seo") for a in optimized if a.get("seo")
            }
    ...
    def step_approval(self) -> bool:
        ...
        for art in enriched:
            ...
            if (
                (tier == "T1" and score >= 80)
                or (is_multi and score >= 75)
                or (verified and score >= 85)
            ):
                art["featured"] = True
            else:
                art["featured"] = False
        ...
        self.log(
            f"Featured: {featured_count}/{len(enriched)} articles marked for homepage"
        )
        ...
        for i, art in enumerate(enriched, 1):
            ...
            review_lines.append(f"{'=' * 80}")
            featured_tag = (
                "*** FEATURED (COPERTINA) ***" if art.get("featured") else "STANDARD"
            )
            ...
            review_lines.append(
                f"Source: {art.get('source_name', art.get('source', 'N/A'))}"
            )
            review_lines.append(f"Category: {art.get('category', 'N/A')}")
            review_lines.append(
                f"Tier: {art.get('tier', 'N/A')} | Score: {art.get('quality_score', 'N/A')}/100"
            )
            ...
            if verif:
                review_lines.append(
                    f"Verification: {verif.get('status', 'not_checked')}"
                )
            # Dossier info
            dossier = art.get("dossier", {})
            if dossier.get("cluster_size", 1) > 1:
                review_lines.append(
                    f"Dossier: {dossier['cluster_size']} sources merged"
                )
                for sup in dossier.get("supporting_sources", []):
                    review_lines.append(
                        f"  + [{sup.get('tier', '?')}] {sup.get('title', '')[:60]} ({sup.get('source', '')})"
                    )
            review_lines.append("")
            ...
            if brief:
                ...
                review_lines.append(
                    f"Should I worry? {brief.get('should_i_worry', 'N/A')}"
                )
            ...
            if meta:
                ...
                review_lines.append(
                    f"Reading time: {meta.get('reading_time_minutes', 'N/A')} min"
                )
        ...
        TG_RECIPIENTS = os.environ.get(
            "TELEGRAM_APPROVAL_CHAT_ID", os.environ.get("TELEGRAM_GROUP_ID", "")
        )
        BACKEND_URL = os.environ.get("BACKEND_URL", "https://nuzantara-rag.fly.dev")
        API_KEY = os.environ.get("SCRAPER_API_KEY", "")

        all_chat_ids = (
            [cid.strip() for cid in TG_RECIPIENTS.split(",") if cid.strip()]
            if TG_RECIPIENTS
            else []
        )
            ...
            if recipient["role"] == "designer":
                ...
                try:
                    from gemini_image_generator import (
                        build_image_prompt as _build_prompt,
                    )
                except ImportError:
                    _build_prompt = None
                    ...
                    try:
                        ...
                        if tg_msg_id and API_KEY and BACKEND_URL:
                            try:
                                ...
                                self.log(
                                    f"Notification mapping failed for {item_id}: {e}",
                                    "WARN",
                                )
                ...
                self.log(
                    f"Sent {min(len(enriched), 20)} article notifications to Damar ({chat_id})"
                )

            else:
                # --- OWNER: compact Italian summary (unchanged) ---
                lines = [
                    f"<b>Intel Pipeline — {len(enriched)} articoli pronti per review</b>\n"
                ]
                for i, art in enumerate(enriched[:20], 1):
                    enr = art.get("enrichment", {})
                    ...
                    priority = enr.get("metadata", {}).get("priority", "?")
                    lines.append(
                        f"{i}. {featured_icon}<b>[{priority.upper()}]</b> <i>{cat}</i>\n   {headline}"
                    )
    ...
    def step_publishing(self) -> bool:
        ...
        approval_status = (
            self.state.get("steps", {}).get("6_approval", {}).get("status", "")
        )
        if approval_status == "pending" and not self.config.get("auto_approve"):
            self.log(
                "BLOCKED: publishing halted, approval still pending. Use --auto-approve to bypass.",
                "WARN",
            )
            self.update_step_status(
                "7_publishing", "blocked", {"reason": "approval_pending"}
            )
            return True
        ...
        TG_RECIPIENTS = _os.environ.get(
            "TELEGRAM_APPROVAL_CHAT_ID", _os.environ.get("TELEGRAM_GROUP_ID", "")
        )
        enriched = [a for a in self.state.get("articles", []) if a.get("enrichment")]
        published = 0
        if not enriched:
            self.log(
                "No enriched articles are eligible for News Room submission; "
                "suppressing completion notification",
                "WARN",
            )
            self.update_step_status(
                "7_publishing",
        ...
        async def _push(art):
            ...
            if art.get("image_path"):
                try:
                    ...
                    if img_p.exists() and img_p.stat().st_size > 5000:
                        payload["cover_image_base64"] = _b64.b64encode(
                            img_p.read_bytes()
                        ).decode("utf-8")
                except Exception as _img_err:
                    self.log(
                        f"Warning: could not read image for upload: {_img_err}", "WARN"
                    )
            async with _httpx.AsyncClient(timeout=60) as c:
                ...
                if r.status_code in (200, 201):
                    ...
                    if item_id:
                        self.log(
                            f"Submitted to staging: {item_id} ({intel_type}) — awaiting News Room approval"
                        )
                    ...
                    try:
                        ...
                        _url = (
                            ...
                            or f"bali-intel-scraper://item/{item_id or 'unknown'}"
                        )
        ...
        if published == 0:
            self.log(
                "No article was accepted by the News Room; suppressing completion notification",
                "ERROR",
            )
            self.update_step_status(
                "7_publishing",
                "failed",
                {
                    ...
                    "submitted": 0,
                    "reason": "all_news_room_submissions_failed",
                },
            )
            return False
        self.log(
            f"✅ Inviati {published}/{len(enriched)} articoli alla News Room per approvazione"
        )

        # Notify only after at least one real News Room submission. A zero-item
        # run is handled above and must never look like successful delivery.
        ...
        try:
            ...
            for art in enriched:
                if not art.get("_staging_item_id"):
                    continue
        ...
        self.update_step_status(
            "7_publishing",
            "completed",
            {"selected": len(enriched), "submitted": published},
        )
        return True
                ...
                articles = [
                    {
                        ...
                        "published_date": (
                            r["published_at"] or r["observed_at"]
                        ).isoformat(),
    ...
    def run(self):
        ...
        # Execute pipeline. Once enrichment fails completely, every downstream
        # side effect is dependency-blocked even under --continue-on-error.
        enrichment_failed = False
        enrichment_dependents = {"5_seo", "6_approval", "8_images", "7_publishing"}
        for step in steps_to_run:
            if enrichment_failed and step in enrichment_dependents:
                self.update_step_status(
                    step,
                    "blocked",
                    {
                        ...
                        "dependency": "3_enrichment",
                    },
                )
                continue
            success = self.run_step(step)
            if not success:
                step_state = self.state.setdefault("steps", {}).setdefault(step, {})
                step_data = step_state.setdefault("data", {})
                if not step_data.get("degraded_ok"):
                    step_data.setdefault("fatal", True)
                    step_data.setdefault("reason", "step_returned_false")
            if step == "3_enrichment" and not success:
        ...
        failed_steps = [
            ...
            if self.state.get("steps", {}).get(step, {}).get("data", {}).get("fatal")
        ]
        if failed_steps:
        ...
        if degraded_steps:
            self.state["degraded_steps"] = degraded_steps
        ...
        # Preserve the last-known-good canonical artifact on fatal runs.
        if not failed_steps:
            try:
                ...
                self.log(
                    f"✅ intel_output_latest.json salvato ({len(self.state.get('articles', []))} articoli)"
                )
            except Exception as e:
                self.log(f"⚠️  Salvataggio intel_output_latest.json fallito: {e}", "WARN")
        else:
            self.log(
                "Canonical intel_output_latest.json preserved after fatal pipeline failure",
                "WARN",
            )
...
def main():
    ...
    parser.add_argument(
        "--categories",
        default="immigration,tax,legal",
        help="Comma-separated categories",
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Limit articles per source"
    )
    parser.add_argument(
        "--min-score", type=int, default=40, help="Minimum quality score"
    )

    # Flow control
    parser.add_argument(
        "--resume-from", choices=PIPELINE_STEPS, help="Resume from specific step"
    )
    parser.add_argument(
        "--skip", dest="skip_steps", help="Comma-separated steps to skip"
    )
    parser.add_argument(
        "--continue-on-error",
        action="store_true",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run (no publishing)"
    )
    parser.add_argument(
        "--max-enrich",
        type=int,
    ...
    if not args.no_cell_emit:
        try:
            ...
            print("⚠️ cell post-emit timeout after 60s (non-fatal)", file=sys.stderr)
        except Exception as cell_exc:
            print(f"⚠️ cell post-emit skipped (non-fatal): {cell_exc}", file=sys.stderr)
