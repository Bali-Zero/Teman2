...
async def test_deduplicator():
    ...
    if not openai_key:
        logger.error(
            "❌ OPENAI_API_KEY non configurato. Impossibile generare embedding."
        )
        return False
    ...
    is_dup, match, score = await dedup.is_duplicate(
        test_article_1["title"], test_article_1["summary"], test_article_1["url"]
    )
    ...
    is_dup, match, score = await dedup.is_duplicate(
        test_article_2["title"], test_article_2["summary"], test_article_2["url"]
    )
    ...
    is_dup, match, score = await dedup.is_duplicate(
        test_article_3["title"], test_article_3["summary"], test_article_3["url"]
    )
    ...
    logger.info("\n💡 Prossimi passi:")
    logger.info(
        "   1. Verifica che la collezione 'balizero_news_history' esista su Qdrant"
    )
