...
async def main():
    ...
    try:
        ...
        if is_dup:
            logger.warning(
                f"   ⚠️ Risultato inatteso: Trovato duplicato (Score: {score:.2f})"
            )
        else:
        ...
        if is_dup:
            logger.success(
                f"   ✅ DUPLICATO RILEVATO correttamente! (Score: {score:.2f})"
            )
    ...
    logger.info(
        "   - I duplicati verranno filtrati PRIMA di chiamare Claude (risparmio $$$)"
    )
