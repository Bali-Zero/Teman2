        ...
        async def async_wrapper(self: "BaseService", *args, **kwargs):
            ...
            with log_context(component=self._component_name):
                ...
                try:
                    ...
                    duration = time.time() - start_time
                    self._metrics.timing(f"{operation_name}.duration", duration)
                    self._metrics.increment(f"{operation_name}.error", 
                                          tags={"error_code": e.error_code})
...
class BaseService(ABC, Generic[T]):
    ...
    async def initialize(self) -> None:
        ...
        self._logger.info(
            f"Initializing {self._service_name} service",
            action=LogAction.START
        )
        ...
        try:
            ...
            self._logger.info(
                f"{self._service_name} service initialized",
                action=LogAction.END
            )
        except Exception as e:
            self._logger.error(
    ...
    async def shutdown(self) -> None:
        ...
        self._logger.info(
            f"Shutting down {self._service_name} service",
            action=LogAction.START
        )
        ...
        try:
            ...
            self._initialized = False
            self._logger.info(
                f"{self._service_name} service shut down",
                action=LogAction.END
            )
        except Exception as e:
            self._logger.error(
