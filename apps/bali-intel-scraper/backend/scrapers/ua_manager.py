...
class UserAgentManager:
    ...
    def get_by_browser(self, browser: str) -> Optional[str]:
        ...
        matching = [ua for ua in self._user_agents if ua.browser.lower() == browser.lower()]
        if matching:
    ...
    def get_by_device(self, device: str) -> Optional[str]:
        ...
        matching = [ua for ua in self._user_agents if ua.device.lower() == device.lower()]
        if matching:
            ua = random.choice(matching)
            ua.usage_count += 1
            ua.last_used = datetime.now()
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "most_used": max(self._user_agents, key=lambda x: x.usage_count).string if self._user_agents else None,
        }
