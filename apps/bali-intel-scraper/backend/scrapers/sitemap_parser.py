...
class SitemapParser:
    ...
    def _parse_url_entry(
        ...
        # Parse lastmod
        lastmod = None
        lastmod_elem = url_elem.find(f'{ns}lastmod', self.NAMESPACES if use_namespace else {})
        ...
        # Parse changefreq
        changefreq = None
        cf_elem = url_elem.find(f'{ns}changefreq', self.NAMESPACES if use_namespace else {})
        if cf_elem is not None and cf_elem.text:
            changefreq = cf_elem.text
        
        # Parse priority
        priority = None
        pri_elem = url_elem.find(f'{ns}priority', self.NAMESPACES if use_namespace else {})
