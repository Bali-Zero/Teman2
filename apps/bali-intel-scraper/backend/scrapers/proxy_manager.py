...
class ProxyManager:
    ...
    async def get_proxy(
        ...
        async with self._lock:
            available = [
                ...
                (target_host is None or target_host not in p.banned_hosts) and
                (country is None or p.country == country)
            ]
    ...
    async def report_success(self, proxy_url: str, response_time_ms: float) -> None:
        ...
        async with self._lock:
            ...
            if proxy:
                ...
                ) if proxy.response_time_ms > 0 else response_time_ms
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "total_requests": sum(p.success_count + p.fail_count for p in self._proxies),
