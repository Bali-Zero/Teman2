...
from backend.scrapers.browser import browser_manager, StealthPlugin
    ...
    async def _get_or_create_context(self, host: Optional[str] = None) -> str:
        ...
        async with self._lock:
            ...
            for context_id, pooled in self._contexts.items():
                if (pooled.status == PoolItemStatus.AVAILABLE and
                    len(self._context_pages[context_id]) < self.max_pages_per_context):
        ...
        async with self._lock:
            ...
            for page_id, pooled_page in list(self._pages.items()):
                if pooled_page.status != PoolItemStatus.IN_USE:
                    ...
                    if (age > self.max_age_seconds or
                        pooled_page.use_count > self.max_uses):
            ...
            contexts_to_remove = []
            for context_id, pooled_context in list(self._contexts.items()):
                if (pooled_context.status != PoolItemStatus.IN_USE and
                    not self._context_pages.get(context_id)):
                    
                    age = now - pooled_context.created_at
                    if age > self.max_age_seconds or pooled_context.use_count > self.max_uses:
                        try:
                            await pooled_context.context.close()
                        except Exception:
                            pass
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            "contexts": {
                ...
                "available": sum(1 for c in self._contexts.values() if c.status == PoolItemStatus.AVAILABLE),
                "in_use": sum(1 for c in self._contexts.values() if c.status == PoolItemStatus.IN_USE),
            },
            "pages": {
                "total": len(self._pages),
                "available": sum(1 for p in self._pages.values() if p.status == PoolItemStatus.AVAILABLE),
                "in_use": sum(1 for p in self._pages.values() if p.status == PoolItemStatus.IN_USE),
            },
            "max_contexts": self.max_contexts,
            "max_pages_per_context": self.max_pages_per_context
        }
