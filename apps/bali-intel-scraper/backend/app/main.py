...
async def _handle_sigterm() -> None:
    ...
    logger.info(
        "SIGTERM received, initiating graceful shutdown", action=LogAction.START
    )
    _shutdown_event.set()
...
def setup_signal_handlers() -> None:
    ...
    try:
        ...
        logger.info("Signal handlers registered", action=LogAction.CONNECT)
    except NotImplementedError:
        # Windows doesn't support add_signal_handler
        logger.warning(
            "Signal handlers not supported on this platform", action=LogAction.SKIP
        )


async def initialize_services() -> None:
    """Initialize all required services."""
    logger.info("Initializing services", action=LogAction.START)
    ...
    logger.info("All services initialized", action=LogAction.END)


async def shutdown_services() -> None:
    """Gracefully shutdown all services."""
    logger.info("Shutting down services", action=LogAction.START)
    ...
    for name, shutdown_func in shutdown_order:
        try:
            ...
            await shutdown_func()
            logger.info(f"{name} shut down successfully", action=LogAction.END)
        except Exception as e:
            logger.error(
    ...
    logger.info("Shutdown complete", action=LogAction.END)
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    logger.info(
        ...
        action=LogAction.START,
        metadata={"environment": settings.env.value, "debug": settings.debug},
    )
    ...
    async def global_exception_handler(request, exc):
        ...
        return JSONResponse(
            ...
            content={
                ...
                "request_id": get_logger(__name__).logger.handlers[0].request_id
                if hasattr(get_logger(__name__).logger.handlers[0], "request_id")
                else None,
            },
        )
