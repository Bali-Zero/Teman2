...
async def deep_health_check() -> HealthStatus:
    ...
    return HealthStatus(
        ...
        uptime_seconds=round(time.time() - _start_time, 2),
        checks={k: v.dict() if isinstance(v, ComponentHealth) else v for k, v in checks.items()}
    )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "p95_response_time_ms": round(
                sorted(self.request_times)[int(len(self.request_times) * 0.95)] * 1000, 2
            ) if len(self.request_times) >= 20 else None
        }
