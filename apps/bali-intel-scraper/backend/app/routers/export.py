...
from backend.models.schemas import ArticleResponse
...
class Exporter:
    ...
    def _export_json(self, articles: list) -> StreamingResponse:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=articles_{datetime.now():%Y%m%d}.json"}
        )
    ...
    def _export_csv(self, articles: list) -> StreamingResponse:
        ...
        if not articles:
            return StreamingResponse(
                io.StringIO(""),
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename=articles_{datetime.now():%Y%m%d}.csv"}
            )
...
<channel>
    ...
    {''.join(rss_items)}
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=articles_{datetime.now():%Y%m%d}.rss"}
        )
...
async def export_stats(format: str = Query("json", regex="^(json|csv)$")):
    ...
    if format == "json":
        ...
        return StreamingResponse(
            output,
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=stats_{datetime.now():%Y%m%d}.csv"}
        )
