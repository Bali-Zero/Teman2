    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Debug semantic search by visualizing retrieval results. 
          See exactly what the agents see.
        </p>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSearch} className="flex gap-4">
            ...
            >
              ...
              {!collections.length && <option value="knowledge_base">knowledge_base</option>}
            </select>
      ...
      <div className="space-y-4">
         <h2 className="text-xl font-semibold tracking-tight">Retrieval Results</h2>
         ...
         {results.map((result, i) => (
           <Card key={result.id || i} className="overflow-hidden transition-all hover:shadow-md">
             <CardHeader className="bg-muted/30 pb-3">
               <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                     <Badge variant={result.score > 0.75 ? "default" : "secondary"}>
             ...
             <CardContent className="pt-4 space-y-3">
                {result.payload?.content ? (
                   <div className="prose prose-sm dark:prose-invert max-w-none">
                     <p className="whitespace-pre-wrap">{String(result.payload.content).slice(0, 500)}...</p>
                   </div>
                ) : (
                ...
                {result.payload?.metadata && (
                  <div className="pt-2 border-t mt-2">
                     ...
                     <div className="flex flex-wrap gap-2">
                        {Object.entries(result.payload.metadata as Record<string, any>).map(([k, v]) => (
                           <div key={k} className="text-xs bg-secondary px-2 py-1 rounded">
