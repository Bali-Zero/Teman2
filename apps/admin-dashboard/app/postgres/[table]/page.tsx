  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
              {params.table} 
              <Badge variant="outline" className="font-normal text-muted-foreground">{data.length} rows visible</Badge>
            </h1>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        {loading ? (
          <div className="flex items-center justify-center h-full text-muted-foreground">Loading data...</div>
        ) : data.length === 0 ? (
          <div className="flex items-center justify-center h-full text-muted-foreground">No records found.</div>
        ) : (
                ...
                <tbody className="[&_tr:last-child]:border-0 divide-y">
                  {data.map((row, i) => (
                    <tr key={i} className="border-b transition-colors hover:bg-muted/50">
                      {columns.map(col => (
                        <td key={`${i}-${col}`} className="p-4 align-middle whitespace-nowrap max-w-[300px] overflow-hidden text-ellipsis">
