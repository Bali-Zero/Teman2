...
export default function CollectionDataPage({ params }: { params: { collection: string } }) {
  ...
  const fetchData = (currentOffset: string | null = null) => {
    setLoading(true);
    const url = `/api/qdrant/points?collection=${params.collection}` + (currentOffset ? `&offset=${currentOffset}` : '');
       ...
       <div className="flex-1 overflow-auto p-4 bg-muted/10">
         {loading ? (
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                 {data.map((point) => (
                    <div key={point.id} className="border rounded-md bg-card p-4 space-y-3 font-mono text-sm shadow-sm overflow-hidden">
                        <div className="flex justify-between items-start border-b pb-2">
                            <span className="text-xs text-muted-foreground font-sans uppercase tracking-wider">ID</span>
                            <span className="bg-primary/10 text-primary px-2 py-0.5 rounded text-xs truncate max-w-[200px]" title={point.id}>{point.id}</span>
                        </div>
