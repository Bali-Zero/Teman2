...
export function ContactOptions({
  ...
  const email = buildEmailIntent({
    subject: `Bali Zero — ${labels[topic]}`,
    body: `Hello Bali Zero, I would like to discuss ${contactTopics[topic]} (from ${source}).`,
  });
  return (
        ...
        onChange={(event) => {
          ...
          if (window.location.pathname === "/contact") {
            window.history.replaceState(
              ...
              "",
              contactPageHref(nextTopic, source),
            );
          }
        }}
      >
