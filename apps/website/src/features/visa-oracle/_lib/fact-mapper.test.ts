...
const REPO_ROOT = path.resolve(__dirname, "../../../../proofs/visa-oracle/contract-source");
...
function representativeAnswer(question: OracleQuestion): string {
  // The gate's explicit negative is its standalone fact-producing answer.
  // "yes" requires the following printed-code question, tested separately.
  if (question.id === "holds_stay_permit") return "no";
...
describe("question registry -> wire coverage", () => {
  it("never leaves an answered FACT/REVIEW_ONLY path as NOT_ASKED", () => {
    for (const question of Object.values(QUESTIONS)) {
      ...
      const result = mapFacts({
        ...(question.id === "business_activity" ? { category: "business" } : {}),
        ...(question.id === "holds_stay_permit" ? { in_indonesia: "no" } : {}),
        [question.id]: representativeAnswer(question),
      });
      if (question.id === "holds_stay_permit") {
        expect(result.facts["immigration.current_status_code"]).toEqual({
          status: "KNOWN",
  ...
  it.each([
    ...
    ["business_activity", "negotiation", "ACTIVITY_BOUNDARY"],
    ["business_activity", "conference", "ACTIVITY_BOUNDARY"],
...
describe("mapDisclosedReviewFlags — monotone abstention metadata", () => {
  const resolvable = [
    ["in_indonesia", "immigration.currently_in_indonesia"],
    ["stay_days", "intent.stay_days"],
    ["family_sponsor_confirmed", "family.sponsor_confirmed"],
    ["wants_onshore_conversion", "process.wants_onshore_conversion"],
  ] as const;

  it.each(resolvable)("keeps %s unknown and delegates its resolution to the engine", (questionId, factPath) => {
    const mapped = mapFacts({ [questionId]: "unsure" });
    expect(QUESTIONS[questionId].decisionMapping).toEqual({ kind: "FACT", factPaths: [factPath] });
    expect(mapped.facts[factPath]).toEqual({ status: "UNKNOWN", reason: "UNVERIFIED" });
    expect(mapped.disclosed_review_flags).toEqual([]);
  });

  it.each(resolvable)("preserves explicit uncertainty and safety disclosures beside %s", (questionId) => {
    expect(mapDisclosedReviewFlags({
      [questionId]: "unsure", review_gate: "not_certain,criminal_record,prior_refusal",
    })).toEqual(["CRIMINAL_RECORD", "NOT_CERTAIN", "PRIOR_VISA_REFUSAL"]);
  });

  it.each(["retirement_basis", "business_activity", "nationalities", "birth_date", "work_indonesia_compensation", "unregistered_future_question"])(
    "retains the uncertainty veto outside the four reviewed questions: %s", (questionId) => {
      expect(mapDisclosedReviewFlags({ [questionId]: "unsure", stay_days: "unsure" })).toContain("NOT_CERTAIN");
    },
  );

  it("does not release activity or ambiguous sponsor containment with a resolvable unknown", () => {
    expect(mapDisclosedReviewFlags({ business_activity: "meetings", stay_days: "unsure" })).toEqual(["ACTIVITY_BOUNDARY"]);
    expect(mapDisclosedReviewFlags({ category: "family", family_relation: "CHILD", in_indonesia: "unsure" })).toEqual(["AMBIGUOUS_SPONSOR"]);
  });
  ...
  it("lets the retirement rules evaluate income and sponsor independently of property context", () => {
...
describe("Loop 06 source-grounded intent boundaries", () => {
  it("maps only an explicit pre-investment activity within Business to INVESTMENT", () => {
    expect(mapPurposes({ category: "business", business_activity: "preinvestment" })).toEqual({ status: "KNOWN", value: ["INVESTMENT"] });
    for (const business_activity of ["meetings", "negotiation", "conference", "unsure"]) {
      expect(mapPurposes({ category: "business", business_activity })).toEqual({ status: "KNOWN", value: ["BUSINESS_MEETINGS"] });
    }
    expect(mapPurposes({ category: "tourism", business_activity: "preinvestment" })).toEqual({ status: "KNOWN", value: ["TOURISM"] });
    expect(mapPurposes({ business_activity: "preinvestment" }).status).toBe("UNKNOWN");
  });
  it.each(["family", "diaspora"])("contains the mixed CHILD/PARENT direction contract for %s without changing the answer", (category) => {
    for (const family_relation of ["CHILD", "PARENT"]) {
      const mapped = mapFacts({ category, family_relation });
      expect(mapped.facts["family.relation_to_sponsor"]).toEqual({ status: "KNOWN", value: family_relation });
      expect(mapped.disclosed_review_flags).toEqual(["AMBIGUOUS_SPONSOR"]);
    }
    for (const family_relation of ["SPOUSE", "STEPCHILD", "SIBLING"]) {
      expect(mapFacts({ category, family_relation }).disclosed_review_flags).toEqual([]);
    }
  });
  it("never converts missing compensation or conversion intent to false for preinvestment", () => {
    const mapped = mapFacts({ category: "business", business_activity: "preinvestment" });
    expect(mapped.facts["work.indonesia_source_compensation"].status).toBe("UNKNOWN");
    expect(mapped.facts["process.wants_onshore_conversion"].status).toBe("UNKNOWN");
  });
});
