...
describe("Visa Oracle authoritative outcome adapter", () => {
  it.each([
    ["C1", "C1_VISIT_ELIGIBLE"],
    ["C6", "C6_SOCIAL_ELIGIBLE"],
  ])("does not claim a supported 121-day %s assessment is within 60 days", (code, reason) => {
    const response = makeVisaOracleResponse();
    response.decision.candidates[0].product_code = code;
    response.decision.candidates[0].reason_codes = [reason];
    response.display.candidates[0].product_code = code;
    const outcome = buildEngineOutcome(response, { facts: { stay_days: "121" } });
    if (outcome.state !== "SUPPORTED_CANDIDATES") throw new Error("unexpected state");
    expect(outcome.candidates.map((candidate) => candidate.code)).toEqual([code]);
    const message = outcome.candidates[0].legal.reasons[0].message;
    expect(message.en).not.toContain("60 days");
    expect(message.id).not.toContain("60 hari");
    if (code === "C6") {
      expect(message.en).toContain("confirmed sponsor");
      expect(message.id).toContain("penjamin terkonfirmasi");
    } else {
      expect(message.en).toContain("tourism or family visit");
      expect(message.id).toContain("wisata atau keluarga");
    }
  });
  it("does not suggest a family alternative absent from the D12 supporting-letter source", () => {
    const response = makeVisaOracleResponse();
    response.decision.candidates[0].product_code = "D12";
    response.decision.candidates[0].reason_codes = ["SUPPORT_LETTER_REQUIRED"];
    response.display.candidates[0].product_code = "D12";
    response.sources[0].canonical_url = "https://www.imigrasi.go.id/wna/daftar-visa-indonesia/D12";
    const outcome = buildEngineOutcome(response);
    if (outcome.state !== "SUPPORTED_CANDIDATES") throw new Error("unexpected state");
    const message = outcome.candidates[0].legal.reasons[0].message;
    expect(message.en).toContain("statement, invitation or correspondence");
    expect(message.id).toContain("pernyataan tertulis, undangan, atau komunikasi tertulis");
    expect(message.en).not.toMatch(/family/i);
    expect(message.id).not.toMatch(/keluarga/i);
    expect(outcome.sources[0].url).toBe(response.sources[0].canonical_url);
  });
  it("does not describe a supported 121-day C2 assessment as a stay within 60 days", () => {
    const response = makeVisaOracleResponse();
    response.decision.candidates[0].product_code = "C2";
    response.decision.candidates[0].reason_codes = ["C2_BUSINESS_ELIGIBLE"];
    response.display.candidates[0].product_code = "C2";
    const outcome = buildEngineOutcome(response, { facts: { stay_days: "121" } });
    expect(outcome.state).toBe("SUPPORTED_CANDIDATES");
    if (outcome.state !== "SUPPORTED_CANDIDATES") throw new Error("unexpected state");
    expect(outcome.candidates.map((candidate) => candidate.code)).toEqual(["C2"]);
    const message = outcome.candidates[0].legal.reasons[0].message;
    expect(message.en).not.toContain("60 days");
    expect(message.id).not.toContain("60 hari");
    expect(message.en).toContain("confirmed sponsor");
  ...
  it.each(["current_status_code", "stay_permit_code"] as const)(
    "reopens the printed %s even when the stay-permit gate was also visited",
    (questionId) => {
      const response = makeVisaOracleResponse("NEEDS_INPUT");
      response.decision.missing_facts = ["immigration.current_status_code"];
      const outcome = buildEngineOutcome(response, {
        editableQuestionIds: ["holds_stay_permit", questionId],
      });
      if (outcome.state !== "NEEDS_INPUT") throw new Error("unexpected state");
      expect(outcome.missingInputs[0].questionId).toBe(questionId);
    },
  );

  it("does not offer the branch gate as a missing printed-status answer", () => {
    const response = makeVisaOracleResponse("NEEDS_INPUT");
    response.decision.missing_facts = ["immigration.current_status_code"];
    const outcome = buildEngineOutcome(response, {
      editableQuestionIds: ["holds_stay_permit"],
      facts: { in_indonesia: "no", holds_stay_permit: "yes" },
    });
    if (outcome.state !== "NEEDS_INPUT") throw new Error("unexpected state");
...
describe("support reasons are sentences, not machine codes", () => {
  ...
  const PACKS_DIR = path.resolve(
    HERE,
    "../../../../proofs/visa-oracle/contract-source",
  ...
  it.each(["C2", "D12"])(
    "explains the %s proposal compensation exclusion without implying support",
    (product) => {
      const code = `${product}_LOCAL_COMPENSATION_NOT_ALLOWED`;
      const message = firstNoPathReason(code);
      expect(message.en).not.toContain(code);
      expect(message.id).not.toContain(code);
      expect(message.en).toContain(`${product} `);
      expect(message.id).toContain(` ${product} `);
      expect(message.en).toContain(
        "do not permit compensation from an Indonesian person or entity",
      );
      expect(message.id).toContain(
        "tidak mengizinkan kompensasi dari orang atau entitas Indonesia",
      );
      expect(message.en).toContain(
        `prevents support for ${product} in this assessment`,
      );
      expect(message.id).toContain(
        `menghalangi dukungan untuk ${product} dalam penilaian ini`,
      );
    },
  );
  ...
  it("localizes the canonical E33G compensation exclusion without inventing a work permission", () => {
    const code = excludeReasonCodeOfRule("hf.e33g.indonesian-source-compensation");
    expect(code).toBe("INDONESIAN_SOURCE_COMPENSATION_BANNED");
    const message = firstNoPathReason(code);
    expect(message.en).toContain("prevents support for E33G");
    expect(message.id).toContain("menghalangi dukungan untuk E33G");
    expect(message.en).not.toContain(code);
    expect(message.id).not.toContain(code);
  });
});
