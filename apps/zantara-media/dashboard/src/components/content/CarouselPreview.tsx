...
const SLIDES_CONTENT: SlideData[] = [
  ...
  },
  {
    ...
    footer:
      'Same weekend: Cyclone 93S hits. Floods in Seminyak, Kerobokan, Denpasar. Viral gridlock videos everywhere.\nYou have to admire the confidence.',
  },
  {
    id: 3,
    ...
    image:
      'https://images.unsplash.com/photo-1515696955266-4f67e13219a8?auto=format&fit=crop&q=80&w=1000',
  },
  {
    id: 4,
    layout: 'text-only',
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1552152370-fb05b263b96e?auto=format&fit=crop&q=80&w=1000',
  },
  {
    id: 6,
    layout: 'text-only',
  ...
  },
  {
    ...
    image:
      'https://images.unsplash.com/photo-1516455207990-7a41ce80f7ee?auto=format&fit=crop&q=80&w=1000',
  },
];
...
const Header = ({ index, total }: { index: number; total: number }) => (
  <div className="absolute top-0 left-0 right-0 z-20 flex justify-between items-center w-full px-6 py-5">
    <div className="flex items-center gap-2">
      <div
        className="w-1.5 h-1.5 rounded-full shadow-[0_0_8px_rgba(212,175,55,0.6)]"
...
const ImageBackground = ({ src, intensity = 0.4 }: { src?: string; intensity?: number }) => {
  ...
  return (
    <>
      ...
      />
      <div className="absolute inset-0 bg-[#1F2023]" style={{ opacity: intensity }} />
...
const CoverSlide = ({ data }: { data: SlideData }) => (
  <div
    className={`h-full flex flex-col relative overflow-hidden text-white font-sans group ${THEME.bg}`}
  >
    ...
    <div className="flex-1 flex flex-col justify-end p-8 pb-20 relative z-10">
      ...
      <div className="flex flex-col gap-3">
        ...
        <h2 className="text-lg font-bold uppercase tracking-wider text-white/90">{data.subhead}</h2>
      </div>
    </div>
    <Footer />
  </div>
  ...
  <div className={`h-full flex flex-col relative overflow-hidden font-sans ${THEME.bgGradient}`}>
    ...
    <div className="flex-1 px-8 pt-20 pb-10 flex flex-col relative z-10">
      ...
      <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide space-y-6 text-center">
        {data.body && (
          <p
            className="text-[14px] leading-relaxed font-light whitespace-pre-line"
        ...
        {data.items && (
          <ul className="space-y-3 text-left pl-2">
            {data.items.map((item, i) => (
              <li
                key={i}
                className="flex gap-3 text-[13px] leading-snug font-light border-b border-white/5 pb-2 last:border-0"
                style={{ color: THEME.text }}
              >
                <span
                ...
                <span>
                  {item.includes('→') ? (
                    <>
                      <strong className="block mb-0.5 text-white font-medium">
                        {item.split('→')[0]}
                      </strong>
          ...
          <div className="grid gap-3 text-left">
            {data.boxes.map((box, i) => (
              <div key={i} className="p-4 rounded bg-white/5 border border-white/5">
                <h4 className="text-[10px] font-bold uppercase mb-1 tracking-widest text-white/60">
                  {box.title}
                </h4>
                <p className="text-xs font-light whitespace-pre-line leading-relaxed text-white/90">
                  {box.content}
                </p>
              </div>
            ))}
      ...
      {data.items && (
        <ul className="space-y-4 text-left pl-2">
          {data.items.map((item, i) => (
            <li
              key={i}
              className="flex items-center gap-3 text-[14px] font-medium text-white drop-shadow-md"
            >
...
const CTASlide = ({ data, index, total }: { data: SlideData; index: number; total: number }) => (
  <div className={`h-full flex flex-col relative font-sans group ${THEME.bg}`}>
    ...
    <div className="flex-1 px-8 pt-20 pb-12 flex flex-col relative z-10 text-center">
      ...
      <p className="text-xs font-medium text-white/80 mb-8 max-w-[80%] mx-auto">{data.body}</p>

      <ul className="space-y-2 mb-auto text-left mx-auto">
        {data.services?.map((service, i) => (
          <li
            key={i}
            className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-white"
          >
      ...
      <div className="relative group">
        ...
        >
          {(() => {
            ...
            switch (slide.layout) {
              ...
              case 'text-only':
                return <TextOnlySlide data={slide} index={currentSlide} total={totalSlides} />;
              case 'cta':
                return <CTASlide data={slide} index={currentSlide} total={totalSlides} />;
              default:
                return <ImageOverlaySlide data={slide} index={currentSlide} total={totalSlides} />;
            }
          })()}
        </div>
        ...
        <div className="absolute -left-20 top-1/2 -translate-y-1/2 flex flex-col gap-2">
          <button
            ...
            className="p-4 rounded-full bg-white/5 text-white hover:bg-white/10 transition-all disabled:opacity-0"
          >
            <ChevronLeft size={20} />
          </button>
        </div>
