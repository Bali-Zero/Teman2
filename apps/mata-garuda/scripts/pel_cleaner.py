#!/usr/bin/env python3
"""W12 PEL-cleaner: systematic recovery from PEL accumulation pattern.

Scans all garuda:*, bridge:*, nexus:* Redis streams. For each consumer group:
  - STALE_PEL (pending>0 AND idle>24h): XCLAIM all pending messages to the
    youngest alive consumer in the group (lowest idle_ms). If no alive consumer
    exists, leave alone (manual decision needed).
...
1 if any XCLAIM/DELCONSUMER call failed.
...
STALE_PEL_IDLE_MS = 24 * 3600 * 1000        # 24h
GHOST_IDLE_MS = 30 * 86400 * 1000           # 30 days
