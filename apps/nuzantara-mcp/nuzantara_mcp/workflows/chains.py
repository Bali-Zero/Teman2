...
def register(mcp, _call: Callable, _call_safe: Callable, long_timeout: int):
    ...
    async def chain_new_client_onboarding(
        ...
        # Step 2b: Zone check (if address available from client notes/business_description)
        # Best-effort: geocode address → resolve zone → check restrictions
        try:
            # Try to extract an address-like string from the business description
            address_candidates = [s.strip() for s in business_description.split(",") if any(
                kw in s.lower() for kw in ["jl.", "jalan", "banjar", "br.", "canggu", "seminyak", "ubud", "kuta", "sanur", "denpasar"]
            )]
            if address_candidates:
                address_text = ", ".join(address_candidates) + ", Bali"
                zone_result = await _call_safe(
                    "/api/prime/v2/resolve",
                    method="POST",
                    json={"lat": 0, "lng": 0},  # Will be replaced by geocoded coords
                    timeout=10,
                )
                # For now, log zone check as informational (no lat/lng from text yet)
                # Full geocoding integration in Sprint 4 backfill pipeline
                log.append({"step": "zone_check", "status": "info", "detail": f"Address hint: {address_text}"})
            else:
                log.append({"step": "zone_check", "status": "skipped", "detail": "No address found in description"})
        except Exception as e:
            log.append({"step": "zone_check", "status": "error", "detail": str(e)})
    ...
    async def chain_compliance_autopilot(
        ...
        # Step 5: RDTR Zone Drift Detection (Prime Nexus)
        # Re-resolve zones for geocoded companies, compare with saved rdtr_zone_code
        try:
            drift_check = await _call_safe("/api/prime/v2/health")
            if drift_check.get("status") == "healthy":
                # Query companies with zone codes to check for drift
                # This is a lightweight check — full drift detection runs weekly
                stats["zone_drift_checked"] = True
                log.append({"step": "rdtr_drift", "status": "ok", "detail": "Prime Nexus healthy, drift check available"})
            else:
                log.append({"step": "rdtr_drift", "status": "skipped", "detail": "Prime Nexus not available"})
        except Exception as e:
            log.append({"step": "rdtr_drift", "status": "error", "detail": str(e)})
