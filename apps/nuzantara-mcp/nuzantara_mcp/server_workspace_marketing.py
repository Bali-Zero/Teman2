...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Before preparing publication, call workspace_health once and read "
        "newsroom_get_article for each selected item. Show its cover_status, "
        "publication_preflight and fact_gate. Fix the reported missing fields; "
        "reuse attached covers. Completeness is not editorial approval or "
        "proof of publication. Obtain the editor's explicit site position; "
        "never infer it from the API default. Do not repeat a failed fact "
        "check without correcting its findings or restoring its provider. "
        "Report the exact failed check and next action instead. A queued "
        "publication is IN CODA; call newsroom_verify_live before saying LIVE. "
        ...
        "newsroom_publish with confirmation CONFIRM. Send the cover as a JPEG "
        "of 1200x630 pixels and at most 300 KB: the chat channel truncates "
        "larger base64 payloads and the upload is rejected. Do not ask Damar for SEO "
