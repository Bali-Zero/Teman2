"""Validate `apps/organism/organism/organs_registry.yaml`.
...
Invocation:
    ...
    python -m organism.tools.validate_organs_registry [PATH] [--bootstrap] [--update-checksum]

Exit code 0 on success, 1 on validation errors. Designed for use as a
pre-commit hook AND a standalone CI step.

NOTE: the SYSTEM CONCEPT remains "Innervation Genoma" (see SYMBIOSIS.md).
This is the FILE/REGISTRY of organs only — the metaphor name (Genoma) is
preserved across docs and the cell-core `Genome` Python class. File renamed
2026-05-08 (IG-3) to disambiguate "genome.yaml the file" from "Genome the
skill DB Python class". Backward-compat alias `validate_genome` re-exports
this module with a DeprecationWarning until 2026-06-08.
"""
...
def validate_data(data: dict, *, bootstrap: bool = False) -> list[str]:
    ...
    if not isinstance(data, dict):
        return ["organs registry must be a mapping at the top level"]
    ...
    if not isinstance(organs, list):
        return errors + ["organs_registry.organs must be a list"]
...
def validate_file(path: Path, *, bootstrap: bool = False) -> list[str]:
    ...
    try:
        ...
        return [f"organs registry file not found: {path}"]
...
def _main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate organs_registry.yaml")
    parser.add_argument(
        "path",
        nargs="?",
        default=str(
            Path(__file__).resolve().parent.parent / "organs_registry.yaml"
        ),
        help="Path to organs_registry.yaml (default: <package>/organs_registry.yaml)",
    )
    parser.add_argument(
        "--bootstrap",
        action="store_true",
    ...
    if errors:
        ...
        return 1
    print(f"✓ organs_registry.yaml valid ({path})")
    return 0


# Public entrypoint name (used by the deprecation shim `validate_genome.main`).
main = _main
