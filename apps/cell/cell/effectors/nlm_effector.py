        ...
        for item in items:
            ...
            try:
                result = subprocess.run(
                    ["nlm", "source", "add", self._nb_id, "--url", url],
                    capture_output=True, text=True, timeout=30,
                )
                if result.returncode == 0:
                    fed += 1
            except Exception as e:
                logger.debug(f"NLM feed failed for {url}: {e}")

        return NLMEffectorResult(
            success=fed > 0,
            detail=f"Fed {fed}/{len(items)} URLs to NLM notebook {self._nb_id[:8]}",
            items_fed=fed,
        )
