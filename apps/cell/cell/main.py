...
async def main() -> None:
    ...
    # was a model ollama cannot load. Air, whose 16GB these were sized for, was
    ...
    try:
        ...
        for _node in _topo["nodes"].values():
            if _node["hostname"] == _hostname and _node.get("ram_gb", 0) <= 16:
                # "sentry" was removed 2026-09-21 (no consumer); .get keeps this
                # dead Air-only branch harmless rather than KeyError-ing.
