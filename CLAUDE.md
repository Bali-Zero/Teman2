...
> **[PROPOSED, pending Zero GO] Fable-paid contingency** (2026-07-11 Gear-3 synthesis — see `research/operations/2026-07-11-fable-paywall-routing.md`, staged report + blind A/B): if Fable-5 becomes a metered/paid endpoint, do NOT reflexively default every interactive session to it at max-effort by habit. The research finds Fable's measured edge over Sonnet+scaffold is concentrated in a *narrow* class — novel/ungrounded judgment with zero catalogued precedent (e.g. WR2 lane-3 brand-voice on uncatalogued cliché); for grounded, structured, or checkable shapes (KBLI bulk generation, WR2 lanes 1/2/4, cron classification, fact-lookup) Sonnet+scaffold is measured-tied or the scaffold makes model tier moot. Interim rule until Zero sets the paid-call threshold (§Solo-operatore in the report): for task shapes the report's routing table marks tie/sonnet-sufficient, route to Sonnet+scaffold and do NOT auto-escalate to paid Fable absent an explicit novelty/ambiguity signal. **This narrows which shapes get a Fable final gate; it does NOT weaken the "final gate never cascades" rule above — where Fable IS the final gate, W65 is unchanged, and no classifier may route the final gate AWAY from the top tier.** (The earlier draft's separate edit to modus §Arsenal was struck by the cross-vendor refuter for breaching exactly that invariant — do not reintroduce it.)
