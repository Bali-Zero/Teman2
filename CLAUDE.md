...
> ⚡ **RULED 2026-08-19 (Zero, verbatim: "fai una piccola modifica al workflow dove Fable 5 è il reviewer finale solo per le grandi feature, fable 5 non fa review di piccoli interventi. Usiamo Opus 5 per quelle."): the final review/gate is SPLIT BY THE DETERMINISTIC GEAR FLOOR.** Gear 3 (large features: architectural work, migrations, cross-system change — the floor computed from the diff, CI-recomputed, never the conductor's choice) keeps the full invariant: **Fable 5 at max effort, never cascades within its class** — window dead → SUSPEND; the 2026-08-09 verdict-stage `gate_degraded` rule is unchanged. Gear 1-2 (small interventions: fixes, docs, mechanical edits, standard small PRs) **close on Opus 5** — Fable is never spent there. The selector is the existing CI-recomputed gear floor, NOT a model's judgment call: this deliberately reuses the one non-gameable classifier the repo already has, so the 2026-07-25 "no task-shape logic in front of the gate" concern is answered by construction — a task cannot be talked into Gear 2 when its diff says 3. Wherever doctrine still reads "unconditionally Fable" for the final on-disk gate, read it as scoped to the Gear-3 class. Protective side-effect: Fable's Team-seat weekly inclusion (~50%) stops being burned on small diffs — which is exactly what keeps the Gear-3 gate alive all week. The WR2 content gate is NOT touched by this ruling (stays unconditionally Fable, no fallback).
...
> clause. *(Scope note 2026-08-19: "the final on-disk gate" here means the Gear-3 gate per the
> gear-split ruling above; the never-pay rule is unchanged and now also covers the Opus-5
> small-intervention gate.)* **Rule for everything else: if/when Fable-5 becomes a metered/paid endpoint, do NOT pay for it
