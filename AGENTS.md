...
- **Gear-3 harness gate (RULED 2026-08-20, supersedes the 2026-08-09 ruling):** **Opus 5 `effort=max`**, rotating across ALL Anthropic accounts (AZ→A2→A3→A1). No fallback tier below it — Fable is out of the workflow, so there is no `gate_degraded: fable→opus` to record anymore. All Anthropic accounts dead → queue. Never pay per-token to unblock.
- **WR2 on-disk content gate (RULED 2026-08-20):** **Opus 5 `effort=max`** — was unconditionally Fable; no fallback either way, window dead → SUSPEND.
