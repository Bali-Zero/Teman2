...
3. **Event-driven, durabilità per canale.** Nessun polling, nessun orchestratore centrale. Ogni canale evento ha la propria strategia di durabilità, scelta in base al consumer:
   ...
   **Empirical count verified 2026-05-12** via `python3 -c "from backend.services.events.event_bus import PG_CHANNEL_MAP; print(len(PG_CHANNEL_MAP))"` → **13 channels** in `PG_CHANNEL_MAP`. Above table: 12 listed in CRM+cognitive+observatory+partners row + 1 separate row for `lkpm_ingest_completed` (Python-emitter-only, no DB trigger) = 13 total. `wr2_status_change` is OUTSIDE PG_CHANNEL_MAP (separate `wr2_supervisor.py` consumer). NB-1 snapshot 2026-03-23 reported 12 channels — that was pre-`partner_commission_changed` addition.
