...
2. **PII/OSINT non trascritti in chiaro — regime a due fasi (ricalibrato 2026-06-23 da Zero).** La frontiera non e' "un LLM non puo' vedere contesto operativo": la frontiera e' che nessun LLM, agente, skill, memoria, report, log, alert, HGT payload, notebook, artefatto pubblico o output condiviso deve trascrivere o persistere in chiaro dati cliente o OSINT (nomi, telefoni, email, passport/KTP/NPWP, chat private, credenziali, raw intelligence). Il processing autorizzato puo' usare contesto operativo quando serve; l'output deve parlare in forma astratta, redatta o referenziata (`client_id`, hash, placeholder).

   **Fase DEV (attuale — fino al go-live prod):** il vincolo "zero-cloud assoluto / mirror Pro-bound" e' **sospeso operativamente** per non bloccare lo sviluppo. In sviluppo si possono usare strumenti cloud (incluso l'IDE Antigravity, agenti cloud, code-indexing) su tutto il repo — **codice, frontend, backend, schema, research** — perche' il dato che circola e' codice e dati di test/sintetici, non il book clienti reale in produzione. Decisione di Zero: «dobbiamo lavorare; quando saremo pronti in prod ci lavoriamo». Resta comunque vietato esportare deliberatamente in chiaro PII REALE di clienti veri (snapshot DB prod, dump WhatsApp reali) verso un terzo — quello e' un atto, non un effetto-collaterale di sviluppo.

   **Fase PROD (al go-live — da ri-armare):** la frontiera PII torna **assoluta** sul percorso che tocca dati cliente reali (mirror OSINT/WhatsApp Pro-bound, snapshot prod, processing PII su Ollama locale). Il riarmo e' un task esplicito di pre-produzione, non automatico — vedi `decision_law2_dev_phase_recalibration_2026_06_23`.
...
3. **Event-driven, durabilità per canale.** Nessun polling, nessun orchestratore centrale. Ogni canale evento ha la propria strategia di durabilità, scelta in base al consumer:
   ...
   **Empirical count re-verified 2026-06-11** via `python3 -c "from backend.services.events.event_bus import PG_CHANNEL_MAP; print(len(PG_CHANNEL_MAP))"` → **17 channels** in `PG_CHANNEL_MAP` (was 13 at the 2026-05-12 snapshot). The table above lists 14 (12 in the CRM+cognitive+observatory+partners row + `lkpm_ingest_completed` + `wa_message_inserted` is the 13th producer row family); the **4 channels added since the 2026-05-12 count are not yet itemised in the table above**: `wa_message_inserted`, `cell_pulse_sustained_red`, `whatsapp_message_received`, `intel_lake_event`. `wr2_status_change` remains OUTSIDE PG_CHANNEL_MAP (separate `wr2_supervisor.py` consumer). NB-1 snapshot 2026-03-23 reported 12 channels — pre-`partner_commission_changed`. **CI pin:** `.github/workflows/catA-channel-count-pin.yml` fails when `len(PG_CHANNEL_MAP)` ≠ the number stated here (single-source drift gate — see Cat-A question #2).
