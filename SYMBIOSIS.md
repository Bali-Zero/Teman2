...
2. **PII/OSINT non trascritti in chiaro — politica interna di sicurezza e minimizzazione, con regime a due fasi (DEV/PROD, ricalibrato 2026-06-23 da Zero).** La frontiera non e' "un LLM non puo' vedere contesto operativo": la frontiera e' che nessun LLM, agente, skill, memoria, report, log, alert, HGT payload, notebook, artefatto pubblico o output condiviso deve trascrivere o persistere in chiaro dati cliente o OSINT (nomi, telefoni, email, passport/KTP/NPWP, chat private, credenziali, raw intelligence). Il processing autorizzato puo' usare contesto operativo quando serve; l'output deve parlare in forma astratta, redatta o referenziata (`client_id`, hash, placeholder). **Questa frontiera-output e' una politica interna piu' rigorosa, non-negoziabile in ogni fase**, coerente con i doveri del titolare di adottare misure di sicurezza, mantenere la riservatezza e prevenire processing/accessi non autorizzati (UU PDP Artt. 35-39). Non presume che ogni comparsa di cleartext integri automaticamente i reati degli Artt. 65/67, che richiedono gli elementi di divulgazione o uso intenzionale e illecito; l'Art. 68 riguarda invece la creazione o falsificazione di dati personali e non e' la base di questa regola.
   ...
   **Perché non è una superficie nuova**: la comunicazione cliente di Bali Zero gira interamente su WhatsApp, quindi nomi e scadenze transitano già per Meta ogni giorno come canale operativo ordinario; e i dati recapitati **vivono già** su Fly Postgres, che è la fonte da cui sono letti. Il rischio che la deroga introduce davvero è un altro ed è di **aggregazione**: un digest concentra 3-5 clienti in un solo messaggio, mentre la conversazione ordinaria è 1:1, e quel messaggio può restare in un backup WhatsApp su dispositivo personale. I limiti di payload qui sotto esistono per quello, non per formalità.
   ...
   - **Destinatari**: solo indirizzi `@balizero.com` presenti a roster con `active = true`, esclusi gli account di servizio, e solo per i clienti con `assigned_to` corrispondente. Mai broadcast, **mai un destinatario di fallback**: un cliente senza proprietario valido resta HELD e non viene recapitato a nessuno.
   ...
   **Stato: autorizzata, NON ancora armata.** L'enforcement (cancello dei destinatari, filtro dei campi, cooldown 90 giorni) vive nel dispatcher e nel suo corpus di colpevolezza+innocenza; finché quella PR non è merged, questa deroga descrive una politica e non un comportamento — «esiste ≠ armato», superscar #2.
