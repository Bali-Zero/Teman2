...
**Test Set 3 - Construction:**
7. Cerca: `"construction"`
...
const testSearch = (query) => {
  ...
  const results = K.filter(item => 
    item[7] && item[7].toLowerCase().includes(query.toLowerCase())
  );
  const end = performance.now();
  console.log(`Query: "${query}" | Results: ${results.length} | Time: ${(end-start).toFixed(2)}ms`);
};
