...
def is_client_folder(name, parent_path):
    ...
    if "titip" in name.lower() or name in VISA_TYPES or name in STATUS_FOLDERS:
        return False
    if (
        any(worker in parent_path for worker in TEAM_FOLDERS)
        or "titip" in parent_path.lower()
    ):
        return False
                ...
                if is_client_folder(folder_name, path):
                    ...
                    if len(files) > 0:
                        ...
                        if self.stats["clients_found"] % 50 == 0:
                            print(
                                f"   Trovati {self.stats['clients_found']} clienti, {self.stats['files_total']} file..."
                            )
