...
class DropboxAPIScanner:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self.log(f"Connected to: {account.name.display_name} ({account.email})", "SUCCESS")
            return True
    ...
    def scan_folder(self, path: str = "", max_depth: int = 3, current_depth: int = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for entry in entries:
                if isinstance(entry, dropbox.files.FileMetadata):
                    ...
                    # Track file type
                    ext = ('.' + entry.name.split('.')[-1]).lower() if '.' in entry.name else '[no extension]'
    ...
    def identify_client_folders(self) -> List[Dict[str, Any]]:
        ...
        for folder_name, folder_info in root.get('subfolders', {}).items():
            if folder_name.upper() in ['INDIVIDUALS', 'COMPANIES']:
                folder_type = 'individual' if 'INDIVIDUAL' in folder_name.upper() else 'company'

                # Each subfolder is likely a client
                for client_name, client_info in folder_info.get('subfolders', {}).items():
...
def main():
    ...
    parser.add_argument('--output', default='dropbox_api_inventory.json', help='Output JSON file')
