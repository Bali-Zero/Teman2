        ...
        for item in items:
            ...
            if is_folder:
                ...
                if subfolder:
                    ...
                    # Aggrega stats
                    folder_data["stats"]["total_folders"] += subfolder["stats"][
                        "total_folders"
                    ]
                    folder_data["stats"]["total_files"] += subfolder["stats"][
                        "total_files"
                    ]
                    folder_data["stats"]["total_size"] += subfolder["stats"][
                        "total_size"
                    ]
                    folder_data["stats"]["client_folders"] += subfolder["stats"][
                        "client_folders"
                    ]

            else:
                file_info = {"name": item["name"], "size": int(item.get("size", 0))}
...
def analyze_structure(data):
    ...
    for folder in data["folders"]:
        ...
        subfolder_count = (
            folder.get("content", {}).get("stats", {}).get("client_folders", 0)
        )
