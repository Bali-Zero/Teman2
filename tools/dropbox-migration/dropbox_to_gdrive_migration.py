...
# File filters
EXCLUDED_EXTENSIONS = {'.tmp', '.bak', '.DS_Store', '.localized', 'desktop.ini', 'Thumbs.db'}
EXCLUDED_FOLDERS = {'Screenshots', 'Other computers', 'Mobile Uploads', 'Camera Uploads'}
...
class FileFilter:
    ...
    def should_skip(file_path: str, file_name: str) -> Tuple[bool, str]:
        ...
        if any(folder in path_parts for folder in EXCLUDED_FOLDERS):
            return True, f"In excluded folder"
    ...
    def categorize_document(file_name: str) -> str:
        ...
        if any(keyword in file_lower for keyword in ['passport', 'kitas', 'kitap', 'visa', 'imta', 'itas', 'merp']):
            return "01_Immigration"
        
        # Company documents
        if any(keyword in file_lower for keyword in ['pt', 'pma', 'nib', 'tdp', 'npwp', 'deed', 'akta', 'sk']):
            return "02_Company"
        
        # Tax documents
        if any(keyword in file_lower for keyword in ['tax', 'spt', 'pajak', 'invoice', 'faktur']):
            return "03_Tax"
        
        # Contracts
        if any(keyword in file_lower for keyword in ['contract', 'agreement', 'mou', 'kontrak', 'perjanjian']):
            return "05_Contracts"
...
class DropboxScanner:
    ...
    def scan(self) -> List[Dict]:
        ...
        mock_clients = [
            '@selese...k (2027)', 'ADITYA', 'ANGEL', 'Adele Marthe',
            'DATA ADI', 'DATA OM DIAN', 'DAVID', 'DINOK',
            'DIRJEN', 'Data Scan', 'Driver', 'EPO',
            'ERSA', 'EXTEND VISA', 'FILE ARIF', 'File dikirim'
        ]
    ...
    def run_initial_migration(self, batch_size: int = 5):
        ...
        for idx, client_folder in enumerate(client_folders, 1):
            ...
            try:
                ...
                gdrive_folder_id = self.migrator.create_client_structure(client_folder['name'])
                ...
                # Migrate each file
                for file in files:
                    should_skip, reason = self.filter.should_skip(file['path'], file['name'])
