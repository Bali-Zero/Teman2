...
CATEGORY_RULES = {
    '01_Immigration': [
        'passport', 'kitas', 'kitap', 'visa', 'imta', 'permit',
        'sponsor', 'extension', 'stay permit', 'immigration',
        'merp', 'limited stay', 'c1', 'c18', 'd1', 'd12', 'e31', 'e33'
    ],
    '02_Company': [
        'pt', 'pma', 'cv', 'nib', 'npwp', 'deed', 'akta',
        'company', 'business', 'license', 'perusahaan',
        'izin', 'usaha', 'modal'
    ],
    '03_Tax': [
        'tax', 'spt', 'pajak', 'pph', 'ppn', 'pbb',
        'tax report', 'annual return', 'fiscal', 'laporan pajak'
    ],
    '04_Family': [
        'spouse', 'wife', 'husband', 'child', 'family',
        'dependent', 'marriage', 'birth certificate',
        'suami', 'istri', 'anak', 'keluarga'
    ],
    '05_Contracts': [
        'contract', 'agreement', 'invoice', 'quotation',
        'service agreement', 'mou', 'kontrak', 'perjanjian'
    ],
...
def run_pilot_migration():
    ...
    pilot_clients = [
        ...
        },
        {
            ...
            'source': DROPBOX_PATH / 'ANGEL' / 'File PT Total Woman Bali (Babette Esmay Monsma)',
    ...
    for i, client in enumerate(pilot_clients, 1):
        ...
        print(f"   📂 {client['source'].relative_to(DROPBOX_PATH) if client['source'].exists() else 'NOT FOUND'}")
        print()
    ...
    print(f"📊 Success rate: {((total_files - total_errors) / max(total_files, 1) * 100):.1f}%")
