...
VISA_TYPE_FOLDERS = {
    ...
    'C11', 'C211', 'D211', 'E31C', '211A', '211B', 'C7', 'C22'
}

# Status folders
STATUS_FOLDERS = {
    'Done', 'On Proses', 'On Process', 'Cancel', 'Pending', 'Sudah Submit'
}
...
UTILITY_FOLDERS = {
    ...
    'Working kitas', 'Spouse Kitas', 'Retirement Kitas', 'Bridging Visa',
    'Set UP PT', 'Invoice Extend', 'Invoice BS', 'Minuta Sirkuler'
}
...
def is_client_folder(path: Path) -> bool:
    ...
    if ' ' in name or any(prefix in name for prefix in ['Mr ', 'Ms ', 'Mrs ', 'Miss ']):
        ...
        if any(word in lower_name for word in ['invoice', 'laporan', 'data', 'catatan', 'new folder']):
            return False
        return True
        ...
        for dir_name in list(dirs):  # Copy list to allow modification
            ...
            if is_client_folder(dir_path):
                ...
                if file_count > 0:  # Only if has files
                    ...
                    print(f"   ✅ {dir_name} ({file_count}{'+ ' if file_count >= 100 else ' '}files)")
...
def main():
    ...
    for client in sorted(clients_list, key=lambda x: x['file_count'], reverse=True)[:30]:
