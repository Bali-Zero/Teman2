...
Nothing in here depends on a colour value. The corpus could not verify which hex
is Bali Zero's brand red (SYNTHESIS Sec.7.1 #2 -- two lanes, two hexes, neither
sourced), so every contrast/APCA gate in Sec.3.1 is out of scope by construction.
(For the record: `#CE1126` does occur in the repo, at
apps/backend-rag/scripts/templates/kbli_magazine.html:105 and
kbli_presentation.html:208. It is absent from the brand-TOKEN files, which is a
different statement. No gate here rests on either premise.)
...
The string gates never run on raw file text. Each file is decomposed into:
  * a COMMENT-BLANKED view (structural gates read this -- so a code comment can
    never trip a gate, and line numbers stay exact), and
...
def _decompose_css(text: str) -> tuple[str, str, list[Span]]:
    def blank(m: re.Match) -> str:
        return "".join("\n" if c == "\n" else " " for c in m.group(0))

    code = _CSS_COMMENT_RE.sub(blank, text)
    spans = [
...
_AFFILIATION_PATTERNS = (
    (re.compile(r"\bofficial\s+(?:partner|agent|reseller|distributor|representative)\b", re.I),
     "official <affiliation noun>"),
    (re.compile(r"\b(?:approved|authorized|authorised)\s+(?:partner|agent|reseller|distributor|representative)\b", re.I),
     "approved/authorised <affiliation noun>"),
    (re.compile(r"\b(?:agen|agent|mitra|partner|perwakilan|distributor|reseller|penyedia|biro)\s+resmi\b", re.I),
     "<affiliation noun> resmi"),
    (re.compile(r"\bresmi\s+(?:partner|mitra|agen|perwakilan)\b", re.I),
...
_ABSOLUTE_CLAIM = (
    r"guarantee[ds]?|approval|approved|success|safe|secure|aman|legal|refund|"
...
def gate_absolute(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    for span in ctx.spans:
        m = _ABSOLUTE_RE.search(span.text)
        if not m:
            continue
...
def gate_rank(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    for span in ctx.spans:
        ...
        if m and _is_prose(span.text) and not _RANK_ENUMERATOR_RE.search(span.text[: m.start()]):
            yield span.line, (
...
def gate_parsefloat(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    ...
    for m in _PARSE_CALL_RE.finditer(ctx.code):
        ...
        if '"' in args or "'" in args or "`" in args:
            continue
        if _id_tokens(args) & PRICE_TOKENS:
            yield line, (
                f"parseFloat/parseInt on a price-named value ({args.strip()[:60]}) — "
                "if it ever holds an id-ID string, this silently divides it by 1000; "
...
_CURRENCY_RE = re.compile(r"""(?:style\s*:\s*["']currency["']|currency\s*:\s*["'])""")
...
def gate_compact(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    ...
    for m in _INTL_RE.finditer(ctx.code):
        ...
        if not _COMPACT_RE.search(args):
            continue
        line = ctx.line_of(m.start())
        named_money = bool(_id_tokens(ctx.window(line, back=2, forward=0)) & PRICE_TOKENS)
        if not (_CURRENCY_RE.search(args) or named_money):
            continue  # compact on a COUNT (followers, cases) is not this gate
        yield line, (
            "Intl.NumberFormat notation:'compact' on a currency value — emits "
...
def gate_clamp(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    for m in _CLAMP_FONTSIZE_RE.finditer(ctx.code):
        line = ctx.line_of(m.start())
        decl = ctx.code_lines[line - 1] if line - 1 < len(ctx.code_lines) else ""
        if ctx.is_css:
            context = f"{_css_selector_for(ctx, line)}\n{decl}"
        else:
            context = ctx.window(line, back=3, forward=1)
        if not (_id_tokens(context) & CLAMP_FORBIDDEN_TOKENS):
            continue
        yield line, (
...
_PRICE_ASTERISK_RE = re.compile(r"(?:\bRp\.?\s?|\bIDR\s?)[\d.,]+\*(?!\*)", re.I)
...
_SUCCESS_WORD_RE = re.compile(
    r"(?i)(success|sukses|berhasil|confirmed|confirmation|completed?\b|\bdone\b|\bpaid\b|thank)"
)
...
_AWAIT_DELAY_RE = re.compile(
    r"new\s+Promise\s*\([^)]{0,80}?setTimeout\s*\([^,]{0,60},\s*(\d{2,7})\s*\)"
)
...
def gate_artificial_delay(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    ...
    for m in _SETTIMEOUT_RE.finditer(ctx.code):
        line = ctx.line_of(m.start())
        window = ctx.window(line, back=0, forward=4)
        if not (_STATE_SET_RE.search(window) and _SUCCESS_WORD_RE.search(window)):
            continue
        if _DISMISS_ARG_RE.search(window):
    ...
    for m in _AWAIT_DELAY_RE.finditer(ctx.code):
        if int(m.group(1)) < ARTIFICIAL_DELAY_MS:
            continue
        line = ctx.line_of(m.start())
        if not _PENDING_CTX_RE.search(ctx.window(line, back=5, forward=3)):
            continue
        yield line, (
            f"hardcoded {m.group(1)}ms await before a submit/verdict/success step — this is "
...
_TW_TRUNCATE_RE = re.compile(r"(?<![\w-])(?:truncate|text-ellipsis)(?![\w-])")
...
MONEY_VERDICT_TOKENS = frozenset(
    {"price", "prices", "harga", "verdict", "putusan", "inclusive", "termasuk"}
)
...
def gate_ellipsis(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    for m in _TEXT_OVERFLOW_RE.finditer(ctx.code):
        ...
        decl = ctx.code_lines[line - 1] if line - 1 < len(ctx.code_lines) else ""
        context = (
            f"{_css_selector_for(ctx, line)}\n{decl}" if ctx.is_css
            else ctx.window(line, back=4, forward=1)
        )
    ...
    for idx, text in enumerate(ctx.code_lines, start=1):
        if _TW_TRUNCATE_RE.search(text) and (_id_tokens(text) & MONEY_VERDICT_TOKENS):
            yield idx, (
                "Tailwind `truncate`/`text-ellipsis` on a price/verdict/inclusion element — "
...
_LANG_SWITCH_RE = re.compile(
    r"(?i)(languageswitch\w*|localeswitch\w*|switchlanguage|langswitch|hreflang|"
...
_EXPLICIT_SWITCHER_RE = re.compile(
    r"(?i)(languageswitch\w*|localeswitch\w*|switchlanguage|langswitch|hreflang|setlocale|changelanguage)"
)
...
def gate_flag_switcher(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    ...
    for m in _FLAG_EMOJI_RE.finditer(ctx.raw):
        candidates.append((ctx.raw.count("\n", 0, m.start()) + 1, "flag emoji"))
...
_FIXED_WIDTH_MSG = (
    "fixed {unit} width on a button/chip/badge/pill — Indonesian short strings run "
    "+35-50% wider than English and overflow it. Use min-width + wrap + a min-height "
...
def gate_fixed_width(ctx: FileCtx) -> Iterator[tuple[int, str]]:
    if ctx.is_css:
        for idx, text in enumerate(ctx.code_lines, start=1):
            ...
            if not _BUTTONISH_RE.search(selector):
                continue
    ...
    if ctx.ext in (".json", ".md", ".mdx"):
        return
    for idx, text in enumerate(ctx.code_lines, start=1):
        m = _TW_FIXED_W_RE.search(text)
        if not m or not _BUTTONISH_RE.search(text):
            continue
        if _SQUARE_EXEMPT_RE.search(text):
            continue
        h = _TW_FIXED_H_RE.search(text)
