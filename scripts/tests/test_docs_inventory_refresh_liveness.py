...
# ---------------------------------------------------------------------------
# Architecture pins: volatile inventory is artifact-only and the production
# liveness caller checks workflow runs, not the retired refresh-PR effect.
# ---------------------------------------------------------------------------
...
def test_refresh_workflow_is_artifact_only() -> None:
    """No branch/PR pipeline remains for volatile inventory state."""
...
def test_production_liveness_skips_retired_pr_effect_check() -> None:
    text = LIVENESS_WORKFLOW.read_text(encoding="utf-8")
    assert "--skip-pr-check" in text
