...
def _send(mailbox_dir: pathlib.Path, key: str, body: str) -> None:
    # --to all + --from (2026-09-18): broadcast now requires an address AND a sender
    # label; retract's own cleanup semantics (per-key, not per-address/sender) are
    # unaffected either way.
    env = dict(os.environ, NUZ_MAILBOX_DIR=str(mailbox_dir))
    proc = subprocess.run(
        ["bash", str(SCRIPT), "local", "broadcast", "--to", "all", "--from", "test-sender",
         "--key", key, body],
