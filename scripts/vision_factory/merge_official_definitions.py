...
def merge_definitions(enriched_path, official_path, output_path):
    print(f"🔄 Merging definitions from {official_path} into {enriched_path}...")
    ...
    if not records:
        print("❌ No records found in enriched data!")
        return
    ...
    try:
        ...
        print(f"❌ Official definitions file not found at {official_path}. Extraction might have failed or is still running.")
        return
    ...
    for rec in records:
        ...
        if code in official_map:
            ...
            if official_rec.get("uraian"):
                ...
                rec["metadata_sumber"]["sumber_definisi"] = "Perban BPS No. 7 Tahun 2025"
                rec["metadata_sumber"]["definisi_original"] = "Replaced with Official Definition"
    ...
    print(f"✅ Merged {updated_count} official definitions. Saved to {output_path}")
