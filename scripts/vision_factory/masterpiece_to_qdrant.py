...
def serialize_for_qdrant(input_path: str, output_path: str):
    print(f"🔌 Serializing {input_path} for Qdrant (Masterpiece V5)...")
    ...
    records = source_data.get('data', []) if isinstance(source_data, dict) else source_data
    ...
    print(f"✅ Serialized {len(qdrant_payloads)} records.")
    if len(qdrant_payloads) > 0:
        print(f"📝 Semantic Text Preview (Record 1):\n---\n{qdrant_payloads[0]['vector_source_text'][:500]}...\n---")
    print(f"💾 Saved to {output_path}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 masterpiece_to_qdrant.py <input_enriched.json> <output_qdrant_ready.json>")
    else:
