...
def extract_official_definitions(pdf_path, output_path):
    ...
    print(f"✅ Text extracted ({len(text)} chars). Parsing content...")
    ...
    print(f"🔍 Found {len(matches)} potential 5-digit KBLI codes.")
    ...
    for i, match in enumerate(matches):
        ...
        end_idx = matches[i+1].start() if i + 1 < len(matches) else len(full_clean_text)
    ...
    print(f"💾 Saved {len(results)} definitions to {output_path}")

if __name__ == "__main__":
    pdf_file = "/Users/antonellosiano/Desktop/nuzantara/peraturan-bps-no-7-tahun-2025.pdf"
