    ...
    try:
        with open(csv_path, "r", encoding="utf-8", errors="replace") as f:
            ...
            if "Kode KBLI" not in headers:
                logger.critical("CRITICAL ERROR: Column 'Kode KBLI' missing from CSV!")
                ...
                try:
                    ...
                    if "(2)" in kbli_code_raw_check:
                        ...
                        if len(valid_parts) > 1:
                            # TRUE MERGE DETECTED
                            # logger.debug(f"Splitting Merged Row: {kbli_code_raw_check}") # Replaced commented print
                            delimiters = {
                                "Kode KBLI": "(2)",
                                "Judul KBLI": "(3)",
                                "Ruang Lingkup": "(4)",
                    ...
                    for row_proc in rows_to_process:
                        ...
                        if kbli_match:
                            ...
                            scope = (
                                str(row_proc.get("Ruang Lingkup") or "")
                                .replace("\n", " ")
                                .strip()
                            )
                            ...
                            active_record = {
                                ...
                                "risk_level": str(
                                    row_proc.get("Tingkat Risiko") or ""
                                ).strip(),
                                "business_scale": str(
                                    row_proc.get("Skala Usaha") or ""
                                ).strip(),
                                "authority": str(
                                    row_proc.get("Kewenangan") or ""
                                ).strip(),
                                "licensing_requirements": str(
                                    row_proc.get("Perizinan Berusaha") or ""
                                ).strip(),
                                "requirements": str(
                                    row_proc.get("Persyaratan") or ""
                                ).strip(),
                                "obligations": str(
                                    row_proc.get("Kewajiban") or ""
                                ).strip(),
                                "issuance_period": str(
                                    row_proc.get("Jangka Waktu") or ""
                                ).strip(),
                            ...
                            if active_record:
                                ...
                                extra_title = str(
                                    row_proc.get("Judul KBLI") or ""
                                ).strip()
                                if extra_title and not active_record["title"].endswith(
                                    extra_title
                                ):
                                    active_record["title"] += f" {extra_title}"

                except Exception as row_e:
                    logger.warning(f"Error processing row {row_count}: {row_e}")
                    continue
        ...
        logger.critical(f"CRITICAL ERROR in CSV Processing: {e}")
        # traceback.print_exc()
        return

    logger.info(f"Extraction finished. Found {len(masterpiece_data)} records.")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        if os.path.exists(input_csv):
            logger.info(f"Running Default Test on {input_csv}")
