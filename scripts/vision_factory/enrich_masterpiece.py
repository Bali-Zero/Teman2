...
SECTOR_INTELLIGENCE = {
    ...
    "WHOLESALE_TRADE": {
        ...
        "checklists_umku": [
            {
                "requirements": ["Izin Gudang (TDG)", "Kontrak Distributor/Keagenan"],
                "sla": "5 Hari",
            }
        ],
    },
    ...
    "RETAIL_TRADE": {
        ...
        "intelligence_tags": [
            "BALI_MORATORIUM_ACTIVE",
            "RETAIL_RESTRICTIONS",
            "FRANCHISE_LIMITATION",
        ],
        "checklists_umku": [
            {
                "requirements": [
    ...
    "FORESTRY": {
        ...
        "intelligence_tags": [
            "FORESTRY_REFORESTATION_1_100",
            "HIGH_CAPITAL_INTENSIVE",
            "MINISTRY_FORESTRY_DIRECT",
        ],
    },
    ...
    "WOOD_INDUSTRY": {
        ...
        "extra_notices": [
            ...
            },
            {
                ...
                "description": "Wajib lapor Rencana Pemenuhan Bahan Baku Industri secara digital. Ketidakcocohan stok fisik vs digital memicu audit.",
        ...
        "intelligence_tags": [
            "SVLK_MANDATORY",
            "EXPORT_ORIENTED",
            "RAW_MATERIAL_TRACING",
        ],
        "checklists_umku": [
            {
                "requirements": [
                    "Sertifikat Standar Verifikasi Legalitas (SVLK)",
                    "Izin Gudang (Tanda Daftar Gudang)",
                ],
    ...
    "INDUSTRY": {
        "prefixes": [
            str(x) for x in range(10, 34) if x != 16
        ],  # 10-33 excluding Wood (16)
        "sanksi_administratif": {
        ...
        "checklists_umku": [
            {
                "requirements": ["Sertifikat Standar", "SPPL/UKL-UPL"],
...
SPECIAL_CODES = {
    ...
    "46333": {
        "extra_notices": [
            {
                "title": "Wajib SKMB (Distributor)",
                "ref": "Permendag Minol",
                "description": "Wajib memiliki Surat Keterangan Minuman Beralkohol (SKMB) sebagai Distributor. Gudang harus terpisah dari barang lain.",
    ...
    "47221": {
        "extra_notices": [
            {
                "title": "Larangan Konsumsi di Tempat",
                "ref": "Regulasi Minol Eceran",
                "description": "Dilarang minum di tempat (kecuali Bar/Resto). Penjualan hanya boleh untuk take-away. Pembeli wajib 21+ tahun.",
                "impact": "Operational Restriction",
            },
            {
                "title": "Separasi Display",
                "ref": "Aturan Ritel Modern",
                "description": "Minuman beralkohol tidak boleh dipajang bercampur dengan produk lain. Rak harus khusus/terpisah.",
                "impact": "Audit Risk",
            },
        ],
...
def enrich_data(input_path, output_path):
    logger.info(f"💎 Enriching: {input_path}")
        ...
        for sector, rule in SECTOR_INTELLIGENCE.items():
            if prefix in rule["prefixes"]:
                ...
                if not rec.get("checklists_umku"):
                    rec["checklists_umku"] = copy.deepcopy(
                        rule.get("checklists_umku", [])
                    )
                ...
                for notice in rule.get("extra_notices", []):
                    if notice["title"] not in [
                        x["title"] for x in rec["legal_notices"]
                    ]:
    ...
    logger.info("🔄 Normalizing to Masterpiece V5 (Indonesian Schema)...")
    normalized_records = []
    ...
    for rec in records:
        norm_rec = {
            ...
            "perizinan_berusaha": rec.get("licensing_requirements")
            or rec.get("perizinan_berusaha", ""),
    ...
    logger.info(
        f"✅ Enriched & Normalized {enriched_count}/{len(records)} records. Saved to {output_path}"
    )


if __name__ == "__main__":
    if len(sys.argv) < 3:
        logger.error("Usage: python3 enrich_masterpiece.py <input.json> <output.json>")
    else:
