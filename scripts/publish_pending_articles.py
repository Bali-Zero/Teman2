...
def convert_to_enriched_article(article_data: Dict) -> Dict:
    ...
    if enriched_content:
        ...
        special_sections = ["**What They Don't Tell You**", "**Our Analysis**", "**Our Advice**", "##"]
    ...
    bali_zero_take = {
        "hidden_insight": extract_section(enriched_content, "What They Don't Tell You")[:500],
    ...
    if not bali_zero_take["hidden_insight"]:
        bali_zero_take["hidden_insight"] = extract_section(enriched_content, "Hidden Insight")
    ...
    tldr = {
        "should_worry": "No" if "not" in ai_summary.lower() or "no" in ai_summary.lower()[:50] else "Maybe",
        "what": ai_summary[:100] if ai_summary else facts[:100],
        "who": "Expats and investors in Indonesia",
        "when": "Now",
        "risk_level": "low" if article_data.get("relevance_score", 50) < 60 else "medium"
    }
...
def publish_article(article_id: str, article_data: Dict) -> bool:
    ...
    # Carica cover image
    cover_image_path = article_data.get("cover_image") or article_data.get("image_url", "")
...
def main():
    ...
    print(f"   GitHub: https://github.com/Balizero1987/Teman2/tree/main/apps/mouth/src/content/articles")
    print(f"   URL: https://balizero.com/{{category}}/{{slug}}")
