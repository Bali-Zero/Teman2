...
REFERENCE_FILE = Path(
    "/Users/antonellosiano/Desktop/KB/nuzantara_laws/apps/scraper/data/raw_laws_local/Company&Licenses/kbli/KBLI_SECTORS_FOREIGN_OWNERSHIP_COMPLETE_REFERENCE_2025.md"
)
...
def parse_pma_from_reference() -> Dict[str, Dict]:
    ...
    sections = re.finditer(
        ownership_section_pattern, content, re.DOTALL | re.IGNORECASE
    )
    ...
    for section_match in sections:
        ...
        kbli_codes_in_section = re.finditer(
    ...
    kbli_section_pattern = (
        r"###\s*KBLI\s*(\d{5})[:\s]*.*?\*\*Foreign Ownership\*\*[:\s]*([^\n]+)"
    )
    kbli_sections = re.finditer(
        kbli_section_pattern, content, re.DOTALL | re.IGNORECASE
    )
    ...
    category_rules = {
        ...
        "41xxx-43xxx": (
            None,
            None,
            "Construction - No percentage limit but partnership required",
        ),
...
def main():
    ...
    if pma_data:
        ...
        for i, (code, info) in enumerate(list(pma_data.items())[:10], 1):
            print(
                f"   {i}. {code}: PMA={info.get('pma_allowed')}, Max={info.get('pma_max_percentage', 'N/A')}, Source={info.get('source')}"
            )
