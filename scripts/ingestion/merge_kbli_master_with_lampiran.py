...
def merge_master_with_lampiran():
    ...
    with_complete_data = sum(
        1 for e in merged_kbli.values() if e.get("data_quality") == "complete"
    )
    ...
    print(
        f"   Con dati completi: {with_complete_data} ({with_complete_data / len(merged_kbli) * 100:.1f}%)"
    )
    print(
        f"   Solo dati base: {with_minimal_data} ({with_minimal_data / len(merged_kbli) * 100:.1f}%)"
    )
