...
class KBLIPlatinumIngestion:
    ...
    async def _qdrant_request(self, client, method: str, endpoint: str, json_data: dict = None) -> dict:
    ...
    async def setup_collection(self):
        ...
        async with httpx.AsyncClient() as client:
            # Check exist
            try:
                await self._qdrant_request(client, "GET", f"/collections/{COLLECTION_NAME}")
                exists = True
            ...
            if exists:
                if self.recreate:
                    ...
                    await self._qdrant_request(client, "DELETE", f"/collections/{COLLECTION_NAME}")
                else:
                    logger.info(f"Collection {COLLECTION_NAME} exists. Appending...")
                    return

            logger.info(f"Creating collection {COLLECTION_NAME}...")
            await self._qdrant_request(client, "PUT", f"/collections/{COLLECTION_NAME}", {
    ...
    async def save_kg_batch(self, nodes: List[Dict], edges: List[Dict]):
        ...
        try:
            ...
            for n in nodes:
                ...
                """, n['id'], n['type'], n['name'], json.dumps(n['props']), COLLECTION_NAME)
            ...
            for e in edges:
                ...
                """, rel_id, e['source'], e['target'], e['type'], json.dumps(e['props']), COLLECTION_NAME)
            ...
            for code, info in batch:
                ...
                for edge in graph_data['edges']:
                    if edge['type'] == 'HAS_pIVOT_STRATEGY':
                        pivot_samples.append(f"{code} -> {edge['target']} ({edge['props']['type']})")
            ...
            if not self.dry_run:
                try:
                    ...
                    for idx, (code, _) in enumerate(batch):
                        points.append({
                            "id": str(uuid.uuid5(uuid.NAMESPACE_DNS, f"kbli_plat_{code}")),
                    ...
                    async with httpx.AsyncClient(timeout=60) as client:
                        await self._qdrant_request(client, "PUT", f"/collections/{COLLECTION_NAME}/points", {"points": points})
