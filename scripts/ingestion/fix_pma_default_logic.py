...
OUTPUT_FILE = (
    PROJECT_ROOT / "reports" / "kbli_extraction" / "pma_complete_with_defaults.json"
)
...
CLOSED_KBLI: Set[str] = {
    "11010",
    "11020",
    "11031",  # Alcolici (Perpres 49/2021)
...
RESTRICTED_CATEGORIES = {
    ...
    "03xxx": (
        None,
        None,
        "Fishing - alcune restrizioni",
    ),  # Da verificare per codice specifico
}
...
def apply_pma_defaults() -> Dict[str, Dict]:
    ...
    restricted = sum(
        1 for v in pma_complete.values() if v.get("pma_max_percentage") != "100%"
    )

    print("\n📊 Statistiche:")
    print(
        f"   PMA permesso (100%): {allowed} ({allowed * 100 / len(pma_complete):.1f}%)"
    )
    print(f"   PMA chiuso (0%): {closed} ({closed * 100 / len(pma_complete):.1f}%)")
    print(
        f"   Con restrizioni (<100%): {restricted} ({restricted * 100 / len(pma_complete):.1f}%)"
    )
