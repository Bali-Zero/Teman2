...
PMA_DATA_FILE = (
    PROJECT_ROOT / "reports" / "kbli_extraction" / "pma_data_from_reference_v2.json"
)
BALI_RESTRICTIONS_FILE = (
    PROJECT_ROOT / "reports" / "kbli_compliance" / "bali_restrictions_mapping.json"
)
...
def format_kbli_content(code: str, info: Dict) -> str:
    ...
    if bali_restrictions:
        ...
        if bali_restrictions.get("temporary_restriction"):
            lines.append(
                f"- Restrizione temporanea: {bali_restrictions.get('restriction_type', 'N/A')}"
            )
        if bali_restrictions.get("affected_areas"):
            lines.append(
                f"- Aree interessate: {', '.join(bali_restrictions['affected_areas'])}"
            )
    ...
    for code in missing_codes:
        ...
        try:
            ...
            point = {
                "id": str(uuid.uuid4()),
                "vector": embedding,
                "payload": {"text": content, "metadata": meta},
            }
            points.append(point)
            added += 1
            ...
            if not dry_run:
                ...
                print(
                    f"  [DRY-RUN] Preparato {code}: {kbli_info.get('judul', 'N/A')[:50]}"
                )
        except Exception as e:
...
def update_existing_kbli_with_pma(dry_run: bool = False) -> int:
    ...
    for code, pma_info in pma_data.items():
        ...
        try:
            ...
            if not dry_run:
                print(
                    f"  ✅ Aggiornato {code}: PMA={pma_info.get('pma_allowed')}, Max={pma_info.get('pma_max_percentage', 'N/A')}"
                )
            else:
                print(
                    f"  [DRY-RUN] Aggiornato {code}: PMA={pma_info.get('pma_allowed')}, Max={pma_info.get('pma_max_percentage', 'N/A')}"
                )
        except Exception as e:
            print(f"  ❌ Errore {code}: {e}")
...
def main():
    ...
    parser.add_argument(
        "--add-missing", action="store_true", help="Aggiungi solo KBLI mancanti"
    )
    parser.add_argument("--update-pma", action="store_true", help="Aggiorna solo PMA")
    parser.add_argument(
        "--add-bali", action="store_true", help="Aggiungi solo restrizioni Bali"
    )
