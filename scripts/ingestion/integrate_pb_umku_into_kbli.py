...
def format_kbli_content_with_pb_umku(
    code: str, kbli_info: Dict, pb_umku_list: List[Dict]
) -> str:
    ...
    if pb_umku_list:
        ...
        lines.append("")
        lines.append(
            "Per svolgere questa attività principale, potresti aver bisogno di questi permessi aggiuntivi:"
        )
        lines.append("")
        ...
        for pb_umku in pb_umku_list:
            ...
            for req in pb_persyaratan:
                if req not in persyaratan:
                    persyaratan.append(
                        f"[PB UMKU {pb_umku.get('nama_pb_umku', '')}] {req}"
                    )

            for kew in pb_kewajiban:
                if kew not in kewajiban:
                    kewajiban.append(
                        f"[PB UMKU {pb_umku.get('nama_pb_umku', '')}] {kew}"
                    )
...
def integrate_pb_umku_into_kbli(dry_run: bool = False):
    ...
    if not pb_umku_file:
        ...
        print(
            "   Esegui prima: python scripts/analysis/extract_pb_umku_from_lampiran_parallel.py"
        )
        return
    ...
    for kbli_code, pb_umku_names in kbli_to_pb_umku.items():
        ...
        try:
            ...
            if not dry_run:
                print(
                    f"  ✅ Aggiornato {kbli_code}: {len(pb_umku_names)} PB UMKU aggiunti"
                )
            else:
                print(
                    f"  [DRY-RUN] Aggiornato {kbli_code}: {len(pb_umku_names)} PB UMKU"
                )
    ...
    print("\n🎯 RISULTATO:")
    print("   Ora ogni KBLI mostra non solo l'attività principale,")
    print("   ma anche TUTTI i permessi di supporto necessari!")
    print("   Questo è un livello di dettaglio che OSS non ha mai raggiunto!")
