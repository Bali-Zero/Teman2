...
DOCUMENTI_PER_RISIKO = {
    "Rendah": {
        "per_operativita": ["Sertifikat standar industri (jika diperlukan)"],
        "per_operativita_en": ["Industrial standard certificate (if required)"],
    },
    "Menengah Rendah": {
        "per_operativita": [
...
def get_documenti_for_risk(risk_level: Optional[str]) -> Dict[str, List[str]]:
    ...
    risk_docs = DOCUMENTI_PER_RISIKO.get(
        risk_level, DOCUMENTI_PER_RISIKO["Tidak diatur"]
    )
...
SKALA_USAHA_DEFAULTS = {
    ...
    "mikro": {"max_revenue_idr": 300000000, "requirements": "NIB saja"},
    "kecil": {"max_revenue_idr": 2500000000, "requirements": "NIB saja"},
    "menengah": {
        "max_revenue_idr": 50000000000,
        "requirements": "NIB + Sertifikat Standar",
    },
    "besar": {"min_revenue_idr": 50000000000, "requirements": "NIB + Izin"},
}
