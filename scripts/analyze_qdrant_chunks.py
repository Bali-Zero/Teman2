...
def analyze_collection(name, sample_size=100):
    ...
    payload = {"limit": sample_size, "with_payload": True, "with_vector": False}
    ...
    print(
        f"   Top Sources: {sorted(sources.items(), key=lambda x: x[1], reverse=True)[:3]}"
    )
