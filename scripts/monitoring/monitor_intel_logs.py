...
def run_fly_logs(limit: int = 100) -> List[str]:
    ...
    try:
        ...
        print(f"{RED}Error: fly CLI not found. Install from https://fly.io/docs/hands-on/install-flyctl/{RESET}")
        return []
...
def analyze_logs(logs: List[str]) -> Dict:
    ...
    for line in logs:
        ...
        # Check for errors
        if "error" in line_lower and ("intel" in line_lower or "staging" in line_lower or "approval" in line_lower):
            errors.append(line)
        
        # Check for warnings
        if "warning" in line_lower and ("intel" in line_lower or "staging" in line_lower):
...
def print_report(analysis: Dict):
    ...
    if analysis["intel_operations"]:
        ...
        for op in analysis["intel_operations"]:
            for endpoint in ["/api/intel/scraper/submit", "/api/intel/staging/", "/api/intel/metrics", "/api/intel/search"]:
                if endpoint in op:
    ...
    print(f"{BLUE}{'='*60}{RESET}")
    if analysis["errors"]:
        print(f"{RED}⚠️  ACTION REQUIRED: {len(analysis['errors'])} errors detected{RESET}")
        return 1
    elif analysis["warnings"]:
        print(f"{YELLOW}⚠️  Review warnings: {len(analysis['warnings'])} warnings detected{RESET}")
        return 0
    else:
