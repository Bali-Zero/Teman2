...
def analyze_intel_metrics(metrics: Dict) -> Dict:
        ...
        for prefix in INTEL_METRICS_PREFIXES:
            if metric_name.startswith(prefix):
                ...
                if "articles_submitted_total" in metric_name:
                    ...
                    if (
                        "labels" in metric_data
                        and "classified_as" in metric_data["labels"]
                    ):
                        classified_as = metric_data["labels"]["classified_as"]
                        analysis["classification_by_type"][classified_as] = (
                            analysis["classification_by_type"].get(classified_as, 0)
                            + metric_data["value"]
                        )
                    ...
                    if (
                        "labels" in metric_data
                        and "intel_type" in metric_data["labels"]
                    ):
                        intel_type = metric_data["labels"]["intel_type"]
                        analysis["staging_queue_sizes"][intel_type] = metric_data[
                            "value"
                        ]
    ...
    if analysis["total_articles_submitted"] > 0:
        duplicate_rate = (
            analysis["total_duplicates"] / analysis["total_articles_submitted"]
        ) * 100
