...
if [[ -z "${DATABASE_URL:-}" ]]; then
    if [[ -n "${DATABASE_URL_LOCAL:-}" ]]; then
        ...
        echo "[wr2-wrapper] ERROR: DATABASE_URL_LOCAL not set in $SECRETS_FILE. Add it (format: postgres://<user>:<password>@127.0.0.1:15432/nuzantara_rag?sslmode=disable) and load com.balizero.wr2.pg-proxy first." >&2
        exit 74
    fi
    export DATABASE_URL
fi
