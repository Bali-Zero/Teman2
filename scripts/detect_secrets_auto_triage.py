...
CONTENT_KEYED_RULES: list[tuple[re.Pattern[str], re.Pattern[str], str]] = [
    ...
    # Translation freshness stamps are SHA-256 digests of the English source
    # body. They are generated by scripts/translate-articles.py and contain no
    # credential material; keep approval restricted to the exact field and
    # 64-character lowercase-hex value shape.
    (
        re.compile(r"^apps/mouth/src/content/articles/.*\.mdx$"),
        re.compile(r'^\s*source_sha256:\s*"[0-9a-f]{64}"\s*$'),
        "mouth article translation freshness stamp: SHA-256 digest of source "
