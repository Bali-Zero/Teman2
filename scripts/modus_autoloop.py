...
Hard invariants (do not relax without a new spec):
  ...
  - Never-cascade final gate (HARD, spec §3): a quota-dead Opus 5.5 window at
    the final gate must route to defer_task(), never to a weaker model
    (RULED 2026-08-20: the final gate is Opus 5.5 xhigh effort, not Fable — see
...
def _spawn_session(task: dict[str, Any]) -> list[str]:
    ...
    argv = [
        ...
        f"Worktree: {worktree}. Final gate is Opus 5.5 xhigh effort and must never "
        "cascade to a weaker model; if the Opus 5.5 window is quota-dead at the "
...
def defer_task(job: dict[str, Any] | str) -> None:
    ...
    Implements the never-cascade invariant (spec §3.1/§3): a dead Opus 5.5
    window at the final gate must never be routed to a weaker model
    (RULED 2026-08-20: the final gate is Opus 5.5, not Fable — CLAUDE.md §5).
    ...
    escalations.write_escalation(
        {
            ...
            "note": (
                "Opus 5.5 final-gate window was quota-dead at execution time. "
