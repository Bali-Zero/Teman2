#!/usr/bin/env python3
"""observe_visa_seq19_fold.py — bites: observation for the seq-19 fold lane.
...
1. ``apps/backend-rag``'s ``test_seq19_pack.py`` — the fold-integrity /
   identity / guard / gold-coverage / gold-replay gate suite for
   ``rulepack-prod-019.source.json``.
2. ``apps/mouth``'s ``engine-adapter.test.ts`` — the cross-app consumer that
   ...
   ``reason_code`` it can emit has copy in ``REVIEW_REASON_COPY``. This is
   the concrete proof that retiring (or NOT retiring) a review-stage rule id
   in the fold is observed outside the Python package that authors it.
...
#: `test_seq19_pack.py`'s own `PROD_TRUST_STORE_JSON` pins, needed because
#: `fold_pack_seq19.fold()` verifies the seq-18 anchor's signature before
#: doing anything else.
_TRUST_STORE_JSON = (
...
def _run(cmd: list[str], *, cwd: Path, env: dict[str, str]) -> int:
    print(f"observe_visa_seq19_fold: running {' '.join(cmd)} (cwd={cwd})")
...
def main() -> int:
    ...
    if rc != 0:
        print("observe_visa_seq19_fold: test_seq19_pack.py FAILED")
        return rc
    ...
    if rc != 0:
        print("observe_visa_seq19_fold: engine-adapter.test.ts FAILED")
        return rc

    print("observe_visa_seq19_fold: both consumers green")
    return 0
