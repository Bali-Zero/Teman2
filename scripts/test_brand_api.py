...
def test_generator_finds_components():
    ...
    # disk-state truth: the real tree has 30 (18 top-level incl. Money +
    # ListPageHeader/SearchBox/FilterBar/StatChips/SubNav + FactBadge added by
    # the WS1 SSOT reconciliation + 11 under apps/ — bump this constant when a
    # component is added/removed, the count is the gate's contract).
    assert len(arts) == 30, f"expected 30 components on disk, got {len(arts)}"
