...
sys.path.append(
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../apps/backend-rag/backend")
    )
)
...
async def run_stress_test():
    ...
    scenario_path = os.path.join(
        os.path.dirname(__file__), "data/stress_test_scenarios.json"
    )
    ...
    for idx, turn in enumerate(scenarios):
        ...
        try:
            ...
            if consecutive_errors >= 4:
                logger.critical(
                    "🚨 CIRCUIT BREAKER TRIPPED: 4 Consecutive Errors. Stopping Test."
                )
                print("\n" + "!" * 60)
