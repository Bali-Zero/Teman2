...
BRIDGED_LABELS: tuple[BridgedLaunchAgent, ...] = (
    ...
    # Restored 2026-06-21: these 3 were silently dropped in the 2026-06-06
    # dict→dataclass refactor (present in .bak-20260606-044211). Their heartbeats
    # froze at 2026-06-05 → tripped 14d staleness 2026-06-20 → false TERMINAL in DLQ
    # while the jobs were verifiably alive (zombie-hunter killing procs, poller
    # polling, webhook on :7788). legacy_job_id MUST match the .last.json names the
    # sentinel reads.
    BridgedLaunchAgent(
        label="com.nuzantara.zombie-hunter",
        organ_id="pro.zombie_hunter",
        daemon=False,
        legacy_job_id="zombie_hunter",
    ),
    BridgedLaunchAgent(
    ...
    BridgedLaunchAgent(
        ...
        organ_id="pro.post_publish_webhook",
        daemon=True,
        legacy_job_id="post_publish_webhook",
    ),
    BridgedLaunchAgent(
