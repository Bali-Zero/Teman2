...
def parse_oss_text_data(text: str) -> Dict:
    ...
    while i < len(lines):
        ...
        if current_section == "uraian":
            ...
            if line and not any(
                x in line
                for x in ["No", "Parameter", "Kewenangan", "Persyaratan", "Kewajiban"]
            ):
...
def fetch_kbli_from_oss(code: str, manual_data: Optional[str] = None) -> Optional[Dict]:
    ...
    for api_url in api_urls:
        try:
            resp = requests.get(
                api_url, timeout=10, headers={"User-Agent": "Mozilla/5.0"}
            )
...
def compare_kbli_data(
    qdrant_data: Optional[Dict], oss_data: Optional[Dict], code: str
) -> Dict:
    ...
    if qdrant_data and oss_data:
            ...
            if q_judul and oss_judul:
                if q_judul == oss_judul:
                    ...
                    comparison["differences"].append(
                        f"Titolo diverso: Qdrant='{q_meta.get('judul')}', OSS='{oss_data.get('judul') or oss_data.get('title')}'"
                    )
        ...
        if "uraian" in oss_data and oss_data["uraian"]:
            if q_text and oss_data["uraian"].lower() in q_text.lower():
                ...
                comparison["differences"].append(
                    "URAIAN presente in OSS ma non trovato esattamente in Qdrant"
                )

        # Verifica RUANG LINGKUP
        if "ruang_lingkup" in oss_data and oss_data["ruang_lingkup"]:
            if q_text and any(
                word in q_text.lower()
                for word in oss_data["ruang_lingkup"].lower().split()[:5]
            ):
                comparison["matches"]["ruang_lingkup"] = "parzialmente presente"
            else:
                comparison["differences"].append(
                    "RUANG LINGKUP presente in OSS ma non chiaramente in Qdrant"
                )

        # Confronto Persyaratan
        q_persyaratan = comparison["qdrant"].get("persyaratan", [])
        if q_persyaratan:
            comparison["matches"]["persyaratan_count"] = (
                f"{len(q_persyaratan)} persyaratan trovati in Qdrant"
            )
            ...
            if "pb_umku" in oss_data and oss_data["pb_umku"]:
                for pb in oss_data["pb_umku"]:
                    if pb.get("persyaratan"):
                        ...
                        if oss_pers_count > 0:
                            comparison["matches"]["persyaratan_oss"] = (
                                f"{oss_pers_count} persyaratan in OSS PB UMKU"
                            )
                            # Confronto dettagliato se possibile
                            if len(q_persyaratan) != oss_pers_count:
                                comparison["differences"].append(
                                    f"Numero persyaratan diverso: Qdrant={len(q_persyaratan)}, OSS={oss_pers_count}"
                                )

        # Confronto Kewajiban
        q_kewajiban = comparison["qdrant"].get("kewajiban", [])
        if q_kewajiban:
            comparison["matches"]["kewajiban_count"] = (
                f"{len(q_kewajiban)} kewajiban trovati in Qdrant"
            )
            ...
            if "pb_umku" in oss_data and oss_data["pb_umku"]:
                for pb in oss_data["pb_umku"]:
                    if pb.get("kewajiban"):
                        ...
                        if oss_kew_count > 0:
                            comparison["matches"]["kewajiban_oss"] = (
                                f"{oss_kew_count} kewajiban in OSS PB UMKU"
                            )
                            if len(q_kewajiban) != oss_kew_count:
                                comparison["differences"].append(
                                    f"Numero kewajiban diverso: Qdrant={len(q_kewajiban)}, OSS={oss_kew_count}"
                                )

        # Verifica PB UMKU
        if "pb_umku" in oss_data and oss_data["pb_umku"]:
            pb_count = len(oss_data["pb_umku"])
            comparison["matches"]["pb_umku_count"] = (
                f"{pb_count} PB UMKU trovati in OSS"
            )
            # Qdrant ha già i dati strutturati nel text
            if q_persyaratan or q_kewajiban:
                comparison["matches"]["pb_umku_qdrant"] = (
                    "Dati PB UMKU presenti in Qdrant (persyaratan/kewajiban estratti)"
                )
        ...
        if "risk_level" in oss_data or "risiko" in oss_data:
            ...
            if q_risk and oss_risk and q_risk != oss_risk:
                comparison["differences"].append(
                    f"Risk level diverso: Qdrant={q_risk}, OSS={oss_risk}"
                )
...
def generate_html_report(comparison: Dict, output_path: str) -> None:
    ...
    if oss.get("pb_umku"):
        pb_items = "".join(
            [
                f"<li><strong>{pb.get('nama_pb_umku', 'N/A')}</strong></li>"
                for pb in oss.get("pb_umku", [])
            ]
        )
...
def main(kbli_code: str = "01111", oss_manual_data: Optional[str] = None):
    ...
    if oss_data:
        ...
        print("   ⚠️  Nessun dato recuperato da OSS.go.id")
    ...
    output_path = os.path.join(
        REPORT_DIR, f"kbli_{kbli_code}_oss_comparison_{today}.html"
    )
