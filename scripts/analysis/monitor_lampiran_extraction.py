...
def analyze_resoconto(file_path):
    ...
    try:
        ...
        return {
            ...
            "completion": (pages_analyzed / total_pages * 100)
            if total_pages > 0
            else 0,
        ...
        return {"pdf_name": file_path.name, "error": str(e), "status": "error"}
...
def main():
    ...
    if resoconto_files:
        ...
        for resoconto_file in resoconto_files:
            ...
            print(
                f"   Pagine: {stats['pages_analyzed']}/{stats['total_pages']} ({stats['completion']:.1f}%)"
            )
            print(f"   Agenti: {stats['num_agents']}")
            print(f"   Tempo: {stats['elapsed_minutes']:.1f} minuti")
            print(
                f"   Successi: {stats['total_success']}, Errori: {stats['total_errors']}"
            )
        ...
        print(
            f"📖 Pagine analizzate: {total_pages_analyzed_all}/{total_pages_all} ({total_pages_analyzed_all / total_pages_all * 100:.1f}%)"
            if total_pages_all > 0
            else "📖 Pagine analizzate: 0"
        )
