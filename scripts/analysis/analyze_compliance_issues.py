...
COMPLIANCE_FILE = (
    PROJECT_ROOT / "reports" / "kbli_compliance" / "kbli_compliance_report.json"
)
...
def analyze_compliance():
    ...
    for comp in comparisons:
        ...
        other_diff = [
            d for d in differences if "Judul" not in d and "judul" not in d.lower()
        ]
    ...
    # Statistiche reali
    total_with_issues = len(
        [c for c in comparisons if c.get("differences") or c.get("missing_fields")]
    )
    ...
    print(
        f"   Solo differenze titolo (non critico): {title_only_count} ({title_only_count * 100 / total_with_issues:.1f}%)"
    )
    print(
        f"   Problemi reali: {real_issues_count} ({real_issues_count * 100 / total_with_issues:.1f}%)"
    )
