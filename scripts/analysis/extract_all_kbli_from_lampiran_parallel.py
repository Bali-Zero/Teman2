...
async def save_checkpoint(
    ...
    try:
        ...
        checkpoint_file = os.path.join(
            OUTPUT_DIR, f"kbli_checkpoint_{checkpoint_type}_{timestamp}.json"
        )
        ...
        print(
            f"💾 Checkpoint salvato: {checkpoint_file} ({len(consolidated)} KBLI, {total_processed}/{len(all_pages)} pagine)",
            flush=True,
        )
    ...
    for lampiran_name, pdf_path, page_num in pages:
        try:
            ...
            async with checkpoint_lock:
                total_processed = sum(
                    stats.get("processed", 0) for stats in progress.values()
                )
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] [Agent {agent_id}] ❌ Errore pagina {page_num}: {e}",
                flush=True,
            )
            processed += 1
...
async def main():
    ...
    output_file = os.path.join(
        OUTPUT_DIR, f"kbli_complete_from_lampiran_{timestamp}.json"
    )
