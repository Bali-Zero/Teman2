...
"""
    ...
    try:
        ...
        if model is None:
            return {"page": page_num, "error": "Vertex AI non inizializzato"}
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = model.generate_content(
                    [prompt, image_part], generation_config={"max_output_tokens": 8192}
                )
                ...
                if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                    ...
                    print(
                        f"  [Agent {agent_id}] ⚠️  Rate limit (429) pagina {page_num}, attendo {wait_time}s...",
                        flush=True,
                    )
                    ...
                    # Timeout - riprova
                    print(
                        f"  [Agent {agent_id}] ⚠️  Timeout pagina {page_num}, retry {attempt + 1}/{max_retries}...",
                        flush=True,
                    )
                    ...
                    print(
                        f"  [Agent {agent_id}] ⚠️  Errore API pagina {page_num}: {error_msg[:100]}...",
                        flush=True,
                    )
                    if attempt == max_retries - 1:
                        raise  # Rilancia se ultimo tentativo
        ...
        if result_text is None:
            return {"page": page_num, "error": "Failed after retries"}
...
async def analyze_pages_range(
    ...
    for page_num in range(start_page, end_page + 1):
        try:
            result = await analyze_page_with_vertex(
...
async def analyze_pdf_with_agents(pdf_name: str, pdf_path: str) -> Dict[str, Any]:
    ...
    if total_pages == 0:
        print("   ⚠️  PDF vuoto o non leggibile", flush=True)
        return None
    ...
    try:
        ...
        print(
            f"✅ Tutti gli agenti completati in {elapsed_time / 60:.1f} minuti",
            flush=True,
        )
    except Exception as e:
        elapsed_time = time.time() - start_time
...
async def main():
    ...
    for idx, (pdf_name, pdf_path) in enumerate(pdfs, 1):
        ...
        try:
            ...
            if idx < len(pdfs):
                print("⏸️  Pausa 30 secondi prima del prossimo PDF...\n", flush=True)
