...
async def extract_pb_umku_from_page(
        ...
        try:
            ...
            if (
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"  [Agent {agent_id}] ❌ Max retries raggiunto per pagina {page_num}",
                        flush=True,
                    )
                    return []
            else:
                print(
                    f"  [Agent {agent_id}] ❌ Errore pagina {page_num}: {e}", flush=True
                )
                return []
    ...
    try:
            ...
            try:
                ...
                try:
                    json_objects = re.findall(
                        r"\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}", result_clean, re.DOTALL
                    )
                    if json_objects:
                        pb_umku_list = []
                        for obj_str in json_objects:
                            if (
                                '"nama_pb_umku"' in obj_str
                                or "'nama_pb_umku'" in obj_str
                            ):
                                try:
        ...
        print(
            f"  [Agent {agent_id}] ⚠️  Errore parsing JSON pagina {page_num}: {e}",
            flush=True,
        )
...
async def agent_worker(
    ...
    progress[f"agent_{agent_id}"] = {"processed": processed, "extracted": extracted}
...
async def main():
    ...
    for agent_id in range(NUM_AGENTS):
        if page_chunks[agent_id]:
            task = agent_worker(
                agent_id, page_chunks[agent_id], vision_service, results, progress
            )
            tasks.append(task)
    ...
    output_file = os.path.join(
        OUTPUT_DIR, f"pb_umku_complete_from_lampiran_{timestamp}.json"
    )
    ...
    for agent_id, stats in sorted(progress.items()):
        print(
            f"   {agent_id}: {stats['processed']} pagine, {stats['extracted']} PB UMKU"
        )
