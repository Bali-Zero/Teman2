...
if VERTEX_API_KEY:
    ...
    print(
        f"✅ Vertex API Key configurata: {VERTEX_API_KEY[:20]}...{VERTEX_API_KEY[-10:]}"
    )
...
async def analyze_pages_range(
                ...
                try:
                    ...
                    if (
                        ...
                        if attempt < max_retries - 1:
                            ...
                            print(
                                f"  [Agent {agent_id}] ⚠️  Quota esaurita pagina {page_num}, retry {attempt + 1}/{max_retries} dopo {delay / 60:.0f} minuti..."
                            )
                            await asyncio.sleep(delay)
                            continue
                        else:
                            print(
                                f"  [Agent {agent_id}] ❌ Quota esaurita dopo {max_retries} tentativi per pagina {page_num}"
                            )
                            raise
                    elif (
                        "503" in error_str
                        ...
                        if attempt < max_retries - 1:
                            ...
                            print(
                                f"  [Agent {agent_id}] ⚠️  API overloaded pagina {page_num}, retry {attempt + 1}/{max_retries} dopo {delay}s..."
                            )
                            await asyncio.sleep(delay)
                            continue
                    raise
            ...
            if result:
                ...
                try:
                    ...
                    if "```json" in result:
                        ...
                        result_clean = re.sub(
                            r"```\s*$", "", result_clean, flags=re.MULTILINE
                        )
                    elif "```" in result:
                    ...
                    resoconto["results"].append(
                        {"page": page_num, "analysis": page_analysis}
                    )
                    pages_analyzed += 1
    ...
    print(
        f"  [Agent {agent_id}] ✅ Completato: {pages_analyzed}/{end_page - start_page + 1} pagine analizzate"
    )
...
async def analyze_pdf_with_agents(
    vision_service: PDFVisionService, pdf_name: str, pdf_path: str
) -> Dict[str, Any]:
    ...
    for i, resoconto in enumerate(resoconti):
        if isinstance(resoconto, Exception):
            ...
            pdf_resoconto["agents"].append({"agent_id": i + 1, "error": str(resoconto)})
        else:
    ...
    print("\n📊 Statistiche:")
    print(
        f"   Pagine analizzate: {total_pages_analyzed}/{total_pages} ({total_pages_analyzed / total_pages * 100:.1f}%)"
    )
    print(f"   Tempo totale: {elapsed_time / 60:.1f} minuti")
    print(
        f"   Tempo medio per pagina: {elapsed_time / total_pages_analyzed:.1f} secondi"
...
async def main(skip_processed: bool = True):
    ...
    for idx, (pdf_name, pdf_path) in enumerate(pdfs, 1):
        ...
        try:
            resoconto = await analyze_pdf_with_agents(
                vision_service, pdf_name, pdf_path
            )
            if resoconto:
