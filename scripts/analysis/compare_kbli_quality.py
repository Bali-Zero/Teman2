        ...
        for point in points:
            ...
            if kode and isinstance(kode, str) and kode.isdigit() and len(kode) == 5:
                ...
                if kode not in kbli_data:
                    ...
                    existing_fields = sum(
                        1 for v in kbli_data[kode].get("metadata", {}).values() if v
                    )
...
def compare_kbli_fields(qdrant_kbli: Dict, extracted_kbli: Dict) -> Dict:
    ...
    for field_name, (qdrant_key, extracted_key) in fields_to_compare.items():
        ...
        if not qdrant_val and extracted_val:
            ...
            comparison["update_reasons"].append(
                f"{field_name}: mancante in Qdrant, presente in estratto"
            )
...
def compare_all_kbli():
    ...
    print(
        f"   KBLI che necessitano aggiornamento: {len(needs_update)}/{len(common_codes)} ({len(needs_update) / len(common_codes) * 100:.1f}%)"
    )
    print()
    ...
    report = {
        ...
        "statistics": {
            ...
            "update_percentage": len(needs_update) / len(common_codes) * 100
            if common_codes
            else 0,
        },
        "field_statistics": {
