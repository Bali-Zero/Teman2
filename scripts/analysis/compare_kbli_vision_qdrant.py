...
def compare_kbli_data(qdrant_data: Dict, vision_data: Optional[Dict]) -> Dict:
    ...
    if q_risk and v_risk:
        ...
        comparison["differences"].append(
            f"Risk level mancante in Qdrant: PP28 ha '{v_risk}'"
        )
    elif q_risk and not v_risk:
        comparison["differences"].append(
            f"Risk level mancante in PP28: Qdrant ha '{q_risk}'"
        )
    ...
    if q_scales and v_scales:
        ...
        comparison["differences"].append(
            f"Skala mancante in Qdrant: PP28 ha {sorted(v_scales)}"
        )
...
async def main():
    ...
    for i, code in enumerate(sample_codes, 1):
        ...
        if not qdrant_data:
            print("  ⚠️  Non trovato in Qdrant")
            continue

        # Cerca in PP 28/2025 con Vision
        print("  👁️  Cercando in PP 28/2025...")
        vision_data = await search_kbli_in_pp28_vision(
            vision_service, code, max_pages=20
        )  # Ridotto per test veloce
    ...
    output_file = os.path.join(
        OUTPUT_DIR, f"vision_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    if stats["total"] > 0:
        print(
            f"✅ Trovati in PP 28/2025: {stats['found_in_pp28']} ({stats['found_in_pp28'] * 100 / stats['total']:.1f}%)"
        )
        print(f"❌ Non trovati in PP 28/2025: {stats['not_found_in_pp28']}")
        print(
            f"✅ Completamente allineati: {stats['fully_matching']} ({stats['fully_matching'] * 100 / stats['total']:.1f}%)"
        )
