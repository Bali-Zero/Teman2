...
def identify_missing_sources():
    ...
    print("      Settori comuni (01xx, 10xx, 85xx, 66xx)")
    print("      Verificare in altri Lampiran o sezioni PP 28/2025")
    print()

    # KBLI che probabilmente non sono regolamentati
    low_priority_codes = [
        c for c in missing_codes if c[:2] not in high_priority_prefixes
    ]

    print(f"   🟡 Priorità MEDIA ({len(low_priority_codes)} KBLI):")
    print("      Altri settori - potrebbero non essere regolamentati in PP 28/2025")
    print("      Usare solo dati BPS + template/defaults")
    print()
    ...
    report = {
        ...
        "prefix_distribution": {
            prefix: len(codes) for prefix, codes in prefix_groups.items()
        },
