    ...
    try:
        while True:
            ...
            if proc_info:
                ...
                print(
                    f"[{datetime.now().strftime('%H:%M:%S')}] ⏱️  Tempo trascorso: {elapsed_str}"
                )
                print(
                    f"   CPU: {proc_info['cpu']}%, Mem: {proc_info['mem']}%, Etime: {proc_info['etime']}"
                )
            ...
            if output_file:
                ...
                if "error" not in stats:
                    ...
                    print(
                        f"   📊 KBLI estratti: {kbli_count} (+{kbli_count - last_kbli_count if last_kbli_count > 0 else 0})"
                    )
                    print(
                        f"   📄 Pagine processate: {pages} (+{pages - last_pages if last_pages > 0 else 0})"
                    )

                    # Progress per agente
                    progress = stats.get("progress", {})
                    if progress:
                        print("   🤖 Progress agenti:")
