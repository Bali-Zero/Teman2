...
def analyze_gap():
    ...
    report: Dict[str, Any] = {
        ...
        "sources": {"bps_master_list": {}, "lampiran_extracted": {}, "qdrant": {}},
        "gap_analysis": {},
        "recommendations": [],
    }
    ...
    print(
        f"   ⚠️  KBLI in BPS ma NON estratti da Lampiran I: {len(missing_from_lampiran)}"
    )
    print(
        f"      Coverage Lampiran I: {len(lampiran_codes) / len(bps_codes) * 100:.1f}%"
    )
    ...
    if not_in_bps:
        ...
        print("   ✅ Tutti i KBLI estratti sono presenti in BPS")

    # KBLI in Qdrant ma non in BPS (obsoleti?)
    obsolete_in_qdrant = qdrant_codes - bps_codes
    if obsolete_in_qdrant:
        print(
            f"   ⚠️  KBLI in Qdrant ma NON in BPS (obsoleti?): {len(obsolete_in_qdrant)}"
        )
        print(f"      Esempi: {sorted(list(obsolete_in_qdrant))[:10]}")
    else:
        print("   ✅ Tutti i KBLI in Qdrant sono presenti in BPS")
    ...
    print(
        f"      Coverage Qdrant: {len(qdrant_codes & bps_codes) / len(bps_codes) * 100:.1f}%"
    )
    ...
    report["gap_analysis"] = {
        ...
        "coverage_lampiran_percentage": len(lampiran_codes) / len(bps_codes) * 100
        if bps_codes
        else 0,
        "coverage_qdrant_percentage": len(qdrant_codes & bps_codes)
        / len(bps_codes)
        * 100
        if bps_codes
        else 0,
    ...
    if len(missing_from_lampiran) > 0:
        ...
        print(
            f"   🔴 Priorità ALTA: {len(missing_from_lampiran)} KBLI mancano da Lampiran I"
        )
        print("      Verificare se sono in altri Lampiran o sezioni PP 28/2025")
    ...
    if len(missing_from_qdrant) > 0:
        ...
        print(
            f"   🟡 Priorità MEDIA: {len(missing_from_qdrant)} KBLI mancano da Qdrant"
        )
    ...
    if obsolete_in_qdrant:
        ...
        print(
            f"   🟢 Priorità BASSA: {len(obsolete_in_qdrant)} KBLI obsoleti in Qdrant da verificare"
        )
    ...
    print(
        f"   Estratti Lampiran I: {len(lampiran_codes)} ({len(lampiran_codes) / len(bps_codes) * 100:.1f}%)"
    )
    print(
        f"   In Qdrant: {len(qdrant_codes)} ({len(qdrant_codes & bps_codes) / len(bps_codes) * 100:.1f}% dei BPS)"
    )
