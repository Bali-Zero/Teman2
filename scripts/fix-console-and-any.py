...
REPLACEMENTS = [
    ...
    (r'console\.log\(([^)]+)\)', r'logger.debug(\1, { component: "AUTO", action: "log" })'),
    (r'console\.error\(([^)]+)\)', r'logger.error(\1, { component: "AUTO", action: "error" }, toError(\1))'),
    (r'console\.warn\(([^)]+)\)', r'logger.warn(\1, { component: "AUTO", action: "warn" })'),
    (r'console\.info\(([^)]+)\)', r'logger.info(\1, { component: "AUTO", action: "info" })'),
    ...
    (r'Record<string,\s*any>', r'Record<string, unknown>'),  # Replace Record<string, any>
...
def needs_import(content: str) -> Tuple[bool, bool]:
    ...
    has_logger = 'logger' in content and ('from' in content and 'logger' in content.split('from')[0] if 'from' in content else False)
    has_to_error = 'toError' in content
    needs_logger_import = 'logger.' in content and not ('import' in content and 'logger' in content)
    needs_to_error_import = 'toError(' in content and not ('import' in content and 'toError' in content)
    ...
    for target_dir in TARGET_DIRS:
        ...
        for file_path in dir_path.rglob('*.ts'):
            ...
            if modified:
                ...
                print(f"✓ {file_path.relative_to(base_path)} ({replacements} replacements)")
