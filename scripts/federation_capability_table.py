...
PIPELINES = {
    ...
    "war-room": {
        "name": "War Room 2.0 (WR2)",
        "role": "Automated editorial pipeline for Bali Zero: research → council → canva carousel",
        "tier": "pipeline",
        "stages": ["research", "council (multi-LLM)", "visual (Imagen)", "canva_renderer (headless claude -p)", "review gate"],
        "files": "apps/backend-rag/backend/services/{war_room,council,visual,canva_renderer,review}/",
        "trigger": "scheduled: launchd + cron on Pro; manual via /api/war-room/* endpoints",
        "status": "active (canva_renderer kill switch gated during rollout)",
    },
    "seo-guardian": {
        "name": "SEO Guardian",
