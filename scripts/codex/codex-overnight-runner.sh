...
3. VERIFY — After all milestones:
   ...
   c. Commit every valid repo change
   d. Run close-out gate: \`./scripts/ai_dev_closeout.sh --strict\`
   e. If everything passes, write final status to /tmp/codex-overnight-${TIMESTAMP}-status.md
...
4. STATUS — At completion (success OR failure), write to /tmp/codex-overnight-${TIMESTAMP}-status.md:
   ...
   - Result of \`./scripts/ai_dev_closeout.sh --strict\`
   - Any blockers encountered
   - Next-step recommendation

HARD RULES (strict):
- Follow docs/operations/AI_DEV_DISCIPLINE.md
...
CLOSEOUT_FILE="/tmp/codex-overnight-${TIMESTAMP}-closeout.md"
CLOSEOUT_STATUS=0
if [ -x "./scripts/ai_dev_closeout.sh" ]; then
    if ./scripts/ai_dev_closeout.sh --strict > "$CLOSEOUT_FILE" 2>&1; then
        log "AI dev close-out passed"
    else
        CLOSEOUT_STATUS=$?
        log "AI dev close-out failed with code=$CLOSEOUT_STATUS"
        if [ "$OUTCOME" = "success" ]; then
            OUTCOME="partial"
            log "Downgraded outcome to partial because close-out failed"
        fi
    fi
else
    CLOSEOUT_STATUS=127
    {
        echo "ERROR: ./scripts/ai_dev_closeout.sh is missing or not executable"
    } > "$CLOSEOUT_FILE"
    log "AI dev close-out script missing or not executable"
    if [ "$OUTCOME" = "success" ]; then
        OUTCOME="partial"
        log "Downgraded outcome to partial because close-out could not run"
    fi
fi
...
- [ ] Manual review
- [x] AI developer close-out: \`./scripts/ai_dev_closeout.sh --strict\`

### AI Developer Close-Out
\`\`\`
$(cat "$CLOSEOUT_FILE" 2>/dev/null | head -80)
\`\`\`
...
[ -f "$CLOSEOUT_FILE" ] && cp "$CLOSEOUT_FILE" "${LOG_DIR}/${TASK_NAME}-${TIMESTAMP}-closeout.md" || true
