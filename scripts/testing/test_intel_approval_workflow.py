...
def check_staging_item_exists(
    base_url: str, intel_type: str, item_id: str, api_key: Optional[str] = None
) -> Dict:
...
def verify_telegram_notification_sent(
    item_id: str, pending_path: str = "/tmp/pending"
) -> Dict:
...
def test_approval_workflow(base_url: str, api_key: Optional[str] = None) -> Dict:
    ...
    if pending_result.get("success"):
        ...
        if count > 0:
            ...
            for item in items[:3]:
                print(
                    f"    • {item.get('id', 'unknown')} - {item.get('title', 'Untitled')[:50]}"
                )
    else:
...
def main():
    ...
    report_file = (
        ...
        / "monitoring"
        / "intel_approval_workflow_test.json"
    )
