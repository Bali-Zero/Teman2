...
def parse_visa_text(file_path):
    ...
    for i in range(1, len(sections), 3):
        ...
        requirements = [
            ...
            for line in requirements_raw.split("\n")
        ...
        if "30 days" in type_info:
            visa_data["duration"] = "30 days"
        if "60 days" in type_info:
            visa_data["duration"] = "60 days"
        if "1 Year" in type_info:
            visa_data["duration"] = "1 Year"
        if "5 Years" in type_info:
            visa_data["duration"] = "5 Years"
...
if __name__ == "__main__":
    ...
    with open(
        "apps/backend-rag/backend/migrations/seed_visa_types_official_2026.py", "w"
    ) as f:
        f.write(seed_content)
    print(
        "Migration file created: apps/backend-rag/backend/migrations/seed_visa_types_official_2026.py"
    )
