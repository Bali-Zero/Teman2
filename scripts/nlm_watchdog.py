...
def _run(cmd: list[str] | str, timeout: int = 30, shell: bool = False,
    ...
    so the Telegram send still works when auth_sentinel cannot be imported."""
    try:
        p = subprocess.run(
...
# Pro's canonical checkout: nb-curator-daily.sh runs nb_generate_inventory.py from here.
