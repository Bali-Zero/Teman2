...
sources:
  ...
  - Kimi K3 cross-family adversarial refutation of this capture, 2026-08-25, independently re-verified (WCAG contrast recomputation) rather than taken on faith
---

# Second Home Studio — Ring-3 UI/UX research capture

This capture records the 2026-08-24 pass that followed ring-2: twelve PRs shipped against the Second Home Studio. Every number below is a direct measurement — painted-pixel, DOM, filesystem, or watcher-log — taken from production, the branch, or the monitor itself; none is a lane's self-report.
...
Reviewed by Kimi K3 (`kimi -m kimi-code/k3`), generator != grader, against this capture as it stood before this review (i.e. with the original, unqualified provenance sentence and no `adversarial_review` key). The refuter was instructed to hunt for five specific classes of defect and to independently recompute anything checkable rather than take it on faith.

- **Numbers that cannot be true together**: none survived. The refuter independently recomputed WCAG 2 relative-luminance contrast from the cited RGB/hex values — `rgb(217,43,58)` vs white → 4.82:1, `#25D366` vs white → 1.98:1, `#22c55e` vs white → 2.28:1 — and all three matched this document's stated figures exactly. This session re-ran the same three computations independently (not trusting the refuter's arithmetic — scar W65, a refuter can hallucinate too) and got identical results. The refuter also checked the twelve-PR count against the table (12 rows, matches), the checklist "widens, never narrows" claim (8/8/9/10, monotone), and the #4831 heading-row table for self-consistency (the apparent non-monotonicity between 1024px and 1180px is explained by the document's own stated 1120px layout-cap breakpoint, not a contradiction).
- **"Measured" claims actually inherited from a prior report**: none found. Each claim in the PR table states its own provenance (painted-pixel, rendered DOM, a rendered production PDF, or an explicitly-disclosed live-DOM CSS injection for #4831) rather than repeating an earlier document's number.
- **A cure proved "live" by a marker the old build could also produce** — this capture's own central lesson (§3): the refuter checked every claim outside §3 against this trap and found none that fail it. The checklist route-awareness claim (#4817) discriminates because a non-route-aware build could not produce four *different* per-route counts; the #4823 PDF claim reads the rendered artifact itself, not a proxy for it; the #4831 "after" values are disclosed as CSS-injection measurements rather than dressed up as a full production deploy.
- **Client PII in cleartext**: none. No client names, phone numbers, passport numbers, or identifiable case detail anywhere in the document.
- **Price figures**: none split. The only base-E33 figure is "Your all-inclusive figure — 35.000.000 IDR" (§2) — never decomposed into PNBP + service fee. The `$130K` proof-of-funds and `Rp 5 billion` property-value figures in §4 are quoted as *defects in the external article corpus* to be corrected, not asserted as Bali Zero's own pricing.

**One defect survived and was fixed**: the intro's blanket claim that "every number below is a painted-pixel or DOM measurement" was false as literally written — `#4825`'s figures come from watcher-log timestamps and `#4824`'s "1219 files" is a filesystem sweep count, neither a painted-pixel nor a DOM measurement, and `#4818`'s row carries no number at all. The substantive point the sentence was making (nothing here is a lane's self-report) was true and is preserved; only the over-narrow "painted-pixel or DOM" wording was corrected to cover the actual provenance types present (painted-pixel, DOM, filesystem, watcher-log).
