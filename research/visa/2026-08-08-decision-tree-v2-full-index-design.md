...
- Conclusive-rate — CORRECTED (house refuter P1 ACCEPTED, item 16): this doc previously stated the measure as a single rule ("proof = replay + synthetic, not organic volume") that contradicted the real-interviews-only definition below. Two explicit layers:
  ...
  (b) **Business observational gate (G-a-vol)**, longer horizon: Conclusive-rate = (SUPPORTED + NO_SUPPORTED_PATH) / valid REAL completed interviews, NO_SUPPORTED_PATH counted only at full coverage, Wilson 95% lower bound > 60%, synthetic excluded and labeled. Honest timeline: at ~0.8 sessions/day real traffic (§1), a volume gate on the order of 1000 sessions/7d (as calculated in `research/visa/2026-07-23-d1-decision-pack.md`) is unreachable — ~3.4 years at current volume. This is why the ≥100 real/14d proposal stays an explicit OPEN Zero decision (§6 item 2), not a silent assumption.
