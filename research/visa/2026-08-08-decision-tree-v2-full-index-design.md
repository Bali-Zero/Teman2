...
- **Real traffic**: ~0.8 sessions/day whole funnel (D1 pack). CORRECTED (house refuter P1 ACCEPTED): this doc previously stated "proof of conclusive-rate must be replay + synthetic, not organic volume" as the sole measure, which contradicted §4's real-interviews-only metric — §4 now splits this into two explicit layers (engineering acceptance vs the business observational G-a-vol gate) so they no longer read as one contradictory rule.
...
4. B1 nationality gate: HARD_FILTER `hf.b1.not-voa-nationality` (EXCLUDE, `on_unknown=HUMAN_REVIEW`, safety_critical) + ELIGIBILITY in-list, gated on the passport actually traveled with — the interview asks "which passport will you travel with?" and the nationality gate evaluates THAT document's country. CORRECTED (Codex P0-2 ACCEPTED): the contract only knows the `person.nationalities` set, not the passport used — with `intersects`, an eligible nationality could wrongly authorize B1 even when the presented document isn't eligible, and vice versa.

   **ESCALATED to a hard REQUIREMENT of pack seq-4 (house refuter P1 ACCEPTED, item 17 — merges with the P0-2 finding above)**: the SET+`intersects` pattern proposed for B1 clones the A1 pattern ALREADY IN PROD, which the refuter proved bypassable TODAY — a traveller holding {IT, MY} bypasses the A1 exclusion. Applied to the proposed B1 gate, {IR, US} would reach `SUPPORTED_CANDIDATES`/`B1_VOA_ELIGIBLE` with NO review at all (Iran is neither VOA-eligible nor on the calling-visa list, but the eligible US nationality clears the set-based check). A `review.citizenship-conflict` rule exists ONLY in the gold test-pack today, not in prod, and the metamorphic property "nationality-set growth never defeats exclusion" has NO test backing it. Pack seq-4 MUST therefore ship all three: (i) a PROD `citizenship-conflict` rule — `len(person.nationalities) > 1` with divergent outcomes across VOA/BVK/calling-visa → `REQUIRE_REVIEW`, fail-closed; (ii) the corresponding metamorphic test in the gold suite as a seq-4 acceptance criterion (adding a nationality to a set can never improve the outcome); (iii) the travel-passport interview question. If the traveller holds multiple nationalities with divergent VOA/BVK/calling-visa outcomes and the travel document is not established → HUMAN_REVIEW (fail-closed). `person.travel_document_country_code` (+ type + expiry) remains the Phase C contract addition that removes this interim approximation.
5. Sweep the remaining 30 purpose-only PRODUCTS reviews: each gains its product's real applicability discriminators (purpose + entry-pattern, per items 1-3 above) so it stops firing out-of-target — proven at the case level in the Acceptance proof below. CORRECTED (Codex P0-1 ACCEPTED): the RulePack contract has NO "non-blocking checklist" effect — Phase A does NOT "demote to checklist" as this doc previously said. Document-requirement copy lives in the UI's outcome copy, not in rules. A genuine CHECKLIST effect in the contract is Phase C work, panel-gated. NOTE (house refuter P2 ACCEPTED, item 18): this sweep is NOT on the critical path of the IT/TOURISM/10-day verification payload — it serves the other purpose categories, not the target case exercised in the Acceptance proof below.
...
- Extended metamorphic properties: UNKNOWN never adds candidates; nationality-set growth never defeats a document-country restriction; SINGLE→MULTIPLE flips; threshold n-1/n/n+1; re-sign identity invariance. FLAGGED (house refuter P1 ACCEPTED, item 17): "nationality-set growth never defeats a document-country restriction" is currently UNTESTED and, per §3 Phase A item 4, the proven A1 bypass ({IT,MY}) shows it is currently FALSE in prod — the new citizenship-conflict rule + its gold metamorphic test are what make this property true, and are now a seq-4 acceptance requirement, not a design aspiration.
- Conclusive-rate — CORRECTED (house refuter P1 ACCEPTED, item 16): this doc previously stated the measure as a single rule ("proof = replay + synthetic, not organic volume") that contradicted the real-interviews-only definition below. Two explicit layers:
  (a) **Engineering acceptance** (immediate gate for shipping the pack seq-4, this session): deterministic replay of the target payload (see Acceptance proof, §3) + the labeled synthetic corpus above. This is what "done" means for Phase A.
  (b) **Business validation gate — team heavy-testing sign-off** (RULED, Zero, 2026-08-08 — supersedes the volume-gate proposal below): no automated observational threshold gates ENFORCE — no 1000 sessions/7d, no ≥100 real/14d, no Wilson 95% lower bound. Instead, the Bali Zero team runs heavy manual testing against the engine in SHADOW mode and verifies the outcomes before ENFORCE is authorized. SHADOW remains the testing substrate for this sign-off — it is where the team's manual test sessions run, not a volume-accumulation phase. Superseded context (kept for the record, no longer operative): at ~0.8 sessions/day real traffic (§1), a volume gate on the order of 1000 sessions/7d (as calculated in `research/visa/2026-07-23-d1-decision-pack.md`) was unreachable — ~3.4 years at current volume — which is what made the ≥100 real/14d fallback proposal moot rather than a live option; that fallback is retired along with the volume-gate approach, not adopted.
...
2. RULED (Zero, 2026-08-08): no volume gate — team heavy-testing + verification validates before ENFORCE; SHADOW is the testing substrate. (Superseded: the standing 1000/7d gate was unreachable at real traffic — ~0.8 sessions/day → ~3.4 years, `research/visa/2026-07-23-d1-decision-pack.md` — and the ≥100 real/14d fallback proposal is retired along with it, not adopted.)
...
| 16 | Measure self-contradiction: §1 "proof = replay+synthetic" vs §4 "real-interviews-only" metric | house refuter (P1) | ACCEPTED | §4 split into two explicit layers: engineering acceptance (replay+synthetic, immediate) vs business observational G-a-vol gate (real interviews only), with honest ~3.4yr timeline at current volume; ≥100/14d carried forward as an open Zero decision |
| 17 | SET+`intersects` B1 gate clones the A1 pattern already bypassed in prod ({IT,MY}); {IR,US} would clear B1 with zero review | house refuter (P1, empirical — merges with Codex C.11/P0-2) | ACCEPTED | Phase A item 4 escalated to a hard seq-4 requirement: PROD citizenship-conflict rule + gold metamorphic test + travel-passport interview question, all three now mandatory |
| 18 | Item 5 (30-rule purpose-only sweep) is on the critical path of the IT/TOURISM verification payload | house refuter (P2) | ACCEPTED — clarified NOT critical path | Item 5 annotated: serves the other purpose categories, not the target case in the Acceptance proof |
| 19 | Phase A items 1-4 alone suffice for the IT/TOURISM/10gg payload (traced empirically across all 110 rules); VOA fee + BVK-19 still current via web | house refuter | UNREFUTED | Retained; cross-checked against the Codex C.9 evaluator run |

House refuter Sonnet delivered its verdict 2026-08-08 05:49 — all 4 findings (16-19) accepted and applied above. Push/PR owned by the orchestrator, not this writer pass.
