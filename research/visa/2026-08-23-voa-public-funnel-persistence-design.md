...
3. **Automatic retention: 90 days**, enforced by a purge job, not a promise in a document. Bali
   Zero already has a working precedent for exactly this shape of decision — the Visa Oracle
   engine's own retention system (`db/migrations_v2/264_visa_decision_retention_policy.sql`,
   `266_visa_retention_evidence.sql`, `268_visa_retention_binding_security_definer.sql`): a
   `visa_decision_retention_policies` table that is **fail-closed until Zero records an explicit
   duration, anchor, effective period, approver, and approval reference** — new inserts are
   rejected until a policy row exists, purge is exposed as a bounded database primitive with no
   invented cadence, and legal-hold transitions are tracked per-decision. The GARUDA rebuild should
   reuse this pattern rather than hand-roll a cron script that silently no-ops if misconfigured
   (cicatrix family #2 — exists ≠ armed).
...
Given the ruled design's 90-day retention (§4.3) and the schema-verified `created_at` column
(§2), the following disposition is proposed for Zero's decision once the count/range above is
known — this document does not execute it:

- **If the measured `max(created_at)` is already >90 days old**: the entire legacy set falls
  outside any retention window this design would apply going forward. Proposed: fold it into the
  same retention policy as new rows (§4.3, the `visa_decision_retention_policies` pattern) rather
  than special-casing it — collapse to the coarse aggregate (§4.6), then purge the rows.
- **If some legacy rows are <90 days old**: those age out under the same policy on their own
  schedule; no special action needed for them beyond enrolling the table under the policy.
