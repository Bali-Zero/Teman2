...
- arXiv:2512.08296 "Towards a Science of Scaling Agent Systems" (Kim, Gu, Park et al., Google, Dec 2025) **is a real paper** (verified abstract). Its abstract-level claim: _"architectures without centralized verification tend to propagate errors more than those with centralized coordination"_ — ⚠️ **not a load-bearing finding** RETRACTED[kim-2025-17x-error-amplification-as-cause]: §4.3 narrates that mechanism, but **Table 4 does not support it** (β̂=0.014, CI [−0.047, 0.074], p=0.658), and the paper attributes the architecture gap to efficiency and overhead "rather than error propagation per se"; it evaluates 5 topologies (Single, Independent, Centralized, Decentralized, Hybrid) and finds coordination yields diminishing returns and mismatched coordination degrades performance (relative range +80.8% to −70.0%).
