...
| `knowledge` rows `type='case_not_resolved'` last 24h | **82** (all `agent='nlm_feeder'`, **0** lhkpn_harvester) | NLM ingestion failures, not gap-dispatch failures. lhkpn_harvester produces 0 rows in any post-dispatch type; whether it is never invoked, or invoked but failing pre-reflection (`process_gap` `error` path raises before any KB write), is the Layer 4 open question |
...
**Layer 4 (downstream — open question, NOT diagnosed by this PR)**: 🔴 `lhkpn_harvester` last reflection 2026-05-08 12:33:13 — 0 in 7 days. The first hypothesis was "dispatcher works but harvester fails to resolve cases", but the empirical evidence does NOT support it: `SELECT agent, COUNT(*) FROM knowledge WHERE type='case_not_resolved' AND created_at > datetime('now','-24 hours') GROUP BY agent` returns `nlm_feeder|82` and **zero rows for `lhkpn_harvester`**. The agent produces no rows in any post-dispatch type; two compatible hypotheses remain (a) it is never invoked, or (b) it is invoked but the lamarckian loop raises before any KB write (`process_gap` returns `status='error'` per `gap_consumer.py:149-156`, which logs but does not store). Reflection itself is gated by `lamarckian.py:204,231,252`, not by `gap_consumer.py:158-167` — `_run_post_reflection` fires on `case_resolved=True`, on retry exhaustion after `case_not_resolved`, and on implicit resolution (no explicit case status), so absence of `case_not_resolved` rows is also absence of reflection by every path. Root cause unknown; the dispatch chain `nexus:gaps → coerce → process_gap → run_with_lamarckian_feedback` needs trace-level investigation in a separate ticket.
...
18. `grep -n "_run_post_reflection" apps/mata-garuda/mata_garuda/runtime/lamarckian.py` → definition at line 76; calls at line 204 (resolved), 231 (retry exhausted after not_resolved), 252 (implicit resolution / no explicit case status) — gating is on retry-exhaust, not strictly on case_resolved=True
