...
| 4   | `kg-linker`                | **(a) Pro-only** | Writes to **Fly Postgres KG** (108k nodes, 242k edges canonical KG, NOT Pro-local — corrected per NB-1 2026-05-12 review). Pro-only because Mini has no replica of source articles it links FROM | Fly Postgres KG writes — DUPLICATE today (idempotent on entity_id? VERIFY) |
| 5   | `wr-topic`                 | **(a) Pro-only** | WR2 topic selector uses Fly Postgres + Canva OAuth (Pro-side credentials)                                                                                                                        | Canva API call — DUPLICATE today                                           |
| 6   | `wr2-bridge`               | **(a) Pro-only** | WR2 bridge talks to **Fly-deployed backend RAG** (corrected per NB-1 2026-05-12 review — NOT Pro-local backend). Pro-only because the bridge holds Pro-side Canva OAuth state                    | Fly backend HTTP calls — DUPLICATE today                                   |
...
**All 14 → Pro-only**. Rationale **CORRECTED after NB-1 review 2026-05-12 04:15 WITA**: the original rationale ("Pro-local DB/file writes") was partially wrong — `kg-linker` writes to Fly Postgres (canonical 108k-node KG) and `wr2-bridge` talks to Fly backend RAG. The verdict direction (all Pro-only) remains correct, but for a **different consolidated reason**:

> Per NB-1 (Mac Mini Scenario A): "Mini-Pro2 prende SSH ops via Tailscale, ha replica codice, NON data. Qualunque LaunchAgent che debba interagire con lo storico OSINT (3407 righe SQLite knowledge.db) o le 2088 memorie MOS DEVE essere Pro-only. Il Mini non ha i dati fisici."

So the canonical rationale is:

1. **Mini has no data replica** — no `knowledge.db`, no MOS memories, no `~/.cell-observatory/observatory.db`. Any agent reading Pro-local state must be Pro-only.
2. **Pro-side credentials** — Canva OAuth tokens, NotebookLM CLI auth, sentinel_ping state files all live on Pro `~/.nuzantara-secrets.env`.
3. **External API duplication waste** — Canva/Brevo/Telegram/NotebookLM API calls cost rate-limit quota; double-firing wastes quota and may violate ToS.
