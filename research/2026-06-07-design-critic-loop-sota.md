...
- **Human-in-the-loop terminal gate.** Consistent with Bali Zero's Legge-5 (no auto-publish): the loop converges to a *proposed* slide + a one-line rationale; Zero's "anteprima → dici cosa cambiare → re-render 2s" is the final stop. The loop's job is to make sure what's presented already passes every objective gate.
