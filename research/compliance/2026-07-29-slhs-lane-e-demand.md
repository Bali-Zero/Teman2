...
> al 2026-05-25". Verificato: **è ferma solo la tabella `whatsapp_message_context`** (la replica del
> mirror WhatsApp verso Fly è morta da ~65 giorni, difetto reale e registrato a parte). Il resto del
> DB prod è vivo e corrente. Sulla base di quella frase avevo declassato per errore i numeri di
> un'altra lane; la correzione è stata fatta. I conteggi di domanda in questo file restano validi —
> vengono dalla fonte viva, come il file stesso dichiara. Lezione: **una superficie stale non rende
> stale il contenitore che la ospita** — nominare la tabella, non il database.
