...
no W60-style flapping). The only genuine *repo* hygiene defect found is a **4519-line
...
1. **`check_health_detailed` reported `status: critical`** with `search`, `ai`,
   `router` = *unavailable*. A single-probe read would have escalated **P0 "RAG is
   down"**. Second probe (`check_health`) returned **`healthy` + embeddings
   operational** with the explicit note *"RAG handled by rag process group"*. The
...
- **PR #972** → `https://github.com/Balizero1987/Teman2/pull/972` (the real, only PR
  for this branch). **Anti-hallucination near-miss**: #970 is a *separate sibling PR*
...
immediately** to a dedicated branch. Root mistake: I correctly isolated the *git
commit* (in a `/tmp` worktree) but wrongly wrote the *docs* to the contested checkout
...
trusted a poll latched onto the wrong number. I caught it by checking each PR's *head
branch* (the discriminator), reopened #972, and re-enabled auto-merge. The fix itself
...
rewritten by a sibling). That hesitation was correct *for the main checkout* but wrong
as a *final decision*: once a fresh worktree off `origin/main` stayed pristine through
