...
- **Blast-radius requirepass 29/6**: client locali non autenticati censiti: `system_doctor.py`, `seed_cell_skills_manual.py`, `sentinel_lib/zombie_hunter.py`, `apps/cell/cell/main.py`+`config.py`, oltre ai 2 curati. Il flusso `organism:events` (XADD dal pg-bridge) è stato **muto dal 29/6** — chi consuma lo stream (Supervisor) ha visto il nulla senza saperlo. Sweep completo dei 5 restanti = follow-up.
