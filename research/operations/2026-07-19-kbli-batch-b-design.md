...
status: DRAFT REV-4 (amended REV-4b) — SIGNED. Post 4-LLM panel (Codex GPT-5.6-sol xhigh
  red-team REJECT with 10 blockers + 14 majors; Gemini 3.1 Pro constructive with 10
  recommendations), a Codex re-check on REV-2 (B1/B4/B9 CURED; B2/B3/B5/B6/B10 PARTIAL; B7/B8
  NOT-CURED), REV-3 closing those 7, a third Codex round on REV-3 (B2/B6/B8 CURED; B3/B5/B7/B10
  still NOT-CURED — determinism gaps), REV-4 freezing those four, then a fourth Codex round on
  REV-4 (B7 CURED; B3/B5/B10 still NOT-CURED — each a real but one-line-closable residual). Per
  the conductor's exit-criterion ruling this was the LAST Codex loop: the conductor closed
  B3/B5/B10 by dictated verbatim amendment (REV-4b) rather than a fifth Codex round. Signed by
  the conducting session 2026-07-19 — see §Sign-off. Phase-0 build and Zero's Legge-5
  ratifications (AQL default, Tier-4 volume) remain open gates before any lot dispatches.
sources:
...
> Conductor note: this document was authored by a dispatched lane (worktree
> `.worktrees/docs-batch-b-design`, branch `agent/air-m5/docs/batch-b-design`), NOT by the
> conductor session itself. It went through four adversarial Codex rounds + one Gemini
> constructive pass (REV-1 REJECT → REV-2 → REV-3 → REV-4 → REV-4b conductor amendment), was
> **signed by the conducting session on 2026-07-19** (see `## Sign-off` below), and is now the
> standard this program pre-registers against. Signing closes the *determinism* gate only —
...
2. **Edge-level precision and recall, frozen numbers (closes B5 residual — REV-2 left this
   ...
   - **Page selection is pre-registered, not hand-picked** — and, closing the REV-3→REV-4
     ...
     2. Rank each page by `SHA256(parser_run_digest + ":" + lampiran_id + ":" +
        zero_padded_page_number)`, hex string, ascending. **`zero_padded_page_number` (REV-4b
        conductor amendment) = the printed page number zero-padded to 4 digits (`0017`).**
     3. **Deterministic greedy fill**: walk the rank-ascending list and take the lowest-ranked
        ...
        **Overlap rule (REV-4b conductor amendment):** a page already taken into the sample
        counts toward EVERY stratum it satisfies — a page that is both wrapped-row and N:M
        advances both stratum counts; the greedy walk never takes the same page twice.
...
10. **Tier-4 AQL parameters — REQUIRED OUTPUT of this gate, not left floating (closes the B3
    ...
    **AQL class = the smallest standard ISO 2859-1 AQL value that is ≥ the Phase-0-measured
    holdout edge error rate** (§1.4 item 2). **Definition, frozen (REV-4b conductor amendment):
    holdout edge error rate := max(1 − precision, 1 − recall), both measured on the holdout
    half** — the worse of the two, so the AQL class is always selected conservatively; never an
    average, never a hand-chosen one of the two — never hand-picked. **Lot size = the stratum's
...
3. **Advancement gate for `inheritance_verdict`, stated as a numbered rule (closes the B10
   ...
   **The table is evaluated as an ORDERED decision procedure — precedence frozen (REV-4b
   conductor amendment, closing the round-4 overlap):** (1) if ANY ancestor lacks both evidence
   classes → `not-adjudicated`, stop. (2) else if the verified evidence shows the regime does
   NOT transfer to the 2025 code (e.g. the successor is OSS-native with its own independent
   regime) → `not-inherited`, stop. (3) else if all verified ancestors point to the SAME regime
   → `inherited-uniform`. (4) else → `inherited-divergent`. Exactly one rule fires; earlier
   rules take precedence — "does it transfer" is adjudicated BEFORE uniformity-vs-divergence,
   which is what makes the four preconditions mutually exclusive. No fifth value, no majority
   rule, no verdict on a subset of ancestors.
...
Signed REV-4b by the conducting session (Fable, f5892d39) 2026-07-19 after 4 adversarial rounds
(Codex GPT-5.6-sol xhigh ×4, Gemini 3.1 Pro ×1): 10/10 original blockers dispositioned, 11
residuals across rounds 2-3 closed in text, round-4's B3/B5/B10 closed by conductor amendment
(this revision), B7 CURED by Codex. Standard met: pre-registration determinism. NOT YET
ARMED — Phase-0 build and Zero's Legge-5 ratifications (AQL default, Tier-4 volume) remain open
gates before any lot dispatches.
