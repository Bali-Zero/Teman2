...
Every fact in this section was measured, not reported: `origin/main` = `03998cf90`, `gh pr view`, and a `/bin/ls` of `apps/backend-rag/backend/db/migrations_v2/` run in one session. Re-measure before trusting it — a board is a snapshot, and this one decays the same way the migration ledger below did.

| Lane | Packet | State | Evidence |
|---|---|---|---|
| `H1` | P04 canonical contracts | **active** | PR [#4586](https://github.com/Bali-Zero/Teman2/pull/4586) opened 2026-08-23T01:26Z, +2940/43 files, `mergeStateStatus=BLOCKED`, unarmed. One red: `CodeQL` — two high-severity `py/redos` in `packages/research-os-core/research_os/primitives.py:21-22`, plus one `note`. The red is the CODE, not a stale merge ref: do not rerun it (rule 3). Conductor gate note posted on the PR. Rule 1 exceeded ~7×; split or justify before gating. |
| `H1` | P04 migration ledger | **decided** | PR [#4584](https://github.com/Bali-Zero/Teman2/pull/4584), ledger revision request 001, `awaiting_conductor_decision` → decided. Blocked on the `R1 gate — adversarial review present` check. |
| `H3` | P03 WR3/FlowKit | not started | Slot free once H1's second slot is claimed; prompt corrected 2026-08-23 (its deliverable 3 ordered a `launchctl` action the §H3 ceiling forbids). |
| `H2` | P01 NEXUS Tasks 1-6 | not started | Prompt corrected 2026-08-23 — the draft ordered bind/plist/credential mutations that §H2's hard boundary forbids in this dispatch. Requires a recoverable immutable snapshot of the dirty external checkout before any edit. |

### Migration-ledger decision 001 (Conductor, 2026-08-23) — binding on seven packets

The freeze reserved `270`–`276` against a migration head of `269`. **All seven are occupied**, by unrelated work merged in the eight days after the freeze; `277` and `278` are occupied too; head is `278`. Independently re-measured at `origin/main` = `03998cf90`, with zero open PRs touching `migrations_v2/`. H1 found this, stopped instead of renumbering, and filed the request — the correct move.
