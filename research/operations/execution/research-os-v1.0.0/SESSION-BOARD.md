...
| `H1` | P04 D2 (migration), D3 (adapters + shadow dual-write + parity probe), D4 (independent contract PASS) | **absent** *(superseded 2026-08-24 — see row below)* | `migrations_v2/` head is `278`; no file matching `research_os` or `contract` exists there; no adapter/dual-write/parity file exists under `packages/research-os-core/`. Cohort B (P05 Intel Lake, P06 NAGA) remains blocked by construction until D4's independent PASS is handed to the Conductor. *(Superseded 2026-08-24 — this row is the Conductor's own 2026-08-23 measurement and is left verbatim; the corrected state is in the row below and in `evidence/p04/contract-pass-001.md` §8, landed via PR #4783.)* |
| `H1` | P04 D2 (migration), D3 (adapters + shadow dual-write + parity probe), D4 (independent contract PASS) — **re-measured 2026-08-24, supersedes the row above** | **D2 landed · D3 partial · D4 landed (`PASS_WITH_LIMITS`)** | Re-measured 2026-08-24 against `origin/main` `35bb9d091` (this row's own base, re-fetched at measurement time) by the lane that authored `evidence/p04/contract-pass-001.md` (PR #4783). **D2 (migration) landed**: head moved `278` → **`280`**, both Research OS files present — `279_research_os_contract_core.sql` and `280_research_os_objects_truncate_guard.sql`. **D3 (adapters) partial**: 3 adapters exist under `apps/backend-rag/backend/services/research_os/` (`action_item_adapter.py`, `action_intent_adapter.py`, `operational_receipt_adapter.py`) — **not** under `packages/research-os-core/`, the subtree the row above searched, which is why it read `absent`; the **shadow dual-write flag and parity probe are still absent** — zero `shadow.py`/`parity` files anywhere in either tree, and the adapter code itself says so (`action_item_adapter.py:373`, `__init__.py:10` cite an unbuilt `shadow.py` as the flag's intended home), tracked as `contract-pass-001.md` §9 condition 2 (D8 phased dual-write/read plan, owner Conductor-assigned lane covering Packets 05–15). **D4 (independent contract PASS) landed**: `evidence/p04/contract-pass-001.md`, verdict `PASS_WITH_LIMITS`. Per its §7, Cohort B (P05 Intel Lake, P06 NAGA) **may** build against the 25 typed models/schemas and fixtures, the closed `ApprovalReceipt`/`OperationalReceipt` matrices, and may treat `research_os_objects` as schema-ready in the strict sense (migration applies/rolls back cleanly against a throwaway database, but is applied in **no** environment — not even staging); Cohort B **may not** rely on deterministic cross-implementation hashing (D7, NOT DELIVERED), a contract registry (D6), the phased dual-write/read plan (D8 second half, the same gap named above), any atomic multi-object repository primitive (D10), or an atomic classification-change primitive (D11) — read §7 in full before assuming a wholesale unblock. |
