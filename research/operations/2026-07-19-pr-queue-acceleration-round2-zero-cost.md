...
| "Alembic snapshot restore" (shared-prompt wording, Appendix P Q1e — not a claim any seat's own response used the term) | **Wrong premise**: the workflow runs `SQLModel.metadata.create_all()` + the PROPRIETARY SQL-v2 runner (`backend.db.migrate apply-all`), not Alembic. Any snapshot key must cover migrations bytes + models feeding create_all + bootstrap + runner code + PG digest/extensions; AND `migration_manager.py:286` has warn/skip paths that must become fail-closed BEFORE any cache trusts it. Only worth it if a probe shows bootstrap+migrations p90 ≥ ~45s. (Sol) |
...
- Fable uniquely: live anatomy grounding (tests.yml single-process/-v/serial mypy, folded into Appendix P's Facts section rather than restated in Appendix C's body) that seeded the round; draft-PR lever proposed AND killed by the panel — recorded as the round's example of why the conductor's ideas face the same refutation bar (W100-family, second instance this day after the R1 misquote catch).


## Adversarial review

**Codex** (`codex exec -m gpt-5.6-terra --sandbox read-only`, cross-family: reviewer ≠ synthesis author Fable/Claude), read-only pass over this file. GLM 5.2 was the first-choice reviewer per protocol — the 1-token probe (`PONG`) came back clean, but the full-file review call hung and was killed after 2 minutes, then again after 5 minutes; abandoned and fell back to Codex GPT-5.6-terra per the standing cascade. Checked: (a) misquotes/misrepresentation of the three seats' verdicts against Appendices A/B/C; (b) internal contradictions across §0-§5; (c) invariant I1-I6 plus the round's new $0-recurring constraint against the adopted plan (§2, §4). 5 findings returned; 4 were real (misquote/precision-class) and applied below, 1 is a re-check that found the text already self-consistent.

1. **WRONG (fixed)** — the "Alembic snapshot restore" table row's "(all three, wording)" tag could be misread as claiming all three seats' own responses used the term "Alembic" — only the shared prompt (Appendix P, Q1e) uses that wording; Fable's own lever list never says "Alembic". Reworded to point at the shared-prompt origin explicitly; the falsification verdict itself is unchanged.
2. **WRONG (fixed)** — the Day-1 item 6 macOS-lint lever said "frees the 5-slot macOS pool", readable as freeing pool CAPACITY when only 2 jobs (per Appendix P's Facts section) actually run on macos-latest today — moving those 2 doesn't expand the 5-slot org-wide cap, it empties this repo's occupancy of it. Reworded for precision; the underlying lever and its benefit are unchanged.
3. **WRONG (fixed)** — the Days 2-7 xdist paragraph described the per-worker DB isolation build (2 known bugs fixed, DSN/DDL census, exact-multiset parity check, 300-run soak) without stating the I4 guilt+innocence requirement on the isolation guard itself — a real invariant-coverage gap, not just wording. Added an explicit clause: the guard ships with guilt (fails the required context on cross-worker DSN/DDL collision) and innocence (passes clean on correct isolation) tests before any soak run counts toward gating.
4. **WRONG (fixed)** — the meta-record credited Fable's unique contribution ("tests.yml single-process/-v/serial mypy") without noting those specific facts live in Appendix P's Facts section (authored by Fable as prompt-writer), not literally restated inside Appendix C's body — a real traceability gap for a reader spot-checking against C alone. Clarified the sourcing; the attribution itself (Fable performed the live read) is correct and unchanged.
5. **OK-BUT-SHARPEN (no fix)** — the meta-record's "Kimi uniquely: ARM-GA/public-4vCPU web verification" was flagged as potentially crediting Kimi with a fact Sol also independently reached. Re-checked against both appendices: Sol had NO network access (Appendix P's Sol-specific note) and reasoned from repo/mechanism, reaching the SAME conclusion via a DIFFERENT method — already credited separately one line above as "Sol uniquely: the x64-already-4vCPU falsification". Kimi's flagged-unique contribution is specifically the WEB-VERIFICATION act (only Kimi had live search), and the line already self-qualifies it as "incomplete without Sol's x64 fact — the two seats' halves only make the whole together". Text already internally consistent; no change applied.

Verified-clean by the reviewer (not re-listed): the other falsification rows (draft-PR types gap, push-count mechanism, `-v` triple-surface, fsync gating, in-job-supervisor arbitration, dedupe-kill 3/3), the §0 seat-independence framing, and no I5/I6/$0-recurring violations found anywhere in the adopted plan.
