...
| company | 9 | Permen_5_2025 | **B** | Full scroll: phrase found, attributed to `Permen_5_2025` (point `0027f82d-…`). **CORRECTED 2026-08-26.** This row previously called `0027f82d` "the same physical point that also carries journey 7's 'Berbasis Risiko' boilerplate". That contradicted this document's own company-J7 row two lines above, which reports exactly **one** normalized match for J7's phrase and locates it at a **different** point, `004ac9d4-…` — two distinct IDs cannot be one physical point, and if the claim held, J7's scan should have returned 2 matches, not 1. The contradiction is provable from this file alone and was verified here. **Which side is wrong was settled by production measurement on the `kb-r1-corrections-0826` lane, not re-run in this session** (no Qdrant read path was available to it): that lane retrieved both full point IDs (`004ac9d4-262b-503e-939c-8f3d6f7d27e0`, `0027f82d-347e-5e83-8371-ee48564649d3`) and cross-checked each against both phrases using `kb.ops.probe_retrieval.normalize` (imported, not reimplemented), finding two distinct points each carrying only its own journey's phrase — so the J9 claim was the wrong one, and J7's "1 exact normalized match" stands. Treat that resolution as **PLAUSIBLE, single-lane measured**; the contradiction it removes is CONFIRMED. Either way this is real, present regulation text, just not ranking for the cross-topic B×C paraphrase. | Ranking (cross-topic compound question competing against single-topic phrasing). | No |
...
- **Class B (19 rows)** — 19, not the 18 an earlier draft of this section said: company-J7 was
  reclassified E->B on 2026-08-26 and the correction reached the aggregate table below but not
  this paragraph (caught 2026-08-26 by cross-family review). It includes the ones that most
  looked like cross-lingual gaps (company J1/J3/J5/J6/J7/J8/J11, tax J9, property J1/J2/J9 — client questions in EN/IT/ES against Indonesian
...
- **Class D (1 row)** and **Class E (1 row)** were never expansion-dependent to begin with (a
  metadata defect and one malformed journey, respectively -- E is 1 after the company-J7
  correction above, matching the aggregate table) — consistent with the measurement.
...
(a ranking-competitor-size fact, not a status value). Tax-J6 cites `LANE-A-1` only for
`Permen_1_2026`'s point-count/contamination scale — same, no status causation. (Corrected
2026-08-26 by cross-family review: an earlier wording said "Tax-J6/J10". The count of three is
right; tax-J10's row carries no `LANE-A-1` string at all — verified by counting the table rows
that contain it, which returns exactly three: immigration-J4, company-J5, tax-J6.) Immigration is also
...
context with the repository in front of it. The seat was instructed to hunt seven specific failure
modes — unreconciled arithmetic, a rate generalised from a partial population, phantom `file:line`
citations, correlation dressed as causation, TRUE-BUT-PARTIAL findings, conclusions stronger than
their evidence, and internal contradictions — and to verify every citation with a command rather
than by reading.

**Verdict: SURVIVES_WITH_LIMITS.** Three findings, all confirmed by an independent re-measurement
before being accepted (a refuter can hallucinate too), all corrected in place above.

| # | severity | finding | disposition |
|---|---|---|---|
| C1 | MINOR | Internal contradiction: the degraded-path section said **Class B (18 rows)** and **Class E (2 rows)**, while the aggregate table says **B = 19, E = 1**. Both slices sum to 24, so no total was wrong — it is an editing leftover: the same-day company-J7 `E -> B` reclassification reached the table and not the prose. The tell the seat spotted: that paragraph's list of cross-lingual B rows omitted company-J7, the single clearest cross-lingual case after the correction. | **Corrected.** Prose now reads B = 19 / E = 1, with company-J7 added to the list and the reason for the earlier number recorded rather than silently overwritten. |
| C2 | MINOR | Overstatement: "the one live exclusion filter that IS wired into every `SearchService.search()` call **regardless of `apply_filters`**". An internal caller passing `apply_filters=False` DOES drop the repealed exclusion. | **Corrected.** Re-read `search_service.py` directly: on that path the filter is deliberately rebuilt as `exclude_repealed=False, exclude_historical=True` for this collection, so what survives is the *historical* guard, not the repeal one. The load-bearing claim — the repeal filter tests `status_vigensi`, a different field from `legal_status` — is untouched by this. |
| C3 | MINOR | "three matches total" is followed by "Tax-J6/J10 cite `LANE-A-1`", which names four rows for a count of three; tax-J10's row contains no such string. | **Corrected.** Verified arithmetically rather than by eye: counting the table rows containing `LANE-A-1` returns exactly **3** — immigration-J4, company-J5, tax-J6 — and `0` for the tax-J10 row. The count was right; the attribution was not. |
| C4 | **SUBSTANTIVE** | Self-contradiction between two rows of the same table: company-J9 called point `0027f82d-…` "the same physical point that also carries journey 7's 'Berbasis Risiko' boilerplate", while company-J7 two rows above reports exactly **one** normalized match for J7's phrase, at the **different** point `004ac9d4-…`. If J9 held, J7's scan would have returned 2. | **Corrected, and the two halves carry different confidence on purpose.** The contradiction itself is CONFIRMED here — it is provable from this file alone, no store access needed, since two distinct point IDs cannot be one physical point. Which side is *wrong* was settled on the `kb-r1-corrections-0826` lane by retrieving both full IDs from production and cross-checking each against both phrases with `kb.ops.probe_retrieval.normalize`; that measurement was **not re-run in this session** (no Qdrant read path available) and is carried as PLAUSIBLE with its provenance named, rather than restated as this session's own. Raised on a lane whose branch the squash orphaned; rescued here because the defect was live on `main`. |

**Limits the seat named, recorded rather than argued away.** Several load-bearing facts here rest
on production-Qdrant scrolls and on a teammate's verbal report, neither of which is reproducible
from the repository alone: the 84,283-point totals, the per-row point ids, the isolated-overlay
re-run, the winner/loser `402/402` vs `340/340` measurement, and the 9-documents/7-contradicted
ground-truth comparison. The seat verified everything the repo *could* answer — per-topic red
counts against the recorded `probe_state`s, `41 = 11+11+10+9`, the class counts `3+19+0+1+1 = 24`,
the B write-split `17+1+1 = 19`, the `legal_status` percentages, the `must_not_retrieve` canary
locations, and the `Permen_1_2026` census summing to 1,506 — and all of it reconciled. What it
could not reach is a verifiability limit of this document, not an error in it, and it is stated
here so a future reader does not mistake "checked" for "checkable".
