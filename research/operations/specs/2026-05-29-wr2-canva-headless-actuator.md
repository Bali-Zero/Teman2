...
| A2  | **Scope MCP ristretto → RE-SCOPED 2026-05-29** | 3/3 (panel) ma design empiricamente invalido | Panel chiedeva `--mcp-config` SOLO-Canva O permission-allowlist + hash skill body. **VERIFICA EMPIRICA 2026-05-29 ha demolito il design flag-based**: `--strict-mcp-config` ESCLUDE Canva (account-hosted, non file-dichiarabile → "CANVA GONE"); `--disallowedTools Bash` IGNORATO sotto `--dangerously-skip-permissions` ("BASH-PRESENT"). L'isolamento via flag NON è ottenibile. **A2 RE-SCOPED (operator, 2026-05-29)**: (1) sanitizzazione del TESTO slide nel `pending_builder` (unica superficie iniettabile — skill body è fisso/hashato); (2) hash sha256 baseline dello skill body come tripwire; (3) rischio residuo documentato nella scar. Razionale: l'attuatore AppleScript ATTUALE gira già con gli stessi built-in → headless NON è regressione. |
