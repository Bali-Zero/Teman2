...
| ✅ **`main_write_via_bash` (W79 — FATTO, testato, NON installato)** | blocca scrittura sul main via **Bash** (`echo >`, `>>`, `tee`, `sed -i`, `cp/mv`, `dd of=` con target sotto `~/Desktop/nuzantara` ESCLUSI i `.worktrees/`) | **buco esistente OGGI** (§9-B): chiuso estendendo `worktree_isolation.py` (commit `da4d3a5e2`), test `infra/claude-hooks/test_w79_shell_write.py` 19/19 PASS, git-regex 20/20 (no regressione). **Resta da INSTALLARE** in `~/.claude/hooks/` sulle 3 macchine (decisione operatore — blast radius globale) |
