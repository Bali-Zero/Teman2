...
- Deep research (4-LLM panel review) = **centralized** parallelizable — ~~+80.9% gain~~ **RITIRATA 2026-08-02** RETRACTED[kim-2025-ranking-supports-the-no-peer-rule]: il +80.8% è **un solo abbinamento task-architettura**, non un guadagno generale del centralized. Verbatim (§1): _"Performance spans +80.8% relative improvement (structured financial reasoning under centralized coordination) to −70.0% degradation (sequential planning under independent coordination), demonstrating that architecture-task alignment, not number of agents, determines collaborative success."_ Sull'aggregato Table 5 `Centralized` 0.463 sta **sotto** il singolo agente 0.466. Non ripristinare la percentuale come guadagno del centralized — e nota che era anche trascritta male (**+80.9%**). RETRACTED[kim-2025-17x-error-amplification-as-cause]
