...
Nuzantara already runs a production-grade Baileys capture bridge (`apps/wa-mirror`, Node.js 22 + `@whiskeysockets/baileys`, deployed on Mini-Pro2 H24) that persists every inbound and outbound team-account WhatsApp message into PostgreSQL `whatsapp_message_context` and emits `whatsapp_message_received` on EventBus. The bridge is currently capture-only — there is no UI to visualize 14k+ accumulated rows, no flow visualization, no human-intervention path. The current research panel evaluated twelve open-source candidates (Evolution API, WAHA, MultiWA, WA-AKG, Chatwoot, Tercela, WPPConnect server+frontend+manager, Evolution Manager v2, Zete dashboard, mohit2777 multi-accounts dashboard, OpenWA) plus three UI pattern libraries (React Flow, chatscope chat-ui-kit, Syncfusion/Assistant-ui) to determine the highest-leverage integration path.

Three LLMs converged on a single architectural verdict: **do not fork any candidate wholesale**. The wa-mirror bridge already owns the Baileys socket layer and the FastAPI monolith already owns CRM/RBAC/Qdrant; introducing a Node.js/NestJS or Ruby/Rails dashboard backend would fragment business logic across two stacks. Instead, build a greenfield dashboard inside the existing Next.js frontend (extend `apps/admin-dashboard` or open a sibling `apps/wa-dashboard`) and lift only the MIT-licensed UI patterns from WA-AKG (chat composer + multi-session switcher) and Evolution Manager v2 (Radix UI dashboard chrome). The conversation rendering layer should be `@chatscope/chat-ui-kit-react`, the automation/handoff flow visualization should be React Flow, the realtime transport should be a hybrid of Postgres LISTEN/NOTIFY (wa-mirror → backend) and Server-Sent Events (backend → browser). WebSocket is acceptable but introduces stateful complexity that the workload (8 accounts, ~5 msg/s peak, 18 concurrent viewers max) does not require.

The two non-negotiable risks are: (a) Baileys breakage cycle of approximately 8-12 weeks against WhatsApp protocol updates, and (b) anti-ban detection on outbound bursts when a human intervenes via the dashboard. Both are mitigatable but must be encoded into the spec before any code is written: Baileys gets pinned to a known-working release with a 10-week retest cadence, and outbound sends go through a per-account rate limiter (10-30s jittered cooldown, identical to WA-AKG's empirical pattern) rather than direct passthrough.
...
DeepSeek throughput math: 8 accounts × peak 50 msg/5min = 480 msg/5min = 1.6 msg/sec sustained, 2.7 msg/sec peak burst. Fanout to 18 concurrent viewers (10 admin + 8 team).
...
| Postgres LISTEN/NOTIFY   | ≤8KB payload, asyncpg >5000 notif/s ceiling | 2.7 notif/s × 18 = 48.6/s      | <10ms intra-process | Internal only (DB → backend), not browser-visible |
...
1. **wa-mirror → backend (intra-machine)**: Postgres LISTEN/NOTIFY. wa-mirror already INSERTs into `whatsapp_message_context`; add a trigger `notify_whatsapp_message_received` that `pg_notify('wa_message_inserted', json_build_object('message_id', NEW.id, 'team_member_phone', NEW.team_member_phone))`. Backend FastAPI background task LISTENs and emits to subscribed SSE clients. Avoids adding Redis Streams to the path. Reuses the same outbox-via-NOTIFY pattern already shipped in the EventBus phase-1+phase-2 fix (migration 144/146).
...
| ------------------ | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Framework          | Next.js 16 + React 19 (existing `apps/mouth` or new `apps/wa-dashboard`) | Match existing stack; App Router for streaming routes; React 19 Suspense for SSE-driven timeline                                                                              |
| App location       | **New `apps/wa-dashboard/`** (sibling to wa-mirror, not nested inside)   | DeepSeek R2: ~30% worktree-contamination risk on a 2-3 week build inside an existing app per cicatrix 2026-04-29 scar. New app + dedicated feature branch reduces collateral. |
...
| i18n               | next-intl OR custom; English-only for internal tooling                   | Per CLAUDE.md §9 owner uses Italian colloquial but team artifacts in English                                                                                                  |

### 5.2 Backend integration

| Layer              | Choice                                                                                                            | Rationale                                                                                                                                                                          |
| ------------------ | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTP layer         | Extend `apps/backend-rag/backend/app/routers/wa_dashboard.py`                                                     | New router namespace; follow existing 271-router pattern; register in `router_registration.py` AND add include_router for both `api`/`both` groups per the PR #423/424 scar lesson |
...
| Outbound queue     | Redis BullMQ-equivalent or asyncio queue + rate limiter                                                           | Cap outbound at 1 msg per (account × 10-30s) jittered; WA-AKG empirical anti-ban pattern                                                                                           |
| wa-mirror dispatch | Add `outbound_command` row insert in PG + LISTEN on wa-mirror side                                                | Avoids adding HTTP server to wa-mirror; bridge already has `pg.ts`                                                                                                                 |
...
What needs adding on top:

```sql
-- migration NNN_wa_dashboard_threads.sql (sketch — not authoritative)
CREATE TABLE wa_dashboard_threads (
  ...
  UNIQUE (counterpart_phone, COALESCE(group_jid,''))
);
...
(Authoritative schema TBD in spec phase; this is illustrative of pattern.)

### 5.4 Local-only deployment posture

The webapp runs on Pro M4 48GB only (per question wording). This means:

- No Fly.io deploy needed; bind `localhost` only.
- No SSL termination needed (or use mkcert for dev).
- Auth can be a simplified version of `nz_access_token` against a hard-coded users list, OR full SSO via the existing `kita.balizero.com` cookie if the dev workflow goes through there.
- Postgres is already local (or via fly-pg-proxy on `localhost:15432` per cicatrix-scars). asyncpg LISTEN/NOTIFY works against both.
- wa-mirror runs on Mini-Pro2 (per README); dashboard on Pro. Two options: (a) dashboard reads PG via Tailscale (~62ms DERP USA latency — acceptable for non-realtime), OR (b) Pro becomes a PG read-replica and writes go through the API. Option (a) is the MVP path.
...
9. **Mini-Pro2 ↔ Pro network latency**. Tailscale DERP USA latency 62ms (per MEMORY). For the SSE read path served from Pro reading PG from Mini, every NOTIFY → query roundtrip incurs 62ms × 2 = 124ms. Not critical for chat UX (target <500ms perceived). Open question: do we colocate the dashboard backend on Mini-Pro2 instead of Pro? Trade-off: Pro has 48GB and the developer's IDE; Mini-Pro2 has 24GB and runs wa-mirror. Probably colocate the backend SSE service on Mini-Pro2 with the FastAPI app, while the Next.js dev server runs on Pro for hot-reload.

10. **DeepSeek output truncation in this very research**. The DeepSeek call exhibited token-budget tightness on first two attempts (forced to drop `reasoning_effort=high` to free output tokens). Pattern noted for future deep-researcher panels: explicitly set `reasoning_effort=low` when output volume is the primary need, not chain-of-thought depth.
...
| Top-1 candidate to lift  | WA-AKG (chat UI + multi-session)            | WA-AKG (closest stack alignment)         | Chatwoot (highest MHI 2.04, but stack-overlap-aware Tercela=8) | **2/3 WA-AKG, DeepSeek raw-numerical favors Chatwoot**         |
| Dashboard app location   | New `apps/wa-dashboard`                     | Inside existing `apps/mouth` Next.js     | New `apps/wa-dashboard` (R2 worktree risk)                     | **2/3 new app, Gemini prefers reuse mouth**                    |
...
- [ ] Confirm dashboard app location: `apps/wa-dashboard/` new app vs extending `apps/admin-dashboard/` (Next.js Pro-only LLM cost dashboard, not deployed) vs `apps/mouth/`. Recommend: extend `apps/admin-dashboard/` (already Pro-only, already Next.js, "Not deployed anywhere" per its `apps/admin-dashboard-local/` README).
- [ ] Confirm v2 send-capability scope addition to wa-mirror — get explicit sign-off from Antonello.
- [ ] Define schema migrations: `wa_dashboard_threads`, `wa_dashboard_outbound_queue`, `wa_dashboard_assignments`, `wa_dashboard_tags`. Lint via Squawk (per pre-deploy hook).
- [ ] Define RBAC matrix: which user roles see which threads, who can claim, who can escalate, who can reassign.
- [ ] Define rate-limit policy: hard-coded 15s default, env override, surfaced as UI countdown.
...
- [ ] Decide colocation: dashboard backend on Pro vs Mini-Pro2.
- [ ] Plan Baileys retest cadence: every 10 weeks calendar entry.
- [ ] Add `lint-launchagents` and `pre-push` hooks already in cicatrix repertoire to the new app's CI.
...
- `apps/wa-mirror/README.md` (capture bridge scope, v1 capture-only constraint)
...
- `.claude/rules/cicatrix-scars.md` (2026-04-29 worktree contamination, 2026-04-29 EventBus PG NOTIFY pattern)
- `MEMORY.md` index entries on Mini-Pro2 24/7 server topology
