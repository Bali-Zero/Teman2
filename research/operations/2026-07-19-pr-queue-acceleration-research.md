...
| Self-hosted runners on the Macs | only ephemeral/JIT VM, no secrets, synthetic fixtures, post-merge/non-required first | GO pilot week 2 — reasons no fork-PR surface exists because current PR authors are the owner's own agents (conflates trusted-current-authors with the repo's actual public exposure) | **NO — repo is PUBLIC** | **REJECTED for required PR checks while the repo is public** (fork-PR arbitrary code on PII-holding production Macs; GitHub's own guidance). Revisit ONLY if repo goes private; then Sol's ephemeral-VM conditions are the bar. Kimi's conflation recorded (see §5). |
...
| TTL >7d idle | yes, receipts, non-security only, time decides hygiene never validity | yes, dry-run first, exclusions | not addressed (proposal D is a visibility/receptor lever, not auto-close) | **ADOPT week 2** with Sol's exclusions — 2/3 (Sol+Kimi); Fable's D is a distinct, complementary visibility lever, not a third TTL vote. |
...
- Convergences 3/3: state-machine diagnosis · steward-first · reports→artifacts · derived-docs-out · MQ-defer · cancel-in-progress · grouping · census-first. (TTL-with-receipts is 2/3 — Sol+Kimi; Fable's proposal D is a distinct visibility receptor, not an auto-close vote — corrected post-R1, see Adversarial review.)
- Arbitrated divergences: R1 waiver (Sol's reject upheld — substance over mechanics); self-hosted (rejected — Kimi's GO reasoned no fork-PR surface exists because current PR authors are the owner's own agents, conflating trusted-current-authors with the repo's actual exposure; the conductor caught it from grounded memory: the repo is PUBLIC, so the surface exists regardless of who currently opens PRs. W100 lesson live: cross-family seats + grounded conductor catch what any single seat misses); admission broker (Sol's design adopted only as contingency — anti-sperpero).
...
**GLM 5.2** (`claude-glm --model glm-5.2`, cross-family: reviewer ≠ synthesis author Fable/Claude), read-only pass over this file: spot-checked the panel verdict matrix (§2) against all three appendices for misquote/misrepresentation, scanned §0-§5 for internal contradictions, and checked the adopted arbitrations against invariants I1-I6 (Appendix P). 7 findings returned; 2 were real (misquote-class) and applied below, 1 is a spec-sharpening note already covered by an existing §4 ratification item, 4 are NONE-FOUND confirmations.

1. **WRONG — self-hosted-runners row mischaracterized Kimi's premise** (§2 row, Kimi column + §5 arbitrated-divergences line). The synthesis said Kimi's GO rested "on the false premise the repo is private" — but Kimi's appendix (Appendix B, §E4 threat-model) never says "private"; its actual (still flawed) reasoning is "PR authors are the owner's own agent sessions, not the public — no fork-PR attack surface," i.e. conflating the CURRENT trusted-author roster with the repo's actual public exposure (a public repo accepts forks from anyone, regardless of who happens to open PRs today). APPLIED: reworded both locations to state Kimi's actual reasoning and name the real conflation, rather than a premise Kimi never asserted. The arbitration outcome (rejected while public) is unchanged — only the characterization of Kimi's reasoning was wrong.
2. **WRONG — TTL >7d idle row/§5 credited Fable with a "yes" and counted it inside "Convergences 3/3."** Fable's appendix (Appendix C) never mentions a TTL/auto-close policy anywhere in its proposals A-E or sequencing; its closest item, proposal D ("Ownership receptor"), is a visibility/dashboard mechanism ("stragglers visible at day 2, not day 6"), not an auto-close policy. Re-checked directly against Appendix C: only Sol (Appendix A) and Kimi (Appendix B) actually endorse TTL — 2/3, not 3/3. APPLIED: Fable's column now reads "not addressed" with the D-is-distinct note; the §5 convergence list no longer claims TTL as 3/3. The arbitration outcome (adopt week 2) is unchanged — it stands on 2/3, which is still a legitimate basis.
3. **OK-BUT-SHARPEN — the R1-waiver arbitration's "byte-compares" step, taken alone, doesn't stop a compromised bot from appending an out-of-scope change to an otherwise-correct regen diff; "checks diff scope" is the load-bearing adversarial substance and should be named as such with its own I4 guilt+innocence corpus.** Not applied as a text edit — this is a specification gap for the bot-lane R1 design, already inside the scope of the existing §4 #2 item (bot-lane R1 policy ratification), not a misquote or contradiction in the capture itself.
4. **NONE-FOUND** on the other 6 spot-checked table rows (R1-waiver Sol summary, merge-queue row, derived-docs row, bot-steward row, workflow-consolidation row, census/dashboard row) — each verified verbatim-faithful to its appendix.
5. **NONE-FOUND** for direct internal contradictions — throughput math (§0→§1), burst counts, straggler anatomy, and the §4 Solo-operatore list are all self-consistent and consistent with Appendix P.
6. **NONE-FOUND** for I1-I6 violations — the R1-waiver and derived-docs-out arbitrations both explicitly preserve I1 (real different-family grader, deterministic validator kept as an ADDITIONAL check, never the grader) and I5 (waived-PR variant, not direct push); fast-gates consolidation is add-then-remove with identical coverage; P3′ is content-keyed not time-keyed (I6); the plan's own kill-condition (§3) covers any I1-I6 breach.

Verified-clean by the reviewer (not re-listed): merge-queue row math (Sol's ~5,000/week vs Kimi's ≥715/day), the §0 measured-ground figures, and the §Solo-operatore list.
