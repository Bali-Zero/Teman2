...
| **Provider-selection / fallback semantics (NEW ROW, added post-refutation)** | `llm_gateway.py` already has a config-flag-gated second-provider egress path: on Gemini failure, `_call_openrouter` (`OpenRouterClient`, imported `:65`) fires if `settings.openrouter_enabled` (`config.py:130`, default `False`). OpenRouter is itself a multi-vendor router — it can already serve OpenAI models today, with zero new code | The most likely way an OpenAI adapter gets wired in practice is by **imitating this existing fallback block** rather than the "shadow-only, explicit flag" design this document assumes — a Gemini quota exhaustion (already happened 4× per the corner) flips traffic to a fallback with nobody making a discrete "arm OpenAI" decision. This document's invariant table had NO row for "which provider answered, and who authorized that routing decision" before this correction | A test asserting the response schema/log always names WHICH provider answered (§Gate V2 G10 already requires schema identity; add: the provider-identity field must be present and asserted in an integration test that actually exercises the OpenRouter/fallback branch, not just the primary-path happy case) |
...
| 2 | Responses payload lacks `store:false` and exposes `previous_response_id` (provider-side state) | **CONFIRMED — and worse than "lacks a flag"** | `openai_responses_client.py::generate()` (`:294-304`) builds the payload with `model`/`input`/`max_output_tokens`, and conditionally `instructions`/`tools`/`previous_response_id` — **no `store` key is ever set**, at any code path. Per the retention research in Deliverable 2 (§2, this turn): the Responses API's default behavior is `store`-driven Application State retention — omitting the key does not mean "not stored," it means "whatever OpenAI's default is," which the fetched docs describe as retained. This is a NEW, sharper finding than my original draft's §(a)/(b) tables assumed (they discussed retention as a policy question; this shows the CLIENT CODE doesn't even set the one flag that would minimize it). `previous_response_id` is accepted as a parameter (`:273`, `:303-304`) — unused by `_shadow_provider.py` today (confirmed: `_run_shadow` at `:146-149` calls `generate(input_text=message, system_prompt=system_prompt)` only, no `previous_response_id`), but the capability exists in the client for a future caller to wire up without re-deriving the retention consequence. **New Gate V2 pattern, G11 below.** |
| 3 | `DEFAULT_MODEL=gpt-5.1` placeholder instead of a benchmarked model | **CONFIRMED, self-disclosed** | `openai_responses_client.py:92` — `DEFAULT_MODEL = "gpt-5.1"`, comment says "Deliberately a placeholder slug, not a verified live one … no model has been benchmarked or approved for this path yet." Additional risk the orchestrator's phrasing didn't quite name: **nobody has verified `"gpt-5.1"` is even a real, currently-available OpenAI API model slug** — a wrong/retired slug fails at request time with a 4xx (not retried, per the client's own retry policy), which is a cheap failure mode for a SHADOW branch (swallowed, logged, nothing user-facing breaks) but would silently mean "the shadow has been logging nothing but 400s" for however long nobody reads the log. Recommend a pre-flight model-existence probe be part of arming, not assumed. |
| 4 | "Available" docs claim live env but the key is cached at construction | **CONFIRMED — the docstring is factually wrong about the code's own behavior** | `OpenAIResponsesClient.__init__` (`:217-234`) sets `self.api_key = api_key if api_key is not None else os.getenv(_ENV_VAR)` **once, at construction**. The `available` property (`:236-242`) docstring says: *"True only when an API key is configured. **Read live (not cached at construction)** so a key provisioned after import still takes effect on the next call"* — but the property body is `return bool(self.api_key)`, reading the INSTANCE ATTRIBUTE set at `__init__`, never re-reading `os.getenv` at all. This is the exact "claim sounds authoritative but is not verifiable/true" pattern I was briefed to hunt for (and Kimi was too — see below). Concretely: `_shadow_provider._get_client()` builds a **module-level singleton** on first use and never rebuilds it (`:58-63`), so if `OPENAI_SHADOW` is armed before `OPENAI_API_KEY` is set (or the key is provisioned/rotated after the process already constructed the singleton), `available` stays `False` (or holds a stale key) until the process restarts — silently, with the misleading docstring telling the next reader this can't happen. |
