...
sources:
  ...
  - "PR #4216 adapter source head 1dcdd670d: Codex subscription adapter, offline harness, tests, CI, and ADR"
...
The selected adapter evidence is frozen at PR #4216 source head `1dcdd670d`. Live-path evidence was
...
- [x] Mandatory final Fable 5 on-disk gate: SHIP on `1dcdd670d`, with no CRITICAL, HIGH, or MEDIUM
      finding; six non-blocking LOW follow-ups recorded in PR #4216.
- [x] #4194 threat/privacy addendum reconciled to source head `1dcdd670d`; its independent Kimi K3
      FIX-FIRST wording findings were corrected.
...
addopts-free pytest process collected and passed **487 tests**: 73 subscription-adapter tests, 166
...
GitHub's backend job independently found that the dormant paid client lacked the mandatory cost
ledger hook. The source was thawed and fixed rather than whitelisted: every handled HTTP-attempt
outcome now emits a fail-open event containing only local model/token/status metadata, and the
repository's `check_llm_cost_tracking.py` passes. Cancellation while a request is still in flight
and zero-token accounting for malformed billed responses remain declared non-blocking LOWs.

After Kimi K3 found that the adapter docstring could be read as runtime authorization, the text was
narrowed to the intended future credential-path choice plus the current human-run offline evidence
boundary. The focused 71-test adapter suite passed at that intermediate freeze; the final head's
expanded 73-test adapter suite is included in the 487-case run above.
...
and returned **SHIP with `degraded_execution: true`**. Fable 5 independently reviewed final source
head `1dcdd670d` and returned **SHIP**, with no CRITICAL, HIGH, or MEDIUM finding. No reviewer
authorized live traffic, credentials, config, gateway or worker wiring, merge, deploy, or cutover.
