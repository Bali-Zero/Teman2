...
**Webhook WA — validazione firma**: `app/routers/whatsapp_chat.py:1159` — `_verify_whatsapp_signature(body, signature_header)`, HMAC-SHA256 su `X-Hub-Signature-256`, usa `hmac.compare_digest` (L1189). Il ramo fail-open **esiste** nel codice (docstring: *"True if signature is valid OR verification is disabled (no app secret configured)"*) ed è deliberato per il dev locale. **CORREZIONE 2026-08-25 (lane WA-bot, Mini): in PRODUZIONE quel ramo NON è quello attivo, e questa riga affermava il contrario.** Misurato dal vivo lo stesso giorno: `WHATSAPP_APP_SECRET` risulta `Deployed` su `nuzantara-rag`, un POST senza `X-Hub-Signature-256` risponde **401** con body `{"detail":"Invalid signature"}` (il 401 generico dell'app dice invece `Authentication required`, quindi il codice di stato da solo non distingue), e webhook reali sono atterrati alle 10:17:55Z / 10:18:32Z / 10:19:40Z sotto la release corrente v4244 (09:53:55Z) — la persistenza avviene solo DOPO la verifica, quindi il canale è verificato e non sordo. **Perché l'errore è istruttivo e non un dettaglio**: questa riga è stata dedotta LEGGENDO il codice e la memoria del 23/8, senza mai sondare l'endpoint vivo; una affermazione sulla POSTURA di produzione non è verificabile dal sorgente, perché dipende da una variabile d'ambiente che il sorgente non conosce. Residuo reale: `META_WEBHOOK_REQUIRE_SIGNATURE` è ancora `False`, quindi un unset del segreto riporterebbe in silenzio al fail-open.
