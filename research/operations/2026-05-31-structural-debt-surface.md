...
- **6 of 10 structural scars are still a live bomb** (4 `still_armed` + 2 `worsened`).
- **4 of 10 are de-facto resolved** — and three of those resolved _silently_ (the fix
  shipped or the mechanism evaporated, but the scar was never marked RESOLVED): EventBus
  doc drift, migrations 129/130, deploy-worktree desync.
- **0 scars are "exploded-silent"** (a bomb you think is fine but is actively broken).
  The deploy-worktree case _looked_ like it could be exploded-silent (empty hollow
  `~/Desktop/nuzantara-deploy`), but the wrapper has a `.git`-missing guard → graceful
  `exit 0`. Not exploded.
...
| #2  | evolver vs deploy-puller shared worktree desync     | **DE-FACTO RESOLVED**       | `~/Desktop/nuzantara-deploy` no longer a git worktree (hollow empty dir, no `.git`, absent from `worktree list`); wrapper now has `.git`-missing guard → `exit 0`; deploy-puller last-exit=0. Collision mechanism eliminated.                                                       |
...
## 2. The "silently-resolved" category (the inverse of exploded-silent)

There are **0 exploded-silent** bombs (the dangerous "you think it's fine but it's
broken" class). But there are **3 silently-RESOLVED** scars — the fix landed (or the
structure evaporated) and the scar was never updated, so anyone reading the cicatrix
file would still treat them as open:
...
3. **#2 deploy-worktree desync** — the shared worktree was deleted entirely and the
   wrapper got a `.git`-missing guard; the A/B/C decision the scar is "pending" on is
   now moot.
...
2. **Rotate the 2 exposed secrets** (`WA_DASHBOARD_DATABASE_URL`,
   ...
   unknown window.
3. **Branch graveyard** — delete zombie `claude/*` (>30d) and stale (>90d): REPORT-ONLY
   by policy.
4. **mata_garuda CI guard** — add `test_genome_no_active_active.py` (optional; bomb
   already disarmed, only the guard is missing).
5. **Demote the other 7 superuser roles** — separate spec.

---

## 6. Top 3 armed bombs by blast radius
...
2. **plist-overwrite + world-readable secrets (#10)** — _blast: world-readable Postgres
...
3. **W62 worktree orphans** — _blast: uncommitted code WIP in 4 stale worktrees lost on
