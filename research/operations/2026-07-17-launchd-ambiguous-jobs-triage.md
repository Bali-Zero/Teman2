...
| 3 | `com.matagaruda.kg-query-api` | 1 | **REAL-CRASH (severe, active)** | `~/logs/mata-garuda-kg-api.err` (116MB): `OSError: [Errno 49] Can't assign requested address` at `apps/mata-garuda/mata_garuda/api/kg_query.py:235` (`socketserver.TCPServer.server_bind`). Root cause: `infra/launchagents/com.matagaruda.kg-query-api.plist` sets `KG_API_BIND=100.93.236.6` — **Mini's** Tailscale IP, being bound on **Pro**. `KeepAlive=true` + `ThrottleInterval=15` respawns it every ~15s; **73,334** occurrences of "Errno 49" counted in the error log (~13 days of continuous crash-loop). Confirmed the bind value is baked into the repo-tracked plist (not live-drift — diffed against `.bak-tcc-20260716`, whose only delta is the `~/Desktop` path fix). | **Escalate P1** to whoever owns Mata Garuda: fix `KG_API_BIND` to Pro's own bind address/interface (e.g. `0.0.0.0` or Pro's own Tailscale IP), then reload. Out of Lane-D scope (different app, not named in this mandate's deliverables). |
| 4 | `com.balizero.wr2.image-generator` | 1 | **INCONCLUSIVE (leans HONEST)** | 526KB `.err.log` (stdout is unused — Python `logging` defaults to stderr): zero tracebacks anywhere; only graceful-degradation WARNINGs (`VLM scoring failed: timed out — accepting image`). Repeated `Done: 0/1 drafts imaged` across many hourly ticks on 07-15, then `Done: 2/2 drafts imaged` + `No drafts in 'drafts' status to process` (looks clean) at 05:39-05:40 on 07-16 — the LAST visible log activity, ~20h before this triage, looks like a clean completion, yet `last_exit=1` is what launchd currently reports. No plist `StartInterval`/`StartCalendarInterval`/`RunAtLoad`/`KeepAlive` key exists at all (event-triggered externally, not launchd-scheduled), so I cannot correlate the recorded exit to a specific dated invocation with certainty. | Capture the next live invocation's exit code together with its log tail to close this out definitively — static log analysis alone is insufficient here (documented, not guessed). |
...
| 8 | `com.balizero.wr2.dossier-compiler` | 2 | **REAL-CRASH** | `~/.openclaw/workspace/logs/war-room-v2/dossier-compiler.error.log`: `asyncpg.exceptions.ConnectionDoesNotExistError: connection was closed in the middle of operation`, plus a recurring `compiler: CLI failed for anchor=<uuid> err=None` pattern spanning 2026-06-30 through TODAY (2026-07-16) — the failure reason itself is being swallowed (`err=None`), compounding the bug. The `.log` file's last recorded batch: `"dossiers_compiled": 0, "dossiers_failed": 11` — 100% failure on the most recent batch. | Fix DB connection lifecycle in `backend/services/intel/dossier_compiler` (asyncpg silent-death class — same family as cicatrix W64/W34) and stop swallowing the real exception into `err=None` (separate lane). |
| 9 | `com.balizero.domain-mesh.foundations.daily` | 1 | **REAL-CRASH (intermittent, ~50%)** | Plist-declared logs are a red herring again (0 bytes since 28 giu — the wrapper `/Users/nuzantara/scripts/domain-mesh-foundations-cron.sh:8,26` redirects everything to a date-stamped `foundations-daily-YYYYMMDD.log`). Yesterday's (07-16) real log shows an unhandled `httpx.ReadError`/`httpcore.ReadError` inside `probe_portal()` (`apps/mata-garuda/mata_garuda/foundations/gov_apis_health.py:58`), crashing the entire `probe_inventory()` batch — no per-endpoint exception isolation. Recent history (8 daily files): 09/10/13/15 lug = full 6205-byte reports (success), 11/12/14/16 lug = 67-byte truncated logs (crash) — **4 of the last 8 days failed** this way. | Wrap each `probe_portal()` call in its own try/except so one flaky government endpoint doesn't crash the whole daily inventory probe (separate lane/Mata Garuda). |
| 10 | `com.nuzantara.intake-gate-count-pusher` | 1 | **REAL-CRASH (data-quality bug, 100% reproducing)** | `~/logs/intake-gate-count-pusher.log` (5.8MB): every single tick (~every 5 min, confirmed across dozens of consecutive timestamps) fails identically: `HTTP 422 ... "loc":["body","counts",0,"user_email"],"msg":"String should have at least 1 character"`. Some record in the counts payload has a blank/missing `user_email`, and the pusher never filters it before POSTing — not a crash in the traditional sense (the HTTP error is caught and logged), but a genuine, persistent defect: the push has not succeeded even once in the visible log window. | Fix the pusher to skip/backfill count entries with a missing `user_email`, or fix the upstream data producing the blank email (separate lane/CRM or backend). |
...
- **REAL-CRASH** (genuine bug, log-confirmed): **5** — #3 (severe, active crash-loop), #5 (hang), #8 (asyncpg + swallowed errors), #9 (intermittent, unhandled network exception), #10 (persistent data-quality bug).
...
continuously crash-looping server bound to the wrong host's IP — the log is large because the
process has failed to bind ~73k times, not because of missing rotation policy alone. Both fixes
are needed (rotation AND the bind-address bug).
