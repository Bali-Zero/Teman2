...
> è marcata **TO VERIFY** nella sezione finale.
...
4. Inclusa nei piani **Pro, Max, Team, Enterprise** senza costo aggiuntivo di prodotto; su Team/Enterprise un Owner deve abilitarla in Organization settings.
...
- **Sandbox con permission card.** Ogni cartella, ogni host di rete, ogni job remoto passa da una card che approvi. Le concessioni permanenti stanno in Settings > Permissions e si revocano. La rete della sandbox è *deny-by-default*.
...
- **Compute remoto.** Può connettersi via SSH a un lab workstation o a un HPC login node, oppure a Modal, oppure a S3/GCS/Azure Blob. **Sì: può toccare sistemi vivi**, se glielo concedi.
...
Tradotto nella nostra grammatica: **il contenuto di ogni file che il modello legge per rispondere esce dall'Indonesia.** "Local-first" descrive dove vive il database dell'app, non dove vive il testo. Per noi il vincolo è identico a quello di qualunque altro seat cloud: **nessuna cartella con PII cliente può essere concessa**. La permission card per cartella è lo strumento che rende questo enforceable, ed è più forte di quanto abbiamo con Claude Code.

Altri fatti utili: il traffico verso SSH host / Modal / endpoint propri **non passa da Anthropic**; su Enterprise con Compliance API attiva Anthropic conserva i transcript di sessione (default 6 anni); la memoria dell'app è locale e non sincronizzata, ma i fatti richiamati in sessione viaggiano nel prompt. Nessuna dichiarazione trovata su residenza dati per il Sud-est asiatico.
...
- risk map del backend (i 1.368 moduli) e riconciliazione catalogo/plist;
- integrità dei link interni e dei redirect.

Il vantaggio non è l'intelligenza: è che produce un **CSV + execution log riproducibile** invece di una chat. Un censimento che possiamo rigirare tra un mese e diffare.
...
3. **Output in schema fisso**, un CSV per famiglia di finding: `finding_id, family, file, line, claim, evidence_excerpt, confidence, runtime_dependent(bool), suggested_fix`. Niente prosa come deliverable primario.
...
5. **Verifica in vivo**: una sessione Claude Code prende il CSV, verifica ogni riga sul disco e sulle macchine, marca `CONFIRMED` / `REFUTED` / `NEEDS-RUNTIME`, e cura solo i confermati in un worktree via broker. Ship con gate e `Bites:` che nomina il consumer.
6. **Ledger**: il CSV originale + il CSV verificato finiscono in `research/agent-craft/` come coppia diffabile. È così che la precisione diventa misurabile.
...
**Punteggio**: per ogni run si registra `precision = CONFIRMED / (CONFIRMED + REFUTED)`, per famiglia. Baseline misurata il 2026-09-09: **~50%**, con una spaccatura netta — precisione alta sulle famiglie "corpus" (link, citazioni, coerenza), precisione bassa o nulla sulle famiglie "runtime" (dead code, orphan daemon, ipotesi di architettura).
...
**Non si innesta.** Non esiste una modalità headless o schedulata: `claude-science serve --detached` accende solo il server, e il lavoro richiede un umano che approvi le permission card nel browser. Mettere un run mensile in un LaunchAgent **oggi non è possibile**.
...
Nota di piano da chiarire prima di installare qualunque cosa: la pagina prodotto pubblicizza un Team plan gratuito **per istituzioni accademiche e nonprofit**. Bali Zero è un'agenzia commerciale: quel canale non ci riguarda, e l'accesso passa dal piano a pagamento che già abbiamo.
...
8. **Prezzo per uso commerciale oltre l'abbonamento.** Nessun listino trovato; le uniche cifre pubbliche sono i grant (fino a $30.000 in crediti per max 50 progetti selezionati, più fino a $2.000 di compute Modal, scadenza domande 15 luglio 2026) e il Team plan gratuito per accademici/nonprofit.
