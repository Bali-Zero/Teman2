...
A synthetic `PreToolUse` Bash event (`{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"ls -la"},"transcript_path":<3.0 MB transcript>,...}`), piped on stdin to every handler registered for PreToolUse in `.claude/settings.json` + `~/.claude/settings.json` whose matcher covers Bash, each timed with `time.perf_counter()` around `subprocess.run(cmd, shell=True, input=event)` — re-measure with `scripts/bench/cc_hook_latency_bench.py` (ships with the C1 lint PR, lane infra/cc-context-diet-lint-v3): **8 blocking handlers, 364 ms serial** (child_workflow 76,
...
parsing every line = 37 ms, a 64 KB tail seek = 17 ms (best of 5, same bench script, `--startup` section). On a 3 MB transcript, then, the row's premise looks wrong in its
detail: one shared reader saves ≤ 60 ms per call and **interpreter start-up is the cost**, and the
...
**Second measurement, same bench, worst-case transcript (2026-09-17 14:2x):** the largest transcript on
this machine is 114,110,555 bytes (session `eb0d5ece`, projects/-Users-balizero-Desktop-nuzantara). Against
it the same 8 handlers take **2,703 ms serial** — `orchestrate_gate.py` alone 738 ms (it counts the
session's lines from the whole file), `worktree_isolation.py` 107, `child_workflow.py context` 100; the
baseline: full read 809 ms, `json.loads` per line 1,063 ms, a 64 KB tail seek **33 ms**. So the row's
premise is right after all, at scale: hook cost grows with the transcript, and every reader that walks the
whole file pays it on every tool call. Two cures, in order: (1) every transcript-reading hook reads a
bounded TAIL (seek from the end) — 809 → 33 ms each, no coupling, one hook at a time, each with its
guilt/innocence test; (2) the dispatcher below for the start-up term.
...
`grep -rn -- --dangerously-skip-permissions scripts infra apps | grep -vE 'tests?/|test_|lint/|\.md:'` (2026-09-17): 13 invocations carry the
...
### After the third gate on the C1 lint lane (2026-09-17, evening) — lane SUSPENDED, spec required

The lint (`scripts/lint/lint_claude_headless_context_diet.py`) went red three times for three
different causes: #6708 (a workflow edit floors the PR at 3 — hot zone), #6715 (a bench quoting
artefact and a false claim derived from it), #6717 (a follow-up row cited in four shipped files
that did not exist, plus an engine blind spot nobody had declared). Builder Contract §1: a
fix-of-a-fix stops at depth 1; the surface is under-specified. The candidate stays on branch
`agent/air-m5/infra/cc-context-diet-lint-v5` (head 15359faf: selftest 18 PASS, lint clean
3763/20/0, pytest 10, three cures verified by the gate); the next attempt starts from a spec.

| id | surface | intervention | risk | status | proof required |
|---|---|---|---|---|---|
| C1d | context · CI | wire the context-diet lint into `.github/workflows/immune-enforcement.yml` (hot-zone path → Gear-3 PR with an Evidence Pack and a council journal), the step the lint's docs already point at as the moment it stops being advisory | medium — hot zone | OPEN — needs the spec below first | the step green on its own PR with `--selftest` proving the guilt path on the runner; a fixture PR with a bare invocation goes red |
| C1e | context · lint engine | the reused engine (`lint_claude_headless_limits.py`) anchors a Python argv only on a literal `"claude"` or a shell `$CLAUDE_BIN`; a Python argv whose binary is a VARIABLE (`CLAUDE_BIN`, `_CLAUDE_BIN`) is never anchored. Verified by the gate: `scripts/cron-agent-python/tdd_pipeline.py` lines 184 and 245 carry bare `-p` invocations that are neither flagged nor grandfathered, and the `claude_vision.py` `--safe-mode` cure is invisible to the lint (removing it yields 0 findings). Together with C1c (shell arg arrays) the lint's "clean" is narrower than it reads | low | OPEN — spec | the engine (or the lint's own pre-pass) anchors `<NAME>_CLAUDE_BIN`/`CLAUDE_BIN` Python argv elements and shell `"${CLAUDE_ARGS[@]}"` shapes, each with a guilt fixture; the grandfather list re-frozen and the delta named |
| C1s | spec | write `research/agent-craft/cc-meta-loop/SPEC-context-diet-lint.md` before any further attempt: (1) the anchor shapes the lint must see — literal `"claude"`, `$CLAUDE_BIN`/`${X_CLAUDE_BIN}` in shell, `CLAUDE_BIN`/`_CLAUDE_BIN` variables in a Python argv, `CLAUDE_ARGS=(...)` arrays — each with one guilt and one innocence fixture taken from a REAL repo file; (2) the arming path in two stages, named honestly in every doc: `scripts/quickcheck.sh` (advisory, pre-push `\|\| true`) now, the CI step (C1d) later; (3) which rows track the debt (the grandfather list, C1c, C1e) so no shipped file cites a row that does not exist; (4) the numbers policy: no frozen token counts in code — only the bench path (`scripts/bench/cc_headless_shape_bench.sh`) and the date/host of the last run | none | OPEN | the spec exists, refuted by a non-author seat (R1), and the v6 PR cites it |
