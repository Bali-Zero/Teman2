...
>
> Created 2026-08-31. Owner: Zero Siano.
...
seven dark tiers into 16 L-points `[M]`, which is precisely why M3 dark reads flat unless you also use
its tint.
...
home-page A/B of a PAS opener **reported** +49% and +46% paid lift at 99% confidence (a 2016 post,
`copyhackers.com/2016/06/great-home-page-copy/`, `VERIFIED-LIVE` _as a publication_) — a vendor's own
published A/B on an unrelated product, i.e. the W7.4 directional-only class: **never quote it to a
client as measured**. What carries here is the mechanism, not the number: stage-of-awareness matching,
and a first-time visa buyer is problem-aware by definition. Same product,
...
Four equations, from a published generator. Three are confirmed empirically against Radix's shipped
values; the fourth (hue drift) is a simplified illustrative model, not a confirmed match — see its own
bullet below. `n` is the normalised ramp position.
...
- **Hue drift compensation**: `H(n) = H_base + 5·(1 − n)` — a simplified illustrative model of the
  Bezold–Brücke correction, with **5° of total compensation across the whole ramp** (`n:0→1`). That
  total span is smaller than the swing Radix's own `redDark` shows across the steps it ships —
  **12.7° at step 3 → 23.0° at step 9**, a **10.3°** difference, more than double what this equation can
  produce end to end (Q13, see Corrections). The two empirical numbers stay the trustworthy measured
  fact; the equation is the illustrative shape of the correction, not a literal reproduction of Radix's
  hand-tuned hex values, and the two must not be read as confirming each other.
...
research capture's numbering — **116 gates exist; the 76 printed below are the ones that bite on these
three surfaces**, and the remaining 40 (form autofill tokens, keyboard hygiene, VA/QRIS/card screen
detail, EAA dates and per-country penalties, secondary type rules) live in the capture.
...
| 2   | Large text (**≥ 24px**, or **≥ 18.66px bold**) and UI text: **≥ 3:1** _and_ **\|Lc\| ≥ 60**. WCAG 2.2 SC 1.4.3 defines large as **18pt / 14pt bold** and CSS 1pt = 1.333px — writing it as 18px/14px demotes regular text 18–24px and bold text 14–18.66px out of gate 1's 4.5:1 tier |
...
| 8   | Dark-mode elevation step: **≥ +4.2 L** per level, and **1.2–1.6× the light step** (gate 9) — at gate 9's −3.5 L floor that is **+4.2 → +5.6 L**. The multiplier is the measured number (W2.4); the absolute step is derived from it, never chosen                                     |
| 9   | Light-mode elevation step: **≥ −3.5 L** per level — the base the dark step is derived from (gate 8)                                                                                                                                                                                   |
...
+53.1% YoY** (L09, DataReportal _Digital 2026 Indonesia_, `VERIFIED-LIVE` 2026-08-31) — the "slow Bali
connection" premise is not the binding constraint at the national median.
...
| 43  | Mobile line length **30–50 characters**, line-height **≥ 1.3**; **14–15px is the floor for secondary/caption text only** — body copy takes gate 45's **≥ 16px**                                              |
| 44  | Desktop line length **≤ 80 characters** (WCAG 1.4.8); copy set wider was **skipped 41% more often** than copy at 60–70 (L02, Baymard `baymard.com/blog/line-length-readability`, `VERIFIED-LIVE` 2026-08-31) |
| 45  | Base body size **≥ 16px**; **18px** on the transactional surfaces. This is the floor for body copy wherever it meets gate 43                                                                                 |
...
| 94  | Two prices (`IDR 750,000` / `IDR 850,000` in `en`; `Rp750.000` / `Rp850.000` in `id` — W6.4) render as a **timeline, never a tier menu**: `Today — IDR 750,000 — 30 days` → `Day ~25, if you want to stay — IDR 850,000 — 30 more days`, with the **sum stated plainly** underneath. Same type size for both (they are equally real), no default selection, no "most popular" badge |
...
| 72  | Guest checkout labelled **explicitly and equally weighted**. In Baymard's qualitative testing **60% of subjects could not locate the guest-checkout option** when it was not presented as a clearly separate, equally-weighted choice; forced account creation is **18% of abandonment** (L06, Baymard, `VERIFIED-LIVE`, `baymard.com/research/checkout-usability` · `baymard.com/blog/mobile-checkout`). _Both figures were struck on 2026-08-31 and the strike was itself retracted the same day — see `## Corrections`, F9._                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 108 | **String blocklist.** _CI-enforced_ (`lint_web_surface.py`, fails the build on a hit): `official` / `approved` / `authorised` **followed by an affiliation noun** (partner · agent · agency · consultant · reseller · distributor · representative — Q16 widened this row to match the lint's own pattern, see Corrections) · **an affiliation noun followed by `resmi`** (`agen resmi`, `mitra resmi`, `biro resmi`, …) and `resmi` followed by one · `guaranteed` · `dijamin` · `no risk` / `tanpa risiko` · `100%` **adjacent to an outcome word** (approval · guarantee · success · refund · aman · pasti · lolos …) · `#1` in prose · `first reseller`. _Review-only, deliberately **not** CI-enforced because the bare string is not decidable_: bare `official` / `approved` / `resmi` (`"Izin resmi"` states a fact about a permit) and bare `100%` (`"100% foreign ownership"` is a regulatory fact gate 110 protects). A reviewer still rejects those in the **claim** sense; CI cannot |
...
| 112 | Success states: **no confetti.** A drawn checkmark (**300–320ms**, no bounce) — clamped to gate 24's `slow` token, which nothing on any surface may exceed — the amount, a reference ID, the next step                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- **Fake / incentivised reviews and unfalsifiable rating strips.** A banned practice under the UK
  **DMCC Act** since April 2025; the CMA opened five ratings investigations on **27 March 2026**, with
  fines to **10% of global turnover**. Keep 4.9 and 693 _numerically_ — never round either toward a
  tidier or a fabricated figure. Purchase likelihood empirically peaks **4.0–4.7** and _falls_ toward
  5.0 (Q14, see Corrections), so a real 4.9 sits past that peak, not inside it — which is exactly why it
  must not be quietly bumped to a nicer-looking 5.0: the real number, even past the peak, still beats a
  fabricated one. Make the string a **live link to the Google Business Profile**, i.e. an artifact that
  breaks if the reviews vanish.
...
**Every `IDR …` example in this file is the `en` form** (comma grouping); the `Rp…` form with dots is
its `id` sibling. An amount quoted from an Indonesian regulation — W7.1's `IDR 500.000` — is a
quotation of a source, not page copy: render it through this table before it reaches a surface.
...
- **The brand red is `#C8102E`.** Verified in this repo: `skills/bali-zero-brand/tokens.json` (line 45,
  ...
  no brand-token file** — its only two occurrences **as a colour value** in this repo are hardcoded
  one-offs in `apps/backend-rag/scripts/templates/kbli_magazine.html` and `kbli_presentation.html`,
  which are not brand surfaces. **State it that way and no other way**: `#CE1126` is _not_ absent from
  this repository, it is absent from the **brand-token** files — the true and the stronger claim, and
  the one that survives a grep. All of W3 and W4.1 assume `#C8102E`. Corroboration from inside the
  repo, which is _not_ a corpus figure: `semantic.css`'s own comment records `#c8102e` on operative
  anthracite at **~2.54:1**, i.e. the codebase had already measured what gate 10 forbids.
...
- **Directional-only numbers that must never be quoted to a client as measured** — countdown-timer lift
  ...
  57%/43% above-the-fold split, video-testimonial statistics, vendor payment-conversion claims, and
  vendor copy/conversion A/B lift claims (the Copyhackers PAS figure in W2.10). The
...
**hard fail**.
W8.9 A string from the blocklist (gate 108) present in the lint's default scan scope → **hard fail**.
Narrowed from "anywhere in the built output" (Q15, see Corrections): the default scope is
`apps/mouth`'s source/UI files, **not** `.md`/`.mdx` article prose — that extension pair is opt-in only
(`--ext .mdx`), a measured decision (see the lint's own docstring: `resmi` alone appears in 317
apps/mouth articles as reported regulatory fact, never as a Bali Zero claim). A blocklist string sitting
only inside an `.md`/`.mdx` file the default scan never reads is a manual review item, not something the
current CI gate catches — if that ever needs to be a hard fail too, the fix is arming `--ext .mdx` in
CI, not this sentence. The **CI-enforced** subset of gate 108 fails the build; the **review-only**
subset (bare `official` / `approved` / `resmi` / `100%`) is a reviewer's rejection, not a build failure.
Gate 108 says which is which — do not read "it fails the build" across all of them.
...
(W6.4) → **hard fail**. Only the `parseFloat` half is greppable and lint-enforced
(`GATE-057-PARSEFLOAT`); the **single-formatter half is a manual review item** — no gate implements it.
...
sourcing, the nine unresolved contradictions with their rulings, or the **40** gates that did not make
the cut above — 116 in the capture, **76** printed in W4.
...
- **Lint** — `scripts/lint_web_surface.py`. The machine-checkable subset: gate 30 (artificial delay),
  gate 108 (**the CI-enforced phrase forms only** — gate 108 names which of its strings are
  review-only), gate 88 (asterisk on a price), the `parseFloat`/`id-ID` trap, `text-overflow: ellipsis`
  on a protected string, `clamp()` on a price or verdict, and overlay-vendor script hosts. **Not** in
  the lint, and claimed here until 2026-08-31: the **single formatter** (W8.12) — no gate implements
  it, so it is a manual review item. A gate that can be grepped belongs there, not in a reviewer's
  memory.
...
**2026-08-31 — adversarial review, Kimi K3 (cross-family).** A model from a different family than the
one that wrote this file recomputed the OKLCH conversions by hand and checked the internal consistency
of every number in it. Thirty-eight findings; the twelve that landed on this file are below, numbered
as it numbered them (`F`_n_ = its item _n_). None is a taste change — each is an arithmetic or
provenance defect that was in the file as shipped.

| Finding | What was wrong                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | What changed                                                                                                                                                                                                                                                                          |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1      | Gate 2 wrote WCAG's large-text threshold as `≥18px / ≥14px bold`. SC 1.4.3 says **18pt / 14pt bold**, and CSS 1pt = 1.333px                                                                                                                                                                                                                                                                                                                                                            | Gate 2 now reads **≥ 24px / ≥ 18.66px bold**. As shipped it demoted regular text 18–24px and bold text 14–18.66px out of gate 1's 4.5:1 tier. No other instance of the confusion is in this file (grepped)                                                                            |
| F2      | Gate 8 (`≥ +7 L`) against gate 9 (`≥ −3.5 L`) is exactly **2.0×** — outside the 1.2–1.6× envelope gate 8's own parenthetical and W2.4 both state                                                                                                                                                                                                                                                                                                                                       | Gate 8 is now **derived**: `≥ +4.2 L`, and 1.2–1.6× the light step. The multiplier is the measured number (W2.4, Radix); the absolute steps were invented, so the invented one moved                                                                                                  |
| F3      | W9 claimed **31** gates did not make the cut                                                                                                                                                                                                                                                                                                                                                                                                                                           | Recounted independently: **76** are printed in W4 (12 + 4 + 8 + 7 + 8 + 12 + 11 + 14), so **40** are not. W4's header and W9 now both carry the arithmetic                                                                                                                            |
| F4      | Gate 24 caps every duration at **320ms**; gate 112 permitted a **300–400ms** checkmark                                                                                                                                                                                                                                                                                                                                                                                                 | Gate 112 capped to **300–320ms**, explicitly clamped to gate 24's `slow` token. No exemption carved — nothing on these surfaces earns one                                                                                                                                             |
| F5      | Gate 43's `min 14–15px` against gate 45's `≥ 16px` made a 14px body simultaneously compliant and non-compliant                                                                                                                                                                                                                                                                                                                                                                         | Gate 43's floor is now **secondary/caption text only**; gate 45 governs body copy. Corroborated by gate 50, which already sets a caveat line at 14px/400                                                                                                                              |
| F7      | `M3 packs seven dark tiers into 16 L-points` carried no `[M]`, while W7.4 declares every Material 3 number unverified-by-construction                                                                                                                                                                                                                                                                                                                                                  | `[M]` added                                                                                                                                                                                                                                                                           |
| F8      | The Copyhackers `+49% / +46% at 99% confidence` lift was stated bare                                                                                                                                                                                                                                                                                                                                                                                                                   | Now attributed (a 2016 post, `VERIFIED-LIVE` **as a publication**), reworded to _reported_, and named as the W7.4 directional-only class. Added to W7.4's directional list                                                                                                            |
| F9      | Three behavioural figures with no attribution and no mark                                                                                                                                                                                                                                                                                                                                                                                                                              | Gate 44's **41%** now cites Baymard (L02, `VERIFIED-LIVE`); W4.5's **45.01 Mbps / +53.1%** now cites DataReportal (L09, `VERIFIED-LIVE`). Gate 72's **60%** was struck as unsourced and the strike was **RETRACTED the same day** — see F9-bis                                        |
| F9-bis  | _The correction to F9 was itself wrong._ Gate 72's 60% was struck on the finding that "L06's only 60% is a different Baymard measurement". L06 carries **two** distinct 60% figures, and the second is verbatim the one the gate cited: "in qualitative testing 60% of subjects struggled to even locate the guest-checkout option when it wasn't presented as a clearly separate, equally-weighted choice" — inside a paragraph the lane marked `VERIFIED-LIVE (fetched 2026-08-31)`. | Gate 72 restored with its citation. The claim never needed defending; it needed reading. Recorded rather than quietly reverted, because a correction that deletes a true statement is more expensive than the error it was aimed at, and only the record makes that visible next time |
| F33     | Cross-document: the lint's docstring still calls the brand red unresolved                                                                                                                                                                                                                                                                                                                                                                                                              | The spec side was already closed and stays closed. Sharpened only the companion fact: `#CE1126` is **not** absent from the repository, it is absent from the **brand-token** files. The lint's side is not this file's to fix                                                         |
| F34     | W9 listed "the single formatter" as lint-covered; **no gate implements it** (verified against the lint's `GATES` registry)                                                                                                                                                                                                                                                                                                                                                             | Removed from W9's list. W8.12 now states that only the `parseFloat` half is greppable (`GATE-057-PARSEFLOAT`) and the single-formatter half is a **manual review item**                                                                                                               |
| F35     | Gate 108 listed bare `resmi` / `official` / `approved` / `100%`, and W8.9 said any blocklist string fails the build — the lint fires only on phrases                                                                                                                                                                                                                                                                                                                                   | Gate 108 split into a **CI-enforced** list and a **review-only** list, each with the reason the bare string is undecidable. W8.9 now says which half fails the build                                                                                                                  |
| F38     | `en`-facing examples wrote `IDR 790.000` with a dot, against W6.4's comma                                                                                                                                                                                                                                                                                                                                                                                                              | Gates 87, 88 and 94 and the W5.2 MDR example corrected to `IDR 790,000` / `IDR 850,000`. W6.4 now states that every `IDR …` example here is the `en` form, and that an amount quoted from a regulation is a quotation, not page copy                                                  |

Findings 10–32, 36 and 37 land on `scripts/lint_web_surface.py`, not on this file.

**Recomputed by the same review and confirmed sound**: W2.2's hue distances, `C(#141218)`,
`κ(#0B1020)`, white on `#C8102E` = 5.88:1, €44 ≈ IDR 790.000, `parseFloat("790.000") = 790`, and the
CrUX percentages. **Found while applying the above and deliberately _not_ fixed here**: findings 1, 2
and 4 are the same errors in the research capture's own gate table (`SYNTHESIS.md` §3, gates 2, 8, 9,
24, 112) — this file is now correct and the cited authority is not, so reconcile the capture before
anyone reads the two side by side; and `#141218` in W2.2 and gate 10 is a Material 3 hex carried
without `[M]`, which the rule behind finding 7 would mark. The review did not name it; it was left
alone rather than changed unasked.

**2026-08-31 — second cross-family review, Qwen 3.8 Max.** A second model, from yet another family,
reproduced every finding against a fixture before anything below was changed (guilty fixtures fired
after the fix and were clean before it; innocent fixtures the reverse). Eighteen findings; the four
that land on THIS file's text are below, keyed `Q`_n_ = its item _n_ so they cannot be confused with
the prior round's `F`_n_ rows or with a gate number. None is a taste change.

| Finding | What was wrong                                                                                                                                                                                                                                                                 | What changed                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q13     | W3's hue-drift formula `H(n) = H_base + 5·(1 − n)` has a total span of exactly 5° end to end; the cited Radix `redDark` values (12.7° at step 3 → 23.0° at step 9) differ by **10.3°**, more than double what the formula can produce, so they cannot be read as confirming it | The bullet no longer claims the formula is confirmed by those values. It is stated as a simplified illustrative model of the Bezold–Brücke correction; the two empirical numbers stay the trustworthy measured fact, the equation stays the illustrative shape, and the two are marked as not confirming each other. W3's intro sentence now says three of the four equations are empirically confirmed, not four |
| Q14     | W5.2 said "keep 4.9 … they sit in the empirically optimal band (purchase likelihood peaks 4.0–4.7 and falls toward 5.0)" — 4.9 is outside 4.0–4.7 by the sentence's own numbers, and on the falling side                                                                       | Reworded: 4.9 sits **past** the peak, not inside it, and that is exactly why the real number must not be quietly bumped toward a nicer-looking 5.0 — the anti-fabrication rule (never round toward a tidier or fabricated figure) is the actual load-bearing claim, and it survives the correction                                                                                                                |
| Q15     | W8.9 said a gate-108 blocklist string "present anywhere in the built output" is a hard fail; the lint's default scope excludes `.md`/`.mdx` (opt-in only, `--ext .mdx`), so an article-prose blocklist string is not actually caught by the current CI gate                    | Narrowed to "present in the lint's default scan scope", with the `.md`/`.mdx` opt-in explained inline and the fix path named (arm `--ext .mdx` in CI) if that coverage is ever wanted. Does not touch the lint's own scope decision, which stays deliberately opt-in for the measured reason its docstring already gives                                                                                          |
| Q16     | Gate 108's row named the CI-enforced affiliation nouns as partner/agent/reseller/distributor/representative; the lint's own pattern is wider (`agency`, `consultant` too), so the doctrine was stricter-sounding than what actually ships                                      | The row's named list now includes `agency` and `consultant`, matching the lint verbatim — the lint's wider coverage is correct (an "official consultant"/"official agency" claim is exactly the same affiliation risk as "official partner"), so the doctrine text moved, not the lint                                                                                                                            |

Findings 1–12, 17 and 18 land on `scripts/lint_web_surface.py`, not on this file — each ships with a
guilty and an innocent fixture in `scripts/tests/test_lint_web_surface.py`'s
`TestQwenReviewRegressions` class. Finding 4 (`official {role}` / `` `official ${role}` `` — a phrase
assembled from a JSX expression or template interpolation) was **declined**: this scanner does not
evaluate `{}`/`${}`, so catching it statically means either flagging "official" before ANY interpolation
(fires on "official {documentType}" and every other innocent dynamic string) or tracing the identifier
back to a literal assignment (covers only the narrow same-file case) — neither is mechanically decidable
without guessing at intent, and the guard-conformance rule forbids inventing a false-positive machine to
close a narrow gap. See the lint's own comment above `_AFFILIATION_PATTERNS` for the full reasoning.
