...
**hard fail**.
W8.9 A string from the blocklist (gate 108) present anywhere in the built output → **hard fail**. The
**CI-enforced** subset of gate 108 fails the build; the **review-only** subset (bare `official` /
`approved` / `resmi` / `100%`) is a reviewer's rejection, not a build failure. Gate 108 says which is
which — do not read "it fails the build" across all of them.
