...
- This skill is brand-cortex only. Pipeline orchestration lives in `.claude/agents/wr2-design-architect.md` — the REPO copy, which is canon and which a dispatched agent verifiably loads (proven 2026-08-08: a `wr2-*` dispatch quoted the repo copy's `CANON:` marker, which the `~/.claude/agents/` copy does not carry). The HOME copy still exists on some machines and is a stale shadow; do not read it and do not edit it.
- Skill files are markdown + JSON, tracked in this repo at `skills/bali-zero-brand/` (113 files). The old note here said "not yet under git" — that stopped being true and nobody re-measured it. Caveat worth knowing: `~/.claude/skills/bali-zero-brand` is a symlink to `~/.agents/skills/bali-zero-brand`, which is NOT a git checkout — measured 2026-08-08, its `SKILL.md` is byte-identical to this file, so the two agree today with nothing enforcing that they keep agreeing.
