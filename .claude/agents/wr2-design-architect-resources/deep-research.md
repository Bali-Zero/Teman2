...
7. **Uncoordinated parallel agents score lowest on one aggregate metric — and that is the whole of it** RETRACTED[kim-2025-ranking-supports-the-no-peer-rule] (the earlier headline "the worst-measured topology" is RETRACTED: `Independent` is lowest on Table 5 Success Rate, `Decentralized 0.477 > SAS 0.466 > Centralized 0.463 > Hybrid 0.452 > Independent 0.370`, while the paper states _"no single architecture dominates across all domains and vendors"_ — and `Centralized` is THIRD, below plain single-agent) — Google's 2025 study (arXiv:2512.08296v3) ranks **Independent** (parallel agents, `Ω=synthesis_only`, no coordination and no cross-validation) **last** on Table 5 Success Rate (0.370), attributing architecture gaps to coordination **efficiency and overhead**. ⚠️ Centralized is **not** the best — Table 5 reads `Decentralized 0.477 > SAS 0.466 > Centralized 0.463 > Hybrid 0.452`, and the paper says _"no single architecture dominates"_. _Mitigation_: orchestrator agent owns canonical state, sub-agents are stateless functions that read shared state and emit deltas. **No autonomous peer-to-peer hand-offs in production** — a repo policy on its own grounds, and on those ALONE (see the correction below: the study's peer-to-peer architecture is `Decentralized`, which it ranks HIGHEST on Table 5; nothing in this item argues from the paper to the policy). RETRACTED[kim-2025-17x-error-amplification-as-cause]
...
**Single agent vs multi-agent verdict** RETRACTED[kim-2025-17x-error-amplification-as-cause] _(2026-08-02 — do not restore the 17.2× rationale; see item 7)_: multi-agent **with strict orchestrator** is correct because specialist roles are genuinely different competencies; and **the paper is not evidence for that in either direction** RETRACTED[kim-2025-ranking-supports-the-no-peer-rule] — an earlier version of this sentence said the study's ranking "argues for centralized state, stateless workers", which it does not: `Centralized` 0.463 sits BELOW plain single-agent 0.466 on Table 5 and the paper states no single architecture dominates. Both **centralized state, stateless workers** and **no peer-to-peer** are **repo policy**, not findings. Avoid temptation to give each sub-agent its own memory. (Corrected 2026-08-02: the warning used to be attributed to the 17.2× error-amplification figure; per the paper's own regression that effect is not significant after controls — the driver is coordination efficiency and overhead. See the correction note at item 7.)
