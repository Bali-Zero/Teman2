...
- **`evidence-carved`**: add `facts: array` (3-5 fact strings, ≤12 words each), `take_line: string` (≤15 words, the Bali Zero editorial comment), `take_label: string` (the kicker above take_line). **Content rule (2026-07-16 — closes the gap this schema table left open since evidence-carved replaced dark-status-list on 2026-05-09)**: `take_label` is a 1-3 word UPPERCASE editorial-stance kicker — see `skills/bali-zero-brand/layouts/evidence-carved.md` '## take_label variants' for the full vocabulary (THE UPSHOT / THE VERDICT / THE NET EFFECT / WHERE THIS LANDS / WHAT WE'RE SEEING / THE STAKES / BETWEEN THE LINES / THE SIGNAL / THE TRADE-OFF / WHAT CHANGES NOW, or coin one in-register). NEVER "THE BOTTOM LINE" — banned below as a filler heading pattern, and that ban applies to `take_label` too (2026-07-16 red-team finding #3: it was recommended here and banned three paragraphs down, a direct contradiction). Pick per carousel; never repeat the immediately previous carousel's choice. NEVER emit `OUR TAKE` / `OUR READ` / `OUR VIEW` (retired single-example anchors) or the `dark-status-list` generic-label ban set (`FACT`/`TAKE`/`NOTE`/`FACTS`/`REALITY`/`KEY FACT`).
...
Zero's mandate: carouseli must read as simple/accessible to the general public, not just to insiders — the August 1 PMK 37/2025 carousel was flagged too hermetic. Rules, tight version — **only the six below are active**; the amendment's rules 1 (gloss-before-code), 5 (qa-dialogue ban), 12 (length polarization), and 17 (categorical-subhead ban) are **NOT active until constitution reconciliation (Zero)** — each contradicts a currently-enforced constitution article or cron validator (see `skills/bali-zero-brand/_proposed-amendments/2026-07-16-accessibility-discipline.md` "## Constitutional conflicts — PENDING ZERO" for the exact conflicts):
...
Full ruleset (17 rules + 5 hook formulas + engagement-data grounding, including the 4 rules NOT active above): `skills/bali-zero-brand/_proposed-amendments/2026-07-16-accessibility-discipline.md`.
