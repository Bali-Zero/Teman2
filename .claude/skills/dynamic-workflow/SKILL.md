...
- **C5** — the LAST Tactics row whose stage names a `gate`/`gates` as a whole word decides (a
  coach may list pre-gates before its final one). That row's seat must match `opus-5` as an
  entity — `opus-5`, `Opus 5`, `fresh opus-5 xhigh` pass; `opus-50`, `opus-5.5`, `opus-5-1` do
