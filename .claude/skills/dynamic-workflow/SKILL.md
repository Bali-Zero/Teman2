...
creates `r1/`, `r2/` under the kit. `{{SEAT}}` stays blank in the master; `check --kit K`
recomputes the brief from `inputs.json` + template and refuses (exit 1) if `BRIEF.md` differs
by one byte, or the ledger fails its own sha check — a hand-edited brief is not a brief (W78).
...
- A file is an answer only if `validate_answer` accepts it (frontmatter `seat:` +
  `objective_sha256:` matching `brief.sha` + all seven `## ` sections present, ≤1,500 words).
  Empty, unparseable or truncated = `dead`: logged, relaunched at most once — a seat with two
  `sent` ledger rows already gets `refused-max-relaunch` on a third `r1`.
...
split into `r2/<seat>.rejected.md`; the ledger row records `kept=<n> rejected=<m>`. There is no
...
robustness, evidence, implementability, fit) as ONE markdown table, no prose. A ballot missing
any letter it was shown, or not a parseable table, is `dead` for that juror — no partial
credit. The script itself tabulates Borda + firsts into `jury/tabulation.md`; no synthesizer
LLM writes this file.
...
`jury`/`anonymise` already computed — it never computes one of its own.
...
exit commands as the phase gates, dead-seat substitution read from `arsenal.json` at start —
`scripts/lint_workflow_script.py` enforces the first and third of these in CI. It runs via the
`Workflow` tool inside a normal modus session; gate, ship and prove-live are modus's, unchanged.
...
`Z-DECISIONI.md`, `OUTCOME.md` (with `rounds_used`, `dead_at_launch`, `wall_clock`, `bites`
keys) already present in the kit; copies them to `--dest`; exits non-zero naming whatever is
missing instead of shipping a partial capture. That outcome is the next brief's F-line.
...
refusal, ledger tamper detection. `scripts/tests/test_dynamic_workflow.py` (pytest, 79 cases)
mirrors it; CI wires it into `.github/workflows/dynamic-workflow-selftest.yml` (PR3c).
