...
F2. Ledger rule (one row per mission, healer at most daily) vs practice: the ledger is touched
far more often than once per mission, and healer-tick closes land in batches on a single day.
(source: `git log --oneline --since=2026-09-12 -- .claude/skills/modus/PENDING-ARMS.md | wc -l`
and `git log --oneline --since=2026-09-16 --until=2026-09-17 --grep=healer -i | wc -l` —
measured by the convener at brief time and pasted into the kit's `BRIEF.md`; a number in this
template is stale by construction)
...
(source: session 2026-09-18, unverified)
F12. Strengths, measured here and globally: Opus 5.5 = architecture, red-team, long-horizon, gate;
...
(source: MODEL_ROSTER.md:71 (`gemini-3.1-pro`: 1M-ctx corpus ingestion, KBLI/visa/regulatory
search) and MODEL_ROSTER.md:91 (`k3` / `kimi-code/k3`: 1M-ctx long-context auditor, multimodal
...
(Filled at invocation from `scripts/arsenal_probe.py --json`: four columns — seat, status,
healthy, latency_ms — and nothing else. The probe's own free-text `evidence` field stays in the
probe's machine-local report and never enters this brief. A seat whose probe status was `TIMEOUT`
reads `unknown (probe budget 15 s)` with `healthy` also `unknown`: the probe's budget is far
shorter than a real round, so **`unknown` is not `dead`** and a seat listed that way may still
answer. Do not write it out of the formation — give it a substitute under F8 and keep it in.)
...
a fresh Sol signs.
C5. Final on-disk gate: a FRESH Opus 5.5 `xhigh` session outside the chain (BLUE). Never cheaper
...
| final gate | opus-5-5 | window | serial | 1 | <command> | <next> |
## Termination
...
**Checked mechanically before any juror reads you.** These are the literals the judge parses,
stated here so that no answer is thrown out for a spelling this brief never gave it:

- All seven `## ` sections present, in the order above. `## Formation` and `## Tactics` each carry
  at least one DATA row past the header — a `|---|` separator line is not a data row.
- Your FINAL gate row in Tactics: the seat cell names `opus-5-5` (`Opus 5.5` and
  `fresh opus-5-5 xhigh` read as the same entity) and the mode cell contains the word `window`
  (C5). List pre-gates
  freely — the LAST Tactics row whose stage names a gate is the one read.
- `round cap` is a BARE INTEGER on EVERY Tactics row: `1`, never "one", "3 rounds", "per stage"
  or "n/a". A round cap written as prose disqualifies (C8).
- Every `## Never` bullet cites at least one F or C by number (C8).
- Formation rows for the imperator seats (`fable-5-1`, `astra`) carry only role `coach` or
  `imperator` (C8).
- Cite C1 by NUMBER. You never need to spell a banned vendor, endpoint or env-var name; if you
  do, it is read as a citation only inside a `## Never` bullet — the same string anywhere else
  disqualifies you (C1).
- `objective_sha256` is the value above byte-for-byte; `facts_cited` and `assumptions` non-empty.

If a later round asks you to object: one objection = one paragraph citing an F/C and carrying a
line that starts with `Test:`, or one row of a table whose header names a `test` column, with an
F/C in the row and a non-empty test cell. No test, no objection.
