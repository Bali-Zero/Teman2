...
| **Kimi K3 (Moonshot)**     | `kimi -p "<prompt>" -m kimi-code/k3` (coding lane: `-m kimi-code/kimi-for-coding`; grunt: `-highspeed`; scriptable seat: `scripts/kimi_client.py`) — ARMED M5+Pro+Mini 2026-07-19 (probed). PII: vendor-parity (Zero ruling 2026-08-24: Chinese-cloud limit ABOLISHED system-wide — same common rules as Anthropic/OpenAI: Law 2 output boundary + Art. 56 basis for PROD transfers); K3 hyper-proactive on ambiguous tasks — scope tight. Desktop = separate operator-driven surface, see `research/operations/2026-07-19-kimi-arsenal-integration-deepseek-retirement.md` | refuter #2 / cross-family second opinion |
| GLM 5.2 (TP1 Alibaba)      | `no door: line in roster`; TP1 seat `tp1-glm-5.2` — probe first with `python3 scripts/arsenal_probe.py --seats tp1-glm-5.2`; OpenAI-compatible base `https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1`, key loaded by `load_tp1_settings_key()` from `~/.qwen/settings.json`. FIRST-CALL refuter for compact reviews; escalate along the cascade (Kimi → Codex) on multi-file scope or probe-fail.                                                                                                                                                    | refuter #1 — USE FIRST, not last-resort  |
