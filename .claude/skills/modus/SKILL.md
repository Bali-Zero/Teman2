...
- **Quota exhaustion:** implementation seats may cascade (Arsenal). **The final gate may NOT** —
  ...
  it does not hand the last grep to a weaker judge. Availability-death >24h (regulatory, not
...
| --------------------------------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Architect · orchestrator · judge · final on-disk gate** | **Fable 5** (max effort)                                   | interactive session default                                                                                 | Always. NEVER delegate or cascade the last grep (W65). Window dead → SUSPEND, don't substitute. Gate-death can be regulatory, not just quota: Fable 5 was export-control-disabled 2026-06-12→30 (back 07-01). Quota-death (hours) → SUSPEND per the standing rule; availability-death >24h → still no substitution, but escalate to the operator as §Solo-operatore (the organism must not freeze silently for weeks).                                                                                                                                                  |
...
| **Refuter (council 3rd seat)**                            | **GLM 5.2 → DeepSeek V4 Pro → Codex** (probe-then-cascade) | `claude-glm` (Keychain; probe first — locked-keychain contexts fail retrieval); DeepSeek key in `~/.openclaw/workspace/.env.master`                                                    | Falsification passes. **PROBE THE SEAT LIVE FIRST** — 2026-06-30 AND 2026-07-02 the key existed while the balance was dead (Esiste≠Armato at the arsenal level). Re-routed 2026-07-03 (Zero GO): GLM-5.2 leads — 51 vs 44 on AA Intelligence Index v4.1, flat Coding Plan vs pay-per-token, and the GLM seat is armed+probed (PONG 2026-07-03) while DeepSeek died 402 at two consecutive councils (topped-up 2026-07-03, HTTP 200). GLM-as-refuter still requires GLM ≠ implementer on the same task — generator≠grader. DeepSeek: explicit `deepseek-v4-pro`/`deepseek-v4-flash` ONLY — legacy aliases `deepseek-chat`/`deepseek-reasoner` RETIRE 2026-07-24 (the silent-downgrade trap becomes a hard-fail). A 2-seat heterogeneous council is acceptable-degraded — DECLARED, never silent. A seat that fails its live probe is itself an un-armed artifact → write a PENDING-ARMS line for the SEAT (artifact = the seat; missing arming step = the root cause — balance, permission allowlist, Keychain access; proof-of-armed = the seat's own 1-token probe passing in a live session). An AMENDMENTS narrative alone leaves the tier dead and untracked. |
...
refuter = GLM/DeepSeek (_falsify the core claim_). Max 3 external seats, rounds capped, NEVER
