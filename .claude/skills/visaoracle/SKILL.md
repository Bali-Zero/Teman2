...
- 2026-07-18 (afternoon): ENGINE track re-claimed by MANDATO S3 re-issue (Pro session, worktree `mouth-visa-engine-pr1-0718`, branch `agent/nuzantara/mouth/visa-engine-pr1-0718`; the mandate text explicitly orders a NEW dated worktree). Program = spec §9 PR ladder as atomic PRs: PR1 contracts → PR2 verify-on-load/anti-rollback (NO key ceremony — Zero's, dev unsigned-behind-flag) → PR3 tri-state evaluator + 20 gold personas + metamorphic → PR4 bitemporal SQL (migration range 250+ reserved; S4 uses 247-249). Seats probed at GROUND: Codex sol PONG, GLM PONG, agy armed (round-4 job as probe). NOT touching worktree `mouth-visa-experience` (sibling's e2e follow-up).
- 2026-07-18 (afternoon): ROUND 4 regulatory re-check COMPLETE + same-day arbitration (Gemini lane → web+NB arbitration lane, generator≠grader): **CORRECTION — Kepmen M.IP-08/2025 effective 2025-06-02, NOT 2026-06-02** (the round-2 line above carries the wrong year; dictum KELIMA +30d from ditetapkan 02 Mei 2025, SSEK/ABNR consensus, ID press live-reporting June 2025; grandfather: old-index documents valid until own expiry). **BVK +6 = Macau, NOT Morocco** (official imigrasi.go.id 19-entry list; Permenimipas 10/2026 ditandatangani 2026-07-07, diundangkan 2026-07-09 BN 463; number-collision trap with Permenkumham 10/2026 Second Home). NB-INTEL-Immigration MISSED Permenimipas 10/2026 entirely (flag nb-curator). Full record: research/visa/2026-07-18-visa-oracle-v2-round4-gemini-delta-recheck.md.
- 2026-07-18 (afternoon): **ENGINE PR1 SHIPPED — PR #2718 open, auto-merge armed (SQUASH)**. `backend/services/visa_engine/` contracts: enums+STAGE_ORDER, strict frozen models, fact registry (35+3 paths), 16-op strong-Kleene AST (no short-circuit, tuples), compiler (limits, recursive literal-type validation w/ canonical dates + set-member typing, scalar/set matrix, commercial-ban in legal stages, pack integrity, env match, required_facts equality), 8 JSON Schemas in `schemas/` (spec §1 layout — NOTE for Track C: NOT `contracts/` as the table below said), 223 tests. Gate chain: Sonnet build → Codex sol xhigh red-team (14 findings FIX-FIRST) → fix round → sol verification (10/11 CURED + 3 residual) → residual round → orchestrator final gate (suite re-run + on-disk P0 spot-checks). Declined-as-out-of-spec: presence-only-support ban (PR3 gold harness will police).
- 2026-07-18 (afternoon): **TWIN-PR COLLISION RESOLVED — #2718 SUPERSEDES #2654** (D12/W92-class). Discovered at merge-conflict time: PR #2654 (M5, `agent/air-m5/mouth/visa-engine-pr1`, opened 2026-07-17 21:43Z, automerge armed 19h without firing, last branch activity 06:05 WITA) implements the same PR1 on the same paths — an earlier run of the same S3 lane. Two armed automerges on one path violate the Track A serial-chain rule. Disposition: #2654 CLOSED with explanatory comment, branch PRESERVED for salvage (its test cases to be mined at PR3); #2718 (3 adversarial rounds, 223 tests) is the single Track A PR1. Root cause: the Track A claim line landed on main after this session's branch cut — stale-read race on the corner claim protocol; the claim check must be `git fetch + read origin/main corner`, not the checkout copy.
...
- LAMPIRAN-OCR (2026-07-18): official Lampiran of Kepmen M.IP-08/2025 (B211*→C-series full mapping) is an image-only, WAF-gated PDF — only LOW-confidence travel-agency reconstruction exists. Arming step: `mcp__ocr-tesseract__perform_pdf_ocr` on the downloaded PDF (kemenimipas.go.id attachment 20250813_09) in a dedicated session. Proof-of-armed: primary-source mapping table in research/visa/. Gates FASE 2 catalog-mapping content, NOT engine PRs.
- NB-INTEL-GAP (2026-07-18): NB-INTEL-Immigration daily feed missed Permenimipas 10/2026 (BVK) one week post-signing. Arming step: nb-curator ingestion + feed-gap diagnosis. Proof-of-armed: NB query returns the Permen with correct number/ministry.
