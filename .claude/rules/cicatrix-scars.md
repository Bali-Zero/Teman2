...
**Reference:** `infra/claude-hooks/worktree_isolation.py` (`_resolve_under_worktrees`, `_worktrees_strictly_inside`, `_unarmed_dirty_removal_target`) · corpus `infra/claude-hooks/test_w105_nested_worktree_removal.py` · registro `infra/guard-conformance/registry.json` + step esecutore in `.github/workflows/guard-conformance.yml` (W81: registrare senza eseguire è teatro). Memoria: `discovery_worktree_hook_blocks_nested_removal_by_path_prefix_2026_07_26`.
...
Il 2026-07-26 il mondo si è invertito: la config è REFUSED (`no access token available`) mentre il token nell'env è valido. Quel `unset` stava buttando via **l'unica credenziale funzionante**. Entrambe le corse notturne (03:00 e 03:20) sono abortite a `could not resolve primary machine for nuzantara-postgres` e la produzione è rimasta **27 ore senza dump**. La produzione era SANA per tutto il tempo (misurato: `/health` 200 `database: connected`; `role=primary` + `role=replica` entrambi `started`) — non era un guasto del database, era la credenziale. `wr2-cron-wrapper.sh` porta lo stesso `unset` per la stessa ragione storica: seconda vittima.
