...
Il 2026-07-26 il mondo si è invertito: la config è REFUSED (`no access token available`) mentre il token nell'env è valido. Quel `unset` stava buttando via **l'unica credenziale funzionante**. L'unica corsa notturna schedulata (crontab `20 3 * * *`) è abortita a `could not resolve primary machine for nuzantara-postgres` in tutti e 3 i suoi tentativi, e la produzione è rimasta **27 ore senza dump** (ultimo buono: 2026-07-25 03:20). La produzione era SANA per tutto il tempo (misurato: `/health` 200 `database: connected`; `role=primary` + `role=replica` entrambi `started`) — non era un guasto del database, era la credenziale. `wr2-cron-wrapper.sh` porta lo stesso `unset` per la stessa ragione storica: seconda vittima.
