...
**Archived 2026-06-16 sweep (W78 + W81 — manual oversize remediation: W82 pushed this file to 43,950 char (>40k auto-load limit, `archive_cicatrix_scars.py:59`); auto-archive could not act because the scars were <14-15 days old. Moved the oldest STRUCTURAL/FIXED scars verbatim. W82 + P0-security kept live):**

- ⚠️ **W78** (P2 STRUCTURAL/META): "il sistema plasma l'agente all'~80%" — no-unlearning of wrong scars + l'-umano-disimpara (escalation drift). Governance scar, not a runtime bug. Full TRAUMA/ANTIBODY/GOTCHA in archive — grep `W78`.
- ✅ **W81** (FIXED): the 3 WR3 learning loops were "green but empty" — "Omeostasi Tautologica" (process-telemetry ≠ state-delta). F20+F21 fixed (114/114 tests), F18 escalated. Full TRAUMA/ANTIBODY/GOTCHA in archive — grep `W81`.
