...
**GOTCHA (c) — il confine è 268, e non è un numero tondo scelto a occhio.** Su 169 file di migrazione ci sono **47** `ALTER TABLE` top-level contro tabelle che prod non lascia possedere all'app. **37 stanno in migrazioni ≤268** e girarono senza incidenti, perché all'epoca il runtime ERA il proprietario; i **10 restanti stanno in migrazioni >268** e sono esattamente i due file che la produzione ha rifiutato (281 ×7, 285 ×3). Zero falsi positivi, zero falsi negativi contro l'incidente osservato. Retrofittare i 37 storici non cambierebbe nulla in nessun database e seppellirebbe i dieci che contano.
...
**CURA — antidoto eseguibile.** Lo sblocco di produzione è stato fatto dalla sessione conductor con membership temporanea (`GRANT visa_ledger_owner TO backend_rag_v2`), rerun verde e **revoca ri-misurata** (`pg_has_role(...) = f`); esito verificato in modo indipendente da una seconda sessione, con il segnale che discrimina davvero — `POST /api/visa/voa/eligibility-checks` passa da `401` a `404`. L'antidoto strutturale è `apps/backend-rag/backend/tests/db/test_post_d1_migrations_guard_ledger_owned_ddl.py`: guardia statica sul testo, `ALTER TABLE` top-level (fuori da ogni corpo dollar-quoted, riconosciuto contando i `$$` — pari/dispari) contro l'insieme misurato, solo per migrazioni >268, con 281/285 in allowlist motivata **più** un test che fallisce se l'allowlist sopravvive alla sua ragione. Colpa e innocenza entrambe provate (4 fixture colpevoli, 5 innocenti). Rosso-prima verificato piantando una migrazione `999` che viola: 2 test rossi con messaggio azionabile, 13/13 verdi dopo la rimozione.
