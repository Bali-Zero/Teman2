...
### 🐛 W132 (2026-08-28): un receptor di posta fleet pensato per le sessioni interattive prolungava OGNI one-shot `claude -p` — e il CLI stampa l'ULTIMO turno, quindi la risposta giusta moriva nel transcript

_Scoperto 2026-08-28 sul Pro, re-ground WR2: il draft-generator falliva 3/3 su ogni draft dal 20/8 («DeckPlan validation exhausted», input pydantic = «Altri 3 fleet-watch… task DeckPlan chiuso»). Curato in PR #5176. Famiglia **#3 (guard-over-match)** nella variante per-PLATEA — il receptor giudica «una sessione che finisce» senza chiedersi QUALE TIPO di sessione — con effetto **#2** (8 giorni di pipeline decapitata, exit 2 letto ma non capito)._

**TRAUMA, misurato non dedotto.** Il transcript della sessione `-p` del planner (`~/.claude/projects/-Users-nuzantara-nuzantara-deploy/c68e9228….jsonl`) mostra la sequenza intera: turno 1 = **il DeckPlan JSON corretto**; poi 8 ondate di `hook_success`/`hook_additional_context` che iniettano `<cross-machine-message from="mini-pro2:fleet-watch">` (PR DIRTY #5037/#4640/#5071…); il modello risponde a ogni ondata («Ricevuto — 3 PR DIRTY… Fuori scope», «Altri 3 duplicati… task DeckPlan chiuso»); la sessione dura 82-241s invece di secondi; `claude -p` stampa **l'ultimo turno** → stdout = chatter fleet. Pydantic fallisce, 3 retry identici, draft `failed`→`rejected`. 12 topic buoni bruciati in 5 giorni, ~3 chiamate opus multi-minuto sprecate per draft per ora — su seat già quota-stressati.

**MECCANISMO.** Lo Stop-hook `mailbox_inject.py` (control-plane `~/.claude/hooks/`, registrato in `settings.json`) consegna la posta cross-machine a fine turno restituendo `additionalContext` — e un Stop-hook con additionalContext NON fa finire la sessione: la riapre con un turno nuovo. Disegno corretto per una sessione interattiva/orchestratore; veleno per un one-shot puro-completion. `claude_oauth_client._build_env` ereditava l'ambiente pieno e non settava `CLAUDE_CONFIG_DIR` → ogni figlio girava sul config-dir interattivo con TUTTI gli hook. Perché dal 20/8: il fleet-watch di mini-pro2 ha iniziato a spool-are messaggi con cadenza di minuti — le sessioni brevi non ne incontravano mai, quelle da minuti (planner opus) sempre.

**GOTCHA — la mia prima ipotesi era sbagliata, e il repro pulito l'ha detto.** Ipotesi 1: «gli hook SessionStart iniettano rumore e il modello risponde al rumore». Due repro fedeli (config-dir default, token_2, stesso modello) sono tornati **puliti** — un prompt breve finisce prima che la posta arrivi. La verità stava solo nel TRANSCRIPT della sessione fallita (le righe `queue-operation`/`attachment`), non nella riproduzione: quando il difetto dipende da un evento asincrono esterno, il repro negativo non prova nulla e l'artefatto persistito è l'unica prova (parente di W127: «riprodotto localmente pulito» ≠ assente).

**GOTCHA — il seat keychain muore sotto il dir sterile, e va bene così.** Il lookup keychain del CLI è config-dir-scoped (misurato: `Not logged in`): la cura uccide il seat sentinella `("", "keychain")`. Accettato e dichiarato nel codice: quel seat girava anch'esso nel dir avvelenato, quindi il suo «successo» era lo stesso ultimo-turno corrotto — ora fallisce subito come `authentication` e va in cooling invece di bruciare minuti. Escape hatch: `CLAUDE_OAUTH_HEADLESS_CONFIG_DIR` può puntare a un profile-dir senza hook.

**GOTCHA — la classe NON è morta repo-wide (il refuter ha corretto il mio claim).** La cura copre i consumer di `claude_oauth_client` (WR2 planner/fact, article composer, coreference, surface router). I fratelli che spawnano `claude -p` per conto proprio — `scripts/zantara-gateway/claude_client.py`, `apps/bali-intel-scraper/scripts/claude_cli_enricher.py` (strippa `CLAUDE_CONFIG_DIR` → default avvelenato) — restano esposti. Riga a ledger per il class-audit (W107: pattern-fix = class-audit, mai rider).

**CURA.** `_build_env` pinna **incondizionatamente** `env["CLAUDE_CONFIG_DIR"]` a `~/.claude-oauth-headless` (un dir nudo non ha hook PER COSTRUZIONE — la classe muore lì, non hook-per-hook; il CLI bootstrappa il dir mancante da solo, misurato anche su path annidati) + `--no-session-persistence` in argv (stesso flag del gemello zantara-gateway): un one-shot non deve persistere una sessione ripristinabile, e il dir sterile non accumula transcript. Test: guilt (un `CLAUDE_CONFIG_DIR` ereditato stile-kaiser NON passa), override+blank-fallback, assert di wiring (pin+flag arrivano all'argv/env del subprocess vero). Mutation-verified con `PYTHONDONTWRITEBYTECODE=1` (W121): pin spento → RC=1, restaurato → RC=0.

**NOTA DI MANUTENZIONE.** Nessuna riga nel ponte `cicatrix-superscar.md`: budget saturo (precedente W130 — corpo qui, ponte intatto).

**Famiglia: #3 (il receptor giudica la FORMA dell'evento — «una sessione stoppa» — non l'ENTITÀ — «che tipo di sessione»), effetto #2.** Parenti: W127 (repro locale pulito ≠ difetto assente), W107 (class-audit), W121 (mutation su bytecode).
