...
**GOTCHA.** `gh pr merge --auto` esce **0 senza output** anche quando `autoMergeRequest` resta `null`. Misurato su **PR #3607** (non su #3615): subito dopo quel comando la GraphQL rispondeva `autoMergeRequest: null` **e** `isInMergeQueue: true` con `mergeQueueEntry: {position 1, AWAITING_CHECKS}` — cioè la PR era già armata per un'altra via e il campo auto-merge non lo diceva. Il gesto è un proxy (W111): interroga `mergeQueueEntry`, non l'exit code del comando. _(Limite dichiarato: #3607 è stata mergiata poco dopo, quindi quello stato non è più ri-osservabile; il fact-check successivo ha guardato #3615, un'altra PR, e trovato — correttamente per QUELLA — nessun evento di coda. Le due osservazioni non si contraddicono, ma questa non è più verificabile a posteriori.)_
