...
**CURA — seam iniettabile, mai il default di produzione.** Aggiunto `now: datetime | None = None` a `run_from_env`, passato a valle a `run_watchdog` — la produzione non cambia comportamento (nessun chiamante reale passa `now`, il fallback resta `datetime.now(timezone.utc)`). Il test guasto ora passa `now=NOW` come ogni altro nel file. Prova di cura, non di silenzio: `test_cli_injected_clock_governs_not_the_real_clock` — STESSA fixture (50 giorni), `now` iniettato un giorno PRIMA dell'expiry → fresh; `now` iniettato 365 giorni DOPO → stale (exit 2) — due asserzioni che non possono passare entrambe a meno che l'iniezione sia reale, indipendentemente da cosa dica l'orologio di sistema in quel momento.
