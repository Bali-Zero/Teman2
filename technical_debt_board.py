...
class TechnicalDebtBoard:
    ...
    def _search_pattern(
        self, pattern: str, type_extractor, extensions: List[str]
    ) -> None:
        ...
        for ext in extensions:
            for file_path in self.repo_root.rglob(f"*{ext}"):
                # Skip excluded paths
                if any(
                    re.search(exclude, str(file_path))
    ...
    def _generate_recommendations(self, by_priority, by_type, by_component) -> str:
        ...
        if p0_count > 0:
            recommendations.append(
                f"🚨 **Address {p0_count} critical issues immediately** - These block production readiness"
            )
        ...
        if bug_count > 5:
            recommendations.append(
                f"🐛 **High bug concentration ({bug_count} items)** - Consider a bug-fix sprint"
            )
        ...
        if backend_count > frontend_count * 2:
            recommendations.append(
                "🔧 **Backend technical debt is high** - Focus on backend refactoring"
            )
        elif frontend_count > backend_count * 2:
            recommendations.append(
                "🎨 **Frontend technical debt is high** - Focus on frontend cleanup"
            )
        ...
        if len(unassigned) > len(self.items) * 0.5:
            recommendations.append(
                f"👥 **{len(unassigned)} items are unassigned** - Consider assigning ownership"
            )
        ...
        if len(no_issue) > len(self.items) * 0.7:
            recommendations.append(
                f"🔗 **{len(no_issue)} items lack GitHub issues** - Create issues for better tracking"
            )

        if not recommendations:
            recommendations.append(
                "✅ **Technical debt is well managed** - Keep up the good work!"
            )
