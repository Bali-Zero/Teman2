...
**Date:** 2026-04-25
**Author:** Claude Opus 4.7 (1M ctx) under Zero's direction
...
| G3.8 | Zero has spent ≥1 day in the last 14 reading the CEP CSV reports manually | meta-gate: human is paying attention          |
...
**Pre-condizione locale:** Zero esprime preferenza esplicita (questo è scope creep verso prodotto, non infrastruttura).
**Sub-gates:**

- A. Solo team-internal, MAI client-facing (NLM warning hallucination).
- B. Fact-check manuale prima di shipping: Zero (10 min lettura settimanale).
...
| G4.4 | Zero can name 5 specific source URLs for the new domain off the top of his head          | meta-gate: human curation possible |
...
**NB-LIFESTYLE** (DeepSeek 3.5 lo ha sconsigliato esplicitamente). Scope creep — sanità, scuole, eventi locali. Volatilità altissima (orari, indirizzi cambiano). Bali Zero non vuole essere TripAdvisor. **Veto permanente** salvo decisione Zero esplicita post Sprint 4.
...
This is the explicit guardrail Zero asked for during the 2026-04-25 brainstorm: _"un sistema RAG che vuole essere SOTA 2026 deve sapere quando NON rispondere, non solo come rispondere"_. The freshness gate (S1.3) implements that for retrieval. This spec implements it for system evolution.
