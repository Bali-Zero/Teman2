...
The `X-API-Key` currently hardcoded as `zantara-secret-2024` in Air crontab must move to GitHub repo secret `NUZANTARA_API_KEY`. Verify the secret exists on Teman2, add if missing.
...
### Step 2.3 — Secrets
`fly secrets set -a nuzantara-cron SENDGRID_API_KEY=xkeysib-... TELEGRAM_BOT_TOKEN=... DEEPSEEK_API_KEY=... ZANTARA_API_KEY=zantara-secret-2024 ...`
...
| `X-API-Key: zantara-secret-2024` hardcoded in multiple places | Phase 1 only moves it to GH secret for Cat1. Broader rotation separate ticket |
