...
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ---------- |
| D0                 | Orchestrator runtime: Python + httpx + official `mcp` SDK 1.12.4 (NOT claude -p subprocess, NOT OpenClaw+GPT-5.5)                                                                 | Zero + 3-LLM panel                    | 2026-05-13 |
| D1                 | File location: NEW `scripts/wr2_canva_pdf_apply.py` accanto al legacy (rollback-friendly)                                                                                         | Zero                                  | 2026-05-13 |
| D2                 | Legacy slides_json schema: adapter inline in orchestrator (riusa `/tmp/wr2_legacy_adapter.py` as `_schema_adapter.py`)                                                            | Zero                                  | 2026-05-13 |
| D3 (v1, discarded) | OAuth token storage: shared `~/.mcp-auth/mcp-remote-0.1.37/` cache                                                                                                                | rejected after 3-LLM panel            | 2026-05-13 |
| D3 (v3, final)     | OAuth token storage: orchestrator-owned `~/.config/wr2/canva_tokens.json` + dedicated bootstrap script + HMAC integrity + proactive refresh + flock + dynamic client registration | Zero + 3-LLM panel + empirical verify | 2026-05-13 |
...
Both are terminal and NOT picked up by the next cron tick. Zero
...
6. Telegram alert to Zero: "WR2 pipeline live, first draft
   processed $DRAFT_ID"
...
| --------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------- |
| 3-LLM spec review           | Claude + Gemini + DeepSeek (+ Codex when alive) | This document already passed sections 1-3. Sections 4-7 panel pending if Zero requests. |
| User review of written spec | Zero                                            | This file approved as-is, or change requests merged via Edit + re-review                |
| Unit test coverage 80%+     | CI                                              | pytest-cov ≥0.8 per module                                                              |
| Bootstrap smoke             | Zero on Pro                                     | `list-brand-kits` returns Bali Zero team                                                |
| End-to-end real draft       | Zero + me on Pro                                | 1 draft `rendered`, Canva edit URL opens, layer-editable                                |
