...
Panel artifacts (ephemeral, not git-tracked — recreate via `scripts/agent-library-evolver-run.sh --dry-run` if needed for audit):

- v1 review outputs: `/tmp/evoskill-review-{gemini,deepseek,codex}.txt`
- v2 review outputs: `/tmp/evoskill-v2-{gemini,deepseek,codex}.txt`
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------- |
| ~~`openai-codex-sdk` still in pyproject~~ **RESOLVED** in commit `e2c834776` (vendor strip removed both `claude-agent-sdk` and `openai-codex-sdk`). `openhands-tools` later removed too in panel round 3 Codex BLOCKING #1 fix (commit `e2e950c75`) because it pulled `browser-use → anthropic` transitively. | LOW (resolved) | Documented in `vendor/evoskill/UPSTREAM.md` §1. No further action.                                               |
...
- ChatGPT Pro subscription / Codex CLI lane: `~/.claude/CLAUDE.md` § "External LLM arsenal" (Tier-3 cascade)
