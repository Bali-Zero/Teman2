...
- Migration allocation is exact: Phase 3 owns only `apps/backend-rag/backend/db/migrations_v2/249_worker_effect_ledger.sql`. It follows `247_event_quarantine.sql` and `248_worker_plane_ownership.sql`. The next free number after this plan is `250`.
...
- Work only in the isolated Pro worktree. Backend commands use `apps/backend-rag/.venv` with `PYTHONPATH=.`. Heavy tests, disposable PostgreSQL suites, and failure injection run on Pro or CI. This transferred task must not edit, dispatch, or persist artifacts on Air-M5.
...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Effect schema      | `backend/db/migrations_v2/249_worker_effect_ledger.sql`, `backend/worker_plane/effects.py`, `effect_ledger.py`                                                    | Stable effect projection, append-only-at-runtime attempts with protected bounded retention, resolution audit, and late-fence transitions.                |
...
Migration 249 creates exactly three tables and no replacement ownership table:
...
Migration 249 defines and tests narrow, workload-specific projection/history
...
`UPDATE` and direct `DELETE` on `worker_effect_attempts`; correction is a later
audited event. History retention is the only bounded exception: two protected,
workload-hard-coded `SECURITY DEFINER` functions may delete at most 10,000
attempt events per call only when `occurred_at < database_now - interval '180
days'`, the event is not the projection's current attempt, and the locked
projection is terminal `confirmed` or terminal non-retryable `failed` with no
unresolved reconciliation. `prepared`, `attempting`, retryable `failed`, and
`outcome_unknown` projections or attempts are never eligible. The functions
run under a dedicated owner with no login and in the same transaction append a
payload-free `effect-attempt-retention` entry to existing
`worker_ownership_audit` with workload, cutoff, first/last event ID, row count,
stable aggregate hash, actor, and reason. The management role receives only
`EXECUTE`; it has no table `DELETE`, trigger bypass, or bulk/all-workload
entrypoint. Invalid/stale transitions use SQLSTATE `55000`.
...
## Task 1: Gate pre-249 Phase 3 admission without a live deployment
...
- [ ] Run RED on Pro/CI:
  ...
  Expected: Phase 3 preflight is absent; no fixed check yet proves head 248 plus all guards unarmed from disposable PostgreSQL.

- [ ] Implement the preflight as a read-only fixed-check runner with SHA-256-bound JSON input and an allowlisted disposable database target. It must reject prose-only evidence, another source commit/artifact hash, unresolved prior-phase findings, any armed guard, any migration head other than 248, or any command/evidence that implies live staging or production mutation. It deliberately performs no effect-ledger or heartbeat check at this pre-249/pre-deploy boundary.
...
- [ ] Run GREEN on Pro/CI:
  ...
  Expected: tests pass; the command exits 0 only for same-branch verifier/review evidence, exact code artifact hash, schema head 248, and all guards unarmed in disposable PostgreSQL. It emits no live staging, heartbeat, or effect-ledger requirement.

- [ ] Obtain a fresh SDD read-only review of the pre-249 boundary, disposable-database allowlist, all-guards-unarmed requirement, evidence freshness, and absence of live staging dependencies; fix and rereview before commit.
- [ ] Commit:
...
## Task 2: Add migration 249 and extend the existing side-effect catalog

**Files:**

- Create: `apps/backend-rag/backend/db/migrations_v2/249_worker_effect_ledger.sql`
- Create: `apps/backend-rag/backend/tests/db/test_migration_249_worker_effect_ledger.py`
...
backend/db/migrations_v2/249_worker_effect_ledger.sql`, and no legacy singular
...
| `worker_effect_attempts`   | `legal-attempt-retention` / `static`          | `api`                                | `retain-terminal-attempts`                                                                                                    |
...
| `worker_effect_attempts`   | `workflow-attempt-retention` / `static`       | `api`                                | `retain-terminal-attempts`                                                                                                    |
...
wrappers hard-code workload identity. The two retention bindings resolve only
to `retain_workflow_terminal_attempts` /
`sql:public.worker_retain_workflow_terminal_attempts` and
`retain_legal_terminal_attempts` /
`sql:public.worker_retain_legal_terminal_attempts`; each hard-codes workload,
requires exact cutoff, batch bound, actor, and reason, and atomically emits the
retention audit summary above. Shared serializers, row mappers, and state
...
Migration 249 also extends the two existing Phase 1 bootstrap-table policies;
...
- [ ] Before writing migration-249 tests/source, run `git fetch origin`, heartbeat all five allocation leases, and from the repository root run `apps/backend-rag/.venv/bin/python scripts/check_worker_plane_migration_allocation.py --base-ref origin/main --feature-ref HEAD --record docs/architecture/worker-plane-migration-allocation.md --next-number 249`. Update the record with the fetched base commit, unchanged upstream-246 blob identity, feature-owned 247/248 proof, and collision proof. Any occupied leased number, missing lease, or mapping drift stops Phase 3 and invalidates affected packets.
- [ ] Rerun the unchanged comprehensive allocation-checker test, then prove the exact feature-owned 247/248 basenames exist once each, 249 is still absent, and 250/251 remain unoccupied. The Phase 0 test already covers this `--next-number 249` state; do not phase-edit the generic checker.
- [ ] Write migration tests for exactly three newly created tables: stable `worker_effect_ledger`, append-only-with-bounded-retention `worker_effect_attempts`, and `worker_cutover_run_audit`, plus ownership-extension tests for the existing `worker_workload_ownership` and `worker_ownership_audit` tables. Cover all columns/constraints, including `worker_effect_attempts.runtime_owner`, `ownership_generation`, `claim_build_epoch`, `claim_token`, `claim_claimed_at`, and dispatch-time `claim_lease_expires_at`; projection indexes such as `(workload_name, state)`; attempt-history indexes such as `(effect_key, attempt_number, event_id)` and `(ownership_generation, event_kind)`; all exact workload-specific SQL functions; fixed row-lock order; SQLSTATE; direct `UPDATE`/`DELETE` denial; non-destructive rollback; `business-context` plus every repeatable writer binding/interface/mode and sorted migration source above; migration uniqueness `247 -> 248 -> 249` with no other 249 file; and the shared G16 checker against a disposable real PostgreSQL schema, refreshed fixture, annotations, and catalog. Prove the two retention functions reject a cutoff newer than 180 days, a batch above 10,000, current attempts, any active/retryable/unknown projection, wrong workload, normal worker role, direct delete, and audit failure; prove an eligible terminal batch deletes exactly the locked IDs and atomically appends the redacted count/range/hash audit. Prove `worker_set_ownership_mode` is an exact expected-field CAS, permits only `active -> draining`, preserves owner/generation/build floor, increments version, and atomically audits; direct bootstrap-table DML and worker-role execution fail. Add guilt fixtures for the legacy singular writer annotation, one generic effect writer accepting `workload_name`, reused interfaces across the workflow/legal bindings, missing SQL/Python twins, a mode/interface operation-key mismatch, a missing 249 migration source, or a set-mode/retention write outside the exact static bindings.
...
- [ ] Run RED on Pro/CI:
  ...
  cd apps/backend-rag
  source .venv/bin/activate
  PYTHONPATH=. pytest backend/tests/db/test_migration_249_worker_effect_ledger.py backend/tests/worker_plane/test_effect_ledger.py backend/tests/architecture/test_catalogs.py scripts/tests/test_check_table_ownership.py -q
  ```

  Expected: migration/types/repository are absent and current capabilities cannot express ambiguity contracts.

- [ ] Implement migration 249, `EffectState`, immutable `EffectRecord`, immutable `EffectAttemptEvent`, read-only `EffectLedgerRepository` queries, the eight grant-fenced workload mutation wrappers, two static workload resolution wrappers, two protected workload-specific retention wrappers, two protected workload-specific cutover-audit wrappers, and the exact static `WorkerPlaneRepository.set_ownership_mode`/`worker_set_ownership_mode` pair named above. Each Python wrapper calls only its matching workload-specific SQL callable and hard-codes workload plus operation; no public or private mutating helper accepts a caller-selected workload. Grant-fenced wrappers require and transactionally revalidate the exact claim/lease stamp; static resolution, retention, and mode-change wrappers are management-plane operations and accept no `ClaimContext`. Run each retention wrapper weekly through the protected management workflow, repeat bounded batches until fewer than 10,000 rows are eligible, and stop on any audit/invariant failure. Reuse canonical `DeliverySemantics`; do not introduce an effect-contract type. Logs contain only key hash, fixed workload, effect, attempt number, generation, state, error class, and retention batch summary.
- [ ] Extend the existing `SideEffectCapability` and `SIDE_EFFECT_CATALOG`; do not create a new registry. Update table ownership for all three new tables with `BusinessContext.PLATFORM` and the exact repeatable grant-fenced/static bindings above; do not serialize a singular runtime writer. Apply migrations through 249 to disposable PostgreSQL, regenerate `backend/tests/fixtures/schema_tables.txt` through the shared checker's explicit refresh mode, review the sorted diff, and keep the fixture staged with this migration.
...
- [ ] Run GREEN on Pro/CI:
  ...
  cd apps/backend-rag
  source .venv/bin/activate
  PYTHONPATH=. pytest backend/tests/db/test_migration_249_worker_effect_ledger.py backend/tests/worker_plane/test_effect_ledger.py backend/tests/architecture/test_catalogs.py backend/tests/db/test_migration_uniqueness.py -q
  ...
  Expected: all tests/checkers pass; 249 is unique/latest; invalid effect transitions and cutover barriers fail closed.

- [ ] Obtain a fresh SDD database/security review of migration 249, transition races, PII surface, catalog compatibility, and rollback; fix and rereview before commit.
- [ ] Commit:

  ```bash
  git add apps/backend-rag/backend/db/migrations_v2/249_worker_effect_ledger.sql apps/backend-rag/backend/tests/db/test_migration_249_worker_effect_ledger.py apps/backend-rag/backend/worker_plane/effects.py apps/backend-rag/backend/worker_plane/effect_ledger.py apps/backend-rag/backend/worker_plane/repository.py apps/backend-rag/backend/tests/worker_plane/test_effect_ledger.py apps/backend-rag/backend/architecture/catalogs/models.py apps/backend-rag/backend/architecture/catalogs/effects.py apps/backend-rag/backend/architecture/catalogs/data/table_ownership.json apps/backend-rag/backend/tests/fixtures/schema_tables.txt apps/backend-rag/backend/tests/architecture/test_catalogs.py apps/backend-rag/scripts/tests/test_check_table_ownership.py docs/architecture/worker-plane-migration-allocation.md
...
The isolated fixture table has logical uniqueness `UNIQUE(workload_name, scheduled_for)`, separate nullable ownership generation/claim fields, and no client data. It is created inside the disposable test transaction, not migration 249. The fixture enqueues a future run, cuts over before due, adopts it in the cutover transaction, executes one logical effect, reverses cutover, and repeats with another future run.
...
- [ ] Write `worker-plane-phase3-legal-checkpoint.md` as a code-readiness handoff: bind it to source commit/artifact, disposable evidence, and fresh complete G9 candidate/comparison hashes, then explicitly defer staging deploy, migration 249, guard arming, scoped grants/secrets, live compatibility-floor heartbeats, forward/reverse cutover, canaries, budget checks, and full-cycle observation to production-rollout Task 2 on the exact protected-merged digest.
...
- [ ] Add CI for migration 249, catalogs/table ownership, effect executor, cutover, ten-run schedule fixture, workflow/legal focused suites, fault matrices, route/import boundaries, prior phase verifiers, Ruff, migration linters, marker scan, and diff check.
...
- [ ] Dispatch Fable as architecture/transaction/reversibility judge, Gemini as constructive distributed-systems/operations reviewer, and GLM as adversarial reviewer through the one checked canonical launcher:
  ...
  The launcher reads the immutable content-addressed packet once, constructs one deterministic manifest-hash/packet-length attestation header plus the exact packet, and sends that identical buffer over stdin to all three subprocesses from a newly created empty `0700` cwd, without a prompt argument or checkout tools. It uses the master plan's exact absolute Claude executable and no-tool safe-mode argv with `plan` for Fable and `dontAsk` for GLM, the committed hash-validated GLM route config and keychain `ANTHROPIC_AUTH_TOKEN` isolation, and absolute `/Users/nuzantara/.local/bin/agy` with exact plan/sandbox argv for Gemini. It atomically preserves raw stdout, stderr, immutable invocation receipts, and launcher-normalized Markdown before any cross-seat visibility. Every receipt has a unique `launcher_invocation_uuid`, common `input_manifest_sha256`, external `packet_sha256`, exact review-input/executable/config/argv/raw hashes, and nullable `provider_session_id`/`reported_model` checked only when emitted; requested route is not provider declaration. Each normalized review repeats only `input_manifest_sha256` and preserves the unedited six-heading body; no manual normalization follows launch.
...
- [ ] Migration sequence is exactly 247 quarantine, 248 ownership, 249 effect ledger; no second Phase 3 migration exists and 250 remains free.
...
- [ ] Migration 249 state transitions prevent concurrent duplicate attempts, blind non-reconcilable retry, terminal reopen, stale effect generation, and activation with attempting/unresolved blockers.
...
- [ ] Migration 249 is applied to disposable PostgreSQL and G16 proves live schema/fixture/catalog/annotation parity with a nonempty `TEST_DATABASE_URL`; the refreshed `schema_tables.txt` and checker tests are staged with migration 249.
- [ ] G16 and the static source ratchet prove `business_context=platform`, the exact sorted workflow/legal grant-fenced automatic bindings, workload-specific static resolution bindings, static cutover-audit binding, and the migration-249 extensions to Phase 1 `worker-grant-admin` plus `worker-ownership-audit`; every Python/SQL pair resolves, operation-mode/interface keys and migration sources match, automatic prepare/begin/finish/reconcile mutations carry the full live grant/domain-claim/lease/attempt fence, resolution and set-mode use only the protected management role, no legacy singular writer metadata or mutation API accepting `workload_name` exists, and no shared helper performs DML.
- [ ] No live staging or production app, database, guard, grant, secret, heartbeat, or ownership row was mutated in Phase 3. Production-rollout Task 2 is the sole live staging actuator and must use the exact protected-merged digest for deploy, migration 249, guard arming, live compatibility-floor proof, workflow then legal cutover, reverse rehearsal, canaries, budget checks, and full-cycle observation.
