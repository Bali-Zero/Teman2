...
- Phase 0 and Phase 1 must be implemented, committed, independently reviewed, and verifier-green earlier on this same feature branch before Phase 2 implementation begins. They do not need to be merged separately; the protected compatibility merge happens only after all implementation phases and their panels pass. Migration `248_worker_plane_ownership.sql` is the latest schema migration entering this phase.
- Phase 2 creates no SQL migration. It reuses `BusinessContext`/`business_context`, `RuntimeOwner`/`runtime_owner`, `worker_workload_ownership`, `worker_owner_heartbeats`, `worker_ownership_audit`, `OwnershipRepository`, `OwnershipService`, `OwnershipGrant`, `OwnerHeartbeat`, `ClaimContext`, `BuildFloorEvidence`, and `LivenessReport` exactly as defined in Phase 1. Business ownership and runtime placement remain independent; this phase introduces no compatibility alias between them. Migration number `249` is reserved for Phase 3; `250` remains free after the Phase 3 plan.
...
- Run backend Python from `apps/backend-rag/.venv` with `PYTHONPATH=.`. Heavy tests and the 30-minute side-effect-free profile run on Pro/CI only. This transferred task must not edit, dispatch, or persist artifacts on Air-M5. Phase 2 must not contact Fly or any live staging database from any machine.
...
The hash has one source of truth: `backend.app.setup.router_manifest.route_catalog_hash()`. Runtime configuration reads an expected `WORKER_ROUTE_CATALOG_HASH`, recomputes the canonical function locally, and fails before opening resources when they differ. The protected deployment workflow computes the expected value **inside the exact immutable deployed digest** with a one-shot, no-network invocation of that same function, requires 64 lowercase hex, binds it to the digest artifact, and injects it into the worker through protected stdin at deploy time. A caller-supplied value, host-checkout computation, stale artifact, digest mismatch, or recomputation mismatch fails before mutation. No worker-specific serializer, copied hash algorithm, or generic `catalog_hash` alias is permitted.
...
`fly.staging.toml` uses `app = "nuzantara-rag-staging"`, `primary_region = "sin"`, the existing Dockerfile, and the exact production API/RAG process commands needed to reproduce the two legacy owners. It sets `ENVIRONMENT=staging`, has no production hostname, mount, public service, or embedded secret, and owns the staging database migration release command `python -m backend.db.migrate apply-all && python -m backend.db.schema_audit`. Its API and RAG readiness probes are private and process-scoped. `fly.worker.toml` uses `app = "nuzantara-worker-staging"`, the same region/Dockerfile, process command `python -m backend.workers.runtime`, environment `PORT=9091`, exactly one `[vm]` declaration with `cpu_kind = "shared"`, `cpus = 1`, and `memory = "1gb"`, and a top-level HTTP check on port 9091 path `/ready`. It contains neither a service block nor a release command. The Dockerfile continues to produce the same image for all planes; it may only add a generic module entry compatibility check, not a worker-specific build stage.

- [ ] Write TOML tests for both exact app names, shared Dockerfile/digest-compatible process commands, region, private checks, missing service blocks, environment keys, exactly one migration owner, and exactly one worker `[vm]` with `cpu_kind="shared"`, `cpus=1`, `memory="1gb"`. Reject a production app/hostname, mount, hardcoded `DATABASE_URL`, provider keys, Qdrant keys, public ports, auto-resize metadata, worker release command, second Dockerfile, second VM group, or any CPU/memory drift. Require the staging primary release command to run both `apply-all` and `schema_audit` and the worker to run neither.
...
In addition to the deployment behavior below, Task 7 adds
artifact/resource/hash guilt tests for missing, duplicate, corrupt, expired,
cross-repository, wrong-run, wrong-commit, and unsuccessful-source artifacts.
They pin the exact v4 upload/download name, retention, missing-file behavior,
producer run ID, same-repository GitHub API success check, and canonical
JSON/SHA-256 fields. They prove `WORKER_ROUTE_CATALOG_HASH` is computed inside
the exact converged digest with networking disabled, is never accepted from
dispatch input or the host checkout, and stale/wrong hash injection fails
before Fly mutation. The protected workflow also re-parses `fly.worker.toml`
and rejects any drift from `cpu_kind=shared`, `cpus=1`, `memory=1gb`.

The staging orchestrator implements post-merge subcommands `inspect`, `deploy-primary-legacy`, `deploy-worker-off`, `admit-workload-capability`, `revoke-workload-capability`, `promote-shadow`, and `rollback-topology`, but Phase 2 never invokes them against Fly. It targets exactly `nuzantara-rag-staging` plus `nuzantara-worker-staging`. Command construction and evidence parsing are dependency-injected so pre-merge tests use fabricated Fly JSON and fake subprocesses. A successful protected `main` deployment in `.github/workflows/fly-deploy.yml` must finish this exact chain: pre-deploy gate; old-image idempotent migration pass; rolling deploy whose fresh-image release command runs `apply-all && schema_audit` before promotion; post-deploy SQL-v2; Python migrations; explicit post-deploy fresh-image `schema_audit`; public health; immutable digest export. A centralized `always()` rollback/escalation job covers every post-promotion failure and digest export depends on every successful post-promotion job. The export resolves one converged immutable Machine `image_ref.digest`, requires `sha256:<64hex>`, binds it to `github.sha` and the workflow run/provenance, and publishes `production-compatibility-digest.json`. The producer uses `actions/upload-artifact@v4` with exact name `worker-plane-production-compatibility-${{ github.run_id }}`, exact file `production-compatibility-digest.json`, `retention-days: 30`, and `if-no-files-found: error`. The canonical JSON binds `github_sha`, `github_run_id`, immutable digest, route-catalog hash computed inside that digest, artifact SHA-256, and workflow provenance. The consumer uses `actions/download-artifact@v4` with the exact producer `run-id`, same name, same repository, and `github-token`; it queries the GitHub API to require that source run's protected-main workflow concluded `success`, rejects a different repository/SHA, duplicate file, corruption, expiry, or hash drift, and verifies the JSON SHA-256 before constructing any mutation.
...
The compatibility-release production job may be dispatched only by rollout Task 3 with the unchanged green post-staging gate artifact, merged commit, exact staging-proven digest, active-goal reference, and matching admission row. Phase 2 deliberately does not yet accept a receipt-activation panel hash: it defines a typed, fail-closed extension seam whose unknown or premature Release-B inputs are rejected. Production-rollout Task 3's focused Release-B PR must modify this workflow, `deploy_worker_production.py`, and their tests so `bootstrap-off` additionally requires and verifies the immutable green Release-B panel hash. Its typed compatibility actuator exposes only `inspect`, `bootstrap-off`, `admit-workload-capability`, `revoke-workload-capability`, and `rollback-digest`. It targets only `nuzantara-worker`, never rebuilds or redeploys API/RAG, and may create the named companion app only after the gate. `bootstrap-off` invokes `worker_database_role.py reconcile-production-base`, installs `WORKER_DATABASE_URL` through stdin without logging it, rejects `DATABASE_URL`, carries no workload provider secret/grant, starts every workload `off`, and proves private readiness plus digest-bound heartbeat. Capability admission accepts exactly the next cataloged workload, reconciles the cumulative grant union, installs only cataloged provider-secret symbols, and cannot arm a guard or change ownership. On failure it restores a recorded prior companion digest; for a first bootstrap it scales the new app to zero, removes its runtime secret, and disables login on the new base role without destroying the app or touching API/RAG.
...
- [ ] Commit the protected primary release ordering, rollback controller, and immutable digest artifact first; run its focused RED/GREEN workflow tests and obtain rereview before advancing:

  ```bash
  git add .github/workflows/fly-deploy.yml apps/backend-rag/scripts/run_migrations.py
  git commit -m "ci(worker-plane): harden primary release ordering" -m "Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>"
  ```

- [ ] Commit the staging/production actuators, protected workflow, capability/fault actions, and their fake tests second; rerun RED/GREEN and rereview this independently reversible slice:

  ```bash
  git add apps/backend-rag/scripts/deploy_worker_staging.py apps/backend-rag/scripts/tests/test_deploy_worker_staging.py apps/backend-rag/scripts/deploy_worker_production.py apps/backend-rag/scripts/tests/test_deploy_worker_production.py .github/workflows/worker-plane-production.yml .github/workflows/worker-plane-phase2.yml
  git commit -m "ci(worker-plane): add protected companion actuators" -m "Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>"
  ```

- [ ] Commit retired shims, the mutation-route ratchet, and ownership protections last; rerun the whole Task 7 suite and obtain the final task rereview:

  ```bash
  git add apps/backend-rag/scripts/deploy_staging.sh apps/backend-rag/scripts/deploy_backend.sh apps/backend-rag/backend/scripts/deploy_fly.sh scripts/preflight.sh scripts/worker_plane/check_live_mutation_routes.py scripts/tests/test_check_live_mutation_routes.py .github/CODEOWNERS .github/workflows/hot-zone-pr-gate.yml
  git commit -m "test(worker-plane): ratchet live mutation routes" -m "Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>"
  # Then record task status, commit SHA, RED/GREEN evidence, and rereview in .superpowers/sdd/progress.md; never stage .superpowers/.
  ```
...
- [ ] Phase 0 and Phase 1 final verifiers remain green; migration 248 remains the latest migration and no Phase 2 schema file exists.
