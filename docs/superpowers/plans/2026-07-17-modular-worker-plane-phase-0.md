...
- Create: `apps/backend-rag/backend/tests/fixtures/workload_source_census.json`
- Modify: `apps/backend-rag/backend/tests/setup/test_router_manifest.py`
...
- Create: `apps/backend-rag/backend/architecture/baselines/direct_asyncpg_upstream_delta.json`
...
The only measurement rule is the repository command below. At the historical reviewed Phase 0 base it yields exactly 67 sorted paths and SHA-256 `4002789a56196bd8cdce5440c1c596191f4e349ae6a91cb7e9f3d8ca8d24991a`:
...
- [ ] Write tests for exact historical-baseline success, one newly introduced router import failure, removal success, changed baseline hash failure, exception acceptance, malformed exception failure, expired exception failure, and the reviewed-upstream-delta branch. An exception record contains `path`, `owner`, `reason`, and `expires_on`; the initial file is `[]`. The upstream-delta record is either the canonical empty object or exactly `{base_commit,head_commit,historical_count,historical_sha256,rebased_count,rebased_sha256,added_paths,removed_paths,reviewer,evidence_sha256}` with sorted sanitized repository paths and no source contents.
- [ ] Run RED:
...
- [ ] Generate the sorted path baseline with the pinned command and write its newline-terminated SHA-256 to the companion file. First compare the rebased `origin/main` measurement with the historical 67/hash pair. If equal, assert the historical pair and write the canonical empty upstream-delta object. If different, require a source review showing that every addition/removal is an upstream-only change between the recorded base and rebased head, record the exact sanitized path delta and its evidence hash, obtain focused review, and pin the newly measured count/hash without weakening the no-addition ratchet. A feature-branch addition, unexplained delta, or mechanical replacement fails.
...
- [ ] Run GREEN:
  ...
  PYTHONPATH=. python scripts/check_router_asyncpg_ratchet.py --verify-recorded-baseline
  ```

  Expected: tests and ratchet pass; the recorded baseline is either the
  historical 67/hash pair or the exact focused-reviewed upstream-only delta,
  and the current measured set adds no path beyond that pinned rebased set.
...
- Modify: `apps/backend-rag/backend/tests/fixtures/workload_source_census.json`
...
- Create: `apps/backend-rag/backend/db/migrations_v2_nontransactional/247_event_quarantine_indexes.sql`
- Create: `apps/backend-rag/backend/db/migrations_v2_nontransactional/manifest.json`
- Create: `apps/backend-rag/scripts/apply_nontransactional_migrations.py`
- Create: `apps/backend-rag/scripts/tests/test_apply_nontransactional_migrations.py`
...
Transactional migration 247 adds nullable `quarantined_at TIMESTAMPTZ`, `quarantine_reason TEXT`, and `quarantined_by TEXT` to `events_outbox`, plus append-only `event_quarantine_audit` metadata with no payload column. It contains no `CREATE INDEX CONCURRENTLY`. The separately hashed sidecar `backend/db/migrations_v2_nontransactional/247_event_quarantine_indexes.sql` contains exactly the partial unconsumed/unquarantined and quarantined indexes as `CREATE INDEX CONCURRENTLY IF NOT EXISTS`. The typed sidecar runner uses an autocommit connection, one advisory lock, bounded `lock_timeout`/`statement_timeout`, an exact manifest entry and source SHA-256, records completion only after both indexes are valid, and refuses to run in a transaction. Neither path duplicates payloads nor deletes/acknowledges outbox rows. Rollback drops the two indexes through the same nontransactional runner before the transactional rollback drops the audit table and columns, and is allowed only before quarantine/audit data exists; the plan's operational exit keeps all additions through the rollback window.
...
- [ ] Write `scripts/tests/test_check_migration_lock_safety.py` RED before migration 247. The fixed checker scans the complete selected migration range and rejects every index/table rewrite that lacks exactly one policy: a dedicated audited `CREATE INDEX CONCURRENTLY` outside the transactional v2 runner, or a `proven-small` record with current production read-only row count/relation bytes, capture timestamp, owner, `lock_timeout_ms <= 1000`, `statement_timeout_ms <= 30000`, scale-clone row count at least the production count, concurrent synthetic-writer maximum stall at most 1000 ms, and zero timed-out statements. Every production/scale-clone capture used at protected Release-A merge must be no more than seven days old at merge time; the checker requires `captured_at`, `valid_until`, and the protected merge-clock input and rejects future, expired, or internally inconsistent evidence. Cover missing/stale counts, evidence older than seven days, transactional `CONCURRENTLY`, an unclassified partial index, smaller clone, writer-stall breach, timeout breach, and one valid mixed bundle. The checker reads no client rows or secrets and emits only relation-level counts, sizes, durations, hashes, and stable reason codes.
- [ ] Implement `scripts/worker_plane/check_migration_lock_safety.py` as a typed, deterministic, no-shell CLI. Its pre-merge invocation is `apps/backend-rag/.venv/bin/python scripts/worker_plane/check_migration_lock_safety.py --migration-dir apps/backend-rag/backend/db/migrations_v2 --nontransactional-dir apps/backend-rag/backend/db/migrations_v2_nontransactional --from 246 --to 251 --evidence docs/architecture/worker-plane-migration-lock-safety.json --as-of "$PROTECTED_MERGE_CLOCK"`. Migration 247's first RED fixture must prove the transactional source contains no concurrent index and classify both sidecar indexes under the manifest-backed nontransactional policy. Later migrations extend the same evidence atomically and rerun the unchanged guilt suite/checker before their packet freezes; a missing later file is allowed only before its allocated phase begins, but protected merge requires the complete 246–251 range and evidence whose `valid_until` covers the merge clock.
...
`backend.services.events.quarantine:quarantine_outbox_event`,
`backend.services.events.quarantine:requeue_quarantined_outbox_event`, plus every
...
`quarantine_outbox_event(conn, outbox_id, reason, quarantined_by) -> bool` atomically updates only `consumed_at IS NULL AND quarantined_at IS NULL`. `requeue_quarantined_outbox_event(conn, outbox_id, expected_quarantined_at, actor, reason) -> bool` is the only recovery interface: a protected static API management role may clear quarantine only when the row is still unconsumed and `quarantined_at` exactly matches the expected timestamp, and the same transaction appends an `event_quarantine_audit` row containing only outbox ID, old quarantine timestamp/reason code, actor, sanitized reason, and action. Normal API/RAG/worker roles, bulk requeue, and direct audit/quarantine DML are denied. Audit insertion failure rolls back the requeue. The recovered row returns to normal deterministic replay and is not marked consumed. `get_quarantine_summary(conn) -> QuarantineSummary` returns count and oldest age without payloads. The existing public `replay_unconsumed(conn, dispatch_fn, *, channel, max_age_minutes) -> int` signature and integer meaning remain stable for all current callers. New `replay_unconsumed_summary(...) -> ReplaySummary` performs the richer selection: it reads unconsumed/unquarantined rows including those older than the replay window, determines policy from `EVENT_CATALOG`, and performs one of three explicit actions: dispatch, quarantine durable stale, or acknowledge explicitly cataloged best-effort expiry. The compatibility wrapper calls the summary function, emits structured `dispatched`, `acknowledged_best_effort`, `quarantined`, and `failed` counts through the caller's existing logger/observation sink, and returns the same successful replay count the old API returned; it never changes existing callers to a dataclass implicitly. EventBus consumes the summary directly. The federation-alert daemon retains the integer wrapper but must emit all four fields before returning `dispatched`; no quarantine may be hidden by the compatibility integer.

- [ ] Write migration tests for the three columns, append-only payload-free audit table, absence of concurrent indexes in transactional migration 247, both exact manifest-bound partial indexes in the nontransactional sidecar, no destructive forward statements, explicit rollback marker, migration number uniqueness, and the exact complete ownership blocks for `public.events_outbox` and `public.event_quarantine_audit`. Reject a missing/delta-only block, a legacy singular writer annotation, catalog/migration binding drift, sidecar hash drift, transaction-wrapped `CONCURRENTLY`, or any unaccounted current mutation interface. Runner tests prove autocommit, advisory exclusion, bounded timeouts, idempotent restart after either index, validity checking, and completion-record hashing without a live production call.
- [ ] Write outbox tests proving a stale durable row is quarantined with `consumed_at IS NULL`, excluded from subsequent replay, visible in the summary, and logged/alerted without its payload. Prove one exact-timestamp protected requeue restores replay, a stale expected timestamp and bulk request fail, normal worker/direct DML are denied, audit failure is atomic, and neither audit nor evidence contains payload bytes.
...
- [ ] Run RED:
  ...
  cd apps/backend-rag
  source .venv/bin/activate
  PYTHONPATH=. pytest backend/tests/db/test_migration_247_event_quarantine.py scripts/tests/test_apply_nontransactional_migrations.py backend/tests/services/events/test_outbox.py backend/tests/services/events/test_outbox_quarantine.py backend/tests/services/events/test_outbox_stale_ttl.py backend/tests/services/events/test_event_bus_replay.py backend/tests/services/federation_alerts/test_daemon.py -q
  ```

  Expected: migration/module tests fail; the durable stale-event assertion shows the current silent acknowledgement.

- [ ] Implement transactional migration 247, the manifest-bound nontransactional index sidecar/runner, and `quarantine.py`. Send an `AlertService` critical alert through an injected callback after the transaction commits; alert failure must not undo quarantine and must be reflected in structured logs/metrics. Bind the requeue wrapper and audit table to the static-management ownership policy and expose no general-purpose SQL mutation.
...
- [ ] Commit the allocation checker and lease record first:

  ```bash
  git add docs/architecture/worker-plane-migration-allocation.md scripts/check_worker_plane_migration_allocation.py scripts/tests/test_worker_plane_migration_allocation.py
  git commit -m "chore(migrations): reserve worker plane migration block" -m "Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>"
  ```

- [ ] Commit the lock-safety engine, sidecar runner, migration, manifest, and focused migration tests second:

  ```bash
  git add apps/backend-rag/backend/db/migrations_v2/247_event_quarantine.sql apps/backend-rag/backend/db/migrations_v2_nontransactional/247_event_quarantine_indexes.sql apps/backend-rag/backend/db/migrations_v2_nontransactional/manifest.json apps/backend-rag/scripts/apply_nontransactional_migrations.py apps/backend-rag/scripts/tests/test_apply_nontransactional_migrations.py apps/backend-rag/backend/tests/db/test_migration_247_event_quarantine.py docs/architecture/worker-plane-migration-lock-safety.json scripts/worker_plane/check_migration_lock_safety.py scripts/tests/test_check_migration_lock_safety.py
  git commit -m "ci(migrations): enforce lock-safe worker plane ddl" -m "Co-Authored-By: Codex Opus 4.8 (1M context) <noreply@anthropic.com>"
  ```

- [ ] Commit quarantine behavior and all audited callers third:

  ```bash
  git add apps/backend-rag/backend/services/events/__init__.py apps/backend-rag/backend/services/events/quarantine.py apps/backend-rag/backend/services/events/outbox.py apps/backend-rag/backend/services/events/event_bus.py apps/backend-rag/backend/services/federation_alerts/daemon.py apps/backend-rag/backend/app/routers/health.py apps/backend-rag/backend/tests/services/events/test_outbox.py apps/backend-rag/backend/tests/services/events/test_outbox_quarantine.py apps/backend-rag/backend/tests/services/events/test_outbox_stale_ttl.py apps/backend-rag/backend/tests/services/events/test_event_bus_replay.py apps/backend-rag/backend/tests/services/federation_alerts/test_daemon.py
...
- [ ] Run the full Phase 0 gate:
  ...
  cd apps/backend-rag
  source .venv/bin/activate
  PYTHONPATH=. pytest backend/tests/architecture backend/tests/worker_plane backend/tests/setup backend/tests/db/test_workflow_migration_provenance.py backend/tests/db/test_migration_247_event_quarantine.py scripts/tests/test_apply_nontransactional_migrations.py backend/tests/services/events backend/tests/services/federation_alerts/test_daemon.py -q
  ...
  apps/backend-rag/.venv/bin/python scripts/worker_plane/check_migration_lock_safety.py --migration-dir apps/backend-rag/backend/db/migrations_v2 --nontransactional-dir apps/backend-rag/backend/db/migrations_v2_nontransactional --from 246 --to 247 --evidence docs/architecture/worker-plane-migration-lock-safety.json --as-of "$PROTECTED_MERGE_CLOCK"
  cd apps/backend-rag
...
- [ ] The checked-in direct-router asyncpg baseline contains the historical 67-path pinned hash or one exact focused-reviewed upstream-only replacement record; the current measured set contains no path outside that pinned rebased baseline, while legitimate removals may make the current set smaller without weakening the ratchet.
