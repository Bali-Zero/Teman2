...
2. **25 watchdog operational guardians = NO bootout architettoricamente giusto**. NB-1 verbatim: *"Operational watchdogs and LaunchAgents send Telegram alerts DIRECTLY. If a disk-monitor or fly-restart-loop-detector attempted to route its SOS signal through the federation_alerts dispatcher during a backend outage, the alert would be silently swallowed by 503."* Cita `heartbeat_monitor.py`, `core_guardian/watchdog.py`, `post_publish_poller.py`.
