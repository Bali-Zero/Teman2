...
- [ ] Write startup conflict tests that distinguish normal ownership from malformed configuration. When the current live grant assigns a workload to another runtime, this process skips task creation, remains request-serving ready, and records a payload-free `not_current_owner` observation; this is required for rollback after all four workloads move to the companion. Initialization fails only for contradictory duplicate-active local declarations, malformed mode/generation data, or an impossible catalog/config binding. Add a full API/RAG boot test with all four pilot grants owned by `worker`: readiness is HTTP 200 and zero pilot durable tasks are created. Legitimate internal concurrency under one owner remains allowed. Cover durable call sites from the Phase 0 classification fixture in `app_factory.py`, `main_api.py`, `service_initializer.py`, and `drive_poll_worker.py`.
- [ ] Run RED:
...
Extend the single checker created in Phase 0; do not create a second migration-only ownership engine. It applies migrations through 248 to disposable PostgreSQL, introspects `pg_catalog`, compares that live set with the refreshed sorted schema fixture, parses every ownership-affecting migration block, resolves the catalog interfaces, and requires each migration block to equal the ownership policy **as of that migration**. Later migrations may extend an existing table, so historical blocks are immutable historical snapshots; the highest-numbered ownership source for a table must equal the current `table_ownership.json` and `WORKLOAD_CATALOG`. Add a two-migration-chain fixture proving a correct historical block plus correct latest block passes, a rewritten/wrong historical block fails, and a stale latest block fails. A dated legacy exception contains `table_name`, `business_context`, `binding_id`, `operation`, `write_interface`, `reason`, `approved_by`, and `expires_on`. It may temporarily account for a known legacy source path already inside a valid finite binding; it can never create a wildcard candidate, waive workload/candidate parity, authorize `off|shadow`, bypass live owner/generation/mode checks, or exempt a control-plane mutation.
