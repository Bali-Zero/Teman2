...
- Quarantine and ambiguity have bounded protected recovery: event requeue is exact-row CAS plus payload-free audit; WA ambiguity pages by 15 minutes and receives an audited confirm or failed-no-retry/manual-hold decision by four hours; neither path permits bulk or automatic ambiguous resend.
- Attempt history is immutable to runtime writers but not unbounded: protected workload-specific retention may remove only unreferenced terminal events older than 180 days in audited batches, never active, retryable, or unknown state.
...
The first panel is allowed one non-runtime bootstrap commit after the documentation rebase. It contains only the canonical freezer, single-buffer launcher, deterministic validator, immutable GLM route configuration, and their guilt tests. These files are committed source dependencies of `H0`, but they are deliberately outside the initial nine-covered-plus-one-instructions review projection. Therefore this initial panel is a review of the plan authority, not independent approval of its own transport/validator implementation: integrity rests on committed source hashes plus executable guilt tests until Phase 0 Task 8 reviews and revalidates the tooling. Record that bounded bootstrap trust gap in the first freeze receipt/disposition; any bootstrap test or provenance failure invalidates the panel. This is not Phase 0 runtime implementation and cannot contain worker-plane behavior, production evidence, or review verdicts.
...
The first plan panel precedes migration source and therefore reviews the
lock-safety contract rather than runtime measurements. Phase 0 Task 7 must add
the fixed `scripts/worker_plane/check_migration_lock_safety.py` plus guilt tests
before migration 247; migrations 247–251 then extend one reviewed evidence
record. Protected Release-A merge is impossible until the checker validates
the complete range against production relation counts and a
scale-representative clone with concurrent synthetic writes, using the
one-second lock-wait and 30-second statement budgets in the rollout plan.
Those captures must be no more than seven days old and their explicit
`valid_until` must cover the protected merge clock. Migration 247's partial
...
- [ ] **Step 2: Resolve every Blocking and Important task-review finding**
- [ ] **Step 3: Prove the Release-A global-ack/receipt dual-read/dual-write bridge, pruning interlock through deterministic pre-existing-row backfill, audited interlock release, all crash points, and the four-cell old/new binary matrix while durable fan-out remains disabled**
- [ ] **Step 4: Prove the dormant Release-B build-floor/catalog-hash/activation-boundary CAS, protected activate/revert/force-bridge actuators, and independent subscription acknowledgement/replay in disposable PostgreSQL while preserving two physical buses; no live receipt activation occurs pre-merge**
- [ ] **Step 5: Run real Mouth/MCP contract compatibility, prohibited-PII pre-publish rejection and sanitized-capture tests, table ownership, direct-SQL/cross-context ratchets, and the Phase 0 Pro/CI code-regression comparator; rollout later establishes environment-local inert G9 denominators**
...
- [ ] **Step 7: After the Release-A post-staging gate, ship the focused protected Release-B receipt-activation PR, including the production-workflow/actuator/test change that makes the green Release-B panel hash mandatory for companion bootstrap; keep its production rolling deploy in bridge mode, deploy the exact Release-B digest to staging, prove build floor/backfill/catalog hash/activation boundaries plus protected bridge -> receipt-authoritative -> bridge -> receipt-authoritative behavior, and pass a fresh immutable three-model activation panel. Then activate production receipts through `activate-receipt-mode` and the observation window; any post-fan-out rollback uses bounded `force-bridge` or keeps Release B deployed fail-closed. Only afterward use `.github/workflows/worker-plane-production.yml` to deploy that exact Release-B digest to the private `nuzantara-worker` companion with every worker workload off, guards unarmed, base control-plane database capability only, and no provider capability; prove private readiness/heartbeat, then execute each workload through a separate protected capability admission and `.github/workflows/worker-plane-live-control.yml` command in fixed order `workflow_queue -> legal_full_ingestion -> notification_scheduler -> wa_outbox`. Every arm/drain/activate/reverse command consumes the immutable effective-capability run ID/hash and immediately re-audits grants plus allowed secret-symbol hashes before CAS; always source drain -> full barrier -> target active at a newer generation, with full-cycle observation and reverse/re-cutover proof before the next workload receives capability**
