---
date: 2026-05-16 (v3.1 revised post 3-LLM panel BLOCK verdict)
domain: automations / infra-hygiene
status: DRAFT v3.2 NON-EXECUTABLE — audit document only, NOT for execution. 5 fix applied post v3.1 panel re-review
v3_1_review_verdicts:
  - gemini_3.1_pro_v3_1: REJECT (silent residuals contradiction + bootout-suicide pattern)
  - codex_gpt_5.5_spalla_v3_1: BLOCKER (7 findings: silent residuals, file pipeline, F9.6 commit wrong target, redis-cli --raw missing, F0.6 narrative stale, "Identical to v2" omissions)
  - deepseek_v4_pro_v3_1: NEEDS_FIX (NOT BLOCK — 0 critical, 6 high pure comment rot from patch cycle, code body correct)
v3_2_fixes_applied:
  - Stripped 6 'silent' residual references (DeepSeek+Codex+Gemini convergent)
  - Reversed bootout-rm order to avoid SIGTERM suicide before rm (Gemini)
  - Added redis-cli --raw flag in plan + materialized script (Codex)
  - Fixed F9.6 commit target → v3.2.md (Codex)
  - Unified Telegram-direct file naming → telegram-direct-mapping.txt (drop senders.txt) (Codex)
v3_2_decision: COMMIT as DRAFT audit document, NOT executed in current session. Antonello reads, decides if scrappare il plan o evolvere
supersedes: docs/superpowers/plans/2026-05-16-automation-cleanup-plan-v2.md (BLOCK), v3 draft (3-LLM BLOCK)
v3_review_verdicts:
...
v3_1_fixes_applied:
  ...
  - File path mismatch fixed (telegram-direct-labels.txt consistent).
...
empirical_baseline_2026_05_16_v3_1:
  - pg_channels_total: 14 (verified grep PG_CHANNEL_MAP)
  ...
  - live_launchagents_total: 137 (launchctl list filter com.{balizero,nuzantara,cell,matagaruda}.* sort -u)
  - telegram_direct_labels_count: 25 (empirical 2026-05-16 03:50 via grep api.telegram.org on 99 programs)
...
# Automation cleanup plan v3.2 — 2026-05-16 (DRAFT NON-EXECUTABLE)
...
# Self-teardown: rm BEFORE bootout (Gemini finding — bootout SIGTERMs us → rm never runs)
...
```bash
# Enumerate ALL program paths referenced by 137 live LaunchAgents
LABELS=$(launchctl list 2>/dev/null | awk 'NR>1 {print $NF}' \
         | grep -E '^(com\.balizero|com\.nuzantara|com\.cell|com\.matagaruda)\.' | sort -u)
...
# Filter for Telegram-direct API callers
TELEGRAM_MAPPING=$DATED_BACKUP/state/telegram-direct-mapping.txt
> $TELEGRAM_MAPPING
while IFS= read -r prog; do
  ...
  if grep -qE '(api\.telegram\.org|telegram_bot.*sendMessage)' "$prog" 2>/dev/null; then
    ...
    for owner in $OWNERS; do
      echo "$owner|$prog" >> $TELEGRAM_MAPPING
    done
  fi
done < $PROGRAMS

echo "Telegram-direct (label|program) pairs: $(wc -l < $TELEGRAM_MAPPING)"
cat $TELEGRAM_MAPPING
...
cat $TELEGRAM_MAPPING | tee -a $ROTATION_SCOPE
...
# expect: observe (unchanged — v3.2 does NOT modify mode)

# Step 5 — Sanity: verify federation_alert_daemon reads federation_alert_mode via repository.get_db_mode()
# The daemon module is a thin launcher (~51 lines) that instantiates FederationAlertDaemon.
# The actual mode logic lives in repository.py — verified empirically 2026-05-16 19:50 WITA
...
- The single label cited as still-running by v2 verdict (`com.nuzantara.federation-alert-dispatcher`) is the federation router's daemon, which reads `federation_alert_mode` via `repository.get_db_mode()` (verified empirically). Current mode `observe` = log-only safer tier; no mode change needed.
...
LAST_ID=$(redis-cli -h "$REDIS_HOST" --raw XREVRANGE organism:events + - COUNT 1 2>/dev/null | head -1 || echo "")
...
<dict>
  ...
  <array>
    <string>/bin/bash</string>
    <string>-lc</string>
    <string>for label in com.balizero.wr2.supervisor com.balizero.wr2.supervisor-watchdog com.balizero.wr2.daily-metrics; do exit_code=\$(launchctl print gui/\$(id -u)/\$label 2>/dev/null | grep -E "last exit" | head -1); echo "\$label: \$exit_code"; done >> ~/.automation-cleanup-2026-05-16/state/pg-proxy-cluster-24h.txt; rm -f ~/Library/LaunchAgents/com.nuzantara.pg-proxy-cluster-recheck-oneshot.plist; launchctl bootout gui/\$(id -u)/com.nuzantara.pg-proxy-cluster-recheck-oneshot 2>/dev/null</string>
  </array>
  <key>StartCalendarInterval</key>
  <dict>
...
2. Update SYMBIOSIS.md source table: PG channels 13 → 14, add `intel_lake_event`
...
#   federation_alert_mode = $PRIOR_FAM (observe, unchanged during cleanup by trap on EXIT re-assert)
# v3.2 does NOT change mode — only observe-locks via FOR UPDATE during transaction window.
# AIL decision at F9: no action required on federation alerts (mode stays observe).

# 9.6 — Atomic commits PER FASE (H5 enforcement)
cd ~/Desktop/nuzantara

# F0/F1 commit (plan + watchdog files in infra/)
git add docs/superpowers/plans/2026-05-16-automation-cleanup-plan-v3.2.md \
...
```
F0 pre-flight (CTE prior snapshot + atrun check + at +4h schedule + Mini reach gate)
   │
   ↓
...
| Mode change tampering during cleanup window | Bassa | Medio | F1.3 Step 2 paranoia race check + trap EXIT re-asserts prior value if changed |
