...
2. `max_concurrent_builders = 4` across all machines. A reviewer that only reads may overlap if it does not contend for a saturated host. A reviewer repair consumes a builder slot.
...
### Task 4: Dispatch Wave 0 with four active builders maximum
