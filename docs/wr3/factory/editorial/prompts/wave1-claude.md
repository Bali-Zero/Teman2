...
Prioritize human psychology, trust, authority, emotional tension, and the moment a reasonable person realizes that the document, promise, shortcut, or identity they trusted may not mean what they thought it meant. Keep every candidate practically useful and sourceable.

## Output contract

Return one JSON object with one key, `candidates`, whose value is an array of exactly 30 objects. Use unique IDs `A01` through `A30`. Every object must contain exactly:
