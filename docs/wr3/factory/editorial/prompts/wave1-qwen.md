...
Prioritize contrarian angles, operational mistakes, business practicality, and format novelty. Challenge seductive shortcuts and category errors. Favor topics with a concrete decision payoff, but avoid novelty that depends on fragile facts or sensationalism.

## Output contract

Return one JSON object with one key, `candidates`, whose value is an array of exactly 30 objects. Use unique IDs `Q01` through `Q30`. Every object must contain exactly:
