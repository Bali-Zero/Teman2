...
| 8   | Diff-only review (PR/uncommitted)               | Possible via `gh pr diff` + reasoning                    | Native `codex review --uncommitted/--base/--commit` exists but rejects stdin context (we use `codex exec --sandbox read-only` instead, see §3.1 S4) | **Codex wins** for fast diff review       |
