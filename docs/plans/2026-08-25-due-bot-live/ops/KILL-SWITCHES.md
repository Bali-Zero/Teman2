...
| `TEAM_BOT_READ_TOOLS_ENABLED`      | `false` | team_replies        | R0/R1 read tools not exposed to the model. Also read by lane B9's `ToolExecutor` as a defense-in-depth check one layer below the loop (`apps/team-bot/team_bot/flags.py::is_team_bot_read_tools_enabled()`).                                                                                                                                                                         | B3/B9       | wired   |
...
| `TEAM_BOT_BRAIN_TP1_ENABLED`       | `false` | team_replies        | `BrainRouter` never attempts any of the three TP1 cloud tiers (qwen3.7-plus / qwen3.6-flash / glm-5.2) and goes straight to the local read-only Mini model (R0 tools only) — the narrower gesture to pull the cloud brain out of rotation without touching `TEAM_BOT_REPLY_ENABLED`, the master send switch. Directive #1 §1. **WIRED, not planned** — `BrainRouter` reads it today. | B4          | wired   |
