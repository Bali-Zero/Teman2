...
- PR #6124 (mappa design): OPEN, auto-merge armato. Il check «Harness floor recompute» resta
  rosso perché il brief gear 2 richiede un verdetto `harness/fable-gate` postato sul head
  `cdfe4a3d26` da un gate fresco (generator ≠ grader) e il rilancio del run `pull_request`
  originale (runbook `merge-queue-discipline.md` §6septies). Stesso percorso per questa PR.
- Questa PR: pacchetto Astra copiato byte-identico (hash in §1) + `FINAL.md` + brief gear 2 +
  panel a quattro seat (§8). La spec v6.2 di topic 3 entra su main con la PR-O0 della finestra
  Oracle, non con questa.
...
| Gemini, default `agy` | si dichiara «Gemini 3.8 Flash» (settings `gemini-3.1-pro-preview`); tentativo 1 vuoto per tool negato in headless                                                                                                           | 16 finding                                                        | conditional |
