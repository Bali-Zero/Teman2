...
## 12. Errata (recorded by the builder of PR2a-v2, 2026-09-12; Imperator decision #36)

Two points where the code that implements this spec departs from its letter, so that the
spec on `main` does not contradict the code on `main`:

- **§3, mutation model E — `swap_case` is not lossy.** `s.swapcase()` is a bijection on ASCII
  letters and has no collision pair, so the derivation rule ("a witness the model fails to
  construct is a test ERROR") would fail on it. The suite uses `casefold` in its place, which is
  lossy (`ab` / `AB` collapse) and exercises the same case-insensitivity class.
- **§4.5, the test policy INSERT — `$2::text::interval`, not `$2::interval`.** asyncpg infers
  the parameter type from the cast and demands a `timedelta` for `interval`, rejecting the
  literal string. Casting through `text` keeps the interval literal readable in the fixture.

Neither changes a property, an observable or a guilt mutation.
