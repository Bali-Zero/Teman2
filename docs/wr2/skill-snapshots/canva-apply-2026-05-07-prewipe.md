...
2. Call `start-editing-transaction` on `template_design_id` with `user_intent="apply carousel text and image replacements"`. Save the returned `transaction_id`.
   ...
   **From the response, build a LIVE TEMPLATE MAP:**
   ...
   - For each page, list of `(element_id, role_index, type)` where:
     - `role_index = 0` for first richtext on the page (assumed headline)
     - `role_index = 1` for second richtext (assumed body/subhead)
     - `role_index = 2` for third (if present)
...
5. **Phase A.5 — PRE-WIPE template decorative residue.** The DAHE6lx1lf8 master template carries 8-11 richtext blocks per page where only 1-3 are heading/body editorial slots; the rest are decorative display-accent baked-in placeholders ("OLD GRANDFATHERED APPROVALS", "AUDIT WINDOW IS OPEN", "TIER-A: PRIORITY SECTORS", etc.) that survive runs unless explicitly wiped. Without this step, the duplicate produced in Phase B inherits stale template text mixed with the new editorial content (the "Frankenstein" failure mode).
   ...
   For each page `i` in `1..live_pages` that has ANY operation in the remapped+clamped array from step 4, build a wipe-all-richtext op set:
   - From the live template map, list every `(element_id, role_index)` where `type == richtext` on page `i`.
   - Emit one `{"type":"replace_text","element_id":<id>,"text":" ","page_index":i}` per richtext element.
   - Prepend these wipe ops to the operations array for that page, so they run BEFORE the editorial replacements.

   Result: each page is zeroed first, then the heading/body remapped ops overwrite the wiped slots with real editorial content. Decorative blocks left untouched after the editorial overwrites stay blank (a single space).
...
8. Call `resize-design` on `template_design_id` (same dimensions) to duplicate. Save the NEW `design_id` — this is the carousel we deliver.
...
11. From the reset transaction's response, enumerate EVERY text element across ALL pages (whatever live_pages is — don't hardcode 11). Build the full list of `(page_index, element_id)` pairs.
...
- `pages=[1..live_pages]`
- `operations`: one `{"type":"replace_text","element_id":<id>,"text":" ","page_index":<page>}` per every text element you enumerated. Use a single space `" "` (not empty string) — some Canva layouts auto-restore a placeholder when given an empty string.
