...
> **HISTORICAL SNAPSHOT (2026-05-03) — Sprint 2 scope, superseded on the operational lane.**
> Every `canva-apply` / `canva-renderer` reference below describes the pre-2026-06-11
> architecture. PR #1236 (2026-06-11) moved the render chokepoint from `canva-apply` to
> `html-apply` (`scripts/wr2_html_render_apply.py`); the Canva lane itself was fully retired
> PR #2396 (2026-07-13) — `canva-renderer`'s own "LEGACY orphan, pending dismissal" note below
> turned out to be right, just early. The L1/L2/L3/L4 cognitive-level classification and the
> non-operational event contracts (connector, dossier-compiler, learner-nightly, measurer,
> trend-hunter, strategos, oracle, pg-proxy, sla-worker, supervisor, hardening) are **not**
> re-verified in this pass and may still be accurate — only the operational carousel-render lane
> (topic-selector → draft-generator → image-generator → **render stage** → newsletter) is known
> stale. Current ground truth for that lane: `docs/wr2/SUPERVISOR.md` +
