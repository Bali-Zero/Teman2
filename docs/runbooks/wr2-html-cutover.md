...
2. Deploy worktree synced: `scripts/wr2_html_render_apply.py`,
   ...
   present in `~/nuzantara-deploy`.
3. Pro-local venv `~/nuzantara-deploy/.venv-wr2-html` exists; `--selftest` OK.
...
source ~/nuzantara-deploy/.venv-wr2-html/bin/activate
WR2_HTML_SHADOW=1 WR2_VISION_REQUIRED=1 DATABASE_URL=... \
  python ~/nuzantara-deploy/scripts/wr2_html_render_apply.py --draft-id <throwaway-uuid>
