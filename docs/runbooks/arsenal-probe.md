...
| codex MODEL_ERR (gpt-6-astra)      | `gpt-6-astra` "requires a newer version of Codex" — upgrade the Codex CLI/app on that machine before re-probing; was misclassified `UNKNOWN_ERR` before 2026-09-07                                                                                                                                                                     |
| claude AUTH/QUOTA                  | `claude auth status`; window cap → wait reset or switch slot                                                                                                                                                                                                                                                                           |
| claude CONTEXT_AUTH (trust dialog) | project trust not accepted for the probed cwd — run `claude` interactively in that project once and accept the trust dialog, or set `projects["<path>"].hasTrustDialogAccepted: true` in `~/.claude/.claude.json` (operator consent, never set programmatically by a session/probe); was misclassified `UNKNOWN_ERR` before 2026-09-13 |
...
Note (2026-09-07, healer tick, Mini): same disease again, one more shape. `codex` probing
`gpt-6-astra` got `"The 'gpt-6-astra' model requires a newer version of Codex. Please upgrade to
the latest app or CLI and try again."` — fell through to `UNKNOWN_ERR` because `_MODEL_ERR_PAT`
only matched `1211`/`unknown model`/`model is not supported`, not this client-version-gated
phrasing. Widened `_MODEL_ERR_PAT` to also match `model requires a newer version`, with a
guilt+innocence pair in `scripts/tests/test_arsenal_probe.py` (innocence: a bare "upgrade to the
latest app or CLI" without the model-bound phrase must not false-positive) and a canned entry in
`_SELFTEST_CANNED`. Operator-gated (upgrade the Codex CLI/app) — this only fixes which cure the
board points at.

Note (2026-09-13, healer tick, Mini): same disease, `claude` this time, but not a credential
shape — a per-PROJECT trust flag. Evidence: `'Please run claude interactively here once and
accept the trust dialog, or set projects["/Users/nuzantara/nuzantara"].hasTrustDialogAccepted:
true in /Users/nuzantara/.claude/.claude.json.'`, carrying no 401/oauth-token/quota marker, fell
through `classify_generic()` to a bare `UNKNOWN_ERR`. Confirmed live on Mini:
`projects["/Users/nuzantara/nuzantara"].hasTrustDialogAccepted` reads `false` in
`~/.claude/.claude.json` even though an interactive session in the same directory works fine —
this is a context limitation (the seat may be LIVE elsewhere), not a credential death, so it is
matched locally in `probe_claude()` to `CONTEXT_AUTH` (mirroring the existing `agy` split) rather
