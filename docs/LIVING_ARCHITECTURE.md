...
_Auto-generated by The Scribe on 2026-02-02 02:57:08_
...
#### `DELETE /clients/{client_id}/drive-folder`
...
**File:** `app/routers/crm_drive_folders.py`
...
#### `GET /clients/{client_id}/drive-folder`
...
#### `GET /clients/{client_id}/drive-folder/stats`
...
#### `GET /clients/{client_id}/drive-folder/structure`
...
#### `GET /clients/{client_id}/drive-folder/{folder_name}/files`
...
**File:** `app/routers/crm_migration.py`

---

#### `GET /migration/documents-by-client/{client_id}`
...
#### `POST /api/intel/staging/{type}/{item_id}/cover`
...
#### `POST /clients/{client_id}/create-drive-folder`
...
#### `POST /clients/{client_id}/drive-folder/{folder_name}/upload`
...
#### `PUT /api/intel/staging/{type}/{item_id}`
...
**File:** `app/routers/article_composer.py`
...
**File:** `app/routers/article_composer_v2.py`
...
**Response Model:** `ComposeResponse`
...
**Response Model:** `PublishResponse`
...
**File:** `app/routers/documents_proxy.py`
...
**File:** `app/routers/kg_agentic.py`
...
**Response Model:** `KGStatsResponse`
...
**Response Model:** `KGQueryResponse`

**File:** `app/routers/kg_agentic.py`

---

### Messaging Identity Admin

#### `GET /api/admin/messaging-identity/lookup/phone/{phone}`

Lookup user by phone number.

Admin only. Returns mapping if exists (team member or portal client).

**File:** `app/routers/messaging_identity.py`

---

#### `GET /api/admin/messaging-identity/lookup/telegram/{chat_id}`
...
#### `GET /api/admin/messaging-identity/mappings`
...
#### `GET /api/admin/messaging-identity/mappings/{user_id}`
...
#### `POST /api/admin/messaging-identity/mappings`
...
#### `POST /api/admin/messaging-identity/mappings/deactivate`
...
#### `GET /api/admin/team-activity/crm-actions`

Get CRM activity log

**File:** `app/routers/admin_team_activity.py`

---

#### `GET /api/admin/team-activity/export/messages`
...
#### `GET /api/admin/team-activity/messages`
...
#### `GET /api/admin/team-activity/overview`
...
#### `GET /api/admin/team-activity/team-stats`
...
#### `GET /api/admin/team-activity/timesheet`
...
#### `POST /api/agentic-rag/proactive-trigger`
...
Returns:
Scheduler status with task details: - running: Whether scheduler is active - task_count: Number of registered tasks - tasks: Details of each task (intervals, run counts, errors)
...
#### `DELETE /api/crm/clients/documents/{document_id}`
...
**Response Model:** `PassportExtractResponse`

**File:** `app/routers/crm_clients.py`

---

#### `POST /api/crm/clients/extract-passport-enhanced`

Enhanced passport OCR using Gemini Vision.
Extracts all visible data from passport and updates client record.

Returns: - passport_number - expiry_date (linked to alert) - full_name (with fuzzy match verification) - gender (M/F) - date_of_birth - birthplace (for enrichment) - nationality - MRZ lines (if visible) - confidence score

**Response Model:** `PassportEnhancedResponse`
...
#### `DELETE /api/crm/documents/{doc_id}`
...
#### `POST /api/crm/clients/{client_id}/documents/bulk`
...
**File:** `app/routers/dashboard_featured_articles.py`
...
#### `POST /api/knowledge/activity/log`

Log a knowledge base view or download activity

**File:** `app/routers/knowledge_activity.py`
...
**File:** `app/routers/whatsapp_chat.py`
...
#### `agents.agents.multi_ai_orchestrator`
...
#### `agents.agents.test_force_orchestrator`
...
#### `agents.agents.unified_test_force_orchestrator`
...
#### `agents.config.qwen_system_prompts`
...
#### `agents.services.cursor_adapter`
...
#### `agents.services.differential_coverage_analyzer`
...
#### `agents.services.google_cloud_shell_adapter`
...
#### `agents.services.google_colab_adapter`
...
#### `agents.services.multi_ai_adapter`
...
#### `agents.services.unified_coverage_collector`
...
#### `agents.services.windsurf_adapter`
...
#### `app.routers.admin_team_activity`
...
#### `app.routers.article_composer_v2`
...
#### `app.routers.dashboard_featured_articles`
...
#### `app.routers.knowledge_activity`
...
#### `app.routers.messaging_identity`
...
#### `db.migrations_legacy_archive.migration_999_fix_rag_tables`
...
#### `migrations.migration_042_passport_enrichment`
...
#### `migrations.migration_043_fix_visa_types_from_qdrant`
...
#### `migrations.migration_043_knowledge_activity_log`
...
#### `migrations.migration_044_seed_practice_types`
...
#### `migrations.migration_050_client_memory_sync`
...
#### `migrations.migration_051_client_messaging_link`
...
#### `migrations.scripts.seed_visa_types`
...
#### `migrations.scripts.seed_visa_types_complete`
...
#### `migrations.scripts.update_visa_types_fc_series`
...
One-time script to add Asya Nadia Firdauzi to team_members table.
Run this on Fly.io: fly ssh console -a nuzantara-rag
Then: cd /app && python -m backend.scripts.add_asya

#### `scripts.check_portal_tables`

Check database tables for portal.

#### `scripts.check_schema`

Check team_members table schema.

#### `scripts.create_portal_test_user`

Create Portal Test User

Quick script to create a test portal user for testing.
Creates a client and portal user account directly.

Usage:
cd apps/backend-rag
source .venv/bin/activate
python backend/scripts/create_portal_test_user.py --email test@example.com --pin 1234

#### `scripts.create_portal_test_user_simple`

Create Portal Test User - Simple Version

Usage:
DATABASE_URL="postgresql://..." python create_portal_test_user_simple.py --email test@example.com --pin 1234

#### `scripts.create_portal_user_for_roberta`

Create Portal User for existing client Roberta (ID 6658)

Usage on Fly.io:
fly ssh console -a nuzantara-rag -C "cd /app && python -m backend.scripts.create_portal_user_for_roberta"
...
Interactive tool to query and explore the Knowledge Graph.
...
source .venv/bin/activate
    ...
    # Show stats
    python -m backend.scripts.kg_query stats

    # Find entity
    python -m backend.scripts.kg_query find "KITAS"

    # Get entity details
    python -m backend.scripts.kg_query get "permit:kitas"

    # Find path between entities
    python -m backend.scripts.kg_query path "company:pt_pma" "permit:kitas"

    # List relationships for entity
    python -m backend.scripts.kg_query rels "permit:kitas"

#### `scripts.search_client`
...
#### `services.article_composer.__init__`
...
#### `services.article_composer.cache`
...
#### `services.article_composer.claude_client`
...
#### `services.article_composer.error_handler`
...
#### `services.article_composer.validators`
...
#### `services.autonomous_agents.__init__`
...
#### `services.crm.birthday_notifier_service`
...
#### `services.crm.birthplace_enrichment_service`
...
#### `services.crm.client_identity_resolver`
...
#### `services.crm.conversation_title_generator`
...
#### `services.crm.document_categorizer`
...
#### `services.crm.lead_assignment_agent`
...
#### `services.integrations.github_publisher`
...
#### `services.integrations.messaging_identity_service`
...
#### `services.integrations.service_account_drive_service`
...
#### `services.integrations.whatsapp_service`
...
#### `services.integrations.whatsapp_triage_service`
...
#### `services.knowledge_graph.incremental_builder`
...
#### `services.rag.agent.__init__`

RAG Agent subpackage for Nuzantara.

Provides agentic RAG capabilities with tool usage and multi-step reasoning.

Modules:
structures: Data structures for agent state, tools, and execution flow.

#### `services.rag.agent.structures`
...
#### `services.rag.agentic.chat_session`
...
#### `services.rag.agentic.kg_orchestrator`
...
Example: >>> gateway = LLMGateway(gemini_tools=[...]) >>> response, model, obj = await gateway.send_message(
... chat=chat_session,
...
#### `services.rag.agentic.memory_handler`
...
#### `services.rag.agentic.orchestrator_context`
...
#### `services.rag.agentic.orchestrator_core`
...
#### `services.rag.agentic.orchestrator_metrics`
...
#### `services.rag.agentic.orchestrator_response`
...
#### `services.rag.agentic.orchestrator_routing`
...
#### `services.rag.agentic.orchestrator_streaming`
...
#### `services.rag.agentic.orchestrator_streaming_core`
...
#### `services.rag.agentic.query_gates`
...
#### `services.rag.agentic.query_helpers`
...
#### `services.rag.agentic.thinking_indicators`
...
#### `services.search.query_expander`
...
#### `tests.integration.article_composer.__init__`

Integration tests for Article Composer

#### `tests.integration.article_composer.test_article_composer_integration`
...
#### `tests.integration.zantara.test_fluidity_and_strength`
...
#### `tests.integration.zantara.test_fluidity_and_strength_simple`

Simple integration tests to verify ZANTARA is fluid and powerful
Tests fluidity (low ABSTAIN rate) and strength (proactive, helpful responses)

#### `tests.integration.zantara.test_tier1_fallback`
...
#### `tests.test_crm_migration_endpoints`
...
#### `tests.test_lead_assignment_flow`
...
#### `tests.unit.llm.providers.test_deepseek`
...
#### `tests.unit.llm.providers.test_ollama`

Unit tests for OllamaProvider
Target: 100% coverage

#### `tests.unit.llm.providers.test_openrouter_comprehensive`

Comprehensive tests for OpenRouterProvider
Target: 100% coverage

#### `tests.unit.llm.providers.test_vertex`

Unit tests for llm.providers.vertex
Auto-generated test template for high coverage

#### `tests.unit.llm.test_base`
...
#### `tests.unit.llm.test_prompt_manager`

Unit tests for PromptManager
Target: 100% coverage

#### `tests.unit.llm.test_provider_registry`

Unit tests for llm.provider_registry
Auto-generated test template for high coverage

#### `tests.unit.llm.test_retry_handler`
...
#### `tests.unit.routers.test_article_composer`
...
#### `tests.unit.services.article_composer.__init__`

Unit tests for Article Composer services

#### `tests.unit.services.article_composer.test_cache`

Unit tests for Cache Service

#### `tests.unit.services.article_composer.test_claude_client`

Unit tests for Claude Client with Retry Logic and Circuit Breaker

#### `tests.unit.services.article_composer.test_error_handler`

Unit tests for Error Handler

#### `tests.unit.services.article_composer.test_validators`
...
#### `tests.unit.services.intel.__init__`

Unit tests for Intel services.

#### `tests.unit.services.intel.test_intel_analytics_service`

Unit tests for IntelAnalyticsService.

#### `tests.unit.services.intel.test_intel_approval_service`

Unit tests for IntelApprovalService.

#### `tests.unit.services.intel.test_intel_classification_service`

Unit tests for IntelClassificationService.

#### `tests.unit.services.intel.test_intel_staging_service`
...
#### `tests.unit.services.misc.conftest`
...
#### `tests.unit.services.misc.test_followup_service_metrics`
...
#### `tests.unit.services.rag.agentic.conftest_ollama`
...
#### `tests.unit.services.rag.agentic.test_context_manager_performance`
...
#### `tests.unit.services.rag.agentic.test_orchestrator_context`

Unit tests for OrchestratorContextManager

Test coverage target: >95%

#### `tests.unit.services.rag.agentic.test_orchestrator_core`
...
#### `tests.unit.services.rag.agentic.test_orchestrator_metrics`

Unit tests for OrchestratorMetricsManager

Test coverage target: >95%

#### `tests.unit.services.rag.agentic.test_orchestrator_response`

Unit tests for OrchestratorResponseBuilder

Test coverage target: >95%

#### `tests.unit.services.rag.agentic.test_orchestrator_routing`

Unit tests for OrchestratorRoutingManager

Test coverage target: >95%

#### `tests.unit.services.rag.agentic.test_orchestrator_streaming`
...
- SystemPromptBuilder.**init**() - cache initialization
...
**Module:** `services.crm.birthday_notifier_service`

Service to send personalized birthday emails to clients.

### `BirthplaceEnrichmentService`

**Module:** `services.crm.birthplace_enrichment_service`
...
**Module:** `services.article_composer.cache`
...
**Module:** `services.intel.intel_analytics_service`
...
**Module:** `services.intel.intel_approval_service`
...
**Module:** `services.intel.intel_classification_service`
...
**Module:** `services.intel.intel_staging_service`
...
**Module:** `services.invoicing.invoice_service`
...
**Module:** `services.integrations.messaging_identity_service`
...
**Module:** `services.integrations.service_account_drive_service`
...
**Module:** `tests.unit.services.article_composer.test_cache`
...
### `TestFollowupServiceHealthCheck`

**Module:** `tests.unit.services.misc.test_followup_service_metrics`

Tests for FollowupService health check

### `TestFollowupServiceLogging`

**Module:** `tests.unit.services.misc.test_followup_service_metrics`

Tests for FollowupService logging

### `TestFollowupServiceMetrics`
...
### `TestIntelAnalyticsService`

**Module:** `tests.unit.services.intel.test_intel_analytics_service`

Test suite for IntelAnalyticsService.

### `TestIntelApprovalService`

**Module:** `tests.unit.services.intel.test_intel_approval_service`

Test suite for IntelApprovalService.

### `TestIntelClassificationService`

**Module:** `tests.unit.services.intel.test_intel_classification_service`

Test suite for IntelClassificationService.

### `TestIntelStagingService`

**Module:** `tests.unit.services.intel.test_intel_staging_service`
...
**Module:** `services.rag.agentic.thinking_indicators`
...
**Module:** `services.integrations.whatsapp_service`

Service for interacting with WhatsApp Business Cloud API.

### `WhatsAppTriageService`

**Module:** `services.integrations.whatsapp_triage_service`
...
### `tests/services/crm/test_lead_assignment_agent.py`

### `services/crm/lead_assignment_agent.py`
...
_Last updated: 2026-02-02 02:57:08_
