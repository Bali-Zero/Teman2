...
- **A1 durable artifacts**: html-apply, after render, copies `slides/*.png` + `caption.txt` +
  `meta.json` to `~/Desktop/nuzantara/apps/war-room/output/carousel/<YYYY-MM-DD>-<slug>/`
...
- **B1 unfreeze now** (ops, this session): `git -C ~/Desktop/nuzantara-deploy checkout -- <plist>`
