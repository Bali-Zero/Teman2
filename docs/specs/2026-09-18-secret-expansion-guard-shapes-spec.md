...
Status: PROPOSED (2026-09-18). Implementation `infra/claude-hooks/secret_expansion_guard.py`,
...
#3), and v2 was re-implemented *against a spec and a case corpus* rather than against its own
...
budget: under 5 ms per call (pure regex over the command string; no file is read unless a shape
already matched a registered path).
...
inverted for this concern: `_strip_heredoc_bodies()` and `_strip_quotes()` *erase* content before
...
denied, and quoting must not hide `"${KEY:-x}"` — the leaking forms are almost always *inside*
...
| X2 | environment dump | `printenv`, `env`, `export`, `set`, `declare` | `printenv` in any form; bare `env`/`set`/`declare` with no following program; `export -p` | `env VAR=val cmd`, `env -i …`, `env -u NAME cmd`, `VAR=val cmd` — these set or strip, they do not print |
...
| X5 | Read tool on a secret store | tools `Read`, `read_file`, `NotebookRead` | `tool_input.file_path` resolves to a registered STORE | a `limit`/`offset` slice does NOT exempt it — the secret is one line, and any slice can contain it |
...
- **Interpreter one-liners.** `python3 -c 'print(os.environ["NAME"])'` is not caught. Detecting it
  means parsing the embedded program, and a name-plus-`print` heuristic would deny the *sanctioned*
