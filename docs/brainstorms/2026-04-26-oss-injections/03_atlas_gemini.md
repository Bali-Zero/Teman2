...
_Caveat:_ Ensure your dev database Docker image or your initial migrations include `CREATE EXTENSION IF NOT EXISTS pg_trgm;` so the dev environment matches production.
...
Atlas will flag _any_ destructive change (like a legitimate `DROP TABLE` you actually want). It is not "noisy" (it doesn't guess), but it will block the PR.
