...
### 🐛 W132 (2026-09-13): the documented cure never reaches the twin file — a fix lands where the failure was, and the generated-contract twin keeps testing the pre-cure world

_Discovered 2026-09-13 in mission SHWEB-20260911, lane W-ORACLE. Family **#3 (Guard-over-match / UNDER-match)**, with a **#2 (Esiste≠Armato)** flavor: the twin guard stays green not because it verified anything, but because the generated contract file it reads had gone stale — a green that means "unregenerated," not "correct"._

**TRAUMA, measured not inferred.** Instance A: PR #5173 (018b55c9cc, 2026-08-28, "the tripwire guarded the wrong enum, and garuda_voa sat in the gap") fixed the backend lead-source tripwire — `apps/backend-rag/backend/services/lead_capture/source.py` plus its own twin test — but never touched the mouth-side twin guard, `apps/mouth/src/app/sitemap.test.ts` ("keeps GARUDA out of the checked-in public API client contract", 2026-07-28). That guard kept asserting `PublicLeadSource` has no `garuda_voa`, and kept two TYPE-NAME markers (`CaseType`, `backend__services__garuda_flow__intake__Purpose`). Nobody noticed for 16 days: the file the guard actually reads, `apps/mouth/src/lib/api/schema.d.ts` (generated by openapi-typescript from the gitignored `apps/backend-rag/openapi.json`), had not been regenerated since 2026-08-24. The first faithful regeneration, on 2026-09-13, is what finally tripped the stale guard — not the original cure. Instance B, same chain: backend fact-vocabulary PR #6396 declared `apps/mouth` "decoupled," yet `apps/mouth/src/app/(visa-oracle)/visa-oracle/_lib/fact-mapper.test.ts` reads `models.py` off disk and diffs its alias count against the hand-kept `apps/mouth/src/lib/api/applicant-fact-paths.ts` (typed against `schema.d.ts`), while `apps/backend-rag/backend/tests/scripts/visa_engine/` holds count literals that move with every fact. Three mouth tests and two backend tests went red; the PR's acceptance probe covered neither.

**MECHANISM.** A cure lands in the file that failed. The TWIN — the guard on the other side of a generated contract, or a count literal in a sibling test tree — is not part of the acceptance probe, so it keeps asserting the pre-cure world. A generated contract file (`schema.d.ts`, or any derived artifact) hides the drift for as long as nobody regenerates it: the guard is syntactically green, but it is testing a snapshot from before the fix, not the fix itself. This is UNDER-match in the #3 sense — the guard's true entity is "the live contract," and it is silently substituting "the last-generated copy of the contract" — wearing the #2 mask of a green that certifies nothing, because regeneration is a step nobody's acceptance probe forces.

**GOTCHA — the guard judges by TYPE NAME, not by entity.** `sitemap.test.ts` asserted against `PublicLeadSource` and `CaseType`/`backend__services__garuda_flow__intake__Purpose` as literal names, not against the endpoint/operationId the contract actually serves. A rename or regeneration cycle can make the assertion pass or fail for reasons that have nothing to do with whether `garuda_voa` is actually excluded from the public surface.

**CURE (rule).** When a PR changes an entity that a generated contract carries, the acceptance probe MUST: (a) regenerate the contract and run the consumer's full test suite — never trust a `schema.d.ts` or equivalent that predates the change; (b) grep every test tree for literals of the old count/name (`git grep -n "<old value>" -- '*test*'`); (c) name each twin file in the PR body as touched or as verified-unaffected. Guards judge by ENTITY (endpoint, operationId), never by type NAME — a name survives a regeneration that changes the shape underneath it.
