...
**Chosen: (c) hybrid** — `~/logs/codex-spalla/<ts>-<slug>.md` for routine runs (per-machine, not git-tracked); only BLOCKER-tagged runs are also copied to `docs/codex-reviews/<ts>-blocker-<slug>.md` for git audit trail.
