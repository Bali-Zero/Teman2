...
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `claude-fable-5`                                                                           | **Imperator window only (RULED 2026-09-06→08): owner-opened with `--model`, effort `xhigh`; never auto-routed, never fan-out, never a gate seat.** No other automated role — no doctrine, skill, cron, or script routes to it (the 2026-08-20 ban on auto-routing stands). Was: final on-disk gate for the Gear-3/large-feature class + WR2 content gate + Phase-2 council judge (see `claude-opus-5` row below for where those roles live now).                                                                                                                                                                                                                                                                                                                                                                                                                                                        | n/a — manual only                                                                                                                                                                                                                                                               |
| `claude-opus-5`                                                                            | Interactive conductor default (ratified 2026-07-25) **+ general/Dux seat and the mandatory seat of the final on-disk gate assignment on a BLUE mission (distinct from the independent review; no substitution without a ruling — and RULED 2026-09-10, PARABELLUM, is such a ruling: on a mission Zero declared ORANGE the Dux and the gate seat are Sol, colour table in `docs/architecture/dual-consul/army-map.md` §1bis), WR2 content gate and Phase-2 council judge — an assignment outside the contribution chain, not a permanent reviewer title (RULED 2026-09-06→08; those roles never route to Fable)**. Architecture, red-team, long-horizon agentic work. Gate roles: xhigh effort, never cascades to a weaker model, window dead → task SUSPENDS. **Thinks by default** — omitting `thinking` now thinks; `max_tokens` caps thinking+answer. Separate rate-limit bucket from the 4.x pool. | `low`/`medium` punch above their weight — primary cost/latency lever. `xhigh` = coding/agentic sweet spot; gate roles run `xhigh`; `max` only on a declared Gear-3 adjudication (RULED 2026-09-06→08). `thinking:{disabled}` only accepted at effort ≤ `high` (400 above that). |
...
| `sol`                 | Red-team + empirical sandbox: migrations (upgrade+downgrade), high-stakes diffs, council red-team seat. **LIVE** via `-m gpt-5.6-sol` (2026-08-27) — the bare `-m sol` slug from the account-rotation note above is still dead; this is a different, versioned slug. Door: `seat_build.sh --seat codex --tier sol` (PR #5044) → `-m gpt-5.6-sol`. Proof: PR #5044's own refuter round ran `codex exec -m gpt-5.6-sol -c model_reasoning_effort="high"` successfully (~16:30Z, teammate-reported) plus an independent live 1-token probe in this PR (`codex exec --sandbox read-only --skip-git-repo-check -m gpt-5.6-sol "reply pong" < /dev/null` → exit 0, 1581 stdout chars, reply `pong`). Gear-3 council role (RULED Zero 2026-09-02): **titolare** — primary review seat in `COUNCIL_REVIEW_SEATS_TITOLARI`. | xhigh/max; `ultra` = max reasoning + auto task delegation |
...
| Seat                                                     | Role / strengths                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gemini-3.1-pro`                                         | Final architectural synthesis, 1M-ctx corpus ingestion, KBLI/visa/regulatory search — Claude hallucinates regulations, this is the CLAUDE.md federation-trigger reason. Gear-3 council role (RULED Zero 2026-09-02): **reserve (fallback)** — counts toward the >=2-seat quorum via `agy-gemini-3.1-pro` in `COUNCIL_REVIEW_SEATS_FALLBACK`, first choice only when a titolare is unavailable (`seat_fallback_reason` required, `scripts/evidence_pack_lint.py`).                                                                  |
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `k3` (`kimi-code/k3`)       | Refuter #2 among the 3 titolari (after Codex Sol; RULED Zero 2026-09-02, now 3 titolari + 3 reserve/fallback), 1M-ctx long-context auditor, **multimodal Evidence-Pack verifier** (screenshots/PDFs/visual artifacts). Never the final gate. |
...
| ------------------------ | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `deepseek-v4-pro`        | `TP1-OAI`                   | Hard logic, complex implementation, architecture counter-analysis, and adversarial refutation. Subscription door only; never conflate it with the retired standalone per-token endpoint.                            | high/max task class                | Implementer/refuter; final: no; **quorum: YES** — reserve/fallback (RULED Zero 2026-09-02), see note below |
...
| `glm-5.2`                | `tp1-glm-5.2` via `TP1-OAI` | Counter-builder and general refuter-ladder hop; useful for independent implementations that challenge an incumbent candidate.                                                                                       | medium/high                        | Implementer/refuter; final: no; **quorum: YES** — reserve/fallback (RULED Zero 2026-09-02), see note below |
| `qwen3.8-max`            | `TP1-OAI`                   | Strategy-panel voice, rigorous instruction following, non-PII mass-document work, and complex pipeline execution. Compliance-exact extraction still requires an independent NotebookLM/Anthropic verification lane. | medium/high                        | Implementer/refuter; final: no; **quorum: YES** — titolare (RULED Zero 2026-09-02), see note below         |
...
> **Why three of the seven TP1 seats say `quorum: YES` (RULED Zero 2026-09-02).** `qwen3.8-max`
> was the first: it is the only TP1 seat that was **promoted ARMED** — 2026-08-14, on 459 measured
...
>
> **Same date, second ruling: the quorum roster split into titolari + reserve (RULED Zero
> 2026-09-02, verbatim: "metti codex, kimi ... qwen, deepseek, glm (questi 3 da TP1), gemini. I
> primi 3 titolari e gli altri fallback nel caso un titolare e' indisponibile").** The Gear-3
> quorum pool grew from 3 seats to 6: **titolari** `codex-gpt-5.6-sol`, `kimi-code/k3`,
> `tp1-qwen3.8-max` (reach for these first); **reserve/fallback** `tp1-deepseek-v4-pro`,
> `tp1-glm-5.2`, `agy-gemini-3.1-pro` (count toward the same >=2-seat quorum — but Zero's own
> mid-day correction, verbatim "le riserve non intervengono se i titolari ci sono": a reserve is
> a SUBSTITUTION, not an equal alternative, so ANY counted reserve — even just one, even standing
> beside a titolare — must be explained via a non-empty `seat_fallback_reason`; second limb of
> `check_council_run_gear3` in `scripts/evidence_pack_lint.py`). `deepseek-v4-pro` and
> `glm-5.2` therefore flip to `quorum: YES` above alongside `qwen3.8-max`; the `codex`/`kimi`/
> `gemini-3.1-pro` rows above carry the matching titolare/reserve marker rather than a `quorum:`
> cell, because they sit outside this TP1 table and `test_roster_quorum_column_matches_the_lint`
> only cross-checks `tp1-`-prefixed seats against `COUNCIL_REVIEW_SEATS` — a literal `quorum:`
> reading on a non-TP1 row would trip that parser against a set it was never meant to check.
>
> **SAETTA review policy v2 (RULED Zero 2026-09-13, mission SAETTA-REVIEW-POLICY; verbatim: "ok inserire anche gemini 3.1 pro high nella lista - e non diminuiamo, ma una regola che tutti vengono invocati i revisori ma basta la risposta di uno").** The Gear-3 review roster is not reduced and gains `agy-gemini-3.1-pro-high` (exact model `gemini-3.1-pro-high`). EVERY eligible independent seat — titolari and reserves alike — is invoked once per frozen candidate revision (max 300s, bounded process-group cleanup, no early-success skip); contributing families (always Anthropic, plus any family that built or supervised) are excluded and recorded; model variants never count as extra families. Quorum is >=1 completed substantive judgment bound to the same candidate SHA and the exact resolved model; exit-0 transport, quota/auth errors, empty output, wrong model and timeouts are NON_JUDGMENT, never PASS. Any unresolved finding blocks release even beside a PASS; every finding is disposed `fixed|rejected` with a rationale; one correction round per mission. The colour-specific fresh final gate is unchanged and is not a council family. This supersedes the 2026-09-02 "reserves only when a titolare is unavailable" limb for every post-ruling pack. Enforced by `python3 scripts/council_journal.py dispatch` (producer) and `check_council_policy_v2` in `scripts/evidence_pack_lint.py` (R9 v2); packs declare `council_policy: saetta-review-policy-v2`, `candidate_sha`, `contributing_families`, `findings_disposition`. Omitting the version does not opt out: v1 survives only for truly historical packs whose every journal line predates 2026-09-13T20:28:23Z. Seat -> family map: `COUNCIL_V2_SEATS` (openai `codex-gpt-5.6-sol`, moonshot `kimi-code/k3`, qwen `tp1-qwen3.8-max`, glm `tp1-glm-5.2`, deepseek `tp1-deepseek-v4-pro`, google `agy-gemini-3.1-pro-high`). The unsuffixed `agy-gemini-3.1-pro` seat stays only for historical v1 packs — the current agy catalog no longer lists it.
