...
| `claude-sonnet-5`                                                                          | **Implementer workhorse** — structured I/O, BUILD-stage default in modus §Arsenal ("Opus 5.5 designs, Sonnet builds, Opus 5.5 verifies"). New tokenizer: **~+30% tokens** for the same text vs 4.6 — re-measure `max_tokens`/compaction triggers with `count_tokens`, never a blanket multiplier.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `xhigh` sweet spot                                                                                                                                                                                                                                                              |
...
no scraping private accounts, no PII. Probe receipts 2026-09-24: `research/operations/2026-09-24-agy-seat-probe.md`. Session and gui/501 LaunchAgent contexts both LIVE; fence stays candidate-only pending the pilot ruled by the 4-seat memo review.
...
- **Standard** → `sonnet-5` (still the modus §Arsenal BUILD default) / `codex-terra` /
  `kimi-for-coding`.
- **Hard / architectural / red-team** → `opus-5-5` at `xhigh` / `codex-sol` at xhigh-max.
- **Effort is a dial, not a constant** — pick per task; `low`/`medium` on Opus 5.5 are a real
...
- **Final gate, all gears (RULED 2026-09-06→08, supersedes the permanent-Opus reviewer of 2026-08-20)** — an
  **assignment to a qualified independent verifier outside the contribution chain**, a Claude session
  on the Opus 5.5 `xhigh` seat, never cascading; the Gear-3 harness verdict gate and the WR2 content gate
  are distinct assignments on the same mandatory seat, each with its own verification (modus §Gate taxonomy). Two imperators (Fable 5.1, Astra) and two generals (Opus 5.5, Sol) with a
