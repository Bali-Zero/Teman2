...
| `sol` (when live)     | Red-team + empirical sandbox: migrations (upgrade+downgrade), high-stakes diffs, council red-team seat. **UNREACHABLE** (2026-08-27 — dead since 2026-07-21 per the account-rotation note above, not independently re-probed here: no `-m` slug selects it until a live probe proves the name again; the `codex` seat itself is reachable, this specific slug is not. `scripts/lint_roster_dispatch.py` requires this marker on a row it cannot resolve to a door — it does not itself verify the claim, see `KNOWN_DEAD_CODEX_SLUGS` in that script.) | xhigh/max; `ultra` = max reasoning + auto task delegation |
| `terra` (when live)   | Standard second-opinion / sandbox builder. **UNREACHABLE** (2026-08-27 — dead since 2026-07-21 per the account-rotation note above, not independently re-probed here: no `-m` slug selects it until a live probe proves the name again; the `codex` seat itself is reachable, this specific slug is not. `scripts/lint_roster_dispatch.py` requires this marker on a row it cannot resolve to a door — it does not itself verify the claim, see `KNOWN_DEAD_CODEX_SLUGS` in that script.)                                                              | medium (own default)                                      |
| `luna` (when live)    | Mechanical/grunt lanes. **UNREACHABLE** (2026-08-27 — dead since 2026-07-21 per the account-rotation note above, not independently re-probed here: no `-m` slug selects it until a live probe proves the name again; the `codex` seat itself is reachable, this specific slug is not. `scripts/lint_roster_dispatch.py` requires this marker on a row it cannot resolve to a door — it does not itself verify the claim, see `KNOWN_DEAD_CODEX_SLUGS` in that script.)                                                                                 | low/medium                                                |
