...
| `sol` (when live)     | Red-team + empirical sandbox: migrations (upgrade+downgrade), high-stakes diffs, council red-team seat. **UNREACHABLE** (2026-08-27, `scripts/lint_roster_dispatch.py` — dead since 2026-07-21 per the row above this table: no `-m` slug selects it until a live probe reproves the name; the `codex` seat itself is reachable, this specific slug is not). | xhigh/max; `ultra` = max reasoning + auto task delegation |
| `terra` (when live)   | Standard second-opinion / sandbox builder. **UNREACHABLE** (2026-08-27, `scripts/lint_roster_dispatch.py` — dead since 2026-07-21 per the row above this table: no `-m` slug selects it until a live probe reproves the name; the `codex` seat itself is reachable, this specific slug is not).                                                              | medium (own default)                                      |
| `luna` (when live)    | Mechanical/grunt lanes. **UNREACHABLE** (2026-08-27, `scripts/lint_roster_dispatch.py` — dead since 2026-07-21 per the row above this table: no `-m` slug selects it until a live probe reproves the name; the `codex` seat itself is reachable, this specific slug is not).                                                                                 | low/medium                                                |
