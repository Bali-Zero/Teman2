...
os.environ["DATABASE_URL"] = (
    "postgresql://postgres:postgres@localhost:5432/nuzantara_db"
)
os.environ["QDRANT_URL"] = "http://localhost:6333"
os.environ["GOOGLE_API_KEY"] = (
    "dummy"  # We might need a real key if we truly hit Gemini, but let's try to mock the LLM Gateway first or use the real one if we have the key.
)
...
async def run_verification():
    ...
    API_KEY = "REDACTED-ROTATED-KEY"  # From previous context
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        response_it = await client.post(
            API_URL, json=payload_it, headers={"X-API-Key": API_KEY}
        )
        print(
            f"🇮🇹 Italian Response ({response_it.status_code}): {response_it.json().get('answer', '')[:100]}..."
        )
        ...
        response_en = await client.post(
            API_URL, json=payload_en, headers={"X-API-Key": API_KEY}
        )
        ...
        response_creator = await client.post(
            API_URL, json=payload_creator, headers={"X-API-Key": API_KEY}
        )
